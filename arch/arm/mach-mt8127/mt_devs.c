...
flashdev_info_t *gen_FlashTable_ptr;
 ...
 * - MT8127 in develop should be 0x3600000
 * - MT8127 SQC should be 0x0
 */
...
//#endif	//End of CONFIG_MT8127_FPGA
...
int __init parse_tag_flashnum_fixup(const struct tag *tags)
{
	...
	//gen_FlashTable_p = (flashdev_info_t*) kzalloc(flash_number*sizeof(flashdev_info_t), GFP_KERNEL);
...
int __init parse_tag_flash_fixup(const struct tag *tags)
{
	...
	int i;
    memcpy(gen_FlashTable_ptr, &tags->u.gen_FlashTable_p, sizeof(flashdev_info_t));
	gen_FlashTable_ptr++;
	return 0;
}
#endif
...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    if (mdesc)
    {
        ...
        unsigned int j = get_devinfo_with_index(15);

	 if (j & (1 << 31))
        {
            if (i & (1 << 13))
                strcpy((void *)mdesc->name, "MT8680");
            else
                strcpy((void *)mdesc->name, "MT8685");
        }
        else
        {
            switch ((i >> 24) & 0xF)
            {
             case 0x0:
                strcpy((void *)mdesc->name, "MT8127");
             break;
             default:
                strcpy((void *)mdesc->name, "MT8117");
             break;
            }
        }
    }
...
// Camera ISP
//=======================================================================
#if 1 //defined(CONFIG_ISP_MT8127)
