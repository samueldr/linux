...
static int ahci_sunxi_set_voltage(struct device *dev, struct ahci_host_priv *hpriv)
{
	int voltage;
	int ret = 0;

	if (IS_ERR_OR_NULL(hpriv->ahci_regulator) || IS_ERR_OR_NULL(hpriv->phy_regulator))
		return 0;

	voltage = regulator_get_voltage(hpriv->ahci_regulator);
	if (regulator_set_voltage(hpriv->ahci_regulator, 2500000, 2500000) != 0) {
		dev_err(dev, "failed to set voltage for ahci_regulator!\n");
		ret = -1;
	} else
		dev_info(dev, "ahci_regulator voltage set: %d to 2500000!\n", voltage);

	voltage = regulator_get_voltage(hpriv->phy_regulator);
	if (regulator_set_voltage(hpriv->phy_regulator, 1200000, 1200000) != 0) {
		dev_err(dev, "failed to set voltage for phy_regulator!\n");
		ret = -1;
	} else
		dev_info(dev, "phy_regulator voltage set: %d to 1200000!\n", voltage);

	return ret;
}

static int ahci_sunxi_enable_regulators(struct device *dev, struct ahci_host_priv *hpriv)
{
	ahci_sunxi_set_voltage(dev, hpriv);

	return ahci_platform_enable_resources(hpriv);
}
...
static int ahci_sunxi_probe(struct platform_device *pdev)
{
	...
	rc = ahci_sunxi_enable_regulators(dev, hpriv);
	if (rc)
		return rc;
