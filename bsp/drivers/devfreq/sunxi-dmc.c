...
static void sunxi_dmcfreq_timer_func(struct timer_list *timer)
{
	...
	mutex_lock(&dmcfreq->input_lock);
	dmcfreq->input_set_target_max = false;
	mutex_unlock(&dmcfreq->input_lock);
}
...
static void sunxi_boost_work_events(struct work_struct *work)
{
	...
	mutex_lock(&dmcfreq->input_lock);
	if (!(dmcfreq->input_set_target_max)) {
		dmcfreq->input_set_target_max = true;
	} else {
		mutex_unlock(&dmcfreq->input_lock);
		goto change_time;
	}
...
static void sunxi_dmcfreq_boost_init(struct sunxi_dmcfreq *dmcfreq)
{
	...
	if (!dmcfreq->boost_workqueue) {
		dev_err(dmcfreq->dev, "Could not create boost workqueue\n");
		return;
	}
						...
						unsigned long *freq, u32 flags)
{
	...
	while ((dmcfreq->rate = clk_get_rate(dmcfreq->dmc_clk)) != target_rate) {
		if (!timeout) {
			dev_err(dev, "change dram clock error!\n");
			rc = -ETIMEDOUT;
			goto out;
		}
...
static int sunxi_dmcfreq_probe(struct platform_device *pdev)
{
	...
	if (rc) {
		pr_err("devfreq read input-boost-enable fail\n");
	...
	if (rc) {
		pr_err("devfreq read input-boost-time fail\n");
