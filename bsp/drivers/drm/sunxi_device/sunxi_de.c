...
static int sunxi_de_clk_enable(void)
{
	int ret;
	...
	clk_set_rate(de_drv->mclk, DE_CORE_CLK_RATE);

	ret = clk_prepare_enable(de_drv->mclk);
	if (ret < 0) {
		DRM_ERROR("Enable de clk failed\n");
		return ret;
	}

	ret = clk_prepare_enable(de_drv->mclk_bus);
	if (ret < 0) {
		DRM_ERROR("Enable de bus clk failed\n");
		return ret;
	}
...
static void sunxi_de_clk_disable(void)
{
	clk_disable_unprepare(de_drv->mclk);
}

#ifndef CONFIG_AW_DRM_DE_V2X
	...
	int nr, struct disp_layer_config_data *data)
{
	...
	csc_cfg.out_fmt =
		(hwde->info.cs == DISP_CSC_TYPE_RGB) ? DE_RGB : DE_YUV;
	if ((data->config.info.screen_win.height < 1280)
	    && (data->config.info.screen_win.height < 720))
		csc_cfg.out_mode = DE_BT601;
	else
		csc_cfg.out_mode = DE_BT709;
	csc_cfg.out_color_range = hwde->info.color_range;

#if defined(CONFIG_AW_DRM_DE_V33X)
#elif defined(CONFIG_AW_DRM_DE_V2X)
	de_al_mgr_apply_color(nr, &csc_cfg);
#elif defined(CONFIG_AW_DRM_DE_SUN50IW1)
#else
	"de_al_mgr_apply_color has NOT been implemented!"
#endif
	...
	sunxi_de_layer_enhance_apply(nr, hwde->layer_config_data,
		sunxi_de_get_layer_count(nr));
#else
	ret = de_al_lyr_apply(nr, hwde->layer_config_data,
		sunxi_de_get_layer_count(nr), false);
	if (ret) {
		DRM_ERROR("de_al_lyr_apply failed\n");
		return ret;
	}
#endif

	/* de_al_mgr_update_regs(nr); */
...
{
#ifndef CONFIG_AW_DRM_DE_V33X
	struct disp_csc_config csc_config;

	if (config->flags & ENH_MODE_DIRTY) {
		de_dcsc_get_config(nr, &csc_config);
		csc_config.enhance_mode = (config->info.mode >> 16) & 0xffff;
		de_dcsc_apply(nr, &csc_config);
	}
#endif
...
#ifndef CONFIG_AW_DRM_DE_SUN50IW1
	de_update_clk_rate(data->config.de_freq);
#endif
	de_update_device_fps(nr, data->config.device_fps);

	ret = de_al_mgr_apply(nr, data);
	if (ret) {
...
static int sunxi_de_event_proc(int nr, bool update)
{
	...
	if (!sunxi_tcon_sync_time_is_enough(tcon_id)) {
		de_al_mgr_update_regs(nr);
		DRM_INFO("WARNING: NO enough time left to sync reg\n");
		return -1;
	}
...
{
#ifdef CONFIG_AW_DRM_DE_V2X
	de_update_clk_rate(rate);
#endif
#ifdef CONFIG_AW_DRM_DE_SUN50IW1
			...
			struct disp_rect *rect)
{
	...
	if (rect)
		memcpy(rect, &info->window, sizeof(info->window));
			   ...
			   char *buf)
{
	ssize_t n = 0;
#if !defined(CONFIG_AW_DRM_DE_V33X)
	/* n += sprintf(buf + n, "hal_debug_level=%d\n",
				de_get_hal_debug_level()); */
#endif
	return n;
}
...
#elif defined(CONFIG_AW_DRM_DE_V33X)
static int sunxi_de_v3x_al_init(struct disp_bsp_init_para *para)
{
	return 0;
}
#elif defined(CONFIG_AW_DRM_DE_V2X)
static int sunxi_de_v2x_al_init(struct disp_bsp_init_para *para)
{
	int i;
	de_al_init(para);
...
#elif defined(CONFIG_AW_DRM_DE_SUN50IW1)
static int sunxi_de_sun50iw1_al_init(struct disp_bsp_init_para *para)
{
	int i;
	de_al_init(para);
...
#elif defined(CONFIG_AW_DRM_DE_V33X)
static int sunxi_de_v3x_al_exit(void)
{
	return 0;
}
#elif defined(CONFIG_AW_DRM_DE_V2X)
static int sunxi_de_v2x_al_exit(void)
{
	int i;
	...
	for (i = 0; i < DE_NUM; i++) {
		...
		if (de_feat_is_support_smbl(i))
			de_smbl_exit(i);
#endif
		kfree(de_drv->hwde[i].layer_config_data);
	}
	return 0;
}
#elif defined(CONFIG_AW_DRM_DE_SUN50IW1)
static int sunxi_de_sun50iw1_al_exit(void)
{
	return 0;
}
#else
...
#elif defined(CONFIG_AW_DRM_DE_V33X)
	return sunxi_de_v3x_al_init(para);
#elif defined(CONFIG_AW_DRM_DE_V2X)
	return sunxi_de_v2x_al_init(para);
#elif defined(CONFIG_AW_DRM_DE_SUN50IW1)
	return sunxi_de_sun50iw1_al_init(para);
#else
	"ERROR Compile!"
#endif
	return 0;
...
static int sunxi_de_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < de_drv->de_cnt; i++) {
		...
		de_drv->hwde[i].vichannel_cnt = de_feat_get_num_vi_chns(i);
		de_drv->hwde[i].uichannel_cnt = de_feat_get_num_ui_chns(i);
		de_drv->hwde[i].layers_per_vichannel = 4;
		de_drv->hwde[i].layers_per_uichannel = 4;
	}

	for (i = 0; i < de_drv->de_cnt; i++) {
		lay_num = sunxi_de_get_layer_count(i);
		de_drv->hwde[i].layer_config_data =
			kzalloc(lay_num * sizeof(struct disp_layer_config_data),
						GFP_KERNEL);
		if (!de_drv->hwde[i].layer_config_data) {
			DRM_ERROR("KZALLOC for disp_layer_config_data failed\n");
			return -1;
		}

		de_drv->hwde[i].funcs = &de_funcs;
	}
...
#elif defined(CONFIG_AW_DRM_DE_V3X)
	return sunxi_de_v3x_al_exit();
#elif defined(CONFIG_AW_DRM_DE_V2X)
	return sunxi_de_v2x_al_exit();
#elif defined(CONFIG_AW_DRM_DE_SUN50IW1)
	return sunxi_de_sun50iw1_al_exit();
#else
	"ERROR Compile!"
#endif
