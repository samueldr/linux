	...
	struct sunxi_connector_work_mode *conn_work_mode)
{
	...
	if (conn_work_mode->color_fmt == COLOR_FMT_YUV420)
		rate /= 2;
		     ...
		     struct sunxi_connector_work_mode *conn_work_mode)
{
	...
	de_update_device_fps(de_id, fps);
...
#if defined(CONFIG_AW_DRM_DE_SUN50IW1)
		tcon1_hdmi_color_remap(de_id, 0);
#else
		tcon1_hdmi_color_remap(de_id, 0, DISP_CSC_TYPE_RGB);
#endif
	} else {
#if defined(CONFIG_AW_DRM_DE_SUN50IW1) || defined(CONFIG_AW_DRM_DE_V33X)
		tcon1_hdmi_color_remap(de_id, 1);
#else
		if (conn_work_mode->color_fmt == COLOR_FMT_YUV444)
			tcon1_hdmi_color_remap(de_id, 1, DISP_CSC_TYPE_YUV444);
		else if (conn_work_mode->color_fmt == COLOR_FMT_YUV422)
			tcon1_hdmi_color_remap(de_id, 1, DISP_CSC_TYPE_YUV422);
		else if (conn_work_mode->color_fmt == COLOR_FMT_YUV420)
			tcon1_hdmi_color_remap(de_id, 1, DISP_CSC_TYPE_YUV420);
#endif
	}

#if defined(CONFIG_AW_DRM_DE_SUN50IW1)
	tcon1_src_select(enc_id, 0);
#else
	tcon1_src_select(enc_id, LCD_SRC_DE, de_id);
#endif

#if defined(CONFIG_AW_DRM_DE_V33X)
	tcon1_black_src(screen_id, 0, conn_work_mode->color_fmt);
#endif
...
/* parse disp_if_top */
	...
	tcon_drv->hwtopif.mclk = devm_clk_get(&pdev->dev, "clk_bus_dpss_top");
	...
	for (i = 0; i < tcon_drv->tcon_cnt; i++) {
		...
		hwtcon->mclk_bus = devm_clk_get(&pdev->dev, id);
		...
		hwtcon->rst_bus_tcon = devm_reset_control_get_shared(&pdev->dev, id);
...
#else
		output_type = DISP_OUTPUT_TYPE_LCD; /*  Fix me */
#endif
