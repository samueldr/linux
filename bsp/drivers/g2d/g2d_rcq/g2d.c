...
#include "g2d_top.h"
#if defined(CONFIG_G2D_MIXER)
#include "g2d_mixer.h"
#endif
#if defined(CONFIG_G2D_ROTATE)
...
int g2d_blit_h(g2d_blt_h *para)
{
	...
	ret = g2d_rotate_set_para(&para->src_image_h,
			    ...
			    para->flag_h);
#else
	G2D_ERR_MSG("Please enable CONFIG_G2D_ROTATE\n");
#endif
	return ret;
}
	...
	case G2D_CMD_TASK_APPLY:
		{
			...
			p_mixer_para = kmalloc(sizeof(*p_mixer_para) * p_task->frame_cnt,
					       GFP_KERNEL | __GFP_ZERO);
			if (!p_mixer_para)
				goto err_noput;
			if (copy_from_user(p_mixer_para, (void __user *)ubuffer[1],
					   sizeof(*p_mixer_para) * p_task->frame_cnt)) {
				ret = -EFAULT;
				goto err_noput;
			}
