...
g2d_ovl_u_submodule_setup(struct g2d_mixer_frame *p_frame)
{
	...
	p_ovl_u = kmalloc(sizeof(*p_ovl_u), GFP_KERNEL | __GFP_ZERO);
	...
	p_ovl_u->reg_blks =
	    kmalloc(sizeof(*p_ovl_u->reg_blks) * p_ovl_u->reg_blk_num,
		    GFP_KERNEL | __GFP_ZERO);
	p_ovl_u->reg_info =
	    kmalloc(sizeof(*p_ovl_u->reg_info), GFP_KERNEL | __GFP_ZERO);
