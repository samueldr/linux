...
g2d_ovl_v_submodule_setup(struct g2d_mixer_frame *p_frame)
{
	...
	p_ovl_v = kmalloc(sizeof(*p_ovl_v), GFP_KERNEL | __GFP_ZERO);
	...
	p_ovl_v->reg_blks =
	    kmalloc(sizeof(*p_ovl_v->reg_blks) * p_ovl_v->reg_blk_num,
		    GFP_KERNEL | __GFP_ZERO);
	p_ovl_v->reg_info =
	    kmalloc(sizeof(*p_ovl_v->reg_info), GFP_KERNEL | __GFP_ZERO);
