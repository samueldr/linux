...
struct scaler_submodule *g2d_scaler_submodule_setup(struct g2d_mixer_frame *p_frame)
{
	...
	p_scal = kmalloc(sizeof(*p_scal), GFP_KERNEL | __GFP_ZERO);
	...
	p_scal->reg_blks =
	    kmalloc(sizeof(*p_scal->reg_blks) * p_scal->reg_blk_num,
		    GFP_KERNEL | __GFP_ZERO);
	p_scal->reg_info =
	    kmalloc(sizeof(*p_scal->reg_info), GFP_KERNEL | __GFP_ZERO);
