...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
	...
	u8 cfg_info_len[] = {CFG_GROUP_LEN(cfg_info_group1), CFG_GROUP_LEN(cfg_info_group2),
						 CFG_GROUP_LEN(cfg_info_group3), CFG_GROUP_LEN(cfg_info_group4),
						 CFG_GROUP_LEN(cfg_info_group5), CFG_GROUP_LEN(cfg_info_group6)};
	...
	if (ts->gtp_cfg_len < GTP_CONFIG_MIN_LENGTH) {
		GTP_ERROR(
			"Sensor_ID(%d) matches with NULL or INVALID CONFIG GROUP! NO Config "
			"Sent! You need to check you header file CFG_GROUP section!",
			sensor_id);
		return -1;
	}
	...
	if (ret == SUCCESS) {
		GTP_DEBUG("CFG_GROUP%d Config Version: %d, IC Config Version: %d", sensor_id + 1, send_cfg_buf[sensor_id][0],
				  opr_buf[0]);
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
	...
	GTP_INFO("FILE HARDWARE INFO:%02x%02x%02x%02x", fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2],
			 fw_head->hw_info[3]);
	...
	if (update_msg.fw_total_len != fw_len) {
		GTP_ERROR(
			"Inconsistent firmware size, Update aborted! Default size: "
			"%d(%dK), actual size: %d(%dK)",
...
static void gup_search_file(s32 search_type)
{
	...
	for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i) {
		...
		if (search_type & AUTO_SEARCH_BIN) {
			GTP_DEBUG("Search for %s, %s for fw update.(%d/%d)", UPDATE_FILE_PATH_1, UPDATE_FILE_PATH_2, i + 1,
			...
			if (got_file_flag & BIN_FILE_READY) {
				...
				if (search_type & AUTO_SEARCH_CFG) {
					i = GUP_SEARCH_FILE_TIMES;	// Bin & Cfg File required to be in the
												// same directory
				} else
#endif
				{
		...
		if ((search_type & AUTO_SEARCH_CFG) && !(got_file_flag & CFG_FILE_READY)) {
			GTP_DEBUG("Search for %s, %s for config update.(%d/%d)", CONFIG_FILE_PATH_1, CONFIG_FILE_PATH_2, i + 1,
					  GUP_SEARCH_FILE_TIMES);
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *path)
{
	...
	got_file_flag = 0x00;
	if (path) {
		GTP_DEBUG("Update File path:%s, %u", path, strlen(path));
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
	...
	ret = gup_load_section_file(fw_boot_isp, (update_msg.fw_burned_len - FW_DSP_ISP_LENGTH), FW_BOOT_ISP_LENGTH,
								SEEK_SET);
	if (FAIL == ret) {
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
	...
	// FW_DSP_ISP_LENGTH + FW_BOOT_ISP_LENGTH; // 32 + 4 + 2 + 4 = 42K
	u16 start_index;
...
s32 gup_clk_calibration(void)
{
	...
	for (i = INIT_CLK_DAC; i < MAX_CLK_DAC; i++) {
		...
		if (ts->gtp_is_suspend) {
			i = 72;  // 80;    // if sleeping while calibrating main clock, set it
					 // default 72
			break;
		}
...
static u8 gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
	...
	if (GTP_FL_FW_BURN == dwn_mode) {
		...
		// ret = gup_check_and_repair(client, 0xC000, 2 * FW_DOWNLOAD_LENGTH,
		// FW_DSP_LENGTH);
...
static s32 gup_check_and_repair(struct i2c_client *client, u16 start_addr, s32 start_index, s32 chk_len)
{
	...
	while ((chk_len > 0) && (chked_times < GTP_CHK_FW_MAX)) {
		...
		for (i = 0; i < cmp_len; ++i) {
			if (chk_cmp_buf[i] != gtp_default_FW_fl[FW_HEAD_LENGTH + start_index + i]) {
				chk_fail = 1;
				i2c_write_bytes(client, cmp_addr + i, &gtp_default_FW_fl[FW_HEAD_LENGTH + start_index + i],
								cmp_len - i);
	...
	if (chk_len > 0) {
		GTP_ERROR("cmp_addr: 0x%04X, start_index: 0x%02X, chk_len: 0x%04X", cmp_addr, start_index, chk_len);
		return FAIL;
	}
	return SUCCESS;
}
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
	...
	for (section = 1; section <= 4; section += 2) {
		...
		if (GTP_FL_FW_BURN == dwn_mode) {
			...
			// ret = gup_check_and_repair(client, burn_addr, start_index,
			// FW_SS51_SECTION_LEN); if (ret == FAIL)
			{
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
	...
	if (!memcmp(path, "update", 6)) {
		...
		for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i) {
			...
			GTP_DEBUG("Search for %s, %s for fw update.(%d/%d)", FL_UPDATE_PATH, FL_UPDATE_PATH_SD, i + 1,
					  GUP_SEARCH_FILE_TIMES);
	...
	if (update_msg.fw_total_len != sizeof(gtp_default_FW_fl)) {
		GTP_ERROR("Inconsistent fw size. default size: %d(%dK), file size: %d(%dK)", sizeof(gtp_default_FW_fl),
				  sizeof(gtp_default_FW_fl) / 1024, update_msg.fw_total_len, update_msg.fw_total_len / 1024);
		set_fs(update_msg.old_fs);
		_CLOSE_FILE(update_msg.file);
		return FAIL;
	}
	...
	ret = update_msg.file->f_op->read(update_msg.file, (char *)gtp_default_FW_fl,
									  update_msg.fw_total_len + FW_HEAD_LENGTH, &update_msg.file->f_pos);
	set_fs(update_msg.old_fs);
	_CLOSE_FILE(update_msg.file);
