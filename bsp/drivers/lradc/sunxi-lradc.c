...
#define SUNXI_LRADC_ADC0_DATA_EN_MASK		(0x01 << 1)
...
static irqreturn_t sunxi_lradc_irq_handler(int irq, void *dummy)
{
	...
	if (reg_val & SUNXI_LRADC_ADC0_DATAPEND) {
		...
		if (chip->compare_before == chip->compare_later) {
...
static int sunxi_lradc_key_setup(struct sunxi_lradc *chip)
{
	...
	if (of_property_read_u32(chip->np, "key_cnt", &key_num)) {
		dev_err(chip->dev, "%s: get key count failed", __func__);
		return -EBUSY;
	}
	dev_dbg(chip->dev, "%s key number = %d.\n", __func__, key_num);
	if (key_num < 1 || key_num >= SUNXI_LRADC_VOL_NUM) {
	...
	key_vol[key_num] = chip->hw_data->measure;
	for (i = 0; i < key_num; i++)
...
static int sunxi_lradc_resource_get(struct sunxi_lradc *chip)
{
	...
	if (chip->irq_num < 0) {
		dev_err(chip->dev, "get irq failed\n");
		return chip->irq_num;
	}
...
static int sunxi_lradc_hw_init(struct sunxi_lradc *chip)
{
	...
	para = SUNXI_LRADC_ADC0_UP_EN | SUNXI_LRADC_ADC0_DOWN_EN | SUNXI_LRADC_ADC0_DATA_EN;
	mask = SUNXI_LRADC_ADC0_UP_EN_MASK | SUNXI_LRADC_ADC0_DOWN_EN_MASK | SUNXI_LRADC_ADC0_DATA_EN_MASK;
	reg_val &= ~mask;
	reg_val |= para;
