...
void log_boot(char *str)
{
	...
	if (log_count >= (LOGS_PER_BUF * BUF_COUNT)) {
		...
		bootevent[log_count / LOGS_PER_BUF] =
			kzalloc(sizeof(*bootevent[0]) * LOGS_PER_BUF,
...
static const struct proc_ops sunxi_bootevent_fops = {
	.proc_open    = sunxi_bootevent_open,
	.proc_read    = seq_read,
	.proc_lseek   = seq_lseek,
	.proc_write   = sunxi_bootevent_write,
...
static int __init init_boot_event(void)
{
	...
	bootevent[0] = kzalloc(sizeof(*bootevent[0]) * LOGS_PER_BUF,
			      GFP_ATOMIC | __GFP_NORETRY | __GFP_NOWARN);
	if (!bootevent[0])
		goto fail;
	sunxi_bootevent_switch(1);
