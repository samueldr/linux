				  ...
				  int bus_width, int timing)
{
	...
	SM_DBG(mmc_dev(host->mmc), "Try set %s clk dly	ok\n",
		mmc_clk_dly[cmod].mod_str);
	SM_DBG(mmc_dev(host->mmc), "drv_ph %d\n",
		mmc_clk_dly[cmod].drv_ph);
	SM_DBG(mmc_dev(host->mmc), "sam_ph %d\n",
...
*/
	...
	writel(rval, ccmu_ptr);
	SM_DBG(mmc_dev(host->mmc), " CCMU_BASE_ADDR %08x\n",
		readl(ccmu_ptr));
	iounmap(ccmu_ptr);
		...
		s32 opha, s32 ipha)
{
	...
	writel(rval, ccmu_ptr);
	SM_INFO(mmc_dev(host->mmc), "CCMU_BASE_ADDR: 0x%08x\n",
		readl(ccmu_ptr));
	iounmap(ccmu_ptr);
}
	...
	u32 rcnt, u32 errno, void *other)
{
	...
	if (rcnt < (SUNXI_RETRY_CNT_PER_PHA_V4P00X*10)) {
		...
		sunxi_mmc_set_dly_raw(host, sunxi_phase[0][0],
							sunxi_phase[0][1]);
		SM_INFO(mmc_dev(host->mmc), "sunxi v4p00x retry give up\n");
		return -1;
	}
	return 0;
}
