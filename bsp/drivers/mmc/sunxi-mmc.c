...
static irqreturn_t sunxi_mmc_irq(int irq, void *dev_id)
{
	...
	SM_DBG(mmc_dev(host->mmc), "irq: rq %p mi %08x idi %08x\n",
...
int sunxi_check_r1_ready(struct sunxi_mmc_host *smc_host, unsigned ms)
{
	...
	SM_INFO(mmc_dev(smc_host->mmc), "wrd\n");
	do {
...
*/
	...
	SM_DBG(mmc_dev(mmc), "hsq cmd %d(%08x) arg %x ie 0x%08x len %d\n",
		cmd_val & 0x3f, cmd_val, cmd->arg, imask,
		mrq->data ? mrq->data->blksz * mrq->data->blocks : 0);
...
*/
	...
	if (host->sunxi_mmc_updata_pha) {
		SM_DBG(mmc_dev(mmc), "not updata pha\n");
		spin_unlock_irqrestore(&host->lock, iflags);
		return -EBUSY;
...
*/
	...
	if (data) {
		if (host->perf_enable && cmd->data)
			host->perf.start = ktime_get();
		SM_DBG(mmc_dev(mmc), "set data\n");
		ret = sunxi_mmc_set_dat(host, mmc, data, true);
		if (ret) {
			spin_unlock_irqrestore(&host->lock, iflags);
			return -EBUSY;
		}
...
MODULE_AUTHOR("David Lanzend�rfer <david.lanzendoerfer@o2s.ch> && lixiang@allwinnertech.com");
