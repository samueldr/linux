	...
	do {
		if ((page_in_chip == buffer->simu_pageno) &&
			...
			if (ret == ECC_LIMIT) {
				...
				awrawnand_info("ecc limit from@0x%llx len@0x%x in page@%d\n",
						from, len, page);

			} else if (ret == ECC_ERR) {
				ecc_failed = true;
				mtd->ecc_stats.failed++;
				awrawnand_err("ecc err from@0x%llx len@0x%x in page@%d\n",
						from, len, page);
				buffer->simu_pageno = INVALID_CACHE;
				buffer->simu_oobno = INVALID_CACHE;
				buffer->chipno = INVALID_CACHE;
			} else if (ret < 0) {
				awrawnand_err("read from @0x%llx len@0x%x fail in page@%d\n",
						from, len, page);
	...
	do {
		...
		if ((page_in_chip == buffer->simu_oobno) &&
			...
			if (ret == ECC_LIMIT) {
				...
				mtd->ecc_stats.corrected += chip->bitflips;
				max_bitflips = max_t(unsigned int, max_bitflips, ret);
				awrawnand_info("ecc limit from@%llx len@%x in page@%d\n",
						from, ops->len, page);

			} else if (ret == ECC_ERR) {
				ecc_failed = true;
				mtd->ecc_stats.failed++;
				awrawnand_err("ecc err from@%llx len@%x in page@%d &&\n",
						from, ops->len, page);
				...
				awrawnand_err("read from @%llx len@%x fail in page@%d $$\n",
						from, ops->len, page);
				buffer->simu_pageno = INVALID_CACHE;
				buffer->simu_oobno = INVALID_CACHE;
				buffer->chipno = INVALID_CACHE;
			   ...
			   struct mtd_oob_ops *ops)
{
	...
	if (ops->len < chip->simu_pagesize)
		...
		if (ops->len & chip->simu_pagesize_mask) {
			awrawnand_err("write len@%u not align to simu pagesize\n", ops->len);
			npage++;
		}
