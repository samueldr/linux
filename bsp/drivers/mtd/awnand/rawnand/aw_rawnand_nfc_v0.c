...
#define CCM_NAND_SRC_SELECT	(0x3 << 24)
#define CCM_NAND_SRC_CSELECT(x)	(((x)& 0x3) << 24)
...
void aw_nfc_reg_dump(struct nfc_reg *reg)
{
	...
	printk("clk------------------\n");
	volatile unsigned *bus_gate = (volatile unsigned *)(0xf1c20060);
	volatile unsigned *bus_reset = (volatile unsigned *)(0xf1c202c0);
	volatile unsigned *mdclk = (volatile unsigned *)(0xf1c20080);
	printk("[%p : %08x]\n", bus_gate, readl(bus_gate));
	printk("[%p : %08x]\n", bus_reset, readl(bus_reset));
	printk("[%p : %08x]\n", mdclk, readl(mdclk));

	printk("pin------------------\n");
	volatile unsigned *pc0 = (volatile unsigned *)(0xf1c20848);
	volatile unsigned *pc1 = (volatile unsigned *)(0xf1c2084c);
	volatile unsigned *pc2 = (volatile unsigned *)(0xf1c20850);
	volatile unsigned *pc3 = (volatile unsigned *)(0xf1c20854);
	volatile unsigned *pc_dri0 = (volatile unsigned *)(0xf1c2085c);
	volatile unsigned *pc_dri1 = (volatile unsigned *)(0xf1c20860);
	volatile unsigned *pc_pull0 = (volatile unsigned *)(0xf1c20864);
	volatile unsigned *pc_pull1 = (volatile unsigned *)(0xf1c20868);
	printk("[%p : %08x]\n", pc0, readl(pc0));
	printk("[%p : %08x]\n", pc1, readl(pc1));
	printk("[%p : %08x]\n", pc2, readl(pc2));
	printk("[%p : %08x]\n", pc3, readl(pc3));
...
static int aw_host_clk_init(struct aw_nand_host *host)
{
	...
	awrawnand_err("get pll rate@%ld Hz\n", rate);
...
static int aw_host_set_clk(struct aw_nand_host *host, uint32_t mdclk)
{
	...
	awrawnand_print("awrawnand(mtd):mdclk:%ld\n",
			clk_get_rate(host->mdclk));
out:
	AWRAWNAND_TRACE_NFC("Exit %s\n", __func__);
	return ret;
}
...
static inline int aw_host_resource_init(struct aw_nand_host *host)
{
	...
	if (IS_ERR_OR_NULL(host->vcc_io)) {
		awrawnand_err("fail to get regulator vcc-io\n");
		goto out;
	}
...
static int aw_host_set_clk_itf_sdr(struct aw_nand_host *host)
{
	...
	AWRAWNAND_TRACE_NFC("Enter %s mdclk@%d", __func__, mdclk);
