		...
		unsigned int pagesize, void *buf, unsigned int len)
{
	...
	do {
		...
		if (ret) {
			pr_err("read boot to blk %d page %d failed\n",
					req.block, req.page);
			return ret;
		}
		if (req.page == 0)
...
int spinand_mtd_upload_boot0(struct mtd_info *mtd, unsigned int len, void *buf)
{
	...
	while (start < end) {
		pr_info("upload boot0 to block %d len %dK\n", start, len / SZ_1K);
		...
		unsigned int len, void *buf)
{
	...
	if (from > ((end - start) * phy_blk_size))
		pr_err("bootpackage size is:0x%x, over read:0x%llx\n",
