...
#define aicbsp_syscfg_tbl_u03 aicbsp_syscfg_tbl
#define aicbsp_syscfg_tbl_u04 aicbsp_syscfg_tbl
...
static int aicbt_init(struct priv_dev *aicdev)
{
	...
	ret = rwnx_plat_bin_fw_upload_android(aicdev, btcfg.addr_adid, aicbsp_firmware_list[aicbsp_info.cpmode].bt_adid);
	if (ret)
		goto err;

	ret = rwnx_plat_bin_fw_upload_android(aicdev, btcfg.addr_patch, aicbsp_firmware_list[aicbsp_info.cpmode].bt_patch);
	if (ret)
		goto err;

	ret = aicbt_patch_table_load(aicdev, &btcfg, head);
	if (ret)
...
int aicbsp_8800d_fw_init(struct priv_dev *aicdev)
{
	...
	uint8_t dout[16];
	uint8_t chip_rev_id = 0, chip_rev_sub_id = 0;
	...
	chip_rev_id = (u8)(rd_mem_addr_cfm.memdata >> 16);

	if (rwnx_send_dbg_mem_read_req(aicdev, 0x00000004, &rd_mem_addr_cfm))
		return -1;

	chip_rev_sub_id = (u8)(rd_mem_addr_cfm.memdata >> 4);

	if (chip_rev_id == CHIP_REV_ID_U02) {
		aicbsp_info.chipinfo->rev = CHIP_REV_U02;
	} else if (chip_rev_id == CHIP_REV_ID_U03) {
		aicbsp_info.chipinfo->rev = CHIP_REV_U03;
		if (chip_rev_sub_id == CHIP_REV_SUB_ID_U04)
			aicbsp_info.chipinfo->rev = CHIP_REV_U04;
	} else {
		pr_err("aicbsp: %s, unsupport chip rev id: %d\n", __func__, chip_rev_id);
		return -1;
	}

	printk("aicbsp: %s, rev id: 0x%x, sub id: 0x%x, chip rev: U%02d\n", __func__, chip_rev_id, chip_rev_sub_id, aicbsp_info.chipinfo->rev);
...
#ifdef AICWF_SDIO_SUPPORT
	if (aicwf_sdio_writeb(aicdev->func[0], SDIOWIFI_WAKEUP_REG, 4)) {
