...
static int aic8800dc_wifi_patch_table_load(struct priv_dev *aicdev, const char *filename)
{
	...
	if (!err && (i < size)) {
		err = rwnx_send_dbg_mem_block_write_req(aicdev, FMACFW_PATCH_TBL_8800DC_U02_DESCRIBE_BASE, FMACFW_PATCH_TBL_8800DC_U02_DESCRIBE_SIZE, &dst[0]);
		if (err)
...
#endif
		ret = rwnx_send_rf_config_req(aicdev, 0, 1, (u8 *)txgain_table, 128);
		if (ret)
			goto out;

		ret = rwnx_send_rf_config_req(aicdev, 0, 0, (u8 *)rxgain_table_24g_20m, 256);
		if (ret)
			goto out;

		ret = rwnx_send_rf_config_req(aicdev, 32,  0, (u8 *)rxgain_table_24g_40m, 256);
	} else {
		if (aicbsp_info.chipinfo->subrev == 0) {
