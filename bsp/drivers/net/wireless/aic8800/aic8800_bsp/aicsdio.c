...
static int aicwf_sdio_flow_ctrl(struct priv_dev *aicdev)
{
	...
	while (true) {
		ret = aicwf_sdio_readb(aicdev->func[0], SDIOWIFI_FLOW_CTRL_REG, &fc_reg);
		if (ret) {
			return -1;
		}
...
static int aicwf_sdio_send_pkt(struct priv_dev *aicdev, u8 *buf, uint count)
{
	...
	ret = sdio_writesb(aicdev->func[0], 7, buf, count);
...
static int aicwf_sdio_send_msg(struct priv_dev *aicdev, u8 *buf, uint count)
{
	...
	sdio_claim_host(aicdev->func[0]);
	ret = sdio_writesb(aicdev->func[1], 7, buf, count);
	...
	u32 size)
{
	...
	ret = sdio_readsb(aicdev->func[1], skbbuf->data, 8, size);
	sdio_release_host(aicdev->func[1]);
...
static int aicwf_sdio_wakeup(struct priv_dev *aicdev)
{
	...
	if (aicdev->state == SDIO_SLEEP_ST) {
		...
		while (write_retry) {
			ret = aicwf_sdio_writeb(aicdev->func[0], SDIOWIFI_WAKEUP_REG, 1);
			if (ret) {
				...
				while (read_retry) {
					u8 val;
					ret = aicwf_sdio_readb(aicdev->func[0], SDIOWIFI_SLEEP_REG, &val);
					if ((ret == 0) && (val & 0x10)) {
						break;
					}
					read_retry--;
...
static int aicwf_sdio_sleep_allow(struct priv_dev *aicdev)
{
	...
	if (bus_if->state == BUS_DOWN_ST) {
		ret = aicwf_sdio_writeb(aicdev->func[0], SDIOWIFI_SLEEP_REG, 0x10);
		if (ret) {
...
static int aicwf_sdio_intr_get_len_bytemode(struct priv_dev *aicdev, u8 *byte_len)
{
	...
	if (aicdev->bus_if->state == BUS_DOWN_ST) {
		*byte_len = 0;
		...
		ret = aicwf_sdio_readb(aicdev->func[0], SDIOWIFI_BYTEMODE_LEN_REG, byte_len);
...
static int aicwf_sdio_bus_start(struct device *dev)
{
	...
	if (aicbsp_info.chipinfo->chipid == PRODUCT_ID_AIC8800D) {
		...
		ret = aicwf_sdio_writeb(aicdev->func[0], SDIOWIFI_INTR_CONFIG_REG, 0x07);
		if (ret != 0)
		...
		//enable sdio interrupt
		ret = aicwf_sdio_writeb(aicdev->func[1], SDIOWIFI_INTR_CONFIG_REG, 0x07);
...
static void aicwf_sdio_hal_irqhandler_func2(struct sdio_func *func)
{
	...
	ret = aicwf_sdio_readb(aicdev->func[1], SDIOWIFI_BLOCK_CNT_REG, &intstatus);
...
static void aicwf_sdio_release(struct priv_dev *aicdev)
{
	...
	for (i = 0; i < funcnum; i++) {
		...
		ret = aicwf_sdio_writeb(aicdev->func[i], SDIOWIFI_INTR_CONFIG_REG, 0x0);
		if (ret < 0) {
...
static int aicwf_sdio_func_init(struct priv_dev *aicdev)
{
	...
	if (aicbsp_info.chipinfo->chipid == PRODUCT_ID_AIC8800DC) {
		...
		ret = aicwf_sdio_writeb(aicdev->func[1], SDIOWIFI_REGISTER_BLOCK, block_bit0);
		if (ret < 0) {
		...
		//1: no byte mode
		ret = aicwf_sdio_writeb(aicdev->func[1], SDIOWIFI_BYTEMODE_ENABLE_REG, byte_mode_disable);
		if (ret < 0) {
	...
	ret = aicwf_sdio_writeb(aicdev->func[0], SDIOWIFI_REGISTER_BLOCK, block_bit0);
	if (ret < 0) {
		bsp_err("reg:%d write failed!\n", SDIOWIFI_REGISTER_BLOCK);
		return ret;
	}
	...
	//1: no byte mode
	ret = aicwf_sdio_writeb(aicdev->func[0], SDIOWIFI_BYTEMODE_ENABLE_REG, byte_mode_disable);
	if (ret < 0) {
		bsp_err("reg:%d write failed!\n", SDIOWIFI_BYTEMODE_ENABLE_REG);
		return ret;
	}
