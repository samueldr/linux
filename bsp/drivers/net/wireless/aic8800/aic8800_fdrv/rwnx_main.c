	...
	do {
		...
		if (strcasecmp(argv[0], "GET_RX_RESULT") == 0) {
			...
			if (setsusp_mode == 1) {
				...
				if (aicwf_sdio_writeb(g_rwnx_plat->sdiodev, SDIOWIFI_WAKEUP_REG, 2)) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
		rwnx_cfg80211_ch_switch_started_notify(dev, &csa->chandef, 0, params->count, false);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(5, 11, 0)
		rwnx_cfg80211_ch_switch_started_notify(dev, &csa->chandef, params->count, params->block_tx);
#else
...
#if IS_ENABLED(CONFIG_AW_MACADDR_MGT)
