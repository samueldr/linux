...
#if defined(ENABLE_INSMOD_NO_FW_LOAD) && !defined(BUS_POWER_RESTORE) && 0
...
sdioh_request_byte(sdioh_info_t *sd, uint rw, uint func, uint regaddr, uint8 *byte)
{
	...
	if (sd_msglevel & SDH_COST_VAL)
		before = ktime_to_timespec64(ktime_get_boottime());
	...
	if (sd_msglevel & SDH_COST_VAL) {
		...
		sd_cost(("%s: rw=%d len=1 cost=%llus %ldus\n", __FUNCTION__,
                                   ...
                                   uint32 *word, uint nbytes)
{
	...
	if (sd_msglevel & SDH_COST_VAL) {
		now = ktime_to_timespec64(ktime_get_boottime());
		sd_cost(("%s: rw=%d, len=%d cost=%llus %ldus\n", __FUNCTION__,
...
txglomfail:
	...
	if (sd_msglevel & SDH_COST_VAL) {
		now = ktime_to_timespec64(ktime_get_boottime());
		sd_cost(("%s: rw=%d, ttl_len=%d, cost=%llus %ldus\n", __FUNCTION__,
	...
	uint addr, uint reg_width, uint buf_len, uint8 *buffer, void *pkt)
{
	...
	if (sd_msglevel & SDH_COST_VAL) {
		now = ktime_to_timespec64(ktime_get_boottime());
		sd_cost(("%s: len=%d cost=%llus %ldus\n", __FUNCTION__,
