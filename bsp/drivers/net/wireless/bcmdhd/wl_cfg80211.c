...
static s32 wl_cfg80211_get_key(struct wiphy *wiphy, struct net_device *dev,
	...
	struct key_params *params));
static s32 wl_cfg80211_config_default_mgmt_key(struct wiphy *wiphy, struct net_device *dev,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 41))
	int link_id,
#endif
	u8 key_idx);
	...
	const struct mesh_setup *setup)
{
	...
	if (beacon_interval) {
		err = wldev_ioctl_set(dev, WLC_SET_BCNPRD, &beacon_interval, sizeof(s32));
		if (err < 0) {
	...
	if (dtim_period) {
		err = wldev_ioctl_set(dev, WLC_SET_DTIMPRD, &dtim_period, sizeof(s32));
		if (err < 0) {
...
static s32
wl_cfg80211_config_default_mgmt_key(struct wiphy *wiphy, struct net_device *dev,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 41))
	int link_id,
#endif
	u8 key_idx)
...
#endif
	pr_err("^^========, %s(%d), wdev->wiphy->interface_modes = %08X\n", __func__, __LINE__, wdev->wiphy->interface_modes);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
				wdev->u.ibss.ssid_len = min(ssid->SSID_len, (uint32)DOT11_MAX_SSID_LEN);
				memcpy(wdev->u.ibss.ssid, ssid->SSID, wdev->u.ibss.ssid_len);
#else
