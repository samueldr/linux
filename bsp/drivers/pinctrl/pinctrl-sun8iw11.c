...
static const struct sunxi_desc_pin sun8iw11_pins[] = {
...
/* The PH and PI irq register are placed on PA position */
static const unsigned int sun8iw11_irq_bank_map[] = {
	SUNXI_BANK_OFFSET('A', 'A'),
};

static const struct sunxi_pinctrl_desc sun8iw11_pinctrl_data = {
	.pins = sun8iw11_pins,
	.npins = ARRAY_SIZE(sun8iw11_pins),
	.irq_banks = ARRAY_SIZE(sun8iw11_irq_bank_map),
	.irq_bank_map = sun8iw11_irq_bank_map,
...
static int sun8iw11_pinctrl_probe(struct platform_device *pdev)
{
	struct resource *res;
	struct clk *bus_clk;
	int ret;
	...
	/* @TOOD:add clk disable to suspend and add clk enable to resume */
	bus_clk = devm_clk_get(&pdev->dev, "bus");
	if (IS_ERR(bus_clk)) {
		dev_err(&pdev->dev, "fail to get bus_clk\n");
		return PTR_ERR(bus_clk);
	}
	ret = clk_prepare_enable(bus_clk);
	if (ret) {
		dev_err(&pdev->dev, "fail to enable bus_clk\n");
		return ret;
	}
	...
	return sunxi_bsp_pinctrl_init(pdev, &sun8iw11_pinctrl_data);
}

static int __maybe_unused sun8iw11_pinctrl_suspend_noirq(struct device *dev)
{
...
static int __maybe_unused sun8iw11_pinctrl_resume_noirq(struct device *dev)
{
	struct sunxi_pinctrl *pctl = dev_get_drvdata(dev);
	unsigned long flags;
...
static struct of_device_id sun8iw11_pinctrl_match[] = {
	{ .compatible = "allwinner,sun8iw11-pinctrl", },
	{}
};
MODULE_DEVICE_TABLE(of, sun8iw11_pinctrl_match);

static const struct dev_pm_ops sun8iw11_pinctrl_pm_ops = {
	.suspend_noirq = sun8iw11_pinctrl_suspend_noirq,
	.resume_noirq = sun8iw11_pinctrl_resume_noirq,
};

static struct platform_driver sun8iw11_pinctrl_driver = {
	.probe	= sun8iw11_pinctrl_probe,
	.driver	= {
		.name		= "sun8iw11-pinctrl",
		.pm = &sun8iw11_pinctrl_pm_ops,
		.of_match_table	= sun8iw11_pinctrl_match,
	},
};

static int __init sun8iw11_pio_init(void)
{
	return platform_driver_register(&sun8iw11_pinctrl_driver);
}
postcore_initcall(sun8iw11_pio_init);
