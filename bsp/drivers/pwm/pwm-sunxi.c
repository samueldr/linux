...
static void sunxi_pwm_resource_put(struct sunxi_pwm_chip *pc)
{
}

static int sunxi_pwm_clk_enable(struct platform_device *pdev,
			     struct sunxi_pwm_chip *pc)
{
	int err;
...
static void sunxi_pwm_clk_disable(struct sunxi_pwm_chip *pc)
{
	if (pc->data->has_clock) {
			     ...
			     struct sunxi_pwm_chip *pc)
{
	...
	err = sunxi_pwm_clk_enable(pdev, pc);
	if (err) {
