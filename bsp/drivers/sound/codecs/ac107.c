			   ...
			   struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_FORMAT_S32_LE:
		...
		break;
	default:
		dev_err(dai->dev, "ac107 unsupport the sample bit: %u\n", sample_bit);
		return -EINVAL;
	}
...
static int ac107_probe(struct snd_soc_component *component)
{
	...
	/* I2S: Turn to hi-z state (TDM) when not transferring slot */
	...
	regmap_write(regmap, I2S_TX_CHMP_CTRL1, pdata->rx_chmap & 0xff);
	regmap_write(regmap, I2S_TX_CHMP_CTRL2, (pdata->rx_chmap >> 8) & 0xff);
...
static int ac107_suspend(struct snd_soc_component *component)
{
	...
	regcache_cache_only(ac107->regmap, true);
	regcache_mark_dirty(ac107->regmap);
...
static int ac107_resume(struct snd_soc_component *component)
{
	...
	regcache_cache_only(ac107->regmap, false);
	regcache_sync(ac107->regmap);
...
static const struct snd_kcontrol_new ac107_snd_controls[] = {
	...
	SOC_SINGLE_TLV("ADC2 Digital Volume", ADC2_DVOL_CTRL, DIG_ADCL2_VOL,
		       0xff, 0, digital_vol_tlv),
	SOC_SINGLE_TLV("ADC1 PGA Gain", ANA_ADC1_CTRL3, RX1_PGA_GAIN_CTRL,
		       0x1f, 0, adc_pga_gain_tlv),
	SOC_SINGLE_TLV("ADC2 PGA Gain", ANA_ADC2_CTRL3, RX2_PGA_GAIN_CTRL,
		       0x1f, 0, adc_pga_gain_tlv),
};
...
static const struct snd_soc_dapm_widget ac107_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA("ADC1 PGA", ADC_DIG_EN, ENAD1, 0, NULL, 0),
	SND_SOC_DAPM_PGA("ADC2 PGA", ADC_DIG_EN, ENAD2, 0, NULL, 0),
...
static const struct snd_soc_dapm_route ac107_dapm_routes[] = {
	...
	{"ADC1 PGA", NULL, "MIC1 PGA"},
	{"ADC2 PGA", NULL, "MIC2 PGA"},

	{"ADC1 MIXER", "ADC1 Switch", "ADC1 PGA"},
	{"ADC1 MIXER", "ADC2 Switch", "ADC2 PGA"},
	{"ADC2 MIXER", "ADC1 Switch", "ADC1 PGA"},
	{"ADC2 MIXER", "ADC2 Switch", "ADC2 PGA"},
...
static const struct snd_soc_component_driver soc_component_dev_ac107 = {
	...
	.resume			= ac107_resume,
	.controls		= ac107_snd_controls,
	.num_controls		= ARRAY_SIZE(ac107_snd_controls),
...
static void ac107_set_params_from_of(struct i2c_client *i2c, struct ac107_data *pdata)
{
	...
	int i, ret;
	const char *str;
	struct of_clksrc_tavle {
		char *name;
		unsigned int val;
	};

	struct of_clksrc_tavle of_pllclk_src_table[] = {
		{ "MCLK",	PLLCLK_SRC_MCLK },
		{ "BCLK",	PLLCLK_SRC_BCLK },
		{ "PDMCLK",	PLLCLK_SRC_PDMCLK },
	};
	struct of_clksrc_tavle of_sysclk_src_table[] = {
		{ "MCLK",	SYSCLK_SRC_MCLK },
		{ "BCLK",	SYSCLK_SRC_BCLK },
		{ "PLL",	SYSCLK_SRC_PLL },
	};
	struct of_clksrc_tavle of_pcm_bit_first_table[] = {
		{ "MSB",	0 },
		{ "LSB",	1 },
	};
	...
	ret = of_property_read_u32(np, "rx-chmap", &pdata->rx_chmap);
	if (ret < 0)
		pdata->rx_chmap = 0xaaaa;
...
MODULE_AUTHOR("Marie Liu <marie@allwinnertech.com>");
MODULE_LICENSE("GPL");
