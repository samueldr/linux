/*
 * sound\soc\sunxi\snd_sunxi_pcm_hdmi.c
...
#define HLOG				"PCMHDMI"
#define SUNXI_DMAENGINE_PCM_DRV_NAME	"sunxi_dmaengine_pcm_hdmi"
...
static int sunxi_pcm_open(struct snd_soc_component *component, struct snd_pcm_substream *substream)
{
	int ret = 0;
...
static int sunxi_pcm_close(struct snd_soc_component *component, struct snd_pcm_substream *substream)
{
	SND_LOG_DEBUG(HLOG, "\n");
...
static int sunxi_pcm_ioctl(struct snd_soc_component *component,
...
static int sunxi_pcm_hw_params_raw(struct snd_soc_component *component,
				   struct snd_pcm_substream *substream,
...
static int sunxi_pcm_hw_free_raw(struct snd_soc_component *component,
				 struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct device *dev = rtd->dev;
...
static int sunxi_pcm_prepare_raw(struct snd_soc_component *component,
				 struct snd_pcm_substream *substream)
{
...
static int sunxi_pcm_trigger_raw(struct snd_soc_component *component,
				 struct snd_pcm_substream *substream,
				 int cmd)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static snd_pcm_uframes_t sunxi_pcm_pointer_raw(struct snd_soc_component *component,
					       struct snd_pcm_substream *substream)
{
	if (g_pcm.hdmi_fmt > HDMI_FMT_PCM) {
...
static int sunxi_pcm_copy_raw(struct snd_soc_component *component,
...
static int sunxi_pcm_construct(struct snd_soc_component *component, struct snd_soc_pcm_runtime *rtd)
{
	int ret;
...
static void sunxi_pcm_destruct(struct snd_soc_component *component, struct snd_pcm *pcm)
{
	int stream;
...
int snd_sunxi_hdmi_platform_register(struct device *dev)
{
	SND_LOG_DEBUG(HLOG, "\n");
	return snd_soc_register_component(dev, &sunxi_soc_platform_raw, NULL, 0);
}
EXPORT_SYMBOL_GPL(snd_sunxi_hdmi_platform_register);

void snd_sunxi_hdmi_platform_unregister(struct device *dev)
{
	SND_LOG_DEBUG(HLOG, "\n");
	snd_soc_unregister_component(dev);
}
EXPORT_SYMBOL_GPL(snd_sunxi_hdmi_platform_unregister);

MODULE_AUTHOR("Dby@allwinnertech.com");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("sunxi ASoC PCM HDMI driver");
