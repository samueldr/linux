...
#if !defined(CONFIG_ARCH_SUN8IW20) && !defined(CONFIG_ARCH_SUN20IW1) && !defined(CONFIG_ARCH_SUN50IW12) && !defined(CONFIG_ARCH_SUN55IW3)
		...
		struct sunxi_hci_hcd *sunxi_hci)
{
	...
	if (ret) {
		DMSG_PRINT("get drvvbus-en is fail, %d\n", -ret);
		sunxi_hci->drvvbus_en_gpio_valid = 0;
	} else {
		/* get drvvbus-en gpio */
		sunxi_hci->drvvbus_en_gpio_set.gpio =
