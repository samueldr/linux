...
int enable_google_vp9_hw_clk(void)
{
	...
	if (clk_prepare_enable(google_vp9_devp->bus_ve_clk)) {
		google_vp9_printk(KERN_ERR, "enable bus ve clk gating failed;\n");
		goto out;
	}

	if (clk_prepare_enable(google_vp9_devp->bus_av1_clk)) {
		google_vp9_printk(KERN_ERR, "enable bus clk gating failed;\n");
		goto out;
	}

	if (clk_prepare_enable(google_vp9_devp->mbus_av1_clk)) {
		google_vp9_printk(KERN_ERR, "enable mbus clk gating failed;\n");
		goto out;
	}

	if (clk_prepare_enable(google_vp9_devp->av1_clk)) {
		google_vp9_printk(KERN_ERR, "enable ve clk gating failed;\n");
		goto out;
	}
	res = 0;
	AW_MEM_INIT_LIST_HEAD(&google_vp9_devp->list);
	google_vp9_printk(KERN_DEBUG, "vp9 clk enable!\n");
out:
...
int disable_google_vp9_hw_clk(void)
{
	...
	res = 0;
	google_vp9_printk(KERN_DEBUG, "vp9 clk disable!\n");
		...
		unsigned long arg)
{
	...
	case VP9_IOCTL_RESET:
		google_vp9_printk(KERN_DEBUG, "vp9 reset!\n");
	...
	case VP9_IOCTL_SET_FREQ:
		{
			...
			google_vp9_printk(KERN_DEBUG, "real_freq:%ld", ret);
			break;
		}
...
static int create_char_device(void)
{
	...
	if (ret < 0) {
		google_vp9_printk(KERN_WARNING, "cedar_dev: can't get major %d\n", vp9_dev_major);
		return ret;
	}
	...
	if (ret) {
		google_vp9_printk(KERN_WARNING, "Err:%d add cedar-dev fail\n", ret);
		goto region_del;
	}
...
static int set_ccmu_by_shelf(char *ccmu_base)
{
	 ...
	 *
	 * if (count >= 10 && reg == 0) {
	 *    google_vp9_printk(KERN_WARNING,"Err: clock is unlock\n");
	 *	return -1;
	 * }
	 */
...
static int googleVp9dev_init(struct platform_device *pdev)
{
	...
	google_vp9_printk(KERN_WARNING, "[google av1]: install start!!!\n");
	...
	if (google_vp9_devp->irq <= 0) {
		google_vp9_printk(KERN_WARNING, "Can't parse IRQ, use defualt!!!");
	    ret = -EINVAL;
		goto free_devp;
	}
	...
	if (!google_vp9_devp->iomap_addrs.regs_ccmu) {
		google_vp9_printk(KERN_WARNING, "vp9 Can't map ccmu registers");
		ret = -EINVAL;
		goto free_devp;
	}
	...
	if (IS_ERR(google_vp9_devp->av1_clk)) {
		google_vp9_printk(KERN_WARNING, "try to get ve clk fail\n");
		ret = -EINVAL;
		goto free_devp;
	}
	...
	if (IS_ERR(google_vp9_devp->bus_av1_clk)) {
		google_vp9_printk(KERN_WARNING, "try to get bus av1 clk fail\n");
		ret = -EINVAL;
		goto free_devp;
	}
	...
	if (IS_ERR(google_vp9_devp->bus_ve_clk)) {
		google_vp9_printk(KERN_WARNING, "try to get bus ve clk fail\n");
		ret = -EINVAL;
		goto free_devp;
	}
	...
	if (IS_ERR(google_vp9_devp->mbus_av1_clk)) {
		google_vp9_printk(KERN_WARNING, "try to get mbus clk fail\n");
		ret = -EINVAL;
		goto free_devp;
	}
	...
	if (IS_ERR(google_vp9_devp->reset)) {
		google_vp9_printk(KERN_WARNING, "get reset fail\n");
		ret = -EINVAL;
		goto free_devp;
	}
	...
	if (IS_ERR(google_vp9_devp->reset_ve)) {
		google_vp9_printk(KERN_WARNING, "get reset all fail\n");
		ret = -EINVAL;
		goto free_devp;
	}

	google_vp9_printk(KERN_WARNING, "[google av1]: install end!!!\n");
	return 0;
free_devp:
...
{
	google_vp9_printk(KERN_WARNING, "sunxi google vp9 version 1.0\n");
