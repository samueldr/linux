...
static int vfe_device_regulator_get(struct device *dev, struct ccm_config *ccm_cfg, int num);
...
//	unsigned int i;
	...
	ret = dev->sd->ops->pad->get_fmt(dev->sd, NULL, fmat);
	if (ret < 0)
		vfe_err("get_fmt from sensor error!\n");

	vfe_print("get mbus_code from sensor 0x%x!\n", fmat->format.code);
							...
							struct v4l2_format *f)
{
	...
	bus_pix_code = (u32)try_fmt_internal(dev, f);
	if (!bus_pix_code) {
		...
		struct v4l2_format *f)
{
	...
	ret = v4l2_subdev_call(dev->sd, pad, get_mbus_config, 0, &mbus_cfg);
	if (ret < 0) {
		vfe_err("v4l2 sub device sensor g_mbus_config error!\n");
		goto out;
	}

	ret = dev->csi_sd->ops->pad->set_mbus_config(dev->csi_sd, 0, &mbus_cfg);
//	ret = v4l2_subdev_call(dev->csi_sd, pad, set_mbus_config, 0, &mbus_cfg);
	if (ret < 0) {
	...
	if (mbus_cfg.type == V4L2_MBUS_CSI2_DPHY) {
		ret = v4l2_subdev_call(dev->mipi_sd, pad, set_mbus_config, 0, &mbus_cfg);
		if (ret < 0) {
...
//	ret = v4l2_subdev_call(dev->sd, pad, set_fmt, NULL, &fmat);
	ret = dev->sd->ops->pad->set_fmt(dev->sd, 0, &fmat);
	if (ret < 0) {
...
//	ret = v4l2_subdev_call(dev->csi_sd, pad, set_fmt, NULL, &csi_fmt);
	ret = dev->csi_sd->ops->pad->set_fmt(dev->csi_sd, 0, &csi_fmt);
	if (ret < 0) {
...
static int vidioc_qbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	return vb2_qbuf(&dev->vb_vidq[dev->cur_ch], dev->v4l2_dev.mdev, p);
}
...
static int __vfe_streamon(struct vfe_dev *dev, void *priv, enum v4l2_buf_type i)
{
	...
	if (dev->mbus_type == V4L2_MBUS_CSI2_DPHY)
		bsp_mipi_csi_protocol_enable(dev->mipi_sel);
#endif
streamon_unlock:
...
static int internal_s_input(struct vfe_dev *dev, unsigned int i)
{
	...
	if (dev->is_isp_used) {
		...
		if (ret != 0)
			vfe_warn("sensor V4L2_CID_VFLIP not defined when vidioc_s_input!input_num = %d\n", i);

		ctrl.id = V4L2_CID_HFLIP;
		ctrl.value = dev->ccm_cfg[i]->hflip;
		ret = v4l2_s_ctrl(NULL, dev->sd->ctrl_handler, &ctrl);
		if (ret != 0)
			vfe_warn("sensor V4L2_CID_HFLIP not defined when vidioc_s_input!input_num = %d\n", i);
	}
						...
						struct v4l2_streamparm *parms)
{
	...
	ret = sensor_g_parm(dev->sd, parms);
	if (ret < 0)
						...
						struct v4l2_streamparm *parms)
{
	...
	ret = sensor_s_parm(dev->sd, parms);
	if (ret < 0)
		vfe_warn("v4l2 sub device s_parm error!\n");

	ret = sunxi_csi_subdev_s_parm(dev->csi_sd, parms);
	if (ret < 0)
		vfe_warn("v4l2 sub device s_parm error!\n");
...
static int vfe_set_sensor_power_on(struct vfe_dev *dev)
{
	...
	vfe_device_regulator_get(&dev->pdev->dev, dev->ccm_cfg[dev->input], dev->input);
...
static int vfe_device_regulator_get(struct device *dev, struct ccm_config *ccm_cfg, int num)
{
	...
	snprintf(ccm_cfg->iovdd_str, sizeof(ccm_cfg->iovdd_str), "csi%d_dev%d_iovdd", pdev->id, num);
	if (strcmp(ccm_cfg->iovdd_str, "")) {
		ccm_cfg->power.iovdd = regulator_get(dev, ccm_cfg->iovdd_str);
		if (IS_ERR_OR_NULL(ccm_cfg->power.iovdd)) {
			vfe_err("get regulator csi_iovdd error!\n");
			goto regulator_get_err;
		}
	}
	snprintf(ccm_cfg->avdd_str, sizeof(ccm_cfg->avdd_str), "csi%d_dev%d_avdd", pdev->id, num);
	if (strcmp(ccm_cfg->avdd_str, "")) {
		ccm_cfg->power.avdd = regulator_get(dev, ccm_cfg->avdd_str);
		if (IS_ERR_OR_NULL(ccm_cfg->power.avdd)) {
			vfe_err("get regulator csi_avdd error!\n");
			goto regulator_get_err;
		}
	}
	snprintf(ccm_cfg->dvdd_str, sizeof(ccm_cfg->dvdd_str), "csi%d_dev%d_dvdd", pdev->id, num);
	if (strcmp(ccm_cfg->dvdd_str, "")) {
		ccm_cfg->power.dvdd = regulator_get(dev, ccm_cfg->dvdd_str);
		if (IS_ERR_OR_NULL(ccm_cfg->power.dvdd)) {
			vfe_err("get regulator csi_dvdd error!\n");
			goto regulator_get_err;
		}
	}
	snprintf(ccm_cfg->afvdd_str, sizeof(ccm_cfg->afvdd_str), "csi%d_dev%d_afvdd", pdev->id, num);
	if (strcmp(ccm_cfg->afvdd_str, "")) {
		ccm_cfg->power.afvdd = regulator_get(dev, ccm_cfg->afvdd_str);
		if (IS_ERR_OR_NULL(ccm_cfg->power.afvdd)) {
			vfe_err("get regulator csi_afvdd error!\n");
			goto regulator_get_err;
		}
	}
	snprintf(ccm_cfg->flvdd_str, sizeof(ccm_cfg->flvdd_str), "csi%d_dev%d_flvdd", pdev->id, num);
	if (strcmp(ccm_cfg->flvdd_str, "")) {
		ccm_cfg->power.flvdd = regulator_get(dev, ccm_cfg->flvdd_str);
...
#ifdef _REGULATOR_CHANGE_
#else
		if (vfe_device_regulator_get(&dev->pdev->dev, dev->ccm_cfg[input_num], input_num)) {
	...
	for (i = 0; i < video_cnt; i++) {
		...
		*vfd = vfe_template[dev->id];
		...
		if (device_valid_count != 0) {
			...
			ret = video_register_device(vfd, VFL_TYPE_VIDEO, video_num);
			if (ret < 0) {
