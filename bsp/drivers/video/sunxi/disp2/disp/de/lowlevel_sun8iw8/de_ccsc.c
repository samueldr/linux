...
static int de_ccsc_set_reg_base(unsigned int sel, unsigned int chno,
				unsigned int base)
{
...
static int de_icsc_set_reg_base(unsigned int sel, unsigned int chno,
				unsigned int base)
{
	__inf("sel=%d, chno=%d, base=0x%x\n", sel, chno, base);
		...
		for (ch_id = 0; ch_id < vi_num[screen_id]; ch_id++) {
			...
			if (screen_id == 0) {
				...
				if (vep_support[screen_id][ch_id]) {
					base_ofst =
					    (ch_id ==
					     0) ? CCSC00_OFST : CCSC01_OFST;
				} else {
					base_ofst =
					    (ch_id ==
					     0) ? CCSC10_OFST : CCSC11_OFST;
				}
			}
			...
			__inf("sel %d, Ccsc_base[%d]=0x%x\n", screen_id, ch_id,
			      base);

			memory = disp_sys_malloc(sizeof(__csc_reg_t));
			if (NULL == memory) {
				__wrn
				    ("malloc Ccsc[%d][%d] memory fail! size=0x%x\n",
				     screen_id, ch_id, sizeof(__csc_reg_t));
				return -1;
			}
			...
			de_ccsc_set_reg_base(screen_id, ch_id,
					     (unsigned int)memory);

			/* input csc */
			if (vep_support[screen_id][ch_id]) {
				base_ofst =
				    (ch_id == 0) ? ICSC0_OFST : ICSC1_OFST;

				base =
				    para->reg_base[DISP_MOD_DE] + (screen_id +
								   1) *
				    0x00100000 + FCE_OFST + 0x40;

				__inf("sel %d, Icsc_base[%d]=0x%x\n", screen_id,
				      ch_id, base);
...
static __s32 iDE_SCAL_Matrix_Mul(__scal_matrix4x4 *in1, __scal_matrix4x4 in2,
		      ...
		      unsigned int out_color_range, int *csc_coeff)
{
	...
	/* BYPASS */
	if (infmt == outfmt && incscmod == outcscmod && out_color_range == 0
		...
		if (infmt == DE_RGB) {
			...
			if (out_color_range == DISP_COLOR_RANGE_16_235) {
				if (incscmod == DE_BT601) {
					coeff[oper] =
					    (__scal_matrix4x4 *) (y2r + 0x80);
					oper++;
				} else if (incscmod == DE_BT709) {
					coeff[oper] =
					    (__scal_matrix4x4 *) (y2r + 0xa0);
					oper++;
				}
			} else {
				if (incscmod == DE_BT601) {
					coeff[oper] =
					    (__scal_matrix4x4 *) (y2r);
					oper++;
				} else if (incscmod == DE_BT709) {
					coeff[oper] =
					    (__scal_matrix4x4 *) (y2r + 0x20);
					oper++;
				} else if (incscmod == DE_YCC) {
					coeff[oper] =
					    (__scal_matrix4x4 *) (y2r + 0x40);
					oper++;
				} else if (incscmod == DE_ENHANCE) {
					coeff[oper] =
					    (__scal_matrix4x4 *) (y2r + 0x60);
					oper++;
				}
			}
		}
