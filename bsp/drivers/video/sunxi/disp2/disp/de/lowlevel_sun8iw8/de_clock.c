...
static de_clk_para de_clk_tbl[] = {
	...
	DE_TOP_CFG(DE_CLK_CORE0, 150000000, 0x04, 0, 0x08, 0, 0x00, 32, 0x00, 0,
		   0x0c, 0, 4)
	    DE_TOP_CFG(DE_CLK_CORE1, 150000000, 0x04, 1, 0x08, 1, 0x00, 32,
		       0x00, 1, 0x0c, 4, 4)
	    DE_TOP_CFG(DE_CLK_WB, 150000000, 0x04, 2, 0x08, 2, 0x00, 32, 0x00,
		       2, 0x0c, 8, 4)
};
		...
		if ((de_clk_tbl[i].clk_no == clk_no)) {
			if (enable) {
				...
				if (de_clk_tbl[i].ahb_reset_shift < 32) {
					reg_val =
					    readl(de_clk_tbl[i].ahb_reset_adr +
						  de_base);
					reg_val =
					    SET_BITS(de_clk_tbl[i].
						     ahb_reset_shift, 1,
						     reg_val, 1);
					writel(reg_val,
					       de_clk_tbl[i].ahb_reset_adr +
					       de_base);
				...
				if (de_clk_tbl[i].ahb_gate_shift < 32) {
					reg_val =
					    readl(de_clk_tbl[i].ahb_gate_adr +
						  de_base);
					reg_val =
					    SET_BITS(de_clk_tbl[i].
						     ahb_gate_shift, 1, reg_val,
						     1);
					writel(reg_val,
					       de_clk_tbl[i].ahb_gate_adr +
					       de_base);
