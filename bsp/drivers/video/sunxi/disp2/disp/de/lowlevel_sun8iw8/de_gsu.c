...
int de_gsu_init(unsigned int sel, unsigned int reg_base)
{
	...
	for (j = 0; j < chno; j++) {
		...
		if (NULL == memory) {
			__wrn
			    ("malloc ui scaler[%d][%d] memory fail! size=0x%x\n",
...
int de_gsu_set_reg_base(unsigned int sel, unsigned int chno, void *base)
{
	...
	gsu_dev[sel][chno] = (__gsu_reg_t *) base;	/* TBD */
...
static unsigned int de_gsu_calc_fir_coef(unsigned int step)
{
	...
	fir_coef_ofst = (int_part == 0) ? GSU_ZOOM0_SIZE :
	    ...
	    (int_part ==
	     2) ? GSU_ZOOM0_SIZE + GSU_ZOOM1_SIZE +
	    (float_part >> 1) : (int_part ==
				 3) ? GSU_ZOOM0_SIZE + GSU_ZOOM1_SIZE +
	    GSU_ZOOM2_SIZE : (int_part ==
			      4) ? GSU_ZOOM0_SIZE + GSU_ZOOM1_SIZE +
	    GSU_ZOOM2_SIZE + GSU_ZOOM3_SIZE : GSU_ZOOM0_SIZE + GSU_ZOOM1_SIZE +
	    GSU_ZOOM2_SIZE + GSU_ZOOM3_SIZE + GSU_ZOOM4_SIZE;
			    ...
			    scaler_para *para)
{
	...
	crop_fix->w =
	    (unsigned int)(((crop.w & 0xffffffff) + (crop.x & 0xffffffff)) >=
			   N2_POWER(1,
				    GSU_FB_FRAC_BITWIDTH) ? ((crop.
							      w >>
							      GSU_FB_FRAC_BITWIDTH)
							     +
							     1) : (crop.
								   w >>
								   GSU_FB_FRAC_BITWIDTH));
		      ...
		      int gsu_sel)
{
	...
	shift =
	    (gsu_sel == 0) ? VSU_PHASE_FRAC_BITWIDTH : GSU_PHASE_FRAC_BITWIDTH;
	half = (1 << (shift - 1));
