...
static unsigned int *pttab[DEVICE_NUM];	/* POINTER of LGC tab */

static unsigned int smbl_hw_base[DEVICE_NUM] = { 0 };
...
*Date :       xx/xx/xx
**********************************************************************************************************/
static unsigned int PWRSAVE_CORE(u32 sel)
{
	s32 i;
...
int de_smbl_update_regs(unsigned int sel)
{
	...
	if (smbl_enable_block[sel].dirty == 0x1) {
		reg_val = get_wvalue(smbl_ctrl_block[sel].off);
		reg_val &= 0x2;
		reg_val |= *((volatile u32 *)smbl_ctrl_block[sel].val);
		put_wvalue(smbl_ctrl_block[sel].off, reg_val);
	}
	return 0;
}
