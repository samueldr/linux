...
int de_fce_update_regs(unsigned int sel, unsigned int chno)
{
	...
	if (fce_para_block[sel][chno].dirty == 0x1) {
		aw_memcpy_toio((void *)fce_para_block[sel][chno].off,
	...
	if (fce_celut_block[sel][chno].dirty == 0x1) {
		...
		/* AHB access CE LUT */
		aw_memcpy_toio((void *)fce_celut_block[sel][chno].off,
...
int de_fce_init(unsigned int sel, unsigned int chno, uintptr_t reg_base)
{
	...
	spin_lock_init(&g_ce_status[sel][chno]->fce_lock);
	return 0;
}
		    ...
		    unsigned int *sum)
{
	...
	aw_memcpy_toio((unsigned char *)hist,
...
{
	aw_memcpy_toio(fce_celut_block[sel][chno].val, (unsigned char *)ce_lut,
		    ...
		    unsigned int frame_cnt)
{
	if ((g_hist_status[screen_id][chno]->isenable)
		...
		|| (HIST_FRAME_MASK == 0x2))) {
		aw_memcpy_toio((unsigned char *)g_hist_p[screen_id][chno],
		...
		unsigned int auto_contrast_dirty)
{
	...
	if (ce_en == 1 && auto_contrast_dirty) {
		...
		if (b_automode) {
			...
			spin_lock_irqsave(&g_ce_status[screen_id][chno]->fce_lock, flags);
			g_ce_status[screen_id][chno]->clr_hist_res = 1;
			spin_unlock_irqrestore(&g_ce_status[screen_id][chno]->fce_lock, flags);
		  ...
		  unsigned int frame_cnt)
{
	...
	spin_lock_irqsave(&g_ce_status[screen_id][chno]->fce_lock, flags);
	if (g_ce_status[screen_id][chno]->clr_hist_res) {
		g_ce_status[screen_id][chno]->clr_hist_res = 0;
