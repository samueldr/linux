...
int de_rtmx_update_regs(unsigned int sel)
{
	...
	if (glb_ctl_block[sel].dirty == 0x1) {
		aw_memcpy_toio((void *)glb_ctl_block[sel].off, glb_ctl_block[sel].val,
	...
	if (dither_ctrl_block[sel].dirty == 0x1) {
		aw_memcpy_toio((void *)dither_ctrl_block[sel].off, dither_ctrl_block[sel].val,
	...
	for (j = 0; j < vi_chno; j++) {
		for (i = 0; i < layno; i++) {
			if (vi_attr_block[sel][j][i].dirty == 0x1) {
				aw_memcpy_toio((void *)vi_attr_block[sel][j][i].off,
		...
		if (vi_fc_block[sel][j].dirty == 0x1) {
			aw_memcpy_toio((void *)vi_fc_block[sel][j].off,
		...
		if (vi_haddr_block[sel][j].dirty == 0x1) {
			aw_memcpy_toio((void *)vi_haddr_block[sel][j].off,
		...
		if (vi_size_block[sel][j].dirty == 0x1) {
			aw_memcpy_toio((void *)vi_size_block[sel][j].off,
		...
		if (lbc_block[sel][j].dirty == 0x1) {
			aw_memcpy_toio((void *)lbc_block[sel][j].off,
			       lbc_block[sel][j].val,
		...
		if (vi_palette_block[sel][j].dirty == 0x1) {
			aw_memcpy_toio((void *)vi_palette_block[sel][j].off,
	...
	for (j = 0; j < ui_chno; j++) {
		for (i = 0; i < layno; i++) {
			if (ui_attr_block[sel][j][i].dirty == 0x1) {
				aw_memcpy_toio((void *)ui_attr_block[sel][j][i].off,
		...
		if (ui_haddr_block[sel][j].dirty == 0x1) {
			aw_memcpy_toio((void *)ui_haddr_block[sel][j].off,
		...
		if (ui_size_block[sel][j].dirty == 0x1) {
			aw_memcpy_toio((void *)ui_size_block[sel][j].off,
		...
		if (ui_palette_block[sel][j].dirty == 0x1) {
			aw_memcpy_toio((void *)ui_palette_block[sel][j].off,
	...
	if (bld_attr_block[sel].dirty == 0x1) {
		aw_memcpy_toio((void *)bld_attr_block[sel].off, bld_attr_block[sel].val,
		       bld_attr_block[sel].size);
		bld_attr_block[sel].dirty = 0;
	}

	if (bld_ctl_block[sel].dirty == 0x1) {
		aw_memcpy_toio((void *)bld_ctl_block[sel].off, bld_ctl_block[sel].val,
		       bld_ctl_block[sel].size);
		bld_ctl_block[sel].dirty = 0;
	}

	if (bld_ck_block[sel].dirty == 0x1) {
		aw_memcpy_toio((void *)bld_ck_block[sel].off, bld_ck_block[sel].val,
		       bld_ck_block[sel].size);
		bld_ck_block[sel].dirty = 0;
	}

	if (bld_out_block[sel].dirty == 0x1) {
		aw_memcpy_toio((void *)bld_out_block[sel].off, bld_out_block[sel].val,
			...
			void *data, unsigned int num)
{
	...
	if (num == 0)
		return 0;
	if (chno >= vi_chno) {
		aw_memcpy_toio(de200_rtmx[sel].ui_palette[chno - vi_chno], data, num * 4);
		...
		aw_memcpy_toio(de200_rtmx[sel].vi_palette[chno], data, num * 4);
...
int de_rtmx_sync_hw(unsigned int sel)
{
	...
	aw_memcpy_fromio(glb_ctl_block[sel].val, (void *)glb_ctl_block[sel].off,
	       glb_ctl_block[sel].size);

	for (j = 0; j < vi_chno; j++) {
		for (i = 0; i < layno; i++) {
			aw_memcpy_fromio(vi_attr_block[sel][j][i].val,
			       (void *)vi_attr_block[sel][j][i].off,
			       vi_attr_block[sel][j][i].size);
		}

		aw_memcpy_fromio(vi_fc_block[sel][j].val,
		    (void *)vi_fc_block[sel][j].off,
		    vi_fc_block[sel][j].size);

		aw_memcpy_fromio(vi_haddr_block[sel][j].val,
		    (void *)vi_haddr_block[sel][j].off,
		    vi_haddr_block[sel][j].size);

		aw_memcpy_fromio(vi_size_block[sel][j].val,
	...
	for (j = 0; j < ui_chno; j++) {
		for (i = 0; i < layno; i++) {
			aw_memcpy_fromio(ui_attr_block[sel][j][i].val,
			    (void *)ui_attr_block[sel][j][i].off,
			    ui_attr_block[sel][j][i].size);
		}

		aw_memcpy_fromio(ui_haddr_block[sel][j].val,
		    (void *)ui_haddr_block[sel][j].off,
		    ui_haddr_block[sel][j].size);

		aw_memcpy_fromio(ui_size_block[sel][j].val,
		    (void *)ui_size_block[sel][j].off,
		    ui_size_block[sel][j].size);
	}

	aw_memcpy_fromio(bld_attr_block[sel].val,
	    (void *)bld_attr_block[sel].off,
	    bld_attr_block[sel].size);

	aw_memcpy_fromio(bld_ctl_block[sel].val,
	    (void *)bld_ctl_block[sel].off,
	    bld_ctl_block[sel].size);

	aw_memcpy_fromio(bld_ck_block[sel].val,
	    (void *)bld_ck_block[sel].off,
	    bld_ck_block[sel].size);

	aw_memcpy_fromio(bld_out_block[sel].val,
