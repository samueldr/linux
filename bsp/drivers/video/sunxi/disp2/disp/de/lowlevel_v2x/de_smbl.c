...
int de_smbl_update_regs(unsigned int sel)
{
	...
	if (smbl_ctrl_block[sel].dirty == 0x1) {
		aw_memcpy_toio((void *)smbl_ctrl_block[sel].off,
...
int de_smbl_set_para(unsigned int sel, unsigned int width, unsigned int height)
{
	...
	/* out_csc coeff */
	aw_memcpy_toio((void *)smbl_csc_block[sel].off,
	       (unsigned char *)csc_bypass_coeff, sizeof(unsigned int) * 12);

	/* filter coeff */
	aw_memcpy_toio((void *)smbl_filter_block[sel].off,
...
int de_smbl_set_lut(unsigned int sel, unsigned short *lut)
{
	...
	aw_memcpy_toio((void *)smbl_lut_block[sel].off, (unsigned char *)lut,
...
{
	/* Read histogram */
	aw_memcpy_toio((unsigned char *)cnt, (unsigned char *)smbl_hist_block[sel].off,
