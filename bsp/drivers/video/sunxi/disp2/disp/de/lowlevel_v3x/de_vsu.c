...
int de_vsu_init(unsigned int sel, uintptr_t reg_base)
{
	...
	for (j = 0; j < chno; j++) {
		...
		if (ed_support[sel][j]) {
			...
			vsu_dev[sel][j]->edictrl.dwval = 0x00000000;
	...
	if (*ovlw != 0 && *ovlh != 0 && bld_rect->w != 0 && bld_rect->h != 0
		...
		if (*ovlw < SC_MIN_WIDTH || bld_rect->w < SC_MIN_WIDTH) {
			...
			if (step->hstep > (1 << shift)) {
				...
				temp = (unsigned long long)(*ovlw) << shift;
				do_div(temp, SC_MIN_WIDTH);
				step->hstep = (unsigned int)temp;
			} else {
				/* scale up */
				*ovlw = SC_MIN_WIDTH;
		...
		if (*ovlh < SC_MIN_HEIGHT || bld_rect->h < SC_MIN_HEIGHT) {
			...
			if (step->vstep > (1 << shift)) {
				...
				temp = (unsigned long long)(*ovlh) << shift;
				do_div(temp, SC_MIN_HEIGHT);
				step->vstep = (unsigned int)temp;
			} else {
				/* scale up */
				*ovlh = SC_MIN_HEIGHT;
