...
{
	int ret;
	int hw_id = hw_private.fb_share.smooth_display_hw_id;
	int fb_stride = hw_private.fb_share.logo.logo_fb.stride;
	int fb_height = hw_private.fb_share.logo.logo_fb.height;
	unsigned long phy_addr = (unsigned long)hw_private.fb_share.logo.logo_fb.phy_addr;
	int count = atomic_read(&hw_private.fb_wait[hw_id].wait_count);
	...
	if (ret <= 0)
		pr_warn("[DISP] %s wait for sync timeout\n", __func__);


	/* FIXME free logo file reserve */
	memblock_free(phy_addr, fb_stride * fb_height);
#ifndef MODULE
	free_reserved_area(__va(phy_addr), __va(phy_addr + PAGE_ALIGN(fb_stride * fb_height)), 0x100, "logo buffer");
#endif
...
{
	if (hw_private.fb_share.smooth_display &&
		hw_id == hw_private.fb_share.smooth_display_hw_id) {
...
#if defined(CONFIG_AW_DISP2_FB_HW_ROTATION_SUPPORT)
	if (info->fb_rot)
...
int logo_display_fb_no_rot(struct fb_hw_info *info, struct logo_info_fb *logo)
{
	...
	src_addr = (char *)fb_map_kernel_cache((unsigned long)src_phy_addr -
					       ...
					       map_offset);
	if (src_addr == NULL) {
		pr_warn("fb_map_kernel_cache for src_addr failed\n");
		return -1;
	}
	...
	if (src_height < dst_height) {
		...
		pr_warn("logo src_height(%d) > dst_height(%d),please cut the height\n",
		      src_height,
		      dst_height);
		goto out_unmap;
	}
	if (src_width < dst_width) {
		...
		pr_warn("src_width(%d) > dst_width(%d),please cut the width!\n",
		      src_width,
		      dst_width);
		goto out_unmap;
	}
...
static int logo_display_fb_rot(struct fb_hw_info *info, struct logo_info_fb *logo, int degree_int)
{
	...
	if (degree_int == 180) { /* copy with rotate 180 */
		...
		pr_warn("input rotate degree error %d\n", degree_int);
	}
...
int platform_fb_init_logo(void *hw_info, const struct fb_var_screeninfo *var)
{
	...
	if (info->fb_info.logo_display) {
		fb_debug_inf("display logo on display %d channel %d layer %d type %d\n",
				info->fb_info.map.hw_display, info->config.channel,
				info->config.layer_id, hw_private.fb_share.logo_type);
		if (hw_private.fb_share.logo_type == LOGO_FILE)
			logo_display_file(info, hw_private.fb_share.logo.logo_file.phy_addr);
		else
			logo_display_fb(info, &hw_private.fb_share.logo.logo_fb);
	}
...
int platform_fb_memory_free(void *hw_info)
{
	...
	disp_free((void *__force)info->dma_mem.virtual_addr,
		  (void *)info->dma_mem.device_addr, info->size);
#endif
...
{
	int num_screens, num_layers, i, j;
	struct disp_manager *mgr = NULL;
	struct disp_layer_config *lyr_cfg;

	num_screens = bsp_disp_feat_get_num_screens();
	lyr_cfg = kzalloc(sizeof(*lyr_cfg) * 16, GFP_KERNEL);

	for (i = 0; i < 4; ++i) {
		for (j = 0; j < 4; ++j) {
			lyr_cfg[i * 4 + j].enable = false;
			lyr_cfg[i * 4 + j].channel = i;
			lyr_cfg[i * 4 + j].layer_id = j;
		}
	}
...
int platform_fb_init(struct fb_info_share *fb_init)
{
	...
	hw_private.fb_wait = kzalloc(sizeof(*hw_private.fb_wait), GFP_KERNEL);
	hw_private.fbs = kzalloc(sizeof(*hw_private.fbs)* fb_init->fb_num, GFP_KERNEL);
	for (i = 0; i < hw_private.num_screens; i++)
		atomic_set(&hw_private.fb_wait->wait_count, 0);
...
int platform_fb_init_for_each(void *hw_info, struct fb_init_info_each *fb, int fb_id)
{
	struct fb_hw_info *info = hw_info;
...
{
	return platform_fb_memory_free(hw_info);
}
