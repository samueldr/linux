...
int init_waveform(const char *path, u32 bit_num)
{
	...
	g_waveform_file.p_init_wf = (unsigned long)(g_waveform_file.p_wf_paddr + *pAddr);/* unsigned long? */
				...
				struct timing_info *timing)
{
	...
	for (buf_id = 0; buf_id < WAVE_DATA_BUF_NUM; buf_id++) {
		...
		eink_cache_sync(queue->wavedata_paddr[buf_id], queue->wavedata_buf_size);
...
void *request_buffer_for_display(struct wavedata_queue *queue)
{
	...
	eink_cache_sync(queue->wavedata_paddr[tmp_tail], queue->wavedata_buf_size);
...
s32 queue_wavedata_buffer(struct wavedata_queue *queue)
{
	...
	if (queue->buffer_state[head] == WV_DECODE_STATE) {
		...
		eink_cache_sync(queue->wavedata_paddr[head], queue->wavedata_buf_size);
	}
				...
				struct timing_info *timing)
{
	...
	eink_cache_sync(mgr->dec_wav_paddr, 2 * wavedata_buf_size);
