...
static struct clk *clk_bus_hdmi0;
static struct clk *clk_bus_hdmi1;
static struct clk *tcon_tv0_clk;
...
#endif
#endif
static struct reset_control	*rst_bus_hdmi0;
static struct reset_control *rst_bus_hdmi1;
...
static int hdmi_clk_enable(void)
{
	...
	if (rst_bus_hdmi0) {
		ret = reset_control_deassert(rst_bus_hdmi0);
		if (ret) {
			pr_err("[%s] de-assert bus hdmi0 failed!\n", __func__);
		}
	}

	if (rst_bus_hdmi1) {
		ret = reset_control_deassert(rst_bus_hdmi1);
		if (ret) {
			pr_err("[%s] de-assert bus hdmi1 failed!\n", __func__);
		}
	}

	if (clk_bus_hdmi0) {
		if (clk_prepare_enable(clk_bus_hdmi0) != 0)
			pr_info("hdmi0 bus clk enable failed!\n");
	}

	if (clk_bus_hdmi1) {
		if (clk_prepare_enable(clk_bus_hdmi1) != 0)
			pr_info("hdmi1 bus clk enable failed!\n");
	}

	if (hdmi_clk) {
		if (tcon_tv0_clk == NULL)
			pr_err("tcon_tv clk get failed\n");
		else
			clk_set_rate(hdmi_clk,
					clk_get_rate(tcon_tv0_clk));

		if (clk_prepare_enable(hdmi_clk) != 0)
...
static int hdmi_clk_disable(void)
{
	...
	if (hdmi_ddc_clk)
		clk_disable_unprepare(hdmi_ddc_clk);
	...
	if (rst_bus_hdmi0) {
		if (reset_control_assert(rst_bus_hdmi0))
			pr_err("[%s] assert bus hdmi0 failed!\n", __func__);
	}

	if (rst_bus_hdmi1) {
		if (reset_control_assert(rst_bus_hdmi1))
			pr_err("[%s] assert bus hdmi1 failed!\n", __func__);
	}
 ...
 * @1 snd_hdmi_audio_enable
 * @2 snd_hdmi_set_audio_para
 *****************************************************************************/
...
static void hdmi_report_hpd_work(struct work_struct *work)
{
	if (hdmi_get_HPD_status()) {
		extcon_set_state_sync(extcon_hdmi, EXTCON_DISP_HDMI, STATUE_OPEN);
		disp_set_hdmi_detect(1);
		hdmi_inf("switch_set_state 1\n");
	} else {
		extcon_set_state_sync(extcon_hdmi, EXTCON_DISP_HDMI, STATUE_CLOSE);
...
s32 hdmi_init(struct platform_device *pdev)
{
	...
	/* get clk */
	hdmi_clk = devm_clk_get(&pdev->dev, "clk_hdmi");
	if (IS_ERR(hdmi_clk)) {
		dev_err(&pdev->dev, "fail to get clk for hdmi\n");
		goto err_clk_get;
	}

	clk_bus_hdmi0 = devm_clk_get(&pdev->dev, "clk_bus_hdmi0");
	if (IS_ERR(clk_bus_hdmi0)) {
		dev_err(&pdev->dev, "fail to get clk for hdmi0 bus\n");
		goto err_clk_get;
	}

	clk_bus_hdmi1 = devm_clk_get(&pdev->dev, "clk_bus_hdmi1");
	if (IS_ERR(clk_bus_hdmi1)) {
		dev_err(&pdev->dev, "fail to get clk for hdmi1 bus\n");
		goto err_clk_get;
	}

	tcon_tv0_clk = devm_clk_get(&pdev->dev, "clk_tcon_tv0");
	if (IS_ERR(tcon_tv0_clk)) {
		dev_err(&pdev->dev, "fail to get clk for tcon tv0\n");
		goto err_clk_get;
	}
	...
	hdmi_ddc_clk = devm_clk_get(&pdev->dev, "clk_ddc");
	...
	rst_bus_hdmi0 = devm_reset_control_get(&pdev->dev, "rst_bus_hdmi0");
	if (IS_ERR(rst_bus_hdmi0)) {
		dev_err(&pdev->dev, "fail to get clk for hdmi rst_bus_hdmi0\n");
		goto err_clk_get;
	}

	rst_bus_hdmi1 = devm_reset_control_get(&pdev->dev, "rst_bus_hdmi1");
	if (IS_ERR(rst_bus_hdmi1)) {
		dev_err(&pdev->dev, "fail to get clk for hdmi rst_bus_hdmi1\n");
		goto err_clk_get;
	}
...
static int hdmi_probe(struct platform_device *pdev)
{
	...
	ghdmi.dev = &pdev->dev;
	if (hdmi_init(pdev) == 0)
		hdmi_inf("hdmi_probe success\n");
	else
		hdmi_inf("hdmi_probe failed!!!!\n");
