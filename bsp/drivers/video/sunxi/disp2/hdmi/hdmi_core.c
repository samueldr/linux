...
static s32 audio_config_internal(u8 audio_reset);
u32	hdmi_print;
...
struct disp_video_timings video_timing[] = {
	...
	{HDMI1440_480I,      0, 13500000,  1,  720,   480,   858,   57,   19,   62,  525,   15,  4,  3,  1,		1,   1,   0,   0},
	...
	{HDMI1080P_25,       0,	74250000,  0,  1920,  1080,  2640,  148,  528,  44,  1125,  36,  4,  5,  1,		1,   0,   0,   0},
	{HDMI1080P_30,       0,	74250000,  0,  1920,  1080,  2200,  148,  88,   44,  1125,  36,  4,  5,  1,		1,   0,   0,   0},
...
static void hdmi_para_init(void)
{
	...
	glb_video_para.hor_sync_polarity = 1;
	glb_video_para.ver_sync_polarity = 1;
...
static s32 audio_config_internal(u8 audio_reset)
{
	...
	static struct audio_para para_old;
	...
	if (video_on) {
		...
		/* different para or reset */
		if (memcmp(&para_old, &glb_audio_para,
			   sizeof(struct audio_para)) ||
		    audio_reset) {
			if (bsp_hdmi_audio(&glb_audio_para)) {
				hdmi_wrn("set hdmi audio error!\n");
				return -1;
			} else
				memcpy(&para_old, &glb_audio_para,
				       sizeof(struct audio_para));
		}
