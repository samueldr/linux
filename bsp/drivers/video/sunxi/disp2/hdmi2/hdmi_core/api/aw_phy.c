...
static void phy_set_mpll(void)
{
	...
	phy_base->pll_ctl1.bits.sdm_en = 0x0;            /* mpll sdm jitter Very large, not used for the time being */
	phy_base->pll_ctl1.bits.sckref = 0;        /* default 1 */
	phy_base->pll_ctl0.bits.slv = 4;
	phy_base->pll_ctl0.bits.prop_cntrl = 7;   /* default 7 */
	phy_base->pll_ctl0.bits.gmp_cntrl = 3;    /* default 1 */
...
static void phy_set_div(void)
{
	...
	phy_base->pll_ctl1.bits.pcnt_n = 1;       /* div6 = 1 (pcnt_en=0)    [div6 = n (pcnt_en = n Note that some multiples are problematic)]  4-256 */
...
static void phy_set_clk(void)
{
	...
	phy_base->phy_ctl6.bits.en_ckdat = 1;       /* default 0 */

	/* phy_base->phy_ctl2.bits.reg_resdi = 0x18; */
	/* phy_base->phy_ctl4.bits.reg_slv = 3;         //low power voltage 1.08V, default 3 */
