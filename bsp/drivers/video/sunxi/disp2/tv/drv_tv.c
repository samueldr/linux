...
static char extcon_name[SCREEN_COUNT][20];
static char extcon_name_char[20];
...
static void tve_clk_init(u32 sel)
{
	...
	clk_set_parent(g_tv_info.screen[sel].clk, g_tv_info.screen[sel].clk_parent);
}
...
#if defined(TVE_SUPPLY_VOLTAGE)
	tv_power_vol = TVE_SUPPLY_VOLTAGE;
#else
	tv_power_vol = 3300000;
#endif
	ret = regulator_set_voltage(g_tv_info.regulator, tv_power_vol, tv_power_vol);
	if (ret < 0) {
		TV_ERR("regulator_set_voltage: set %duV failed!\n", tv_power_vol);
		return -1;
	}
...
s32 tv_init(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(extcon_dev[sel]))
		return -1;
	memset(extcon_name_char, 0, sizeof(extcon_name_char));
	switch (interface) {
	case DISP_TV_CVBS:
		snprintf(extcon_name_char, sizeof(extcon_name_char), "tve%d_cvbs", sel);
		break;
	case DISP_TV_YPBPR:
		snprintf(extcon_name_char, sizeof(extcon_name_char), "tve%d_ypbpr", sel);
		break;
	case DISP_TV_SVIDEO:
		snprintf(extcon_name_char, sizeof(extcon_name_char), "tve%d_svideo", sel);
		break;
	case DISP_VGA:
		snprintf(extcon_name_char, sizeof(extcon_name_char), "tve%d_vga", sel);
		break;
	default:
		break;
	}

	memcpy(&extcon_name[sel][0], extcon_name_char, strlen(extcon_name_char) + 1);

	devm_extcon_dev_register(&pdev->dev, extcon_dev[sel]);
	extcon_dev[sel]->name = &extcon_name[sel][0];
...
static int tv_probe(struct platform_device *pdev)
{
	...
	g_tv_info.screen[pdev->id].clk_parent = clk_get_parent(g_tv_info.screen[pdev->id].clk);
