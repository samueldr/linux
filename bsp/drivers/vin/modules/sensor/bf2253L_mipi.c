/*
 * A V4L2 driver for BF2253L Raw cameras.
 *
...
static int sensorGainMapping[MaxGainIndex][2] = {
	{1024, 15},
	{1088, 16},
	{1150, 17},
	{1214, 18},
...
{
	struct sensor_info *info = to_state(sd);
	unsigned int All_gain, iI, RegGain = 0;

	All_gain = gain_val << 6;

	if (All_gain < 1024)
	    All_gain = 1024;
	else if (All_gain > 10856)
		All_gain = 10856;
	...
	for (iI = 0; iI < (MaxGainIndex - 1); iI++) {
		if ((All_gain >= sensorGainMapping[iI][0]) && (All_gain < sensorGainMapping[iI + 1][0])) {
			if ((All_gain - sensorGainMapping[iI][0]) <= (sensorGainMapping[iI + 1][0] - All_gain))
				RegGain = sensorGainMapping[iI][1];
			else
				RegGain = sensorGainMapping[iI + 1][1];
			break;
		}
	}
...
static void sensor_s_sw_stby(struct v4l2_subdev *sd, int on_off)
{
	...
	sensor_read(sd, 0xe0, &rdval);

	if (on_off == STBY_ON) {

		sensor_write(sd, 0xe0, rdval | 0x01);
	} else {
		sensor_write(sd, 0xe0, rdval & 0xfe);
	}
}
