...
#define SENSOR_NAME "gc2355_mipi"
...
static struct regval_list sensor_1600x1200p30_regs[] = {
	...
	{0x05, 0x00},
	{0x06, 0xa2},
	{0x11, 0x2b}, /* raw8 2a raw10 2b */
	{0x12, 0xd0},
	{0x13, 0x07}, /* raw8 0x640, raw 10 0x7d0 */
	{0x15, 0x60},
	{0x21, 0x10},
	{0x24, 0x02},
	{0x26, 0x08},
...
static int sensor_s_gain(struct v4l2_subdev *sd, unsigned int gain_val)
{
	...
	if ((ANALOG_GAIN_1 <= All_gain) && (All_gain < ANALOG_GAIN_2)) {
		...
		Digital_gain = 64*All_gain/ANALOG_GAIN_6;
		sensor_write(sd, 0xb1, Digital_gain >> 6);
		sensor_write(sd, 0xb2, (Digital_gain << 2)&0xfc);
	} else {
		sensor_write(sd, 0xb6, 0x06);
		Digital_gain = 64*All_gain/ANALOG_GAIN_7;
		sensor_write(sd, 0xb1, Digital_gain >> 6);
		sensor_write(sd, 0xb2, (Digital_gain << 2)&0xfc);
	}
...
{
	data_type rdval_1, rdval_2, rdval_3;

	sensor_read(sd, 0xf7, &rdval_1);
	sensor_read(sd, 0xf9, &rdval_2);
	sensor_read(sd, 0x10, &rdval_3);

	if (on_off == STBY_ON) {
		sensor_write(sd, 0xfc, 0x01);
		sensor_write(sd, 0xf2, 0x00);
		sensor_write(sd, 0xf7, rdval_1 & 0xfe);
		sensor_write(sd, 0xf9, rdval_2 | 0x01);
		sensor_write(sd, 0x10, rdval_3 & 0xef);
	} else {
		sensor_write(sd, 0xfc, 0x06);
		sensor_write(sd, 0xf2, 0x00);
		sensor_write(sd, 0xf7, rdval_1 | 0x01);
		sensor_write(sd, 0xf9, rdval_2 | 0x01);
		sensor_write(sd, 0x10, rdval_3 | 0x10);
	}
}
