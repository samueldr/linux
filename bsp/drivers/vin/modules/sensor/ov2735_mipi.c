/*
 * A V4L2 driver for ov2735_mipi cameras.
 *
...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for ov2735 sensors");
MODULE_LICENSE("GPL");
...
/*
 * The ov2735_mipi sits on i2c with ID 0x78
 */
#define I2C_ADDR 0x78

#define SENSOR_NAME "ov2735_mipi"
...
static int sync_cnt;
static int repeat_sync_flag;
static int ov2735_sensor_vts;
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
	...
	if (exp_val > ((ov2735_sensor_vts - 6) << 4))
		exp_val = (ov2735_sensor_vts - 6) << 4;
	if (exp_val < 16)
		exp_val = 16;

	if (vinc->vin_status.frame_cnt < SYNC_FRAME) {
		info->exp = 0x4650;
	...
	sensor_print("sensor_s_exp info->exp %d\n", exp_val);
	info->exp = exp_val;
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	if (ret < 0) {
		sensor_err("%s sensor write addr(0xfd) err\n", __func__);
		return ret;
	}

	ret = sensor_read(sd, 0x02, &rdval);
	if (ret < 0) {
		sensor_err("%s sensor read addr(0x02) err\n", __func__);
		return ret;
	}

	if (rdval != ((V4L2_IDENT_SENSOR >> 8) & 0xFF)) {
		sensor_err("%s sensor read addr(0x02) return 0x%x\n",
			   __func__, rdval);
		return -ENODEV;
	}

	ret = sensor_read(sd, 0x03, &rdval);
	if (ret < 0) {
		sensor_err("%s sensor read addr(0x03) err\n", __func__);
		return ret;
	}

	if (rdval != (V4L2_IDENT_SENSOR & 0xFF)) {
		sensor_err("%s sensor read addr(0x03) return 0x%x\n",
			   __func__, rdval);
		return -ENODEV;
	}
