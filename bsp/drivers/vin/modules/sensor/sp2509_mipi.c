/*
 * A V4L2 driver for SP2509 Raw cameras.
 *
 * Copyright (c) 2018 by Allwinnertech Co., Ltd.  http:
 *
...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for SP2509 sensors");
MODULE_LICENSE("GPL");

#define MCLK              (24*1000*1000)
#define V4L2_IDENT_SENSOR 0x2509
...
#define SENSOR_NAME "sp2509_mipi"


struct sensor_exp_gain global_exp_gain = {2052, 64};
...
{
	sensor_write(sd, 0x03, (exp_val >> 12) & 0xFF);
	sensor_write(sd, 0x04, (exp_val >> 4) & 0xFF);
	return 0;
}
...
{
	sensor_write(sd, 0x24, gain_val<<1);
	return 0;
}
				...
				struct sensor_exp_gain *exp_gain)
{
	...
	global_exp_gain.exp_val = exp_gain->exp_val;
	global_exp_gain.gain_val = exp_gain->gain_val;
...
static int sensor_reg_init(struct sensor_info *info)
{
	...
	exp_gain.exp_val = global_exp_gain.exp_val;
	exp_gain.gain_val = global_exp_gain.gain_val;
