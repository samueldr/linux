...
//./modules/nand/common1/phy-nand/rawnand/controller/ndfc_base.h
 ...
 * x >= 1; 00 <= ab <= 99
 */
#define NAND_COMMON1_PHY_DRV_VERSION "v2.09 2022-09-07 11:06"
...
#ifdef CONFIG_AW_RAWNAND_CD
int nand_gpio_alloc(struct sunxi_ndfc *host)
{
	struct nand_gpio *ctx = devm_kzalloc(host->dev,
					    sizeof(*ctx), GFP_KERNEL);

	if (ctx) {
		ctx->cd_debounce_delay_ms = 200;
		ctx->cd_label = devm_kasprintf(host->dev, GFP_KERNEL,
				"%s cd", dev_name(host->dev));
		if (!ctx->cd_label)
			return -ENOMEM;
...
int nand_gpio_get_cd(struct sunxi_ndfc *host)
{
	struct nand_gpio *ctx = host->slot.handler_priv;
	int cansleep;
...
static irqreturn_t nand_gpio_cd_irqt(int irq, void *dev_id)
{
	/* Schedule a gpio detection after a debounce timeout */
	struct sunxi_ndfc *host = dev_id;
	struct nand_gpio *ctx = host->slot.handler_priv;
	struct nand_controller_info *nctri = g_nctri;
	int pwroff = nand_gpio_get_cd(host);

	if (ctx->cd_debounce_delay_ms)
		msleep(ctx->cd_debounce_delay_ms);
	nand_print("cd irq, %x\n", pwroff);
	if (pwroff) {
		nand_clk_release(host, nctri->channel_id);
		nand_print("Got power off signal gate off control, %x\n", pwroff);
	} else {
		nand_print("Not power off signal, %x\n", pwroff);
	}
...
/**
 * nand_gpiod_request_cd - request a gpio descriptor for powerdown-detection
 * @host: nand host
 ...
 */
int nand_gpiod_request_cd(struct sunxi_ndfc *host, const char *con_id,
			 ...
			 unsigned int debounce, bool *gpio_invert)
{
	...
	desc = devm_gpiod_get_index(host->dev, con_id, idx, GPIOD_IN);
	if (IS_ERR(desc)) {
		nand_print("des error\n");
		return PTR_ERR(desc);
	}
...
void nand_gpiod_request_cd_irq(struct sunxi_ndfc *host)
{
	...
	if (host->slot.cd_irq >= 0 || !ctx || !ctx->cd_gpio) {
		nand_print("request cd irq arg failed\n");
		return;
	}
	...
	if (irq >= 0) {
		if (!ctx->cd_gpio_isr)
			ctx->cd_gpio_isr = nand_gpio_cd_irqt;
	...
	if (irq < 0) {
		nand_print("request pwr off irq failed\n");
	} else
		nand_print("request pwr off irq ok\n");

}

int nand_pwroff_pio_request(struct sunxi_ndfc *host)
{
		struct device *dev = host->dev ;
		u32 cd_debounce_delay_ms = 0;
		int ret = 0;

		ret = nand_gpio_alloc(host);
		if (ret) {
			nand_print("Alloc power off GPIO\n");
			return ret;
		}

		ret = device_property_read_u32(dev, "cd-debounce-delay-ms",
					     &cd_debounce_delay_ms);
		if (ret)
			return ret;

		ret = nand_gpiod_request_cd(host, "pwroff", 0, false,
					   cd_debounce_delay_ms * 1000,
					   NULL);
		if (!ret)
			nand_print("Got power off GPIO\n");
		else if (ret != -ENOENT && ret != -ENOSYS) {
			nand_print("Got power off GPIO failed\n");
			return ret;
		 } else {
			nand_print("Unknow err %d\n", ret);
			return ret;
		 }


		nand_gpiod_request_cd_irq(host);
		return ret;
}
#endif
