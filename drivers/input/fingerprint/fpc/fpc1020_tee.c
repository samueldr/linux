	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (gpio_is_valid(fpc1020->pwr_gpio)) {
		gpio_set_value(fpc1020->pwr_gpio, enable);
	}
	mutex_unlock(&fpc1020->lock);
...
static int hw_reset(struct fpc1020_data *fpc1020)
{
	...
	/*dev_err(dev, "==chenhui==fpc1020_reset_active rc %d\n", rc);*/
	usleep_range(RESET_HIGH_SLEEP1_MIN_US, RESET_HIGH_SLEEP1_MAX_US);

	rc = select_pin_ctl(fpc1020, "fpc1020_reset_reset");
	if (rc)
		goto exit;
	/*dev_err(dev, "==chenhui==fpc1020_reset_reset rc %d\n", rc);*/
	...
	dev_err(dev, "==fpc1020=IRQ after reset %d\n", rst_gpio);
	return rc;

exit:
	dev_err(dev, "==fpc1020==hw_reset error\n");
	return rc;
}
...
static int device_prepare(struct fpc1020_data *fpc1020, bool enable)
{
	...
	pr_info("=fpc1020 device_prepare enable:%d, prepared:%d\n", enable, fpc1020->prepared);
	mutex_lock(&fpc1020->lock);
...
*/
		...
		if (gpio_is_valid(fpc1020->pwr_gpio)) {
			pr_info("fpc1020 device_prepare2 enable:%d, prepared:%d\n", enable, fpc1020->prepared);
			gpio_set_value(fpc1020->pwr_gpio, enable);
		}
	...
	char *buf)
{
	...
	/*debug*/
    /*dev_err(dev, "=chenhui=IRQ %d\n", gpio_to_irq(fpc1020->irq_gpio));
	dev_err(dev, "=chenhui=rest %d\n", gpio_to_irq(fpc1020->rst_gpio));
	dev_err(dev, "=chenhui=pwr %d\n", gpio_to_irq(fpc1020->pwr_gpio));
	dev_err(dev, "=chenhui=rest value %d\n", gpio_get_value(fpc1020->rst_gpio));
	dev_err(dev, "=chenhui=pwr value %d\n", gpio_get_value(fpc1020->pwr_gpio));*/
...
static int fpc1020_probe(struct platform_device *pdev)
{
	...
	rc = fpc1020_request_named_gpio(fpc1020, "fpc,gpio_irq",
			&fpc1020->irq_gpio);
	if (rc) {
		dev_err(dev, "no fpc,gpio_irq node found\n");
		goto exit;
	}
	rc = fpc1020_request_named_gpio(fpc1020, "fpc,gpio_rst",
			&fpc1020->rst_gpio);
	if (rc) {
		dev_err(dev, "no fpc,gpio_rst node found\n");
		goto exit;
	}
	rc = fpc1020_request_named_gpio(fpc1020, "fpc,gpio_pwr",
			&fpc1020->pwr_gpio);
	if (rc) {
		dev_err(dev, "no fpc,gpio_pwr node found\n");
		goto exit;
	}
	...
	if (of_property_read_bool(dev->of_node, "fpc,enable-on-boot")) {
		...
		/* power on */
		if (gpio_is_valid(fpc1020->pwr_gpio)) {
			dev_info(dev, "Enabling hardware pwr_gpio output high\n");
...
*/
	if (gpio_is_valid(fpc1020->pwr_gpio)) {
		gpio_set_value(fpc1020->pwr_gpio, 0);
	}
	dev_info(&pdev->dev, "%s\n", __func__);
...
static int __init fpc1020_init(void)
{
	...
	/*if (is_fpc1028_fp()) {
		...
		pr_info("fingerprint is not fpc1028, exit\n");
		return 0;
	}*/
