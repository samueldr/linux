...
#include <linux/clk.h>
//zhenglihong@wind-mobi.com 20160511 begin
#ifdef CONFIG_WIND_HALL_SUPPORT
#include <linux/irq.h>
#include <linux/gpio.h>
#include <linux/interrupt.h>
#define GPIO_HALL_1_PIN 9
extern 	unsigned int hall_irq;
#endif
//zhenglihong@wind-mobi.com 20160511 end
...
static void kpd_hall_handle(unsigned long data);
static DECLARE_TASKLET(kpd_hall_tasklet, kpd_hall_handle, 0);
 u8 kpd_hall_state;
 u8 cur_hall_state;
 u8 next_hall_state;
#endif
...
//zhenglihong@wind-mobi.com 20150611 begin
#ifdef CONFIG_WIND_HALL_SUPPORT

extern void kpd_hall_init(void);
//extern int mt_get_gpio_in(unsigned long pin);
int __init kpd_hall_init_status(void)
{
  kpd_hall_state = gpio_get_value(GPIO_HALL_1_PIN);
  printk("ciel_test_hall_init kpd_hall_state =%d\n",kpd_hall_state);
  return 0;
}

void  kpd_hall_handle(unsigned long data)
{

	 
   
	   cur_hall_state =  gpio_get_value(GPIO_HALL_1_PIN); 
     next_hall_state = !cur_hall_state;
     
    
     printk("ciel:cur_hall_state==%d kpd_hall_state =%d next_hall_state = %d\n",cur_hall_state,kpd_hall_state,next_hall_state);
...
irqreturn_t  kpd_hall_eint_handler(int dev, void *dev_id)
{
   // printk("ciel eint\n");
	disable_irq_nosync(hall_irq);
	tasklet_schedule(&kpd_hall_tasklet);
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
    ...
    printk("yutao add test kpd_pdrv_probe!!\n");
...
#endif
//zhenglihong@wind-mobi.com 20160513 begin
#ifdef CONFIG_WIND_HALL_SUPPORT
    __set_bit(EV_SW, kpd_input_dev->evbit);
	__set_bit(SW_LID, kpd_input_dev->swbit);
 ...
 #endif
//zhenglihong@wind-mobi.com 20160513 end  
...
late_initcall(kpd_hall_init_status);
#endif
//zhenglihong@wind-mobi.com 20160511 end 
