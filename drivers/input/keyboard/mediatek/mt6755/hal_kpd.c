...
static const struct of_device_id hall_dev_of_match[] = {
{.compatible = "mediatek,hall-eint",},
	{},
};
extern  irqreturn_t  kpd_hall_eint_handler(int , void *);
void  kpd_hall_init(void)
{
	...
	if (node) {
		...
		printk("hall:hall_irq=%d \n",hall_irq);
		
	
	
	} else {
		printk("[%s] hall request_irq can not find touch eint device node!.", __func__);
	}
	


	//Work functon
	if (hall_irq) {/*INT mode*/

	

	    ret = request_irq(hall_irq, kpd_hall_eint_handler, IRQF_TRIGGER_NONE, "hall-eint", NULL);
		        if(ret > 0){
				    ret = -1;
			     	printk("hall request_irq IRQ LINE NOT AVAILABLE\n");
				}
