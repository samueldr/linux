...
static ssize_t status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if ((touchStatus & 0x04) && (touchStatus & 0x02) && (touchStatus & 0x01)) {
		return snprintf(buf, 64, "7\n");/*CS0,CS1 add CS2 all near*/
	} else if ((touchStatus & 0x04) && (touchStatus & 0x02)) {
		return snprintf(buf, 64, "6\n");/*CS1 add CS2 both near*/
	} else if ((touchStatus & 0x04) && (touchStatus & 0x01)) {
		return snprintf(buf, 64, "5\n");/*CS0 add CS2 both near*/
	} else if (touchStatus & 0x04) {
		return snprintf(buf, 64, "4\n");/*CS2 near*/
	} else if ((touchStatus & 0x02) && (touchStatus & 0x01)) {
		return snprintf(buf, 64, "3\n");/*CS0 add CS1 both near*/
	} else if (touchStatus & 0x02) {
		return snprintf(buf, 64, "2\n");/*CS1 near*/
	} else if (touchStatus & 0x01) {
...
#ifdef SNS_SAR_USE_CS0_PIN
			write_register(this,  SX932x_CTRL1_REG, 0x27);
			write_register(this,  SX932x_AFE_PH0_REG, 0x3D);
			write_register(this,  SX932x_PROX_CTRL1_REG, 0x12);
			write_register(this,  SX932x_PROX_CTRL7_REG, 0x26);
			write_register(this,  SX932x_ADV_CTRL10_REG, 0x00);
#endif
...
static int sx932x_parse_dt(struct sx932x_platform_data *pdata,  struct device *dev)
{
	...
	irq_gpio_num = pdata->irq_gpio;
		pr_info("[SENSOR]: %s - get irq_gpio irq_gpio_num is %d\n",  __func__, irq_gpio_num);
...
static int sx932x_init_platform_hw(struct i2c_client *client)
{
	...
	pr_info("[SX932x] : %s init_platform_hw start!\n", __func__);
...
static int sx932x_fb_callback(struct notifier_block *nfb,
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		if (*blank == FB_BLANK_UNBLANK)	{
			if (sx932x_chip) {
			    u8 touchStatus = 0;
				read_register(sx932x_chip, SX932x_IRQSTAT_REG, &touchStatus);
				pr_info("sx932x_fb_callback.FB_BLANK_TO_UNBLANK for read status %d!\n", touchStatus);

				manual_offset_calibration(sx932x_chip);
			}
		}
	}
...
static struct notifier_block __refdata sx932x_fb_notifier = {
	.notifier_call = sx932x_fb_callback,
};
...
static int sx932x_probe(struct i2c_client *client,  const struct i2c_device_id *id)
{
	...
	if (this) {
		...
		* (1->NIRQ=0,  0->NIRQ=1) */
		...
		/*set pinctrl*/
		this->pinctrl = devm_pinctrl_get(&client->dev);
		if (IS_ERR_OR_NULL(this->pinctrl)) {
			pr_err("%s, error devm_pinctrl_get()\n", __func__);
			goto probe_error;
		} else {
			this->sx932x_active = pinctrl_lookup_state(this->pinctrl, "sx932x_active");
			if (IS_ERR_OR_NULL(this->sx932x_active)) {
				pr_err("%s, error pinctrl_lookup_state() for SX932x_ACTIVE\n", __func__);
				goto probe_error;
			}
			this->sx932x_sleep = pinctrl_lookup_state(this->pinctrl, "sx932x_sleep");
			if (IS_ERR_OR_NULL(this->sx932x_sleep)) {
				pr_err("%s, error pinctrl_lookup_state() for SX932x_SLEEP\n", __func__);
				goto probe_error;
			}
		}

		if (this->pinctrl) {
			err = pinctrl_select_state(this->pinctrl, this->sx932x_sleep);
			if (err) {
				pr_err("select sx932x_sleep failed with %d\n", err);
				return err;
			}
			msleep(20);
			err = pinctrl_select_state(this->pinctrl, this->sx932x_active);
			if (err) {
				pr_err("select sx932x_active failed with %d\n", err);
				return err;
			}
			msleep(20);
		}
...
/*
	if (this->sx932x_vdd)
		err = regulator_enable(this->sx932x_vdd);
	*/
	...
	sx932x_chip = this;
	fb_register_client(&sx932x_fb_notifier);
...
/* def CONFIG_PM */
#if  defined(SUSPEND_SUPPORT) && (SUSPEND_SUPPORT == 1)
/*====================================================*/
/***** Kernel Suspend *****/
...
void sx93XX_suspend(psx93XX_t this)
{
	if (this)
		...
		dev_err(this->pdev, "sx932x suspend!\n");
...
void sx93XX_resume(psx93XX_t this)
{
	if (this) {
		...
		/*if (this->init)
			this->init(this);*/
			dev_err(this->pdev, "sx932x resume!\n");
