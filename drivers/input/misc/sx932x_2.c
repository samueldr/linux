...
static int sx932x_2_fb_callback(struct notifier_block *nfb,
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		if (*blank == FB_BLANK_UNBLANK)	{
			if (sx932x_2_chip) {
			    u8 touchStatus = 0;
				read_register(sx932x_2_chip, SX932x_IRQSTAT_REG, &touchStatus);
				pr_info("sx932x_2_fb_callback.FB_BLANK_TO_UNBLANK for read status %d!\n", touchStatus);

				manual_offset_calibration(sx932x_2_chip);
			}
		}
	}
...
static struct notifier_block __refdata sx932x_2_fb_notifier = {
	.notifier_call = sx932x_2_fb_callback,
};
	...
	if (this) {
		...
		* (1->NIRQ=0,  0->NIRQ=1) */
		...
		if (IS_ERR_OR_NULL(this->pinctrl)) {
			...
			this->sx932x_active = pinctrl_lookup_state(this->pinctrl, "sx932x_2_active");
			if (IS_ERR_OR_NULL(this->sx932x_active)) {
				pr_err("%s, error pinctrl_lookup_state() for SX932x_ACTIVE\n", __func__);
				goto probe_error;
			}
			this->sx932x_sleep = pinctrl_lookup_state(this->pinctrl, "sx932x_2_sleep");
		...
		if (this->pinctrl) {
			...
			if (err) {
				pr_err("select sx932x_2_sleep failed with %d\n", err);
				return err;
			}
			msleep(20);
...
/*
	...
	*/
	...
	sx932x_2_chip = this;
	fb_register_client(&sx932x_2_fb_notifier);
...
void sx93XX_2_suspend(psx93XX_t this)
{
	if (this)
		...
		dev_err(this->pdev, "sx932x_2 suspend!\n");
...
void sx93XX_2_resume(psx93XX_t this)
{
	if (this) {
		...
		/*if (this->init)
			...
			dev_err(this->pdev, "sx932x_2 resume!\n");
