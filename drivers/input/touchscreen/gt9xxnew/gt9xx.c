...
#define GT_LONGPRESS_TIME_MS           1000
#define GT_DEBOUNCE_TIME_MS           1000
...
#if 0
static ssize_t gtpclass_idle_enable_show(struct class *cls,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", (int) idle_data);
}

static ssize_t gtpclass_idle_enable_store(struct class *cls,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int error = kstrtoul(buf, 10, &idle_data);
	if (error)
		return error;

	if (!idle_data)
		tp_wakeup_status = TP_WAKEUP_TRIGGER;
	else
		tp_wakeup_status = TP_WAKEUP_INIT;

	return count;
}

static struct class_attribute ctp_class_attrs[] = {
	__ATTR(tp_idle, S_IRUGO | S_IWUSR | S_IWGRP, gtpclass_idle_enable_show, gtpclass_idle_enable_store),
	__ATTR_NULL,
};
...
//#endif
	...
	if (!idle_data) {
		if ((tp_wakeup_status == TP_WAKEUP_TRIGGER) && (pre_touch > 0)) {
			tp_wakeup_status = TP_WAKEUP_CHECK;
			tp_wakeup_timecheck = jiffies
					      + msecs_to_jiffies(GT_LONGPRESS_TIME_MS);
		} else if (tp_wakeup_status == TP_WAKEUP_CHECK) {
			if (!time_after(jiffies, tp_wakeup_timecheck) && (pre_touch == 0)) {
				GTP_DEBUG("touch KEY_POWER");
				input_report_key(ts->input_dev, KEY_POWER, 1);
				input_sync(ts->input_dev);
				msleep(10);
				input_report_key(ts->input_dev, KEY_POWER, 0);
				input_sync(ts->input_dev);
				tp_wakeup_status = TP_WAKEUP_DEBOUNCE;
				tp_wakeup_timecheck = jiffies
						      + msecs_to_jiffies(GT_DEBOUNCE_TIME_MS);
			} else if (time_after(jiffies, tp_wakeup_timecheck)
				   && (pre_touch == 0)) {
				tp_wakeup_status = TP_WAKEUP_TRIGGER;
			} else {
				GTP_DEBUG("tp_wakeup_status %d", tp_wakeup_status);
			}
		} else if (tp_wakeup_status == TP_WAKEUP_DEBOUNCE) {
			if (time_after(jiffies, tp_wakeup_timecheck)) {
				tp_wakeup_status = TP_WAKEUP_INIT;
			}
		}
	}
...
//       ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_SENSOR_ID, &sensor_id, 1);
	...
	GTP_INFO("fly-->X_MAX: %d, Y_MAX: %d, TRIGGER: 0x%02x", ts->abs_x_max, ts->abs_y_max, ts->int_trigger_type);

	if ((ts->abs_x_max == 0) && (ts->abs_y_max == 0)) {
		printk("config[RESOLUTION_LOC + 1]=0x%x,config[RESOLUTION_LOC]=0x%x\n",config[RESOLUTION_LOC + 1],config[RESOLUTION_LOC]);
		printk("config[RESOLUTION_LOC + 3]=0x%x\n,config[RESOLUTION_LOC + 2]=0x%x\n",config[RESOLUTION_LOC + 3],config[RESOLUTION_LOC + 2]);
