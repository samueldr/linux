...
static int himax_diag_seq_read(struct seq_file *s, void *v)
{
	...
	if (g_diag_command >= 1 && g_diag_command <= 6) {
		if (g_diag_command <= 3) {
...
#endif
#ifdef HX_TP_PROC_2T2R
		}else if(Is_2T2R && g_diag_command == 4 ) {
	...
	} else if (dsram_type > 0)
	{
		himax_diag_arrange(s);
		seq_printf(s, "\n");
	}
	if((g_diag_command >= 1 && g_diag_command <= 8) || dsram_type > 0)
	{
...
static ssize_t himax_diag_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	...
	//coordinate dump start
	else if (storage_type == 0 && g_diag_command == 0x08)	{
	...
	else if (storage_type > 0 && rawdata_type > 0 && rawdata_type <= 3)
	{
		I("%s,dsram\n",__func__);
	...
	size_t len, loff_t *pos)
{
	...
	if(!HX_PROC_SEND_FLAG)
	{
		...
		{
			if(himax_read_trigger_way(private_ts->client) < 0)
		...
		{
			if(himax_read_trigger_way(private_ts->client) == 1) //Edgd = 1, Level = 0
				count += sprintf(temp_buf + count, "INT is edge trigger.\n");
			else if(himax_read_trigger_way(private_ts->client) == 0)
				count += sprintf(temp_buf + count, "INT is level trigger.\n");
			else
				count += sprintf(temp_buf + count, "Unkown trigger type.\n");
		}
