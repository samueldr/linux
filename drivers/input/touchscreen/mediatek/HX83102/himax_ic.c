...
// VCOM		//0x9008_806C ==> 0x0000_0001
	...
	himax_83110_Flash_Write_Burst(tmp_addr, tmp_data);

	msleep(20);

	himax_83110_SenseOff();
...
// Switch Type (BaseC)	//0x8002_0324 ==> 0x0000_000D
//[Option:0x0A(Result),0x0B(Dev Rx Error),0x0C(Dev Tx Error),0x0D(BaseC),0x0E(Golden BaseC)]
	tmp_addr[3] = 0x80; tmp_addr[2] = 0x02; tmp_addr[1] = 0x03; tmp_addr[0] = 0x24;
...
//Read Data_Depth(0x9006_000C)
		...
		RegisterRead83110(tmp_addr, 1, &info_data[0]);
...
//Receive 128 Bytes(0x9006_0000)
			tmp_addr[3] = 0x90; tmp_addr[2] = 0x06; tmp_addr[1] = 0x00; tmp_addr[0] = 0x00;
			RegisterRead83110(tmp_addr, 31, &info_data[0]);
			I("info_data header is 0x%x, 0x%x, 0x%x, 0x%x \r\n", info_data[0], info_data[1], info_data[2], info_data[3]);
//#ifdef HI_MAX_DEEBUG
//		 I("info_data header is 0x%x, 0x%x, 0x%x, 0x%x \r\n", info_data[0], info_data[1], info_data[2], info_data[3]);
//#endif
...
//Receive stack_data Bytes(0x9006_0000)
		       	tmp_addr[3] = 0x90; tmp_addr[2] = 0x06; tmp_addr[1] = 0x00; tmp_addr[0] = 0x00;
		        RegisterRead83110(tmp_addr, stack_data, &info_data[0]);
		        I("info_data header is 0x%x, 0x%x, 0x%x, 0x%x \r\n", info_data[0], info_data[1], info_data[2], info_data[3]);
//#ifdef HI_MAX_DEEBUG
//	   			 I("info_data header is 0x%x, 0x%x, 0x%x, 0x%x \r\n", info_data[0], info_data[1], info_data[2], info_data[3]);
//#endif
...
// Inspection complete check	//0x9008_8084
	while ( check_cnt <100) {
		tmp_data[3] = 0x00; tmp_data[2] = 0x00; tmp_data[1] = 0x00; tmp_data[0] = 0x00;
		tmp_addr[3] = 0x90; tmp_addr[2] = 0x08; tmp_addr[1] = 0x80; tmp_addr[0] = 0x84;
		RegisterRead83110(tmp_addr, 1, tmp_data);
		if (tmp_data[0]==0xAA){
	...
	// Switch to normal type	//0x8002_0324 ==> 0x0000_0000
	tmp_addr[3] = 0x80; tmp_addr[2] = 0x02; tmp_addr[1] = 0x03; tmp_addr[0] = 0x24;
	tmp_data[3] = 0x00; tmp_data[2] = 0x00; tmp_data[1] = 0x00; tmp_data[0] = 0x00;
	himax_83110_Flash_Write_Burst(tmp_addr, tmp_data);
...
void himax_sense_on(struct i2c_client *client, uint8_t FlashMode)
{
	...
	for (i = 0; i < 5; i++)
	{
		...
		if (tmp_data[1] != 0x01 || tmp_data[0] != 0x00)
		{
			tmp_addr[0] = 0x98;
			tmp_data[0] = 0x00;
			himax_register_write(client, tmp_addr, 4, tmp_data, false);
...
int fts_ctpm_fw_upgrade_with_sys_fs_64k(struct i2c_client *client, unsigned char *fw, int len, bool change_iref) //Alice - Un
{
	...
	CRC_from_FW = himax_hw_check_CRC(client,tmp_data, 0x0F000);
	I("%s: CRC_from_FW =%x \n", __func__,CRC_from_FW);
	burnFW_success = Calculate_CRC_with_AP(fw, CRC_from_FW,fw_image_60k);
	if (!himax_program_reload(client))
    {
			E("Program reload CRC fail!\n");
			return false;
    }
...
int himax_read_trigger_way(struct i2c_client *client)
{
...
void himax_read_FW_ver(struct i2c_client *client)
{
	...
	//=====================================
	// Read FW version : 0x1000_7000
	//=====================================
	
	cmd[3] = 0x10; cmd[2] = 0x00; cmd[1] = 0x70; cmd[0] = 0x00;
	himax_register_read(client, cmd, 4, data, false);

	ic_data->vendor_fw_ver = data[2] << 8 | data[3];
...
void himax_get_DSRAM_data(struct i2c_client *client, uint8_t *info_data)
{
	...
	for (cnt = 0; cnt < 100; cnt++)
	{
		...
		msleep(10);
		if(tmp_data[1] == 0xA5 && tmp_data[0] == 0x5A )
			{
	...
	{
		check_sum_cal += (info_data[i+1]*256 + info_data[i]);
	}
