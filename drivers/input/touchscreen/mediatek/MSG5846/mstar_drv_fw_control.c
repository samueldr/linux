//liupeng@wind-mobi.com 20160530 begin
////////////////////////////////////////////////////////////////////////////////
//
...
/*=============================================================*/
//wangjun@wind-mobi.com 20170607 begin
...
#endif
//wangjun@wind-mobi.com 20170607 end
...
#include "msg28xx_xxxx_update_bin_0x16.h" // for MSG28xx wangjun@wind-mobi modify ad 20170630
...
u32 g_GestureWakeupMode[2] = {0x00000000, 0x00000000};//liupeng@wind-mobi.com 20160606
...
/*=============================================================*/
//qiumeng@wind-mobi.com 20161013 begin for disable tp auto update FW function
#if 0
...
#endif
//qiumeng@wind-mobi.com 20161013 end for disable tp auto update FW function
...
#define N_BYTES_EVERY_TIME (DEMO_MODE_PACKET_LENGTH-2)
u8 bCallApkToGetFwLog = 0;
unsigned char fwLog[256] = {0};
static int apkPrintFwLog(unsigned char *pPacket)
{
	int i = 0;
	static int cnt = 0;
	 if (pPacket[0] == 0x2C)
	{
		...
		{
			fwLog[i+cnt*N_BYTES_EVERY_TIME] = pPacket[2+i];
		}
		
		if(pPacket[1]==1)
		{
			printk("[Mstar]APK_PRINT_FW_LOG:%s\n",fwLog);
			cnt=0;
			if (g_TouchKObj != NULL)
			{
			    char *pEnvp[2];
...
//                    input_report_key(g_InputDevice, KEY_M, 1);
                    input_report_key(g_InputDevice, 187, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_M, 0);
                    input_report_key(g_InputDevice, 187, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x65:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_W_CHARACTER_FLAG;
        ...
        // wangjun@wind-mobi.com 20170607 begin
         ...
         if (g_ChipType == CHIP_TYPE_MSG26XXM)
            ...
            
        	{
        		u16 fwvr;
        		u16 ver;
        		DrvIcFwLyrGetCustomerFirmwareVersion(&ver, &fwvr, &_gFwVersion);
...
//            DBG(&g_I2cClient->dev, "szDbBusRxData[%d] = 0x%x , %c \n", i*4+3, szDbBusRxData[i*4+3], szDbBusRxData[i*4+3]); // add for debug
    ...
    else if (g_ChipType == CHIP_TYPE_MSG28XX)   
    {
        ...
        mdelay(I2C_SMBUS_READ_COMMAND_DELAY_FOR_SPRD_PLATFORM);
#endif //CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM
		mdelay(10);//liupeng@wind-mobi.com 20160712 
...
//liupeng@wind-mobi.com 20160530 end