...
//lihaiyan@wind-mobi.com 20170412 begin
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
	u32 g_GestureWakeupMode_tmp[2] = {g_GestureWakeupMode_wind,0x00000000};
#endif
//lihaiyan@wind-mobi.com 20170412 end
    ...
    if (pEventData && pEventData->data && nEvent == FB_EVENT_BLANK)
    {
        ...
        else if (*pBlank == FB_BLANK_POWERDOWN)
        {
...
#endif //CONFIG_ENABLE_HOTKNOT
            {
				...
				if (g_GestureWakeupMode_wind != 0x00000000)
				{
					//DrvIcFwLyrOpenGestureWakeup(&g_GestureWakeupMode[0]);
					DrvIcFwLyrOpenGestureWakeup(&g_GestureWakeupMode_tmp[0]);
					return;
				}
				//lihaiyan@wind-mobi.com 20170412 end
            }
...
// wangbing@wind-mobi.com 20170421 begin
#ifdef CONFIG_WIND_DEVICE_INFO
    {
        u16 fwvr = 0;
        u16 ver = 0;
        u8 *_gFwVersion_tmp = NULL;
        
		DrvIcFwLyrGetCustomerFirmwareVersion(&ver, &fwvr, &_gFwVersion_tmp);
...
s32 /*__devexit*/ MsDrvInterfaceTouchDeviceRemove(struct i2c_client *pClient)
{
	...
	tp_gesture_proc_remove(); //lihaiyan@wind-mobi.com 20170412 
