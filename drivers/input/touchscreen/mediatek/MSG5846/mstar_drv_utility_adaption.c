...
s32 IicWriteData(u8 nSlaveId, u8* pBuf, u16 nSize)
{
    ...
    if (g_I2cClient != NULL)
    {
        ...
        {
            PRINTF_ERR("[Mstar]Not allow to execute SmBus command while update firmware.\n");
        }
        else
        {
            ...
            {
                PRINTF_ERR("[Mstar]IicWriteData() error %d\n", rc);
            }
        }
    }
    else
    {
        PRINTF_ERR("[Mstar]i2c client is NULL\n");
    }
    ...
    if (g_I2cClient != NULL)
    {
        ...
        else
        {
            ...
            if (rc < 0)
            {
                PRINTF_ERR("[Mstar]IicWriteData() error %d, nSlaveId=%d, nSize=%d\n", rc, nSlaveId, nSize);
            }
        }            
    }
    else
...
s32 IicReadData(u8 nSlaveId, u8* pBuf, u16 nSize)
{
    ...
    if (g_I2cClient != NULL)
    {
        ...
        else
        {
            ...
            {
                PRINTF_ERR("[Mstar]IicReadData() error %d\n", rc);
            }
        }
    }
    else
    ...
    if (g_I2cClient != NULL)
    {
        ...
        else
        {
            ...
            if (rc < 0)
            {
                PRINTF_ERR("[Mstar]IicReadData() error %d, nSlaveId=%d, nSize=%d\n", rc, nSlaveId, nSize);
            }
        }            
    }
    else
...
s32 IicSegmentReadDataByDbBus(u8 nRegBank, u8 nRegAddr, u8* pBuf, u16 nSize, u16 nMaxI2cLengthLimit)
{
    ...
    if (g_I2cClient != NULL)
    {
        ...
        PRINTF_ERR("[Mstar]nSegmentLength = %d\n", nSegmentLength);	// add for debug
        ...
        while (nLeft > 0)
        {
            ...
            PRINTF_ERR("[Mstar]nRegBank = 0x%x\n", nRegBank);	// add for debug
            PRINTF_ERR("[Mstar]nRegAddr = 0x%x\n", nRegAddr);	// add for debug
            ...
            if (nLeft > nSegmentLength)
            {
                if ((nRegAddr + nSegmentLength) < MAX_TOUCH_IC_REGISTER_BANK_SIZE)
                {
                    ...
                    PRINTF_ERR("[Mstar]nNextRegAddr = 0x%x\n", nNextRegAddr);	// add for debug
                ...
                else if ((nRegAddr + nSegmentLength) == MAX_TOUCH_IC_REGISTER_BANK_SIZE)
                {
                    ...
                    PRINTF_ERR("[Mstar]nNextRegBank = 0x%x\n", nNextRegBank);	// add for debug
                ...
                else // ((nRegAddr + nSegmentLength) > MAX_TOUCH_IC_REGISTER_BANK_SIZE)
                {
                    ...
                    PRINTF_INFO("[Mstar]nNextRegBank = 0x%x\n", nNextRegBank);	// add for debug
                    
                    nOver = (nRegAddr + nSegmentLength) - MAX_TOUCH_IC_REGISTER_BANK_SIZE;

                    PRINTF_ERR("[Mstar]nOver = 0x%x\n", nOver);	// add for debug
...
//                    nOffset += msgs[1].len;
            ...
            {
                PRINTF_ERR("[Mstar]IicSegmentReadDataByDbBus() -> i2c_transfer() error %d\n", rc);
    ...
    if (g_I2cClient != NULL)
    {
        ...
        if (NULL != I2CDMABuf_va)
        {
            ...
            while (nLeft > 0)
            {
                ...
                if (rc < 0)
                {
                    PRINTF_ERR("[Mstar]IicSegmentReadDataByDbBus() -> i2c_master_send() error %d\n", rc);
                ...
                if (rc < 0)
                {
                    PRINTF_ERR("[Mstar]IicSegmentReadDataByDbBus() -> i2c_master_recv() error %d\n", rc);
        ...
        {
            PRINTF_ERR("[Mstar]IicSegmentReadDataByDbBus() -> I2CDMABuf_va is NULL\n");
        }
#else
        while (nLeft > 0)
        {
...
//                nOffset += msgs[1].len;
            ...
            {
                PRINTF_ERR("[Mstar]IicSegmentReadDataBySmBus() -> i2c_transfer() error %d\n", rc);
    ...
    if (g_I2cClient != NULL)
    {
        ...
        while (nLeft > 0)
        {
            ...
            if (rc < 0)
            {
                PRINTF_ERR("[Mstar]IicSegmentReadDataBySmBus() -> i2c_master_send() error %d\n", rc);
            ...
            if (rc < 0)
            {
                PRINTF_ERR("[Mstar]IicSegmentReadDataBySmBus() -> i2c_master_recv() error %d\n", rc);
