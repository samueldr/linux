...
#define FTS_I2C_SLAVE_ADDR                  0x3A   // 0X38 lihaiyan@wind-mobi.com 
...
// wangjun@wind-mobi.com 20170606 begin
...
// wangjun@wind-mobi.com 20170606 end
...
void fts_irq_disable(void)
{
    ...
    if (!ft_irq_disable) {
        disable_irq_nosync(ft_touch_irq);  //lihaiyan@wind-mobi.com 20170313 FAE suggests 
        ft_irq_disable = 1;
    }
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("[wjwind] ft8006 probe()\n");
    if (fts_i2c_client->addr != FTS_I2C_SLAVE_ADDR) {
        FTS_INFO("[TPD]Change i2c addr 0x%02x ,start to change ", fts_i2c_client->addr);
...
reset_proc:
    ...
    printk("[wjwind] ft8006: err = %d data = %d\n",err, data);
...
// wangjun@wind-mobi.com 20170606 begin
    ...
    msleep(5);    
    {
        u8 fwvr = 0xFF;
        u8 ver = 0xFF;
        printk("ft8006 get TP information\n");
        //fts_i2c_read_reg(client, 0xA6, &fwvr);
        //fts_i2c_read_reg(client, 0xA8, &ver);
        sprintf(wind_device_info.ctp_module_info.ic_name, "%s", "ft8006");
...
//lihaiyan@wind-mobi.com 20160209 add  begin
...
#endif
//lihaiyan@wind-mobi.com 20170209 end
    //  fts_release_all_finger();  //lihaiyan@wind-mobi.com FAE suggestS move
