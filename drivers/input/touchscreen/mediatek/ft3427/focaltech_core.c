...
int fts_power_init(void)
{
   ...
   if (tpd->tpd_dev->of_node) {
	...
	printk("ft3427 of_find_compatible_node-->mediatek,mt6797-touch,error\n");
   }
...
reset_proc:
	...
	tpd_gpio_output(ft_tpd_rst_gpio_number, 1);
	msleep(400);
	err = fts_i2c_read_reg(fts_i2c_client, 0x00, &data);
	printk("ft3427 fts_i2c: data = %d\n", data);
	if(err< 0 || data!=0)// reg0 data running state is 0; other state is not 0
	{
	    	printk("ft3427 I2C transfer error\n");
...
//hebiao@wind-mobi.com 20160101 begin
	...
	msleep(5);
	{
		...
		fts_i2c_read_reg(client, 0xA6, &fwvr);
		fts_i2c_read_reg(client, 0xA8, &ver);
...
static void tpd_suspend(struct device *h)
{
    ...
    AF_Into_Power_Down_Mode();  //hebiao@wind-mobi.com for comsumption
