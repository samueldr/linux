...
u8 fts_ctpm_check_fw_status(struct i2c_client * client)
{
    ...
    auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] = 0x00;
    fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
    printk("ft3427 ID1=0x%x, ID2 = 0x%x\n", reg_val[0], reg_val[1]);
#endif
    ...
    for (i = 0; i < 5; i++) {
        ...
        if ((uc_chip_id == chip_types.chip_idh) && ((uc_tp_vendor_id == FTS_VENDOR_1_ID) || (uc_tp_vendor_id == FTS_VENDOR_2_ID))) {
		break;
        }
        else {
            continue;
     ...
     FTS_DEBUG
        ("[UPGRADE]: uc_tp_vendor_id = %x, uc_chip_id = %x, chip_types.chip_idh = %x, FTS_VENDOR_1_ID = %x, FTS_VENDOR_2_ID=0x%x!!",
         uc_tp_vendor_id, uc_chip_id, chip_types.chip_idh, FTS_VENDOR_1_ID, FTS_VENDOR_2_ID);
//hebiao@wind-mobi.com begin at 20170101 for guoxian and txd compatible
