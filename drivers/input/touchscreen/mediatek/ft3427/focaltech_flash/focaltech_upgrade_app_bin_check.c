...
u16 DrvCRC(u8 * pbt_buf, u16 addr, u16 length)
{
    ...
    FTS_DEBUG("[UPGRADE][ECC] : %04x  data:%04x, len:%04x!!", (addr), debug,
              length);
    for (i = 0; i < length; i++) {
...
bool fts_check_app_bin(u8 * pbt_buf, u32 dw_lenth)
{
    ...
    // 2 PART1 ECC
    if (!task_check_mem_ecc
        (pbt_buf, APP_ADDR_PART1_START, APP_ADDR_PART1_LEN, ADDR_PART1_ECC)) {
    ...
    return task_check_mem_ecc(pbt_buf, APP_ADDR_PART2_START, len1,
                              ADDR_PART2_ECC);
}
...
bool fts_check_app_bin(u8 * pbt_buf, u32 dw_lenth)
{
    ...
    return task_check_mem_ecc(pbt_buf, APP_ADDR_PART2_START, len,
                              ADDR_PART2_ECC);
}
#else
