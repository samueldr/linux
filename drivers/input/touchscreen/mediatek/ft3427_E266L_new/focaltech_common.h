...
#define FTS_CHIP_TYPE_MAPPING {_FT3427, 0x54, 0x22, 0x54, 0x22, 0x00, 0x00, 0x54, 0x2C}
...
extern struct fts_Upgrade_Fun fts_updatefun_onsell;

/* i2c communication*/
int fts_i2c_write_reg_onsell(struct i2c_client *client, u8 regaddr, u8 regvalue);
int fts_i2c_read_reg_onsell(struct i2c_client *client, u8 regaddr, u8 * regvalue);
int fts_i2c_read_onsell(struct i2c_client *client, char *writebuf, int writelen,
                 char *readbuf, int readlen);
int fts_i2c_write_onsell(struct i2c_client *client, char *writebuf, int writelen);
int fts_i2c_init_onsell(void);
int fts_i2c_exit_onsell(void);
...
#define FTS_INFO(fmt, args...) do { \
            if (g_show_log_onsell) {printk(KERN_ERR "[FTS][Info]"fmt"\n", ##args);} \
        }  while (0)

#define FTS_ERROR(fmt, args...)  do { \
             if (g_show_log_onsell) {printk(KERN_ERR "[FTS][Error]"fmt"\n", ##args);} \
        }  while (0)
