...
int sleep_flag; //0 wake 1 sleep 


/*****************************************************************************
* Private constant and macro definitions using #define
*****************************************************************************/
#define FTS_DRIVER_NAME_ON_SELL                     "fts_ts_onsell"
#define INTERVAL_READ_REG                   20  //interval time per read reg unit:ms
#define TIMEOUT_READ_REG                    300 //timeout of read reg unit:ms
#define FTS_I2C_SLAVE_ADDR_ON_SELL                  0x38
#define FTS_READ_TOUCH_BUFFER_DIVIDED_ON_SELL       1
/*****************************************************************************
* Static variables
*****************************************************************************/
struct i2c_client *fts_i2c_client_onsell;
struct input_dev *fts_input_dev_onsell;
struct task_struct *thread_tpd_onsell;
...
#if FTS_DEBUG_EN
int g_show_log_onsell = 1;
#else
int g_show_log_onsell = 0;
#endif

unsigned int ft_tpd_rst_gpio_number = 0;
unsigned int ft_tpd_int_gpio_number = 1;
...
*****************************************************************************/
static const struct i2c_device_id fts_tpd_id[] = { {FTS_DRIVER_NAME_ON_SELL, 0}, {} };
...
/*****************************************************************************
*  Name: fts_wait_tp_to_valid_onsell
...
*****************************************************************************/
int fts_wait_tp_to_valid_onsell(struct i2c_client *client)
{
    int ret = 0;
    int cnt = 0;
    u8 reg_value = 0;
    ...
    do {
        ret = fts_i2c_read_reg_onsell(client, FTS_REG_CHIP_ID, &reg_value);
        if ((ret < 0) || (reg_value != chip_types_onsell.chip_idh)) {
            FTS_INFO("TP Not Ready, ReadData = 0x%x", reg_value);
        }
        else if (reg_value == chip_types_onsell.chip_idh) {
...
*  Return:
*****************************************************************************/
void fts_tp_state_recovery_onsell(struct i2c_client *client)
{
    /* wait tp stable */
    ...
    fts_ex_mode_recovery_onsell(client);
...
/*****************************************************************************
*  Name: fts_reset_proc_onsell
*  Brief: Execute reset operation
*  Input: hdelayms - delay time unit:ms
*  Output:
*  Return: 0 - Get correct Device ID
*****************************************************************************/
int fts_reset_proc_onsell(int hdelayms)
{
    tpd_gpio_output(ft_tpd_rst_gpio_number, 0);
    msleep(20);
    tpd_gpio_output(ft_tpd_rst_gpio_number, 1);
    msleep(hdelayms);
...
int fts_power_init_onsell(void)
{
   int ret;
    /*set TP volt */
   ...
   if (tpd->tpd_dev->of_node) 
   {
	...
	ret = regulator_set_voltage(tpd->reg, 2800000, 2800000);	//modified by hebiao 2016907
	if (ret != 0) {
		printk("regulator_set_voltage(%d) failed!\n", ret);
		return -1;
		}
	}else {
	printk("ft3427 of_find_compatible_node-->mediatek,mt6755-touch,error\n");
   }
...
void fts_power_suspend_onsell(void)
{
    int retval;
...
int fts_power_resume_onsell(void)
{
    int retval = 0;
...
static void tpd_down(int x, int y, int p, int id)
{
    ...
    if (FTS_REPORT_PRESSURE_EN) {
        FTS_DEBUG("[A]P%d(%4d,%4d)[p:%d] DOWN!", id, x, y, pressure);
    }
    else {
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
...
#if FTS_READ_TOUCH_BUFFER_DIVIDED_ON_SELL
    u8 pointnum;
    memset(data, 0xff, POINT_READ_BUF);
    ret = fts_i2c_read_onsell(fts_i2c_client_onsell, writebuf, 1, data, 3);
    if (ret < 0) {
    ...
    if (pointnum > 0) {
        writebuf[0] = 0x03;
        ret =
            fts_i2c_read_onsell(fts_i2c_client_onsell, writebuf, 1, data + 3,
...
#else
    ret = fts_i2c_read_onsell(fts_i2c_client_onsell, writebuf, 1, data, POINT_READ_BUF);
    if (ret < 0) {
        FTS_ERROR("[B]Read touchdata failed, ret: %d", ret);
        FTS_FUNC_EXIT();
        return ret;
...
static int fts_read_touchdata(struct ts_event *data)
{
    ...
    buf[0] = 0x0;
    ret = fts_i2c_read_onsell(fts_i2c_client_onsell, buf, 1, buf, 3);
    if (ret < 0) {
        FTS_ERROR("[B]Read touchdata failed, ret: %d", ret);
        return ret;
    }
    ...
    if (data->touch_point_num > 0) {
        ret =
            fts_i2c_read_onsell(fts_i2c_client_onsell, buf + 3, 1, buf + 3,
...
#else
    ret = fts_i2c_read_onsell(fts_i2c_client_onsell, buf, 1, buf, POINT_READ_BUF);
    if (ret < 0) {
        FTS_ERROR("[B]Read touchdata failed, ret: %d", ret);
        FTS_FUNC_EXIT();
        return ret;
...
static int fts_report_value(struct ts_event *data)
{
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (data->au8_touch_event[i] == 0 || data->au8_touch_event[i] == 2) {
            ...
            //data->au16_x[i] = tpd_dts_data.tpd_resolution[0] - 1 - data->au16_x[i];
            //data->au16_y[i] = tpd_dts_data.tpd_resolution[1] - 1 - data->au16_y[i];
...
#if FTS_GESTURE_EN
        ret = fts_i2c_read_reg_onsell(fts_i2c_client_onsell, FTS_REG_GESTURE_EN, &state);
        if (ret < 0) {
            FTS_ERROR("[Focal][Touch] read value fail");
        }
        if (state == 1) {
            fts_gesture_readdata(fts_i2c_client_onsell);
            continue;
        }
#endif

#if FTS_PSENSOR_EN
        if (fts_proximity_readdata(fts_i2c_client_onsell) == 0)
            continue;
#endif
...
extern  struct i2c_client *GT9762AF_I2Cclient_common_E266;
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("[wjwind] ft3427 probe() \n");
    if (fts_i2c_client_onsell->addr != FTS_I2C_SLAVE_ADDR_ON_SELL) 
    {
        FTS_INFO("[TPD]Change i2c addr 0x%02x to 0x38", fts_i2c_client_onsell->addr);
        fts_i2c_client_onsell->addr = FTS_I2C_SLAVE_ADDR_ON_SELL;
        FTS_DEBUG("[TPD]fts_i2c_client_onsell->addr=0x%x\n", fts_i2c_client_onsell->addr);
    }
    ...
    tpd_gpio_as_int(ft_tpd_int_gpio_number);
    /* Init I2C */
    fts_i2c_init_onsell();
reset_proc:
    /* Reset CTP */
    ...
    tpd_gpio_output(ft_tpd_rst_gpio_number, 1);
    msleep(400);
    err = fts_i2c_read_reg_onsell(fts_i2c_client_onsell, 0x00, &data);
    // printk("qiangangft3427 fts_i2c: data = %d\n", data);
    if(err< 0 || data!=0)// reg0 data running state is 0; other state is not 0
    {
        // printk("qiangangft3427 I2C transfer error\n");
		...
		fts_i2c_exit_onsell();
		gpio_free(ft_tpd_rst_gpio_number);
		gpio_free(ft_tpd_int_gpio_number);
		return -1;
	}
   ...
   // fts_i2c_init_onsell();
    ...
    tp_gesture_proc_init();             // wangbing@wind-mobi.com 20170503
#endif
    ...
    fts_reset_proc_onsell(200);
    fts_wait_tp_to_valid_onsell(client);

    tpd_load_status = 1;

    fts_ctpm_get_upgrade_array_onsell();

#if FTS_SYSFS_NODE_EN
    fts_create_sysfs_onsell(client);
#endif

    fts_ex_mode_init_onsell(client);

#if FTS_APK_NODE_EN
    fts_create_apk_debug_channel_onsell(client);
#endif
    ...
    if (IS_ERR(thread_tpd_onsell)) {
        ...
        FTS_ERROR("[TPD]Failed to create kernel thread_tpd_onsell,ret=%d!", retval);
    }
...
// wangbing@wind-mobi.com 20170502 begin
    ...
    msleep(5);    
    {
        u8 fwvr;
        u8 ver;
        printk("ft3427 get TP information\n");
        fts_i2c_read_reg_onsell(client, 0xA6, &fwvr);
...
static int tpd_remove(struct i2c_client *client)
{
...
#if FTS_SYSFS_NODE_EN
    fts_remove_sysfs_onsell(client);
#endif

    fts_ex_mode_exit_onsell(client);
...
#if FTS_APK_NODE_EN
    fts_release_apk_debug_channel_onsell();
#endif
    ...
    tp_gesture_proc_remove();           // wangbing@wind-mobi.com 20170503
#endif
...
static void tpd_suspend(struct device *h)
{
...
#if FTS_GESTURE_EN
    retval = fts_gesture_suspend(fts_i2c_client_onsell);
    if (retval == 0) {
    ...
    /* TP enter sleep mode */
    retval =
        fts_i2c_write_reg_onsell(fts_i2c_client_onsell, FTS_REG_POWER_MODE,
...
static int touch_irqdepth(int irqnum)
{
...
#if FTS_GESTURE_EN
    if (fts_gesture_resume(fts_i2c_client_onsell) == 0) {
        FTS_FUNC_EXIT();
		//hebiao@wind-mobi.com 20161224 begin for tp no_function
		irq_depth = touch_irqdepth(ft_touch_irq);
		printk("ft3427 irq = %d touch_irq_no=%d\n", irq_depth, ft_touch_irq);
...
//hebiao@wind-mobi.com 20161224 end for tp no_function
    ...
    fts_tp_state_recovery_onsell(fts_i2c_client_onsell);
...
#ifdef CONFIG_MTK_SENSOR_HUB_SUPPORT
    fts_sensor_enable(fts_i2c_client_onsell);
#else
    //hebiao@wind-mobi.com 20161224 begin for tp no_function
	irq_depth = touch_irqdepth(ft_touch_irq);
	printk("ft3427 irq = %d touch_irq_no=%d\n", irq_depth, ft_touch_irq);
...
struct device_attribute *fts_attrs_onsell[] = {
#ifdef CONFIG_MTK_SENSOR_HUB_SUPPORT
...
static struct tpd_driver_t tpd_device_driver = {
    .tpd_device_name = FTS_DRIVER_NAME_ON_SELL,
    .tpd_local_init = tpd_local_init,
    .suspend = tpd_suspend,
    .resume = tpd_resume,
    .attrs = {
              .attr = fts_attrs_onsell,
              .num = ARRAY_SIZE(fts_attrs_onsell),
              },
};
