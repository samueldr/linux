...
static void esd_process(u8 * writebuf, int buflen, bool flag)
{
    if (flag) {
        if ((writebuf[1] == 0xFC) && (writebuf[2] == 0x55) && (buflen == 0x03)) {
            /* Upgrade command */
            FTS_DEBUG("[ESD]: Upgrade command(%x %x %x)!!", writebuf[0],
                      writebuf[1], writebuf[2]);
            fts_esdcheck_switch(DISABLE);
        }
        else if ((writebuf[1] == 0x00) && (writebuf[2] == 0x40)
                 && (buflen == 0x03)) {
            /* factory mode bit 4 5 6 */
            FTS_DEBUG("[ESD]: Entry factory mode(%x %x %x)!!", writebuf[0],
                      writebuf[1], writebuf[2]);
            fts_esdcheck_switch(DISABLE);
        }
            ...
            FTS_DEBUG("[ESD]: Exit factory mode(%x %x %x)!!", writebuf[0],
                      writebuf[1], writebuf[2]);
        ...
        if ((writebuf[1] == 0x07) && (buflen == 0x02)) {
            FTS_DEBUG("[ESD]: Upgrade finish-trigger reset(07)(%x %x)!!",
...
#endif
            ret =
                fts_updatefun_curr_onsell.upgrade_with_app_bin_file(fts_i2c_client_onsell,
                                                             upgrade_file_path);
#if FTS_ESDCHECK_EN
            fts_esdcheck_switch(ENABLE);
#endif
                              ...
                              size_t count, loff_t * ppos)
{
    ...
    case PROC_UPGRADE:
        ...
        ret = fts_i2c_read_reg_onsell(fts_i2c_client_onsell, regaddr, &regvalue);
        if (ret < 0)
            num_read_chars = sprintf(buf, "%s", "get fw version failed.\n");
        else
            num_read_chars =
                sprintf(buf, "current fw version:0x%02x\n", regvalue);
        break;
    case PROC_READ_REGISTER:
        readlen = 1;
        ret = fts_i2c_read_onsell(fts_i2c_client_onsell, NULL, 0, buf, readlen);
        if (ret < 0) {
#if FTS_ESDCHECK_EN
            fts_esdcheck_proc_busy(0);
#endif
...
static int fts_debug_write(struct file *filp,
                           const char __user * buff, unsigned long len,
                           void *data)
{
    unsigned char writebuf[WRITE_BUF_SIZE];
    int buflen = len;
...
#endif
/************************************************************************
* Name: fts_create_apk_debug_channel_onsell
* Brief:  create apk debug channel
* Input: i2c info
* Output: no
* Return: success =0
***********************************************************************/
int fts_create_apk_debug_channel_onsell(struct i2c_client *client)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
...
/************************************************************************
* Name: fts_release_apk_debug_channel_onsell
* Brief:  release apk debug channel
* Input: no
* Output: no
* Return: no
***********************************************************************/
void fts_release_apk_debug_channel_onsell(void)
{
                                ...
                                struct device_attribute *attr, char *buf)
{
    ...
    fts_esdcheck_proc_busy(1);
#endif
    if (fts_i2c_read_reg_onsell(fts_i2c_client_onsell, FTS_REG_FW_VER, &fwver) < 0) {
...
static void fts_str_to_bytes(char *bufStr, int iLen, char *uBytes,
                             int *iBytesLen)
{
    int i = 0;
    int iNumChLen = 0;
                                ...
                                struct device_attribute *attr, char *buf)
{
    ...
    if (!g_rwreg_result.op) {
        if (g_rwreg_result.result == 0) {
            count =
                sprintf(buf, "Read %02X: %02X\n", g_rwreg_result.reg,
                        g_rwreg_result.value);
        }
        else {
            count =
                sprintf(buf, "Read %02X failed, ret: %d\n", g_rwreg_result.reg,
        ...
        if (g_rwreg_result.result == 0) {
            count =
                sprintf(buf, "Write %02X, %02X success\n", g_rwreg_result.reg,
                        g_rwreg_result.value);
        }
        else {
            count =
                sprintf(buf, "Write %02X failed, ret: %d\n", g_rwreg_result.reg,
                        g_rwreg_result.result);
        }
    }
    mutex_unlock(&fts_input_dev_onsell->mutex);
                                 ...
                                 size_t count)
{
    ...
    if (2 == num_read_chars) {
        ...
        /*read register */
        regaddr = wmreg;
        g_rwreg_result.result = fts_i2c_read_reg_onsell(client, regaddr, &regvalue);
        if (g_rwreg_result.result < 0) {
        ...
        g_rwreg_result.reg = regaddr;
        g_rwreg_result.value = regvalue;
        g_rwreg_result.result = fts_i2c_write_reg_onsell(client, regaddr, regvalue);
        if (g_rwreg_result.result < 0) {
            FTS_ERROR("Could not write the register(0x%02x)", regaddr);

        }
        else {
            FTS_INFO("Write 0x%02x into register(0x%02x) successful", regvalue,
                     regaddr);
            g_rwreg_result.result = 0;
        }
    }
                                  ...
                                  const char *buf, size_t count)
{
    ...
    if (i_ret == 0) {
        msleep(300);
        uc_host_fm_ver = fts_updatefun_curr_onsell.get_app_i_file_ver();
        FTS_DEBUG("[EX]: upgrade to new version 0x%x!!", uc_host_fm_ver);
    }
    else {
        FTS_ERROR("[EX]: upgrade failed ret=%d!!", i_ret);
    }
                                      ...
                                      const char *buf, size_t count)
{
    ...
    fts_esdcheck_switch(DISABLE);
#endif
    fts_updatefun_curr_onsell.upgrade_with_app_bin_file(client, fwname);
#if FTS_ESDCHECK_EN
    fts_esdcheck_switch(ENABLE);
#endif
    enable_irq(client->irq);
                                      ...
                                      struct device_attribute *attr, char *buf)
{
    ...
    count +=
        sprintf(buf + count, "FTS_DEBUG_EN: \t\t\t%s\n",
                FTS_DEBUG_EN ? "ON" : "OFF");
#if defined(FTS_MT_PROTOCOL_B_EN)
    count +=
        sprintf(buf + count, "TS_MT_PROTOCOL_B_EN: \t\t%s\n",
                FTS_MT_PROTOCOL_B_EN ? "ON" : "OFF");
#endif
    count +=
        sprintf(buf + count, "FTS_GESTURE_EN: \t\t%s\n",
                FTS_GESTURE_EN ? "ON" : "OFF");
    count +=
        sprintf(buf + count, "FTS_ESDCHECK_EN: \t\t%s\n",
                FTS_ESDCHECK_EN ? "ON" : "OFF");
#if defined(FTS_PSENSOR_EN)
    count +=
        sprintf(buf + count, "FTS_PSENSOR_EN: \t\t%s\n",
                FTS_PSENSOR_EN ? "ON" : "OFF");
#endif
    count +=
        sprintf(buf + count, "FTS_GLOVE_EN: \t\t\t%s\n",
                FTS_GLOVE_EN ? "ON" : "OFF");
    count +=
        sprintf(buf + count, "FTS_COVER_EN: \t\t%s\n",
                FTS_COVER_EN ? "ON" : "OFF");
    count +=
        sprintf(buf + count, "FTS_CHARGER_EN: \t\t\t%s\n",
                FTS_CHARGER_EN ? "ON" : "OFF");

    count +=
        sprintf(buf + count, "FTS_REPORT_PRESSURE_EN: \t\t%s\n",
                FTS_REPORT_PRESSURE_EN ? "ON" : "OFF");
    count +=
        sprintf(buf + count, "FTS_FORCE_TOUCH_EN: \t\t%s\n",
                FTS_FORCE_TOUCH_EN ? "ON" : "OFF");

    count +=
        sprintf(buf + count, "FTS_TEST_EN: \t\t\t%s\n",
                FTS_TEST_EN ? "ON" : "OFF");
    count +=
        sprintf(buf + count, "FTS_APK_NODE_EN: \t\t%s\n",
                FTS_APK_NODE_EN ? "ON" : "OFF");
    count +=
        sprintf(buf + count, "FTS_POWER_SOURCE_CUST_EN: \t%s\n",
                FTS_POWER_SOURCE_CUST_EN ? "ON" : "OFF");
    count +=
        sprintf(buf + count, "FTS_AUTO_UPGRADE_EN: \t\t%s\n",
                                ...
                                struct device_attribute *attr, char *buf)
{
    ...
    fts_i2c_read_reg_onsell(client, FTS_REG_POWER_MODE, &regvalue);    //power mode 0:active 1:monitor 3:sleep
    count += sprintf(tmp + count, "Power Mode:0x%02x\n", regvalue);

    fts_i2c_read_reg_onsell(client, FTS_REG_FW_VER, &regvalue);    //FWver
    count += sprintf(tmp + count, "FW Ver:0x%02x\n", regvalue);

    fts_i2c_read_reg_onsell(client, FTS_REG_VENDOR_ID, &regvalue); //Vendor ID
    count += sprintf(tmp + count, "Vendor ID:0x%02x\n", regvalue);

    fts_i2c_read_reg_onsell(client, FTS_REG_LCD_BUSY_NUM, &regvalue);  //LCD Busy number
    count += sprintf(tmp + count, "LCD Busy Number:0x%02x\n", regvalue);

    fts_i2c_read_reg_onsell(client, FTS_REG_GESTURE_EN, &regvalue);    // 1 Gesture mode,0 Normal mode
    count += sprintf(tmp + count, "Gesture Mode:0x%02x\n", regvalue);

    fts_i2c_read_reg_onsell(client, FTS_REG_CHARGER_MODE_EN, &regvalue);   // 3 charge in
    count += sprintf(tmp + count, "charge stat:0x%02x\n", regvalue);

    fts_i2c_read_reg_onsell(client, FTS_REG_INT_CNT, &regvalue);   //Interrupt counter
    count += sprintf(tmp + count, "INT count:0x%02x\n", regvalue);

    fts_i2c_read_reg_onsell(client, FTS_REG_FLOW_WORK_CNT, &regvalue); //Flow work counter
...
*/
static DEVICE_ATTR(fts_fw_update, S_IRUGO | S_IWUSR, fts_fwupdate_show,
                   fts_fwupdate_store);
...
*/
static DEVICE_ATTR(fts_upgrade_app, S_IRUGO | S_IWUSR, fts_fwupgradeapp_show,
...
static DEVICE_ATTR(fts_dump_reg, S_IRUGO | S_IWUSR, fts_dumpreg_show,
                   fts_dumpreg_store);
static DEVICE_ATTR(fts_show_log, S_IRUGO | S_IWUSR, fts_show_log_show,
                   fts_show_log_store);
static DEVICE_ATTR(fts_module_config, S_IRUGO | S_IWUSR, fts_module_config_show,
                   fts_module_config_store);
#if FTS_ESDCHECK_EN
static DEVICE_ATTR(fts_esd_check, S_IRUGO | S_IWUSR, fts_esdcheck_show,
                   fts_esdcheck_store);
#endif
...
/************************************************************************
* Name: fts_create_sysfs_onsell
* Brief:  create sysfs for debug
* Input: i2c info
* Output: no
* Return: success =0
***********************************************************************/
int fts_create_sysfs_onsell(struct i2c_client *client)
{
    int err;
...
/************************************************************************
* Name: fts_remove_sysfs_onsell
* Brief:  remove sys
* Input: i2c info
* Output: no
* Return: no
***********************************************************************/
int fts_remove_sysfs_onsell(struct i2c_client *client)
{
    sysfs_remove_group(&client->dev.kobj, &fts_attribute_group);
    return 0;
}
