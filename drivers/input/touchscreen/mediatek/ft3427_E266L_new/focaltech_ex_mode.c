...
struct fts_mode_flag g_fts_mode_flag_onsell;
                                    ...
                                    struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "Glove: %s\n",
                    g_fts_mode_flag_onsell.fts_glove_mode_flag ? "On" : "Off");
}
                                     ...
                                     const char *buf, size_t count)
{
    ...
    if (val == 1)               // enter glove mode
    {
        if (!g_fts_mode_flag_onsell.fts_glove_mode_flag) {
            g_fts_mode_flag_onsell.fts_glove_mode_flag = true;
            ret = fts_enter_glove_mode(fts_i2c_client_onsell, true);
        }
        else
            return count;
    }
    else                        // exit glove mode
    {
        if (g_fts_mode_flag_onsell.fts_glove_mode_flag) {
            g_fts_mode_flag_onsell.fts_glove_mode_flag = false;
            ret = fts_enter_glove_mode(fts_i2c_client_onsell, false);
        }
        else
            return count;
    }
...
int fts_enter_glove_mode(struct i2c_client *client, int mode)
{
    ...
    ret = fts_i2c_write_reg_onsell(client, buf_addr[0], buf_value[0]);
    if (ret < 0) {
        FTS_DEBUG("[fts][Touch] fts_enter_glove_mode write value fail \n");
    }
    FTS_DEBUG("[fts][Touch] glove mode :  %d \n", mode);
...
*
*/
static DEVICE_ATTR(fts_touch_glove_mode, S_IRUGO | S_IWUSR,
                                    ...
                                    struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "Cover: %s\n",
                    g_fts_mode_flag_onsell.fts_cover_mode_flag ? "On" : "Off");
}
                                     ...
                                     const char *buf, size_t count)
{
    ...
    val = simple_strtoul(buf, 0, 10);
    if (val == 1)               // enter cover mode
    {
        if (!g_fts_mode_flag_onsell.fts_cover_mode_flag) {
            g_fts_mode_flag_onsell.fts_cover_mode_flag = true;
            ret = fts_enter_cover_mode(fts_i2c_client_onsell, true);
        }
        else
            return count;
    }
    else                        // exit cover mode
    {
        if (g_fts_mode_flag_onsell.fts_cover_mode_flag) {
            g_fts_mode_flag_onsell.fts_cover_mode_flag = false;
            ret = fts_enter_cover_mode(fts_i2c_client_onsell, false);
        }
        else
            return count;
    }
...
int fts_enter_cover_mode(struct i2c_client *client, int mode)
{
    ...
    ret = fts_i2c_write_reg_onsell(client, buf_addr[0], buf_value[0]);
    if (ret < 0) {
        FTS_DEBUG("[fts][Touch] fts_enter_cover_mode write value fail \n");
    }
    FTS_DEBUG("[fts][Touch] cover mode :  %d \n", mode);
...
*
*/
static DEVICE_ATTR(fts_touch_cover_mode, S_IRUGO | S_IWUSR,
                                      ...
                                      struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "Charger: %s\n",
                    g_fts_mode_flag_onsell.fts_charger_mode_flag ? "On" : "Off");
}
                                       ...
                                       const char *buf, size_t count)
{
    ...
    if (val == 1)               // enter charger mode
    {
        if (!g_fts_mode_flag_onsell.fts_charger_mode_flag) {
            g_fts_mode_flag_onsell.fts_charger_mode_flag = true;
            ret = fts_enter_charger_mode(fts_i2c_client_onsell, true);
        }
        else
            return count;
    }
    else                        // exit charger mode
    {
        if (g_fts_mode_flag_onsell.fts_charger_mode_flag) {
            g_fts_mode_flag_onsell.fts_charger_mode_flag = false;
            ret = fts_enter_charger_mode(fts_i2c_client_onsell, false);
        }
        else
            return count;
    }
...
int fts_enter_charger_mode(struct i2c_client *client, int mode)
{
    ...
    ret = fts_i2c_write_reg_onsell(client, buf_addr[0], buf_value[0]);
    if (ret < 0) {
        FTS_DEBUG("[fts][Touch] fts_enter_charger_mode write value fail \n");
    }
    FTS_DEBUG("[fts][Touch] charger mode :  %d \n", mode);
...
*
*/
static DEVICE_ATTR(fts_touch_charger_mode, S_IRUGO | S_IWUSR,
...
#if FTS_GLOVE_EN
    &dev_attr_fts_touch_glove_mode.attr,
#endif

#if FTS_COVER_EN
    &dev_attr_fts_touch_cover_mode.attr,
#endif

#if FTS_CHARGER_EN
    &dev_attr_fts_touch_charger_mode.attr,
#endif
...
int fts_ex_mode_init_onsell(struct i2c_client *client)
{
    int err = 0;
    ...
    if (0 != err) {
        FTS_DEBUG
            ("[focal] %s(void) - ERROR: fts_touch_glove_init(void) failed.\n",
             __func__);
        sysfs_remove_group(&client->dev.kobj, &fts_touch_mode_group);
        return -EIO;
    }
    else {
        FTS_DEBUG("[focal] %s(void) - fts_touch_glove_init(void) succeeded.\n",
                  __func__);
    }
...
int fts_ex_mode_exit_onsell(struct i2c_client *client)
{
    sysfs_remove_group(&client->dev.kobj, &fts_touch_mode_group);
    return 0;
}

int fts_ex_mode_recovery_onsell(struct i2c_client *client)
{
    int ret = 0;
#if FTS_GLOVE_EN
    if (g_fts_mode_flag_onsell.fts_glove_mode_flag)
