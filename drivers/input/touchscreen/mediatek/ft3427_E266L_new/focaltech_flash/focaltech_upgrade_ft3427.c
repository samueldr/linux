...
extern int compatible_flag; //hebiao@wind-mobi.com begin at 20170101 for guoxian and txd compatible
/*****************************************************************************
...
extern unsigned char CTPM_FW_HLT[];
//add by qiangang@wind-mobi.com 20170329 end
struct fts_Upgrade_Fun fts_updatefun_onsell = {
    .chip_idh = 0x54,
    .chip_idl = 0x22,
...
{
    u8 *pbt_buf = NULL;
    int fwsize = fts_GetFirmwareSize_onsell(firmware_name);
    ...
    if (fts_ReadFirmware_onsell(firmware_name, pbt_buf)) {
...
{
    u16 ui_sz;
    ui_sz = fts_getsize_onsell(FW_SIZE);
    if (ui_sz > 2) {
//hebiao@wind-mobi.com begin at 20170101 for guoxian and txd compatible
	if (compatible_flag == 1) {
		FTS_DEBUG("[UPGRADE]:get gx fw_versino\n");
        	return CTPM_FW_GX[ui_sz - 2];
	}else if (compatible_flag == 2) {
		FTS_DEBUG("[UPGRADE]:get txd fw_versino\n");
		return CTPM_FW_TXD[ui_sz - 2];
//add by qiangang@wind-mobi.com 20170329 begin
	}else if (compatible_flag == 3) {
		FTS_DEBUG("[UPGRADE]:get hlt fw_versino\n");
		return CTPM_FW_HLT[ui_sz - 2];

// wangbing@wind-mobi.com 20170502 begin
    }else if (compatible_flag == 4) {
        printk("[UPGRADE]:get hs fw_versino\n");
        return CTPM_FW_HS[ui_sz - 2];  
// wangbing@wind-mobi.com 20170502 end

// wangjun@wind-mobi.com 20170522 begin
    }else if (compatible_flag == 5) {
        printk("[UPGRADE]:get ykl fw_versino\n");
        return CTPM_FW_YKL[ui_sz - 2];  
// wangjun@wind-mobi.com 20170522 end

//add by qiangang@wind-mobi.com 20170329 end
	}else {
		FTS_DEBUG("[UPGRADE]:unknown type\n");
	}
                                       ...
                                       u32 dw_lenth)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ...
        fts_i2c_write_reg_onsell(client, FTS_RST_CMD_REG1, FTS_UPGRADE_AA);
        msleep(fts_updateinfo_curr.delay_aa);

        /*write 0x55 to register FTS_RST_CMD_REG1 */
        fts_i2c_write_reg_onsell(client, FTS_RST_CMD_REG1, FTS_UPGRADE_55);
        msleep(200);
        ...
        if (reg_val[0] == chip_types_onsell.bootloader_idh
    ...
    for (j = 0; j < packet_number; j++) {
        ...
        for (i = 0; i < 30; i++) {
            ...
            FTS_DEBUG("[UPGRADE]: reg_val[0] = %x reg_val[1] = %x!!",
                      reg_val[0], reg_val[1]);
            //msleep(1);
            fts_ctpm_upgrade_delay_onsell(1000);
        }
    }
...
//hebiao@wind-mobi.com begin at 20170101 for guoxian and txd compatible
	if (compatible_flag == 1) {
		FTS_DEBUG("[UPGRADE]:get gx fw size\n");
        	fw_len = fts_getsize_onsell(FW_SIZE);
	}else if (compatible_flag == 2) { 
		FTS_DEBUG("[UPGRADE]:get txd fw size\n");
		fw_len = fts_getsize_onsell(FW_SIZE);
//add by qiangang@wind-mobi.com 20170329 begin
	}else if (compatible_flag == 3) { 
		FTS_DEBUG("[UPGRADE]:get hlt fw size\n");
		fw_len = fts_getsize_onsell(FW_SIZE);
		// printk("qiangangfw_len= %d ",fw_len);
//add by qiangang@wind-mobi.com 20170329 end

// wangbing@wind-mobi.com 20170502 begin
    }else if (compatible_flag == 4) {
        FTS_DEBUG("[UPGRADE]: get hs fw size\n");
        fw_len = fts_getsize_onsell(FW_SIZE);
        printk("[UPGRADE]: fw_len = %d ", fw_len);
// wangbing@wind-mobi.com 20170502 end

// wangjun@wind-mobi.com 20170522 begin
    }else if (compatible_flag == 5) {
        FTS_DEBUG("[UPGRADE]: get ykl fw size\n");
        fw_len = fts_getsize_onsell(FW_SIZE);
        printk("[UPGRADE]: fw_len = %d ", fw_len);
// wangjun@wind-mobi.com 20170522 end
...
//hebiao@wind-mobi.com end at 20170101 for guoxian and txd compatible
    ...
    //hebiao@wind-mobi.com begin at 20170101 for guoxian and txd compatible
	if (compatible_flag == 1) {
		FTS_DEBUG("[UPGRADE]:get gx fw data\n");
        	pbt_buf = CTPM_FW_GX;
	}else if (compatible_flag == 2) { 
		FTS_DEBUG("[UPGRADE]:get txd fw data\n");
		pbt_buf = CTPM_FW_TXD;
//add by qiangang@wind-mobi.com 20170329 begin
	}else if (compatible_flag == 3) { 
		FTS_DEBUG("[UPGRADE]:get txd fw data\n");
...
//add by qiangang@wind-mobi.com 20170329 end
    ...
    else if (compatible_flag == 4)          // wangbing@wind-mobi.com 20170502 begin
    { 
        FTS_DEBUG("[UPGRADE]:get hs fw data\n");
        pbt_buf = CTPM_FW_HS;
    }                                       // wangbing@wind-mobi.com 20170502 end
     else if (compatible_flag == 5)          // wangjun@wind-mobi.com 20170522 begin
    { 
        FTS_DEBUG("[UPGRADE]:get ykl fw data\n");
        pbt_buf = CTPM_FW_YKL;
    }                                       // wangjun@wind-mobi.com 20170522 end
    else 
    {
        FTS_DEBUG("[UPGRADE]:unknown type data\n");
    }
//hebiao@wind-mobi.com end at 20170101 for guoxian and txd compatible
                                                                ...
                                                                *client)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ...
        i_ret = fts_ctpm_i2c_hid2std_onsell(client);
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ...
        i_ret = fts_i2c_read_onsell(client, auc_i2c_write_buf, 0, reg_val, 4);
        if (i_ret < 0) {
            FTS_DEBUG
                ("[UPGRADE]: read vendor id from flash error when i2c write, i_ret = %d!!",
                 i_ret);
