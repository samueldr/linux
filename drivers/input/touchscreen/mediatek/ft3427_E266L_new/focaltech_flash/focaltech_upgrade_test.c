...
*****************************************************************************/
struct wake_lock ps_lock_onsell;
...
int fts_ctpm_auto_upgrade_onsell(struct i2c_client *client)
{
    ...
    wake_lock_init(&ps_lock_onsell, WAKE_LOCK_SUSPEND, "tp_wakelock");

    wake_lock(&ps_lock_onsell);
    ...
    do {
        ...
        FTS_DEBUG_UPGRADE("upgrade %d times, error %d times!!", uc_UpgradeTimes,
                          uc_ErrorTimes);
    }
    while (uc_UpgradeTimes < (FTS_UPGRADE_TEST_NUMBER));

    wake_unlock(&ps_lock_onsell);
