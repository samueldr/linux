...
*****************************************************************************/
#ifdef CONFIG_MTK_I2C_EXTENSION
static void fts_i2c_msg_dma_alloct_onsell(void);
static void fts_i2c_msg_dma_release_onsell(void);
#endif
/*****************************************************************************
* functions body
*****************************************************************************/
                 ...
                 char *readbuf, int readlen)
{
    ...
    if ((writelen > 0) && (writelen <= DMA_BUFFER_LENGTH)) {
        ...
        ret = i2c_master_send(client, (unsigned char *)g_dma_buff_pa_onsell, writelen);
        if (ret != writelen) {
...
static void fts_i2c_msg_dma_alloct_onsell(void)
{
    ...
    if (NULL == g_dma_buff_va_onsell) {
        ...
        g_dma_buff_va_onsell =
            (u8 *) dma_alloc_coherent(&tpd->dev->dev, DMA_BUFFER_LENGTH,
                                      &g_dma_buff_pa_onsell, GFP_KERNEL);
...
static void fts_i2c_msg_dma_release_onsell(void)
{
    FTS_FUNC_ENTER();
    if (g_dma_buff_va_onsell) {
        dma_free_coherent(&tpd->dev->dev, DMA_BUFFER_LENGTH, g_dma_buff_va_onsell,
                          g_dma_buff_pa_onsell);
...
int fts_i2c_exit_onsell(void)
{
    FTS_FUNC_ENTER();
#ifdef CONFIG_MTK_I2C_EXTENSION
    fts_i2c_msg_dma_release_onsell();
#endif
    FTS_FUNC_EXIT();
    return 0;
}
