...
*/
static DEVICE_ATTR(fts_touch_proximity_mode, S_IRUGO | S_IWUSR,
                   fts_touch_proximity_show, fts_touch_proximity_store);
...
static int fts_enter_proximity_mode(struct i2c_client *client, int mode)
{
    ...
    ret = fts_i2c_write_reg_onsell(client, buf_addr, buf_value);
    if (ret < 0) {
                          ...
                          int *actualout)
{
    ...
    case SENSOR_ENABLE:
        if ((buff_in == NULL) || (size_in < sizeof(int))) {
            ...
            /* Enable proximity */
            err = fts_enter_proximity_mode(fts_i2c_client_onsell, value);
        }
        break;
...
int fts_proximity_readdata(struct i2c_client *client)
{
    ...
    fts_i2c_read_reg_onsell(client, FTS_REG_FACE_DEC_MODE_EN, &regvalue);
