...
static s8 fts_sensor_enter_doze(struct i2c_client *client)
{
    ...
    ret = fts_i2c_write_reg_onsell(i2c_client, FTS_REG_GESTURE_EN, gestrue_on);
    if (ret < 0) {
    ...
    for (i = 0; i < 10; i++) {
        fts_i2c_read_reg_onsell(i2c_client, FTS_REG_GESTURE_EN, &gestrue_data);
        if (gestrue_data == 0x01) {
            FTS_DEBUG("FTP has been working in doze mode!");
            break;
        }
        msleep(20);
        fts_i2c_write_reg_onsell(i2c_client, FTS_REG_GESTURE_EN, gestrue_on);
...
void fts_sensor_enable(struct i2c_client *client)
{
    ...
    data = 0x00;
    fts_i2c_write_reg_onsell(client, FTS_REG_GESTURE_EN, data);
}
...
void fts_sensor_suspend(struct i2c_client *i2c_client)
{
    ...
    if (scp_init_flag == 0) {
        ...
        ipi_pkt.param.tcs.i2c_num = TPD_I2C_NUMBER;
        ipi_pkt.param.tcs.int_num = CUST_EINT_TOUCH_PANEL_NUM;
        ipi_pkt.param.tcs.io_int = ft_tpd_int_gpio_number;
        ipi_pkt.param.tcs.io_rst = ft_tpd_rst_gpio_number;
