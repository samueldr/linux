...
*******************************************************************************/
#define FOCALTECH_TEST_INFO  "File Version of  test library:  V1.1.0 2016-08-01"
...
*****************************************************************************/
#if 0
//  old fts_i2c_read_onsell/write function. need to set fts_i2c_client_onsell.
extern struct i2c_client *fts_i2c_client_onsell;
extern int fts_i2c_read_onsell(struct i2c_client *client, char *writebuf, int writelen,
                        char *readbuf, int readlen);
extern int fts_i2c_write_onsell(struct i2c_client *client, char *writebuf,
                         int writelen);
#endif
static int fts_test_i2c_read(unsigned char *writebuf, int writelen,
                             unsigned char *readbuf, int readlen)
{
    int iret = -1;
#if 1
    //  old fts_i2c_read_onsell function. need to set fts_i2c_client_onsell.
    //Modify the i2c_read function that is used in this project
    iret = fts_i2c_read_onsell(fts_i2c_client_onsell, writebuf, writelen, readbuf, readlen);
#else
    iret = fts_i2c_read_onsell(writebuf, writelen, readbuf, readlen);
#endif
...
static int fts_test_i2c_write(unsigned char *writebuf, int writelen)
{
    int iret = -1;
#if 1
    //  old fts_i2c_write_onsell function.  need to set fts_i2c_client_onsell.
    //Modify the i2c_read function that is used in this project
    iret = fts_i2c_write_onsell(fts_i2c_client_onsell, writebuf, writelen);
#else
    iret = fts_i2c_write_onsell(writebuf, writelen);
#endif
...
#if 0
    init_i2c_write_func(fts_i2c_write_onsell);
    init_i2c_read_func(fts_i2c_read_onsell);
#else
...
{
    char fwname[128] = { 0 };
    struct i2c_client *client = fts_i2c_client_onsell;
