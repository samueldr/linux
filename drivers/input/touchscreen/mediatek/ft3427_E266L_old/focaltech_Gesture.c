//tuwenzan@wind-mobi.com add at 20160311 begin
/*
 *
...
{
    char *envp[2]={" "," "};
	printk("tp_esd fts gesture_id==0x%x\n ",gesture_id);
    
	switch(gesture_id)
	{
		printk("hb gesture gesture_id=0x%x\n", gesture_id);
		case GESTURE_LEFT:
	...
	envp[1]=NULL;	
	printk("tp_esd going to kobject_uevent_env gesture is %s \n", envp[0]);
...
int fts_read_Gestruedata(void)
{
    ...
    ret = fts_i2c_read_onsell(fts_i2c_client_onsell, buf, 1, buf, FTS_GESTRUE_POINTS_HEADER);
    ...
    /* FW */
	 ...
	 if((pointnum * 4 + 2)<255)
	 {
	    	 ret = fts_i2c_read_onsell(fts_i2c_client_onsell, buf, 1, buf, (pointnum * 4 + 2));
	 }
	 else
	 {
	        ret = fts_i2c_read_onsell(fts_i2c_client_onsell, buf, 1, buf, 255);
	        ret = fts_i2c_read_onsell(fts_i2c_client_onsell, buf, 0, buf+255, (pointnum * 4 + 2) -255);
	 }
	 if (ret < 0)
	 {
	       printk( "%s read touchdata failed.\n", __func__);
	...
	if((pointnum * 4 + 8)<255)
	{
		ret = fts_i2c_read_onsell(fts_i2c_client_onsell, buf, 1, buf, (pointnum * 4 + 8));
	}
	else
	{
	     ret = fts_i2c_read_onsell(fts_i2c_client_onsell, buf, 1, buf, 255);
	     ret = fts_i2c_read_onsell(fts_i2c_client_onsell, buf, 0, buf+255, (pointnum * 4 + 8) -255);
	}
	if (ret < 0)
	{
	    printk( "%s read touchdata failed.\n", __func__);
...
#endif
//tuwenzan@wind-mobi.com add at 20160311 end