...
//printk("\n  zax v= %d \n",apk_debug_flag);
	...
	case PROC_UPGRADE:
		...
		regaddr = 0xA6;
		ret = fts_read_reg(fts_i2c_client_onsell, regaddr, &regvalue);
		if (ret < 0)
	...
	case PROC_READ_REGISTER:
		...
		if (ret < 0) {
					...
					esd_switch(1);apk_debug_flag = 0;
				#endif
			dev_err(&fts_i2c_client_onsell->dev, "%s:read iic error\n", __func__);
			return ret;
		} 
		num_read_chars = 1;
		break;
	...
	if (copy_to_user(buff, buf, num_read_chars)) {
		dev_err(&fts_i2c_client_onsell->dev, "%s:copy to user error\n", __func__);
#if FT_ESD_PROTECT
					esd_switch(1);apk_debug_flag = 0;
				#endif
		return -EFAULT;
...
static ssize_t fts_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (fts_read_reg(fts_i2c_client_onsell, FTS_REG_FW_VER, &fwver) < 0)
		return -1;
...
static ssize_t fts_fwupdate_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	i_ret = fts_ctpm_fw_upgrade_with_i_file(client, boot_id);
	if (i_ret == 0)
	{
		msleep(300);
	...
	//fts_ctpm_auto_upgrade_onsell(client);
