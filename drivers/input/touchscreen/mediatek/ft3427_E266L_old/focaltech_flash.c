...
//lihaiyan@wind-mobi.com modify at 20160414 begin
static unsigned char CTPM_FW[] = {
	#include "YD_E266_HS_Oncell_FT3427_VID0xA6_V0x04_20170427_app.i"
};
static unsigned char CTPM_FW_GX[] = {
	#include "GX0xda_FT34x7_V06_20161115_app.i"
};
static unsigned char CTPM_FW_TXD[] = {
	#include "TXD0xf0_FT34x7_V02_20161117_app.i"
};
//lihaiyan@wind-mobi.com modify at 20160414 end
...
void fts_update_fw_vendor_id(struct fts_ts_data *data)
{
	...
	err = fts_i2c_read_onsell(client, &reg_addr, 1, &data->fw_vendor_id, 1);
	if (err < 0)
...
void fts_update_fw_ver(struct fts_ts_data *data)
{
	...
	err = fts_i2c_read_onsell(client, &reg_addr, 1, &data->fw_ver[0], 1);
	if (err < 0)
		dev_err(&client->dev, "fw major version read failed");

	reg_addr = FTS_REG_FW_MIN_VER;
	err = fts_i2c_read_onsell(client, &reg_addr, 1, &data->fw_ver[1], 1);
	if (err < 0)
		dev_err(&client->dev, "fw minor version read failed");

	reg_addr = FTS_REG_FW_SUB_MIN_VER;
	err = fts_i2c_read_onsell(client, &reg_addr, 1, &data->fw_ver[2], 1);
	if (err < 0)
...
int fts_ctpm_fw_upgrade_ReadProjectCode(struct i2c_client *client, char *pProjectCode)
{
	...
	for (j = 0; j < 33; j++) 
	{
		...
		fts_i2c_read_onsell(client, auc_i2c_write_buf, 4, pProjectCode+j, 1);
		if (*(pProjectCode+j) == '\0')
			break;
	}
...
void fts_get_upgrade_array(void)
{
	...
	chip_id = FTS_CHIP_ID; 
#else
	ret = fts_read_reg(fts_i2c_client_onsell, FTS_REG_ID,&chip_id);
#endif
	if (ret<0) 
	{
...
int fts_6x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	fts_i2c_write_onsell(client, auc_i2c_write_buf, 5);
	...
	for (j = 0; j < packet_number; j++) 
	{
		...
		fts_i2c_write_onsell(client, packet_buf, FTS_PACKET_LENGTH + 6);
	...
	if ((dw_lenth) % FTS_PACKET_LENGTH > 0) 
	{
		...
		fts_i2c_write_onsell(client, packet_buf, temp + 6);
...
int fts_6x06_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < 6; i++) 
	{
		...
		bt_ecc ^= packet_buf[6];
		fts_i2c_write_onsell(client, packet_buf, 7);
		msleep(20);
	}
...
int fts_5x26_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	fts_i2c_write_onsell(client,auc_i2c_write_buf,2);
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	int	fw_filenth1 = sizeof(CTPM_FW_GX); //tuwenzan@wind-mobi.com add at 20160504
	int	fw_filenth2 = sizeof(CTPM_FW_TXD);//huyunge@wind-mobi.com 20160819
	if((CTPM_FW[fw_filenth-12] == 30) || (CTPM_FW_GX[fw_filenth1-12] == 30) || (CTPM_FW_TXD[fw_filenth2-12] == 30)) //tuwenzan@wind-mobi.com modify at 20160504   huyunge@wind-mobi.com 20160819
	{
		is_5336_fwsize_30 = 1;
	}
	else 
...
int fts_5822_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < 15; i++) 
	{
		auc_i2c_write_buf[0] = 0x6a;
		reg_val[0] = reg_val[1] = 0x00;
		fts_i2c_read_onsell(client, auc_i2c_write_buf, 1, reg_val, 2);
	...
	i_ret = fts_i2c_write_onsell(client, auc_i2c_write_buf, 6);
	msleep(dw_lenth/256);
	for (i = 0; i < 100; i++) 
	{
		auc_i2c_write_buf[0] = 0x6a;
...
int fts_5x06_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	TPD_DEBUG("Step 4:erase app and panel paramenter area\n");
	auc_i2c_write_buf[0] = FTS_ERASE_APP_REG;
	fts_i2c_write_onsell(client, auc_i2c_write_buf, 1);	/*erase app area */
	msleep(fts_updateinfo_curr.delay_erase_flash);
	/*erase panel parameter area */
	auc_i2c_write_buf[0] = FTS_ERASE_PARAMS_CMD;
	fts_i2c_write_onsell(client, auc_i2c_write_buf, 1);
...
{	
	...
	u8 bt_ecc;
	int i_ret;
printk("********************wind_tp Enter CTP Auto Upgrade777777777777777777777777777********************\n");
	i_ret = hidi2c_to_stdi2c(client);
	if(i_ret == 0)
	{
...
int  fts_8606_writepram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < 20; i++) 
	{
		...
		/*********Step 2:Enter upgrade mode *****/
		auc_i2c_write_buf[0] = FTS_UPGRADE_55;
		i_ret = fts_i2c_write_onsell(client, auc_i2c_write_buf, 1);
		if(i_ret < 0)
		{
...
int  fts_8606_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	i_ret =fts_i2c_read_onsell(client, auc_i2c_write_buf, 1, reg_val_id, 1);
	if(dw_lenth == 0)
	{
	...
	{		
		...
		cmd[2] = 0x00;//???	
		fts_i2c_write_onsell(client, cmd, 3);
	}
	
	{
		cmd[0] = 0x09;
		cmd[1] = 0x0B;
		fts_i2c_write_onsell(client, cmd, 2);
	}
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	{
		fts_ctpm_auto_clb_onsell(client);
	}
...
{
	//tuwenzan@wind-mobi.com add at 20150504 begin
	u16 ui_sz,ui_sz1,ui_sz2;//huyunge@wind-mobi.com 20160819
	int fw_id = 0;  //sunjingtao@wind-mobi.com 20160825
	ui_sz = sizeof(CTPM_FW);
	ui_sz1 = sizeof(CTPM_FW_GX); 
	ui_sz2 = sizeof(CTPM_FW_TXD); //huyunge@wind-mobi.com 20160819
		if(tp_vendor_id==0xA6)
		{
			fw_id = CTPM_FW[ui_sz - 2];
		}
		else if (tp_vendor_id==0xE4 || tp_vendor_id == 0xDA) //modified by hebiao at 1119 for guoxian
		{
			fw_id = CTPM_FW_GX[ui_sz1 - 2];
			return fw_id;
		}
		else if (tp_vendor_id==0xF0 || tp_vendor_id == 0x79)  //modified by hebiao at 1119 for txd
		{
			fw_id = CTPM_FW_TXD[ui_sz2 - 2];
			printk("hb update fw_id=%d\n", fw_id);
			return fw_id;
		}
	if (ui_sz > 2)
	{
		printk("hb fts_updateinfo_curr.CHIP_ID=0x%x\n", fts_updateinfo_curr.CHIP_ID);
		...
		//printk("tuwenzan fw_id = %d\n",fw_id);
		  return fw_id; //CTPM_FW[ui_sz - 2];
	}
	}
	//tuwenzan@wind-mobi.com add at 20150504 end
	return 0x00;
}
/************************************************************************
...
* Return: fail <0
***********************************************************************/
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client, u8 boot_id)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	...
	int fw_len1 = sizeof(CTPM_FW_GX);
	int fw_len2 = sizeof(CTPM_FW_TXD);//huyunge@wind-mobi.com 20160819
	u8  tp_vendor_id;
	printk("********************wind_tp Enter CTP Auto Upgrade3333333333333333333333333333********************\n");
	fts_read_reg(client, FTS_REG_VENDOR_ID, &tp_vendor_id);
	printk("hb tp_vendor_id=0x%x\n", tp_vendor_id);
        //tp_vendor_id = FTS_Vendor_3_ID;//0xf0;
	...
	*/
	...
	{
	   printk("********************wind_tp Enter CTP Auto Upgrade44444444444444444444444444444444********************\n");
	...
	printk("********************wind_tp Enter CTP Auto Upgrade555555555555555555555555********************\n");
		if(tp_vendor_id==0xA6)
		{
		//	printk("twz vendor_name=FT3427 \n");
			...
			{
			#ifdef AUTO_CLB
				fts_ctpm_auto_clb_onsell(client);  /*start auto CLB*/
			#endif
			}
		}
		//huyunge@wind-mobi.com 20160819 start
		else if(tp_vendor_id == 0xE4 || boot_id == 0xDA)   //modified by hebiao at 1119 for guoxian
		{
		    printk("hb for guoxian update ic=ft3427 tp_vendor_id=0x%x\n", tp_vendor_id);
			...
			pbt_buf = CTPM_FW_GX;
			/*call the upgrade function*/
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_GX));
			if (i_ret != 0)
			{
					dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
			}
		...
		else if (tp_vendor_id == 0xF0 || boot_id == 0x79) //modified by hebiao at 20161119 for txd
		{
			printk("hb for txd update ic=ft3427 tp_vendor_id=0x%x\n", tp_vendor_id);
			if (fw_len2 < 8 || fw_len2 > 54 * 1024) 
			{
			pr_err("FW length error\n");
			return -EIO;
			}
			...
			pbt_buf = CTPM_FW_TXD;
			/*call the upgrade function*/
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TXD));
			if (i_ret != 0)
			{
					dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
			}
		...
		else if(tp_vendor_id==0x8D)  //sunjingtao@wind-mobi.com 20160825
		{
...
static unsigned char ft5x46_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	...
	for(i = 0;i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		i_ret = fts_i2c_write_onsell(client, auc_i2c_write_buf, 4);
		if (i_ret < 0)
		{
...
int fts_ctpm_auto_upgrade_onsell(struct i2c_client *client)
{
	...
	printk("ft3427 code_fwvr_num uc_host_fm_ver = 0x%x\n",uc_host_fm_ver);
	printk("ft3427 uc_tp_fm_ver = 0x%x, uc_tp_vendor_id = 0x%x, code_fwvr=0x%x, uc_boot_vendor_id=0x%x\n",uc_tp_fm_ver, uc_tp_vendor_id, uc_host_fm_ver, uc_boot_vendor_id);
	/* read information end */
	
	if((uc_tp_vendor_id != FTS_Vendor_1_ID) &&( uc_tp_vendor_id != 0xf0))
	{
		uc_boot_vendor_id = ft5x46_ctpm_VidFWid_get_from_boot(client);
		printk("ft3427 uc_boot_vendor_id= 0x%x!\n", uc_boot_vendor_id);
		if((uc_boot_vendor_id == 0xDA)|| (uc_boot_vendor_id == FTS_Vendor_2_ID)|| (uc_boot_vendor_id == FTS_Vendor_3_ID) ||(uc_boot_vendor_id == 0x79))
		{
			uc_tp_fm_ver = 0;//force to upgrade the FW
			uc_tp_vendor_id = uc_boot_vendor_id;
			printk("ft3427 force update\n");
		}
		else
		{
			printk("hb [FTS] FW .i unmatched,stop upgrade\n");
	...
	if (uc_tp_fm_ver == FTS_REG_FW_VER || uc_tp_fm_ver < uc_host_fm_ver ) // (uc_tp_fm_ver == FTS_REG_FW_VER ||uc_tp_fm_ver < uc_host_fm_ver ||uc_tp_fm_ver == 0) //for not fw update
	{
		msleep(100);
		printk("ft3427 update begin\n");
		i_ret = fts_ctpm_fw_upgrade_with_i_file(client, uc_boot_vendor_id);
		if (i_ret == 0)	
		{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver(uc_tp_vendor_id);
			dev_dbg(&client->dev, "[FTS] upgrade to new version 0x%x\n",uc_host_fm_ver);
			printk("ft3427 upgrade to new version 0x%x\n",uc_host_fm_ver);
		} 
		else
		{
			pr_err("ft3427 upgrade failed ret=%d.\n", i_ret);
			printk("ft3427 upgrade failed ret=%d.\n", i_ret);
			return -EIO;
		}
	}else {
		printk("ft3427 not update, uc_tp_vendor_id=0x%x, uc_tp_fm_ver=0x%x\n", uc_tp_vendor_id, uc_tp_fm_ver);
	}
	return 0;
}
