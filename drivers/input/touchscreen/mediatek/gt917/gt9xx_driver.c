...
// wangbing@wind-mobi.com 20171016 begin
...
void gtp_glove_mode_switch(int on);
#endif
// wangbing@wind-mobi.com 20171016 end
...
//qiangang@wind-mobi.com 20170727 begin
...
//qiangang@wind-mobi.com 20170727 end
...
// wangbing@wind-mobi.com 20171011 begin
static int clk_tick_cnt = 200;
// wangbing@wind-mobi.com 20171011 end
...
static u8 pnl_init_error;//modify by qiangang@wind-mobi.com 20170930
u8 cfg_len = 0;
u8 gtp_resetting = 0;
...
static int gtp_get_gpio_res(void)
{
	...
	if (ret < 0) {
		...
		printk("qiangang Unable to request gpio reset_pin\n");
		return -1;
	}
...
static s32 gtp_send_cfg(struct i2c_client *client)
{
		...
		u8 retry = 0;//modify by qiangang@wind-mobi.com 20170930
...
// wangbing@wind-mobi.com 20171016 begin
	//add by iven
#ifdef CONFIG_GTP_GLOVE_MODE
	u8 cfg_info_group0_glove[] = GTP_CFG_GROUP0_GLOVE;
#endif
// wangbing@wind-mobi.com 20171016 end
	...
	(!cfg_info_len[3]) && (!cfg_info_len[4]) && (!cfg_info_len[5])) {
		...
		ver_gtp = sensor_id;//add by qiangang@wind-mobi.com 20170727 
...
// wangbing@wind-mobi.com 20171016 begin
		...
		memcpy(&gtp_glove_config[GTP_ADDR_LENGTH], cfg_info_group0_glove, CFG_GROUP_LEN(cfg_info_group0_glove));
#endif
// wangbing@wind-mobi.com 20171016 end
...
static ssize_t gtp_read_gloves(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    u32 nLength = 0;
    // u8 nGloveMode = 0;
...
static ssize_t gtp_write_gloves(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    u32 nGloveMode = 0;
    u32 i = 0;
    char *pCh = NULL;
    ...
    if (pBuffer != NULL)
    {
		...
		gtp_glove_mode_switch(nGloveMode);
        printk("[wind_tp]Glove Mode = 0x%x\n", nGloveMode);
    }
...
static const struct file_operations proc_gloves_ops= {
    .read = gtp_read_gloves,
    .write = gtp_write_gloves,
};

int tp_gloves_proc_init(void)
{
	_gProcClassEntry = proc_mkdir("class", NULL);												// proc/class
	_gProcMsTouchScreenMsg20xxEntry = proc_mkdir("ms-touchscreen-msg20xx", _gProcClassEntry);	// proc/class/ms-touchscreen-msg20xx
	_gProcDeviceEntry = proc_mkdir("device", _gProcMsTouchScreenMsg20xxEntry);					// proc/class/ms-touchscreen-msg20xx/device
	_gProcGloveModeEntry = proc_create("glove_mode", 0666, _gProcDeviceEntry, &proc_gloves_ops);// proc/class/ms-touchscreen-msg20xx/deviceglove_mode
	if(_gProcGloveModeEntry == NULL)
    {
        printk(" %s: tpd_gesture_mode create failed!\n", __func__);
        remove_proc_entry("glove_mode", _gProcGloveModeEntry);
        return -ENOMEM;
    }
	return 0;
}

int tp_gloves_proc_remove(void)
{
	remove_proc_entry("glove_mode", _gProcGloveModeEntry);
	return 0;
}
...
#endif
	printk("qiangang enter gt917 tpd_i2c_probe\n");
	i2c_client_point = client;
	client->addr = 0x14;
	of_get_gt9xx_platform_data(&client->dev);
printk("qiangang enter gt917 tpd_i2c_probe111\n");
	ret = gtp_get_gpio_res();
	if (ret < 0) {
		GTP_ERROR("Failed to get gpio resources");
		printk("qiangang failed to get gpio resource\n");
		return ret;
	}
printk("qiangang enter gt917 tpd_i2c_probe 222\n");
	ret = tpd_power_on(client);
	if (ret < 0) {
		GTP_ERROR("I2C communication ERROR!");
		goto out;
	}
printk("qiangang enter gt917 tpd_i2c_probe 333 \n");
...
// wangbing@wind-mobi.com 20171010 begin
	...
	input_set_capability(tpd->dev, EV_KEY, KEY_F21);
	input_set_capability(tpd->dev, EV_KEY, KEY_F22);
	input_set_capability(tpd->dev, EV_KEY, KEY_F23);
// wangbing@wind-mobi.com 20171010 end
#endif
...
void gtp_glove_mode_switch(int on)
{
	...
	s32 retry = 0;
	if (on) {
		printk("[wind_tp]glove_mode On");
		for (retry = 0; retry < 5; retry++) {
			ret = gtp_i2c_write(i2c_client_point, gtp_glove_config, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
			if (ret > 0)
				break;
		}	

	} else {
		printk("[wind_tp]glove_mode Off");
		for (retry = 0; retry < 5; retry++) {
...
{
	int retval;
	printk("qiangang enter tpd_local_init");
  ...
  //  }
	...
	printk("qiangang enter i2c_add_driver\n");
	if (i2c_add_driver(&tpd_i2c_driver) != 0) {
		printk("qiangang enter i2c_add_driver error\n");
		GTP_INFO("unable to add i2c driver.");
		return -1;
	}
printk("qiangang enter i2c_add_driver OK\n");
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
	...
	while (retry++ < 10) {
		...
		{
#endif
// wangbing@wind-mobi 20171011 begin
			// gtp_gpio_output(GTP_IRQ_GPIO, 1);
// wangbing@wind-mobi 20171011 end
			gtp_reset_guitar(client, 20);//qiangang@wind-mobi.com 20170929
			msleep(20);
			gtp_irq_enable();//qiangang@wind-mobi.com 20170929
		}
...
static int __init tpd_driver_init(void)
{
	...
	tpd_get_dts_info();
	printk("qiangang enter tpd_driver_init");
	if (tpd_driver_add(&tpd_device_driver) < 0)
		
		GTP_INFO("add generic driver failed");
	printk("qiangang out tpd_driver_init");
		return 0;
}
