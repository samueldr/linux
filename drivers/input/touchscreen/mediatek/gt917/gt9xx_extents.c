...
//add by qiangang@wind-mobi.com 20170929 begin
#define  CONFIG_GTP_GESTURE_WAKEUP
//add by qiangang@wind-mobi.com 20170929 end
...
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	if (DOZE_ENABLED == gesture_data.doze_status) {
		...
		if ((g_GestureWakeupMode_wind & GESTURE_MODE_DOUBLE_CLICK_FLAG) && (doze_buf[0] == 0xCC)) {
			key_code = KEY_F21;
		} else if ((g_GestureWakeupMode_wind & GESTURE_MODE_UP_DIRECT_FLAG) && (doze_buf[0] == 0xBA)) {
			key_code = KEY_F21;
		} else if ((g_GestureWakeupMode_wind & GESTURE_MODE_V_FLAG) && (doze_buf[0] == 0x76)) {
			key_code = KEY_F16;
		} else if ((g_GestureWakeupMode_wind & GESTURE_MODE_C_FLAG) && (doze_buf[0] == 0x63)) {
			key_code = KEY_F23;
		} else if ((g_GestureWakeupMode_wind & GESTURE_MODE_E_FLAG) && (doze_buf[0] == 0x65)) {
			key_code = KEY_F15;
		} else if ((g_GestureWakeupMode_wind & GESTURE_MODE_W_FLAG) && (doze_buf[0] == 0x77)) {
			key_code = KEY_F13;
		} else if ((g_GestureWakeupMode_wind & GESTURE_MODE_S_FLAG) && (doze_buf[0] == 0x73)) {
			key_code = KEY_F22;
		} else if ((g_GestureWakeupMode_wind & GESTURE_MODE_Z_FLAG) && (doze_buf[0] == 0x7A)) {
			key_code = KEY_F17;
		} else {
