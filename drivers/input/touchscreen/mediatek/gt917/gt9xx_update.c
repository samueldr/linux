...
#define CONFIG_GTP_AUTO_UPDATE
#define CONFIG_GTP_REQUEST_FW_UPDATE

#ifdef CONFIG_GTP_REQUEST_FW_UPDATE
#define GT9XX_FW_NAME "gt9xx_firmware_v7003.h"
#include "include/firmware/gt9xx_firmware_v7003.h"
#endif
...
static u8 gup_check_update_file(struct i2c_client *client, struct st_fw_head *fw_head, u8 *path)
{
		...
		if (path) {
			...
			//modify by qiangang@wind-mobi.com 20170929 begin
			/*
			...
			*/
			//add by xueqiang start
			
		     update_msg.fw_total_len =  gtp_fw_size - FW_HEAD_LENGTH;
		     if ( gtp_fw_size  <
		    //add by xueqiang end
	         ...
	         for (i = 0; i < update_msg.fw_total_len; i += 2) {
	             fw_checksum +=
	                 (gtp_default_FW[FW_HEAD_LENGTH + i] << 8) + gtp_default_FW[FW_HEAD_LENGTH + i + 1];
	         }
	         ...
	         return SUCCESS;
//modify by qiangang@wind-mobi.com 20170929 end
#else
#ifdef CONFIG_GTP_AUTO_UPDATE_CFG
