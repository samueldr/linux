//hebiao@wind-mobi.com 20161213 begin
...
int himax_irqdepth(int irqnum); //modified by hebiao at 2016705
...
static int lcd_flag = 0; //added by hebiao at 2016905

//dingyisheng@wind-mobi.com 20160407 begin i_CTPM_FW_HLT
//auto FW_UPDATE select among  BOE, HLT and other modules.
	...
	{
		#include "E286_HLT_Wind_E500_C02_20161213.i" //BOE FW update!
	};
	static unsigned char i_CTPM_FW_HLT_HX8394F[]=
	{
		#include "E286_HLT_Wind_E500_C02_20161213.i"//HLT FW update!
	};
	static unsigned char i_CTPM_FW_HLT_ILI9881[]=
	{
		#include "E286_HLT_Wind_E500_C02_20161213.i"//HLT FW update!
	};
#endif 
//dingyisheng@wind-mobi.com 20160407 end
#ifdef MTK
...
/* //liupeng@wind-mobi.com 20160127 begin
...
//zhaozhensen@wind-mobi.com 20160303 begin
...
#endif
//zhaozhensen@wind-mobi.com 20160303 end
...
static int himax_input_register(struct himax_ts_data *ts)
{
	...
	set_bit(KEY_F13, ts->input_dev->keybit);
	set_bit(KEY_F15, ts->input_dev->keybit);
	set_bit(KEY_F19, ts->input_dev->keybit);
	set_bit(KEY_F14, ts->input_dev->keybit);
	set_bit(KEY_F17, ts->input_dev->keybit);
	set_bit(KEY_F18, ts->input_dev->keybit);
#endif	
...
//dingyisheng@wind-mobi.com 20160407 begin
...
static int i_update_FW(void)
{
	...
	if(private_ts->vendor_sensor_id == 0x12) {
		...
		if (lcd_flag == 1) { 
			ImageBuffer = i_CTPM_FW_HLT_HX8394F; //helitai_module
			fullFileLength = sizeof(i_CTPM_FW_HLT_HX8394F); 
		} else if (lcd_flag == 2){
			ImageBuffer = i_CTPM_FW_HLT_ILI9881; //helitai_module
			fullFileLength = sizeof(i_CTPM_FW_HLT_ILI9881); 
...
int fts_ctpm_fw_upgrade_with_i_file_flash_cfg(struct himax_config *cfg)
{
	...
	if(private_ts->vendor_sensor_id == 0x12) {
		...
		if (lcd_flag == 1) {
			...
			ImageBuffer = i_CTPM_FW_HLT_ILI9881;
		} else {
			ImageBuffer = NULL;  //unknown_module
		}
	}  else {
...
//qiumeng@wind-mobi.com 20160427 begin
#if defined(HX_USB_DETECT2)
		ts->usb_connected = 0;
#endif
//qiumeng@wind-mobi.com 20160427 end
...
//hebiao@wind-mobi.com 20161105 end
	...
	if(data[0] == 0x05 && data[1] == 0x85 &&
		(data[2] == 0x25 || data[2] == 0x26 || data[2] == 0x27 || data[2] == 0x28)){
			sprintf(wind_device_info.ctp_module_info.ic_name,"Hx%02X%02XE",data[1],data[2]);
	} else {
			E("Himax IC package incorrect!!PKG[0]=%x,PKG[1]=%x,PKG[2]=%x\n",data[0],data[1],data[2]);
			return;
	}
	
	wind_device_info.ctp_module_info.fwvr  = private_ts->vendor_config_ver;
	if(private_ts->vendor_sensor_id == 0x12) {
		wind_device_info.ctp_module_info.vendor = 0x01;  //boyi_module
	} else if (private_ts->vendor_sensor_id == 0x22) {
		wind_device_info.ctp_module_info.vendor = 0x02;  //helitai_module
	} else if (private_ts->vendor_sensor_id == 0x21) {
		wind_device_info.ctp_module_info.vendor = 0x03;  //yassy_module
	} else {
		wind_device_info.ctp_module_info.vendor = 0xFF;  //unknown_module
	}
#endif	
...
{
	int ret=0;
	int himax_irqdepth_r  =  0; //modified by hebiao at 20160705

	I(" %s: POLLING_COUNT=%x, STATUS=%x \n", __func__,HX_CHIP_POLLING_COUNT,ret);

	himax_irqdepth_r = himax_irqdepth(private_ts->client->irq); //modified by hebiao at 20160705
		I("[himax] %s: himax_irqdepth_r =%d\n", __func__, himax_irqdepth_r); //modified by hebiao at 20160705
...
*/	
	printk("himax_gesture_flag=0x%x", gesture_flag);
	return gesture_flag;
}
...
//							100);
					...
					input_report_key(ts->input_dev, KEY_MENU, 1);//yanglong@wind-mobi.com modify  20151217 #92975
		}
...
#endif
//qiumeng@wind-mobi.com 20160418 start
#if defined(HX_USB_DETECT2)
	himax_cable_detect_func();
#endif
//qiumeng@wind-mobi.com 20160418 end
#ifdef HX_CHIP_STATUS_MONITOR
		HX_CHIP_POLLING_COUNT=0;
...
if((diag_cmd) || (HW_RESET_ACTIVATE))
	...
	else
	{
		...
		if ((check_sum_cal != 0x00) )
		{
			...
			for(i=0;i<(hx_touch_info_size/4);i++){
			I("Check sum : buf[%d] = %x, buf[%d] = %x, buf[%d] = %x, buf[%d] = %x \n", 
			i*4, buf[i*4], i*4+1, buf[i*4+1], i*4+2, buf[i*4+2],i*4+3, buf[i*4+3]);
			}
...
static irqreturn_t tpd_eint_interrupt_handler(int irq, void *desc)
{
	...
	//himax_int_enable(private_ts->client->irq,0,true); //modified by hebiao at 2016705
...
int ret_event = 0, KEY_EVENT = 0;	
	...
	do
	{
		...
		//printk("wait_interrupt\n");
		wait_event_interruptible(waiter,tpd_flag!=0);
		//printk("awake ssuccess\n");
		...
		if (private_ts->debug_log_level & BIT(2)) {
			...
			if (atomic_read(&private_ts->suspend_mode)&&(!FAKE_POWER_KEY_SEND)&&(private_ts->SMWP_enable)) {
				...
				//printk("gesture ret_event=%d\n", ret_event);
				switch (ret_event) {
				case 0x01: //up
					if (private_ts->gesture_cust_en[0] == 1) {
						KEY_EVENT = KEY_POWER;
					}
				break;
				case 0x80:
						printk("gesture wake\n");//
					if (private_ts->gesture_cust_en[1] == 1) {
						KEY_EVENT = KEY_POWER; //double click
					}
				break;
				case 0x0B:
					if (private_ts->gesture_cust_en[2] == 1) {
						KEY_EVENT = KEY_F13; //W
					}
					break;
				case 0x0C:
					if (private_ts->gesture_cust_en[3] == 1) {
						KEY_EVENT = KEY_F15; //E
					}
				break;
				case 0x05:
					if (private_ts->gesture_cust_en[4] == 1) {
						KEY_EVENT = KEY_F19; //C
					}
				break;
				case 0x09:
					if (private_ts->gesture_cust_en[5] == 1) {
						KEY_EVENT = KEY_F14; //S
					}
				break;
				case 0x06:
					if (private_ts->gesture_cust_en[6] == 1) {
						KEY_EVENT = KEY_F17; //Z
					}
				break;
				case 0x0A:
					if (private_ts->gesture_cust_en[7] == 1) {
						KEY_EVENT = KEY_F18;  //V
					}
				break;
				default:
				...
				printk("himax gesture_flag ret_event = %d,KEY_EVENT = %d,SMWP_enable = %d\n",ret_event,KEY_EVENT,private_ts->SMWP_enable);
				if(KEY_EVENT) {
			...
			//himax_int_enable(private_ts->client->irq,1,false); //modified by hebiao at 2016705
				continue;
			}
#endif

		//printk("enter_himax_ts_work\n");
		himax_ts_work();
		//printk("out_himax_ts_work\n");
		//himax_int_enable(private_ts->client->irq,1,true); //modified by hebiao at 2016705
	...
	printk("kthread_stop func==%s line=%d\n", __func__, __LINE__);
...
static void himax_cable_detect_func(void)
{
	...
	//connect_status = 0;
	//qiumeng@wind-mobi.com 0160418 start
...
//hebiao@wind-mobi.com 20160601 begin 
		self_test_lop_counter++;
		if (self_test_lop_counter < 0x64)
//hebiao@wind-mobi.com 20160601 end
...
//modified by hebiao 2016726 begin
...
//modified by hebiao 2016726 end
}
...
#if 0
//liqiang@wind-mobi.com 2016830 begin
static char *find_lcm_name(void)
{
		char *p, *q;
		static char lcm_name[126] = { 0 };
		...
		strncpy((char *)lcm_name, (const char *)p, (int)(q - p));
		lcm_name[q - p + 1] = '\0';

		return lcm_name;

}
//liqiang@wind-mobi.com 2016830 end
#endif
static int himax852xes_probe(struct i2c_client *client, const struct i2c_device_id *id)
{	

	int err = -ENOMEM;
	//char *lcd_name = NULL; //added by hebiao at 2016905
	struct himax_ts_data *ts;
	struct himax_i2c_platform_data *pdata;
	printk("hb enter probe\n");
...
himax_gpio_power_config(ts->client,pdata);
	...
	if(i_update_FW()==false) {
		...
		//ranyanhao@wind-mobi.com 20160323 begin
		himax_read_TP_info(client);  //Read TP info again,when UPDATE_FW.
		//ranyanhao@wind-mobi.com 20160323 end
	}
#endif
...
#endif
//qiumeng@wind-mobi.com 20160505 start
#if defined(HX_USB_DETECT2)
	himax_cable_detect_func();
#endif
//qiumeng@wind-mobi.com 20160505 end
/*#ifdef HX_SMART_WAKEUP
//hebiao@wind-mobi.com 2016726 begin
	buf[0] = 0x8F;
	buf[1] = 0x00;
	ret = i2c_himax_master_write(ts->client, buf, 2, DEFAULT_RETRY_CNT);
	if (ret < 0)
...
//hebiao@wind-mobi.com 2016726 end
	if(ts->SMWP_enable)
	{
		I("%s:SMWP_enable = %d \n", __func__, ts->SMWP_enable);
		//Sense Off
...
//qiumeng@wind-mobi.com 20160525 begin
	...
	irq_enable_count=1;
	atomic_set(&ts->pdata->int_state, 1);
	himax_depth_r=himax_irqdepth(ts->client->irq);
...
MODULE_LICENSE("GPL");
//hebiao@wind-mobi.com 20161213 end