...
#endif
{
	...
	switch(cmd)
	{
		...
		case ILITEK_IOCTL_DEBUG_SWITCH:
			rc = copy_from_user(szBuf, (unsigned char*)arg, 1);
			printk("ilitek The debug_flag_ilitek = %d.\n", szBuf[0]);
			if (szBuf[0] == 0)
			{
				debug_flag_ilitek = false;
			}
			else if (szBuf[0] == 1)
			{
				debug_flag_ilitek = true;
			}
			break;
#endif //DEBUG_NETLINK
...
ssize_t ilitek_file_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	if (strcmp(buffer, "dbg_enable") == 0) {
		...
		debug_flag_ilitek =!debug_flag_ilitek;
		printk("%s, %s debug_flag_ilitek message(%X).\n",__func__,debug_flag_ilitek?"Enabled":"Disabled",debug_flag_ilitek);
	}
#endif //DEBUG_NETLINK
