...
static u8 *_gPlatformFwVersion = NULL; // internal use firmware version for MStar
 u8 *_gFwVersion = NULL; // customer firmware version modify by qiangang
...
// wangbing@wind-mobi.com 20170801 begin >> note: report normal pressure
    /*
    if (nId == 0)
    {
    ...
    */
// wangbing@wind-mobi.com 20170801 end
...
static u16 _DrvChangeFirmwareMode(u16 nMode)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2118A || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)  
    {
		...
		//IicWriteData(SLAVE_I2C_ID_DWI2C, &szTxData[0], 1);
		//mdelay(20);
		//IicReadData(SLAVE_I2C_ID_DWI2C, &szRxData[0], 8);
		//mdelay(5000);
		//szTxData[0] = 0x53;
		//szTxData[1] = 0x00;
		//szTxData[2] = 0x08;
		...
		//mdelay(20);
		//IicReadData(SLAVE_I2C_ID_DWI2C, &szRxData[0], 306);
        if (i == 5)
        {
...
static void _DrvGetTouchPacketAddress(u16* pDataAddress, u16* pFlagAddress)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2118A || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121 || (g_ChipType == CHIP_TYPE_MSG22XX && IS_SELF_FREQ_SCAN_ENABLED))   
    {
        ...
        while (i < 5)
        {
			...
			//if(g_ChipType == CHIP_TYPE_MSG22XX)
			//{
			//	rc = IicReadData(SLAVE_I2C_ID_DWI2C, &szRxData[0], 2);
			//}
			//else
			{ 
...
static ssize_t _DrvProcfsMpTestRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    nLength = sprintf(pBuffer, "%d", DrvMpTestGetTestResult());
...
set_bit(191, g_InputDevice->keybit);
	set_bit(192, g_InputDevice->keybit);
	set_bit(193, g_InputDevice->keybit);
	set_bit(183, g_InputDevice->keybit);
	set_bit(187, g_InputDevice->keybit);
	set_bit(186, g_InputDevice->keybit);
	set_bit(185, g_InputDevice->keybit);

    //hebiao@wind-mobi.com 20170404 begin   
    set_bit(184, g_InputDevice->keybit);
    set_bit(188, g_InputDevice->keybit);
    set_bit(189, g_InputDevice->keybit);
    set_bit(190, g_InputDevice->keybit);
...
int apk_printf(unsigned char *pPacket)
{
	int i = 0,	len=0;
	static int cnt = 0;
	 ...
	 if (pPacket[0] == 0x2C)
	{
		if(pPacket[1]==1)
		{
			for(i=0;i<(MUTUAL_DEMO_MODE_PACKET_LENGTH-2);i++)
			{
				fwLog[i+cnt*(MUTUAL_DEMO_MODE_PACKET_LENGTH-2)] = pPacket[2+i];
			}
			cnt=0;
...
//add by qiangang 20170823 end
    ...
    if (g_GestureWakeupFlag == 1)
    {
        ...
        if (bIsCorrectFormat)
        {
            ...
            {
                case 0x58:
//modify  by qiangang 20170823
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_DOUBLE_CLICK_FLAG;

                    DBG(&g_I2cClient->dev, "Light up screen by DOUBLE_CLICK gesture wakeup.\n");
				printk("qiangang Light up screen by DOUBLE_CLICK gesture wakeup.\n");
                    input_report_key(g_InputDevice, KEY_F21, 1);
                    input_sync(g_InputDevice);
                    input_report_key(g_InputDevice, KEY_F21, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x60:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_UP_DIRECT_FLAG;
                    ...
                    DBG(&g_I2cClient->dev, "Light up screen by UP_DIRECT gesture wakeup.\n");
					printk("qiangang Light up screen by UP_DIRECT gesture wakeup.\n");
...
//                    input_report_key(g_InputDevice, KEY_M, 0);
                ...
                case 0x65:
                    ...
                    DBG(&g_I2cClient->dev, "Light up screen by W_CHARACTER gesture wakeup.\n");
					printk("qiangang Light up screen by W_CHARGER gesture wakeup.\n");

//                    input_report_key(g_InputDevice, KEY_W, 1);
                    input_report_key(g_InputDevice, KEY_F13, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_W, 0);
                    input_report_key(g_InputDevice, KEY_F13, 0);
                    input_sync(g_InputDevice);
                    break;		
                case 0x66:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_C_CHARACTER_FLAG;
                    ...
                    DBG(&g_I2cClient->dev, "Light up screen by C_CHARACTER gesture wakeup.\n");
					printk("qiangang Light up screen by C_CHARGER gesture wakeup.\n");

//                    input_report_key(g_InputDevice, KEY_C, 1);
                    input_report_key(g_InputDevice, 193, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_C, 0);
                    input_report_key(g_InputDevice, 193, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x67:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_e_CHARACTER_FLAG;
                    ...
                    DBG(&g_I2cClient->dev, "Light up screen by e_CHARACTER gesture wakeup.\n");
					printk("qiangang Light up screen by e_CHARGER gesture wakeup.\n");

//                    input_report_key(g_InputDevice, KEY_E, 1);
                    input_report_key(g_InputDevice, KEY_F15, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_E, 0);
                    input_report_key(g_InputDevice, KEY_F15, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x68:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_V_CHARACTER_FLAG;
                    ...
                    DBG(&g_I2cClient->dev, "Light up screen by V_CHARACTER gesture wakeup.\n");
					printk("qiangang Light up screen by V_CHARGER gesture wakeup.\n");

//                    input_report_key(g_InputDevice, KEY_V, 1);
                    input_report_key(g_InputDevice, 186, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_V, 0);
                    input_report_key(g_InputDevice, 186, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x69:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_O_CHARACTER_FLAG;
...
//                    input_report_key(g_InputDevice, KEY_O, 0);
                ...
                case 0x6A:
                    ...
                    DBG(&g_I2cClient->dev, "Light up screen by S_CHARACTER gesture wakeup.\n");
					printk("qiangang Light up screen by S_CHARGER gesture wakeup.\n");

//                    input_report_key(g_InputDevice, KEY_S, 1);
                    input_report_key(g_InputDevice, 192, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_S, 0);
                    input_report_key(g_InputDevice, 192, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x6B:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_Z_CHARACTER_FLAG;
                    ...
                    DBG(&g_I2cClient->dev, "Light up screen by Z_CHARACTER gesture wakeup.\n");
					printk("qiangang Light up screen by Z_CHARGER gesture wakeup.\n");

//                    input_report_key(g_InputDevice, KEY_Z, 1);
                    input_report_key(g_InputDevice, KEY_F17, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_Z, 0);
                    input_report_key(g_InputDevice, KEY_F17, 0);
                    input_sync(g_InputDevice);
//modify by qiangang 20170823 end
                    break;
                case 0x6C:
                    _gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_RESERVE1_FLAG;
...
#endif //CONFIG_ENABLE_HOTKNOT
            {
                DBG(&g_I2cClient->dev, "111111111111111111111111111 g_GestureState = %d, g_ChipType = 0x%X\n", g_GestureState, g_ChipType);
...
s32 /*__devinit*/ MsDrvInterfaceTouchDeviceProbe(struct i2c_client *pClient, const struct i2c_device_id *pDeviceId)
{
    ...
    DrvTouchDevicePowerOn();
    DBG(&g_I2cClient->dev, "*** 11111111111111111111111111111 int_status = %d ***\n", gpio_get_value(MS_TS_MSG_IC_GPIO_INT));
    ...
    DBG(&g_I2cClient->dev, "*** 222222222222222222222222222222 int_status = %d ***\n", gpio_get_value(MS_TS_MSG_IC_GPIO_INT));
