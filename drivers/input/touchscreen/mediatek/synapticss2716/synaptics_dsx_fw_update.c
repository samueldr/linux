...
#define FW_IMAGE_NAME "synapticss2716/E286L_HLT_ILI9881_S2716_30303032.img"

#define DO_STARTUP_FW_UPDATE
#ifdef DO_STARTUP_FW_UPDATE
#include "SynaFirmwareImage_16.h"
#endif
...
extern int synaptics_rmi4_i2c_readx(struct synaptics_rmi4_data *rmi4_data,
		unsigned short addr, unsigned char *data, unsigned short length);
#endif
//hebiao@wind-mobi.com 20160907 end for 0x04
...
static int fwu_read_flash_status(void)
{
	...
	//printk("hb fwu_read_flash_status 2\n");
	...
	retval = fwu->fn_ptr->read(rmi4_data,
			...
			&command,
			sizeof(command));
	//printk("hb fwu_read_flash_status 3\n");
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
	...
	//printk("hb fwu_read_flash_status end\n");
...
static int fwu_wait_for_idle(int timeout_ms, bool poll)
{
	...
	//printk("hb fwu_wait_for_idle timeout_count=%d begin\n", timeout_count);
	do {
		usleep_range(MIN_SLEEP_TIME_US, MAX_SLEEP_TIME_US);
		//printk("hb fwu_read_flash_status count=%d\n", count);
		count++;
		if (poll || (count == timeout_count)) {
			//printk("hb fwu_read_flash_status count=%d\n", count);
			fwu_read_flash_status();
			//printk("hb after fwu_read_flash_status count=%d command=0x%x flash_status=0x%x\n", 
				//count, fwu->command, fwu->flash_status);
		}
		if ((fwu->command == CMD_IDLE) && (fwu->flash_status == 0x00)) {
			//printk("hb fwu_read_flash_status susccess count=%d command=0x%x flash_status=0x%x\n", 
				//count, fwu->command, fwu->flash_status);
			return 0;
		}
...
#endif
	retval = fwu_write_f34_command(CMD_ENABLE_FLASH_PROG); //hebiao modified at 919CMD_ENABLE_FLASH_PROG
	if (retval < 0) {
		//printk("hb 74\n");
		return retval;
	}
	//printk("hb fwu_wait_for_idle begin\n");
	retval = fwu_wait_for_idle(ENABLE_WAIT_MS, false); 
	if (retval < 0) {
		//printk("hb 75\n");
		return retval;
	}
	//printk("hb fwu_wait_for_idle end\n");
	if (!fwu->in_bl_mode) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
...
static int fwu_start_reflash(void)
{
	...
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
	//printk("hb func==%s line=%d fwu_start_reflash begin\n", __func__, __LINE__);
	if (rmi4_data->sensor_sleep) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
				"%s: Sensor sleeping\n",
				__func__);
...
//hebiao@wind-mobi.com end 20161126 for esd polling
	...
	if (!fwu->force_update && fwu->new_partition_table) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
				...
				__func__);
		retval = -EINVAL;
		//printk("hebiao update failed5\n");
		goto exit;
	}

	retval = fwu_read_flash_status();
	if (retval < 0) {
		//printk("hebiao update failed6\n");
		goto exit;
	}
	if (fwu->in_bl_mode) {
		dev_info(&fwu->rmi4_data->i2c_client->dev,
...
//hebiao@wind-mobi.com begin 20161126 for esd polling
#ifdef SYNAPTICS_ESD_PROTECTION_EN
	rmi4_data->esd_detect_halt = 0;
#endif
//hebiao@wind-mobi.com end 20161126 for esd polling
	//printk("hb func==%s line=%d fwu_start_reflash end\n", __func__, __LINE__);
	return retval;
}
...
//hebiao@wind-mobi.com 20160907 begin
	...
	fwu->rmi4_data->config_id = be_to_uint(fwu->config_id);
	sprintf(wind_device_info.ctp_module_info.ic_name,"%s", "synaptics_s2716");
	printk("ranyanhao %s,%d,%s\n",__func__,__LINE__,wind_device_info.ctp_module_info.ic_name);
	wind_device_info.ctp_module_info.fwvr = rmi4_data->config_id;
	printk("ranyanhao %s,%d,%x\n",__func__,__LINE__,wind_device_info.ctp_module_info.fwvr);
	wind_device_info.ctp_module_info.vendor = rmi4_data->firmware_id; 
	printk("ranyanhao %s,%d,%x\n",__func__,__LINE__,wind_device_info.ctp_module_info.vendor);
#endif
//hebiao@wind-mobi.com 20160907 end
...
#endif
//printk("hb func==%s line=%d synaptics_fw_updater begin\n", __func__, __LINE__);
	synaptics_fw_updater(SynaFirmware);
//printk("hb func==%s line=%d synaptics_fw_updater end\n", __func__, __LINE__);
	return;
}
#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("hebia write dore flash begin update\n");
	...
	printk("hebia write dore flash end update\n");
	retval = count;
...
//hebiao@wind-mobi.com 20160907 begin for 0x04
#ifdef SYNAPTICS_ESD_PROTECTION_EN
	fwu->fn_ptr->read =  synaptics_rmi4_i2c_readx ;//hebiao@wind-mobi.com 20160907  for 0x04 
#else 
