...
#if 1
#define SYNAPTICS_PROC_TOUCH_FOLDER "android_touch"
#define SYNAPTICS_PROC_SMWP_FILE "SMWP"
#define SYNAPTICS_PROC_GESTURE_FILE "GESTURE"
static struct proc_dir_entry *synaptics_proc_GESTURE_file = NULL;
static struct proc_dir_entry *synaptics_touch_proc_dir = NULL;
static struct proc_dir_entry *synaptics_proc_SMWP_file = NULL;
static struct synaptics_rmi4_data  *private_ts; 
...
#define UBL_I2C_ADDR 0x20   //2C modified by hebiao at 20161123

#define SENSOR_MAX_X 720
#define SENSOR_MAX_Y 1280

//ranyanhao@wind-mobi.com 20161021 begin
#define WAKEUP_GESTURE false
//ranyanhao@wind-mobi.com 20161021 end
...
// hebiao@wind-mobi.com 20161124 begin for esd polling
#ifdef SYNAPTICS_ESD_PROTECTION_EN
#define ESD_PROTECTION_WAIT_TIME 		2000	//ms  //hebiao added at 20161123 for esd
static struct task_struct *thread_esd_protection = NULL;       //hebiao added at 20161123 for esd
...
#endif
// hebiao@wind-mobi.com 20161124 end for esd polling
...
//hebiao@wind-mobi.com 20161126 begin for esd polling
extern void pmic_ldo_vldo28_sw_en(int en_value);
//hebiao@wind-mobi.com 20161126 end for esd polling
...
#if defined SYNPTICS_USB_DETECT
//hebiao@wind-mobi.com 20161209 begin

extern bool bat_is_charger_exist(void);
static int synaptics_cable_detect_func(void)
{
	struct synaptics_rmi4_data  *ts = private_ts; 
	u32 connect_status = 0;
	unsigned retval = 0;
	unsigned char data = 0;
	//unsigned char data1 = 0;
	static unsigned flag1 = 0 ;
	static unsigned flag2 = 0 ;

	
	connect_status = bat_is_charger_exist();
	//printk("hb write begin flag1=%d flag2=%d connect_status=%d\n", flag1, flag2, connect_status);
	
	if (connect_status > 0 && flag1 == 0) {
		flag1 = 1;
		flag2 = 0;
		data = 0x40;
		retval =  synaptics_rmi4_i2c_write(ts, 0x000D, &data, sizeof(data));
		if (retval < 0)
			return retval;

		printk("synaptics enter charging mode connect_status=%d\n", connect_status);
	} else if (connect_status == 0 && flag2 == 0) {
		flag1 = 0;
		flag2 = 1;
		data = 0;
		retval =  synaptics_rmi4_i2c_write(ts, 0x000D, &data, sizeof(data));
		if (retval < 0)
			return retval;
		printk("synaptics enter no_charging mode connect_status=%d\n", connect_status);
	}
	
	//printk("hb write end\n");
	//tpd_i2c_read_data(ts,ts->i2c_client, 0x0D, &data1, sizeof(data1));
	//printk("hb charging_data = 0x%x\n", data1);
	
	return 0;	
}

//hebiao@wind-mobi.com 20161209 end
#endif

//hebiao@wind-mobi.com 20160907 begin for 0x04
#ifdef SYNAPTICS_ESD_PROTECTION_EN
int synaptics_irqdepth(int irqnum)
{
...
int synaptics_rmi4_i2c_readx(struct synaptics_rmi4_data *rmi4_data,
		unsigned short addr, unsigned char *data, unsigned short length)
{
...
int  synaptics_esd_protection_suspend(void)
{
	g_start_esd_protection = 0;
	printk("synaptics suspend  stop esd check\n");
	return 0;
}

int  synaptics_protection_resume(void)
{
	g_start_esd_protection = 1;
	printk("synaptics resume start esd check\n");
	return 0;
}

static int synaptics_esd_protection_notice(void)
{
	int i = 0;
	...
	if(i == 10)
	{
		//printk("synaptics Failed to read/write i2c,  the i2c communication has been accounted for ESD PROTECTION.\n");
		return -1;
	}

	
	do_gettimeofday(&g_last_comm_time);
	//printk("synaptics  succeed to read/write i2c.\n");
	return 0;
}

static void synaptics_esd_protection_check(void)
{
	unsigned char esd = 0x1;
	unsigned char config_id[2] = {0};
	unsigned char para_e = 0x0;
	int retval = 0;
	
	struct synaptics_rmi4_data  *ts = private_ts; 
	retval = tpd_i2c_read_data(ts,ts->i2c_client, 0x04, &esd, sizeof(esd));
	retval = tpd_i2c_read_data(ts,ts->i2c_client, 0x09, config_id, 2);
	retval = tpd_i2c_read_data(ts,ts->i2c_client, 0x0E, &para_e, sizeof(para_e));
	//printk("synaptics esd status=%d config_id[0]=0x%x config_id[1]=0x%x para_e=0x%x\n", esd, config_id[0], config_id[1], para_e);
	if (retval < 0) {
		printk("synaptics esd read i2c transfer error\n");
		...
		retval = synaptics_rmi4_reinit_device(ts);
		if (retval < 0) {
			printk("%s:synaptics esd  Failed to reinit device\n",__func__);
		}
	}

	if (esd == 0x0 && config_id[0] == 0x32 && config_id[1] == 0x37 && para_e == 0x0f) {
		//printk("synaptics esd ok\n");
	}else {
		printk("synaptics esd status=%d config_id[0]=0x%x config_id[1]=0x%x para_e=0x%x\n", esd, config_id[0], config_id[1], para_e);
		printk("synaptics esd failed\n");
		pmic_ldo_vldo28_sw_en(0);
		msleep(10);
		pmic_ldo_vldo28_sw_en(1);
		msleep(50);
...
static int synaptics_esd_protection_timeout(void *unused)  
{
	...
	do
	{
		...
		if (ts->esd_detect_halt == 1 || 0 == g_start_esd_protection) {
			
			//printk("synaptics esd_detect_halt=%d g_start_esd_protection=%d,esd detect pause\n",ts->esd_detect_halt,  g_start_esd_protection);
			continue;
		} 
		
		irq_depth = synaptics_irqdepth(touch_irq);
		printk("synaptics irq = %d\n", irq_depth);
		
		do_gettimeofday(&tv);
		iDeltaTime = (tv.tv_sec - g_last_comm_time.tv_sec)*MSEC_PER_SEC + (tv.tv_usec - g_last_comm_time.tv_usec)/1000;
		//printk("tv.tv_sec=%d, tv.tv_usec=%d, g_last_comm_time.tv_sec=%d, g_last_comm_time.tv_usec=%d\n", 
			//tv.tv_sec, tv.tv_usec, g_last_comm_time.tv_sec, g_last_comm_time.tv_usec);

		if(ESD_PROTECTION_WAIT_TIME < iDeltaTime)
		{
			g_esd_protection_use_i2c = 1;//
			g_esd_protection_checking = 1;//
			synaptics_esd_protection_check();
...
int synaptics_esd_protection_init(void)
{
	int err = 0;
	do_gettimeofday(&g_last_comm_time);
	g_start_esd_protection = 1;
	...
	thread_esd_protection = kthread_run(synaptics_esd_protection_timeout, 0, "synaptics_esd_protection");
	...
	printk("tp_esd %s is successful init",__func__);
	return 0;
}

int synaptics_esd_protection_exit(void)
{
	
	kthread_stop(thread_esd_protection);
	msleep(ESD_PROTECTION_WAIT_TIME);
	printk("tp_esd %s is successful exit",__func__);
	return 0;
}
#endif
//hebiao@wind-mobi.com 20160907 end
...
#if 1
//hebiao@wind-mobi.com 20160907 begin
static ssize_t synaptics_SMWP_read(struct file *file, char *buf,
...
static ssize_t synaptics_SMWP_write(struct file *file, const char *buff,
	size_t len, loff_t *pos)
{
	struct synaptics_rmi4_data  *ts = private_ts; 
	char buf[80] = {0};
	int i;
	if (ts->sensor_sleep == true) {
		printk("TP is resumne,can not write gesture\n");
		return len;
	}
...
static struct file_operations synaptics_proc_SMWP_ops =
{
	.owner = THIS_MODULE,
	.read = synaptics_SMWP_read,
	.write = synaptics_SMWP_write,
};

static ssize_t synaptics_GESTURE_read(struct file *file, char *buf,
	size_t len, loff_t *pos)
{
	struct synaptics_rmi4_data  *ts = private_ts; 
	size_t ret = 0;
	...
	{
		ret = sprintf(buf, "ges_en[%d]=%d \n",GESTURE_SEND_FLAG ,ts->gesture_cust_en[GESTURE_SEND_FLAG]);
		GESTURE_SEND_FLAG++;
	}
	else
	{
...
static ssize_t synaptics_GESTURE_write(struct file *file, const char *buff,
	size_t len, loff_t *pos)
{
	struct synaptics_rmi4_data  *ts = private_ts; 
	int i =0;
...
static struct file_operations synaptics_proc_Gesture_ops =
{
	.owner = THIS_MODULE,
	.read = synaptics_GESTURE_read,
	.write = synaptics_GESTURE_write,
};
static int synaptics_touch_proc_init(void)
{
	synaptics_touch_proc_dir = proc_mkdir(SYNAPTICS_PROC_TOUCH_FOLDER, NULL);
	if (synaptics_touch_proc_dir == NULL)
	{

		printk(" %s: synaptics_touch_proc_dir file create failed!\n", __func__);
		return -ENOMEM;
	}

	synaptics_proc_SMWP_file = proc_create(SYNAPTICS_PROC_SMWP_FILE, (S_IWUSR|S_IRUGO|S_IWUGO), 
		synaptics_touch_proc_dir, &synaptics_proc_SMWP_ops);
	if(synaptics_proc_SMWP_file == NULL)
	{
		printk(" %s: proc SMWP file create failed!\n", __func__);
		goto fail_0;
	}
	synaptics_proc_GESTURE_file = proc_create(SYNAPTICS_PROC_GESTURE_FILE, (S_IWUSR|S_IRUGO|S_IWUGO), 
		synaptics_touch_proc_dir, &synaptics_proc_Gesture_ops);
	if(synaptics_proc_GESTURE_file == NULL)
	{
	...
	fail_0: remove_proc_entry( SYNAPTICS_PROC_SMWP_FILE, synaptics_touch_proc_dir );
	fail_1: remove_proc_entry( SYNAPTICS_PROC_GESTURE_FILE, synaptics_touch_proc_dir );	
	return -ENOMEM;
}

static void synaptics_touch_proc_deinit(void)
{

		remove_proc_entry( SYNAPTICS_PROC_GESTURE_FILE, synaptics_touch_proc_dir );
		remove_proc_entry( SYNAPTICS_PROC_SMWP_FILE, synaptics_touch_proc_dir );
}
//hebiao@wind-mobi.com 20160907 end
#endif
...
static ssize_t synaptics_rmi4_wake_gesture_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
//hebiao@wind-mobi.com 2016912 begin
static ssize_t synaptics_rmi4_wake_gesture_flag_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t synaptics_rmi4_wake_gesture_flag_store(struct device *dev,
		struct device_attribute *attr, const char *buff, size_t len);
//hebiao@wind-mobi.com 2016912 end
#endif
...
//hebiao@wind-mobi.com 2016912 begin
	__ATTR(wake_gesture_flag, 0660,
			synaptics_rmi4_wake_gesture_flag_show,
			synaptics_rmi4_wake_gesture_flag_store),	
//hebiao@wind-mobi.com 2016912 end
#endif
};
...
static int tpd_irq_registration(void)
{
	...
	//printk("synaptics tpd_irq_registration\n");
	...
	if (node) {
		...
		ret = request_irq(touch_irq, (irq_handler_t) tpd_eint_handler, IRQF_TRIGGER_LOW,
				"TOUCH_PANEL-eint", NULL);//IRQF_TRIGGER_LOW
	...
	if (copy_from_user(buf, buff, len))
	{
		...
		if(buf[0] == '0')
			...
			if (rmi4_data->enable_wakeup_gesture == 0){
				for (i=0;i<16;i++)
				{
					rmi4_data->gesture_cust_en[i]= 0;
				}
			} 
		printk("%s: enable_wakeup_gesture = %d.\n", __func__, rmi4_data->enable_wakeup_gesture);
		...
		struct device_attribute *attr, char *buf)
{
	...
	size_t ret = 0;
	if (GESTURE_SEND_FLAG < 16) {
		ret = sprintf(buf, "ges_en[%d]=%d \n",GESTURE_SEND_FLAG ,rmi4_data->gesture_cust_en[GESTURE_SEND_FLAG]);
		GESTURE_SEND_FLAG++;
	} else {
		GESTURE_SEND_FLAG = 0;
		ret = 0;
	...
	for (i=0;i<16;i++)
		{
			if (buf[i] == '0')
	    		...
	    		rmi4_data->gesture_cust_en[i]= 1;
			else
				rmi4_data->gesture_cust_en[i]= 0;
			printk("gesture en[%d]=%d \n", i, rmi4_data->gesture_cust_en[i]);
		}
...
static int synaptics_rmi4_i2c_read(struct synaptics_rmi4_data *rmi4_data,
		unsigned short addr, unsigned char *data, unsigned short length)
{
//hebiao@wind-mobi.com begin 20161126 for esd polling
#ifdef SYNAPTICS_ESD_PROTECTION_EN
	int i = 0;
	int ret = 0;
	for(i = 0; i < 3; i++)
	{
		ret = synaptics_esd_protection_notice();
		if(0 == ret){
			//printk("synaptics tp_esd %s tp is ok.\n", __func__);
			break;	// can use I2C
		}else
		{
			//printk("synaptics i=%d tp_esd %s tp is fail return :%d.\n", i,  __func__,ret);
			continue;
		}
	}
#endif
	//printk("synaptics i2c read11111\n");
//hebiao@wind-mobi.com end 20161126 for esd polling
	return tpd_i2c_read_data(rmi4_data,rmi4_data->i2c_client, addr, data, length);
}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->sensor_sleep && rmi4_data->SMWP_enable) {
		retval = synaptics_rmi4_i2c_read(rmi4_data,
...
static int synaptics_rmi4_f12_abs_report(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *fhandler)
{
	//struct synaptics_rmi4_data  *ts = private_ts;  //added by hebiao 2016912 
	int retval;
	unsigned char touch_count = 0; /* number of touch points */
	unsigned char finger;
	...
	int wy;
	int temp;
	int KEY_EVENT = 0; //added by hebiao 2016913

//ranyanhao@wind-mobi.com 20161103 begin
	char ASUS_Gestures[10] = "NULL";
	char *P_ASUS_Gestures = ASUS_Gestures;
//ranyanhao@wind-mobi.com 20161103 begin
...
//ranyanhao@wind-mobi.com 20161103 begin
	if (rmi4_data->sensor_sleep && rmi4_data->SMWP_enable) {
		...
		printk("ranyanhao dg[0]=%x, dg[2]=%x\n", detected_gestures[0],detected_gestures[2]);

		if (detected_gestures[0] && (detected_gestures[0]!= F12_UDG_DETECT)) {

			switch (detected_gestures[0]) {

				case 0x07:   //up
				
					if (rmi4_data->gesture_cust_en[0] == 1) {
						KEY_EVENT = KEY_F20;
						P_ASUS_Gestures = "KEY_POWER";
					}
					break;
					
				case 0x03:	// double click

					if (rmi4_data->gesture_cust_en[1] == 1) {
						KEY_EVENT = KEY_F20;
						P_ASUS_Gestures = "KEY_POWER";
					}
					break;
				...
				case 0x0b: 	// W,E,C,S,Z
					...
					switch (detected_gestures[2]) {
						...
						case 0x77:	// W
							...
							if (rmi4_data->gesture_cust_en[2] == 1) {
								KEY_EVENT = KEY_F13; //183
								P_ASUS_Gestures = "KEY_W";
							}
							break;
							
						case 0x65:	// E
						
							if (rmi4_data->gesture_cust_en[3] == 1) {
								KEY_EVENT = KEY_F15; //185
								P_ASUS_Gestures = "KEY_E";
							}
							break;
							
						case 0x63:	// C
						
							if (rmi4_data->gesture_cust_en[4] == 1) {
								KEY_EVENT = KEY_F19; //189
								P_ASUS_Gestures = "KEY_C";
							}
							break;
							
						case 0x73:	// S
						
							if (rmi4_data->gesture_cust_en[5] == 1) {
								KEY_EVENT = KEY_F14; //184
								P_ASUS_Gestures = "KEY_S";
							}
							break;
							
						case 0x7a:	// Z
						
							if (rmi4_data->gesture_cust_en[6] == 1) {
								KEY_EVENT = KEY_F17; //187
								P_ASUS_Gestures = "KEY_Z";
							}
							break;
				...
				case 0x0a:	// V
						if (rmi4_data->gesture_cust_en[7] == 1) {
							KEY_EVENT = KEY_F18; //188
							P_ASUS_Gestures = "KEY_V";
						}	
					break;
			...
			if (KEY_EVENT) {
				input_report_key(rmi4_data->input_dev, KEY_EVENT, 1);
				input_sync(rmi4_data->input_dev);
				input_report_key(rmi4_data->input_dev, KEY_EVENT, 0);
				input_sync(rmi4_data->input_dev);
				//rmi4_data->sensor_sleep = false;
				printk("ranyanhao KEY_EVENT =%d,ASUS_Gestures is %s\n", KEY_EVENT, P_ASUS_Gestures);
			}

		}
		return 0;
	}
//ranyanhao@wind-mobi.com 20161103 end
...
*/
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if (finger_status) {
			...
			printk("synaptics_station x=%d, y=%d\n", x, y);
...
//hebiao@wind-mobi.com 20161209 begin
#if defined SYNPTICS_USB_DETECT
		synaptics_cable_detect_func();
#endif
//hebiao@wind-mobi.com 20161209 end
	}
/*
//hebiao@wind-mobi.com 20161212 begin
#if defined SYNPTICS_USB_DETECT
		synaptics_cable_detect_func();
#endif
//hebiao@wind-mobi.com 20161212 end
*/
...
static void synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (status.unconfigured && !status.flash_prog) {
		...
		printk("synaptics tp power off\n");
		if (get_boot_mode() == 4) {
			printk("tp idle not power\n");
		}else {
			pmic_ldo_vldo28_sw_en(0);
			msleep(10);
			pmic_ldo_vldo28_sw_en(1);
 ...
 */
 ...
 static irqreturn_t tpd_eint_handler(unsigned irq, struct irq_desc *desc)
 {
	...
	//printk("enter_intterupt\n");
	tpd_flag = 1;
	//printk("synaptics no_function enter interrupt\n");
...
static int touch_event_handler(void *data)
{
	...
	do{
		...
		//printk("synaptics no_function wake_up success\n");
		tpd_flag = 0;
		TPD_DEBUG_SET_TIME;
...
#ifdef CONFIG_OF_TOUCH
		//printk("synaptics no_function report end, on  irq\n");
		...
		bool enable)
{
	...
	if (enable) {
		...
		if (retval < 0) {
			printk("synaptics no_function i2c read failed,clear interrupt failed\n");
			return retval;
		}
...
#ifdef CONFIG_OF_TOUCH
//hebiao@wind-mobi.com 20161224 begin for tp no_function
			irq_depth = synaptics_irqdepth(touch_irq);
			printk("synaptics irq = %d touch_irq_no=%d\n", irq_depth, touch_irq);

			if(irq_depth==1){
				enable_irq(touch_irq);
			}else if(irq_depth > 1) {
				for(i=0; i < irq_depth; i++) {
				enable_irq(touch_irq);
				msleep(10);
				}
			}else if(irq_depth < 0){
				irq_depth = (irq_depth<<1)>>1; 
				for(i=0; i < irq_depth; i++) {
					disable_irq_nosync(touch_irq);
					msleep(10);
				}
			...
			//enable_irq(touch_irq);
//hebiao@wind-mobi.com 20161224 end for tp no_function
#else
...
flash_prog_mode:
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->SMWP_enable= WAKEUP_GESTURE;
	else
		rmi4_data->SMWP_enable= false;
...
#endif
//hebiao@wind-mobi.com 2016913 begin
	set_bit(KEY_F21, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_01, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_02, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_03, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_04, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_05, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_06, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_07, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_08, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_09, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_10, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_11, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_12, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_13, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_14, rmi4_data->input_dev->keybit);
	set_bit(KEY_CUST_15, rmi4_data->input_dev->keybit);
//hebiao@wind-mobi.com 2016913 end
...
{
	int retval,ret ;
	int err = -ENOMEM; //added by hebiao 2016827
	unsigned char attr_count;
	struct synaptics_rmi4_data *rmi4_data;

	//unsigned char f01_query[F01_STD_QUERY_LEN]; //added by hebiao 2016921
...
#if !defined CONFIG_MTK_LEGACY// add 2.8v begin
...
/*
	...
	if (retval < 0)
		return retval;
	printk("f01_query=%s\n", f01_query);
	printk("version_major = 0x%x, version_minor=0x%x, manufacturer_id=0x%x\n", 
	rmi4_data->rmi4_mod_info.version_major, rmi4_data->rmi4_mod_info.version_minor, rmi4_data->rmi4_mod_info.manufacturer_id);
*/
	...
	synaptics_touch_proc_init(); //added by hebiao at 20160907
	if (get_boot_mode() == 4) {
	} else {
//hebiao@wind-mobi.com begin 20161126 for esd polling
	#ifdef SYNAPTICS_ESD_PROTECTION_EN
	synaptics_esd_protection_init();
	#endif
//hebiao@wind-mobi.com end 20161126 for esd polling
	}
...
err_set_input_dev:
// hebiao@wind-mobi.com 2016827 begin
	err = regulator_disable(tpd->reg);
	if (err != 0)
		printk("Failed to disable reg-vgp6: %d\n", err);
	regulator_put(tpd->reg);
	gpio_free(0);  //free rst
	gpio_free(1);  //free int
// hebiao@wind-mobi.com 2016827 end
	kfree(rmi4_data);
...
#endif
{
	...
	synaptics_touch_proc_deinit(); //added by hebiao at 20160907
#ifdef USE_I2C_DMA
	if(wDMABuf_va){
...
//ranyanhao@wind-mobi.com 20161028 begin
/*
 * This function will be performed only once
 */
extern struct i2c_client *GT9762AF_I2Cclient;
void AF_Into_Power_Down_Mode(void)
{
	...
	if((AF_flag == 0) && (GT9762AF_I2Cclient != NULL)) {
		AF_addr_temp = GT9762AF_I2Cclient->addr;
		GT9762AF_I2Cclient->addr = 0x18 >> 1;
		ret = i2c_master_send(GT9762AF_I2Cclient, AF_Cmd, 2); 
    	if (ret < 0) {
		...
		GT9762AF_I2Cclient->addr = AF_addr_temp;
	}
}
//ranyanhao@wind-mobi.com 20161028 end	
...
//hebiao@wind-mobi.com 20161126 begin for esd polling
#ifdef SYNAPTICS_ESD_PROTECTION_EN
	synaptics_esd_protection_suspend(); 
#endif
//hebiao@wind-mobi.com 20161126 end for esd polling
	}
//ranyanhao@wind-mobi.com 20161028 begin
	AF_Into_Power_Down_Mode();
//ranyanhao@wind-mobi.com 20161028 end

	if (rmi4_data->staying_awake)
		return ;

	if (rmi4_data->SMWP_enable) {
...
static void synaptics_rmi4_resume(struct device *dev)
{
	...
	int irq_depth = 0;
	int i = 0;
	printk("synaptics enter resume\n");

	if (get_boot_mode() == 4) {
	}else {
//hebiao@wind-mobi.com 20161126 begin for esd polling
#ifdef SYNAPTICS_ESD_PROTECTION_EN
	synaptics_protection_resume();
#endif 
//hebiao@wind-mobi.com 20161126 end for esd polling
	}
	if (rmi4_data->staying_awake)
	...
	if (rmi4_data->SMWP_enable) {
		irq_depth = synaptics_irqdepth(touch_irq);
		printk("synaptics gesture irq = %d touch_irq_no=%d\n", irq_depth, touch_irq);
