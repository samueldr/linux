...
#define FLAG_DUAL_CAM_ADDR 0x14F8
#define FLAG_AEC_ADDR 0x1CFA
...
#define CS_DUAL_CAM_S_ADDR 0x14F9
#define CS_DUAL_CAM_E_ADDR 0x1CF8
#define CS_DUAL_CAM_ADDR 0x1CF9

#define CS_AEC_S_ADDR 0x1CFB
#define CS_AEC_E_ADDR 0x1D04
#define CS_AEC_ADDR 0x1D05
...
int s5k3l6_validflag_check_eeprom(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	if (e_ctrl->cal_data.mapdata[FLAG_DUAL_CAM_ADDR] != FLAG_VALID_VALUE) {
		pr_err("%s :%d: DUAL_CAM flag invalid 0x%x\n",
			__func__, __LINE__, e_ctrl->cal_data.mapdata[FLAG_DUAL_CAM_ADDR]);
		flag |= 0x30;
	}

	if (e_ctrl->cal_data.mapdata[FLAG_AEC_ADDR] != FLAG_VALID_VALUE) {
		pr_err("%s :%d: AEC flag invalid 0x%x\n",
			__func__, __LINE__, e_ctrl->cal_data.mapdata[FLAG_AEC_ADDR]);
		flag |= 0x40;
	}
...
int s5k3l6_checksum_eeprom(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	for (j = CS_DUAL_CAM_S_ADDR; j <= CS_DUAL_CAM_E_ADDR; j++)
		checksum += e_ctrl->cal_data.mapdata[j];

	if ((checksum % 256) != e_ctrl->cal_data.mapdata[CS_DUAL_CAM_ADDR]) {
		pr_err("%s :%d: aec info checksum fail\n", __func__, __LINE__);
		rc  |= 0x30;
	}
	checksum = 0;

	for (j = CS_AEC_S_ADDR; j <= CS_AEC_E_ADDR; j++)
		checksum += e_ctrl->cal_data.mapdata[j];

	if ((checksum % 256) != e_ctrl->cal_data.mapdata[CS_AEC_ADDR]) {
		pr_err("%s :%d: aec info checksum fail\n", __func__, __LINE__);
		rc  |= 0x40;
	}
...
static const struct of_device_id s5k3l6_eeprom_dt_match[] = {
	{ .compatible = "zte,s5k3l6_047-eeprom", .data = &s5k3l6_eeprom_func_tbl},
};
