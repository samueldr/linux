// hebiao@wind-mobi.com 2015118 begin
/* Copyright Statement:
 *
...
static int N2DM_ReadAccRawData(struct i2c_client *client, s16 * data);
...
static int n2dm_SetWorkMode(struct i2c_client *client, u8 work_mode)
{
	...
	if((res =n2dm_Update_Sensitivity(client)) != 0)
	{
...
static int n2dm_ReadAccData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if((res = N2DM_ReadAccRawData(client, obj->data))  != 0)
	{        
		GSE_ERR("I2C error: 11111111111111ret value=%d", client->addr);
		return -4;
	}
	else
	{
	#if 1
		obj->data[N2DM_AXIS_X] = (s16 )(obj->data[N2DM_AXIS_X]) * obj->sensitivity*GRAVITY_EARTH_1000/(16*1000); //default high resolution, 16
...
static ssize_t show_layout_value(struct device_driver *ddri, char *buf)
{
	...
	data = i2c_get_clientdata(n2dm_i2c_client);
	if(NULL == data)
	{
...
static DRIVER_ATTR(status,              S_IWUSR | S_IRUGO, show_status_value,        NULL); //
...
static int n2dm_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj_i2c_data = obj;
	//client->addr = N2DM_I2C_SLAVE_ADDR; move to dts
	obj->client = client;
...
/*----------------------------------------------------------------- N2DM ------------------------------------------------------------------*/
// hebiao@wind-mobi.com 2015118 end