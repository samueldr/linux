...
//liukangping  add psensor factory calibration 20161208 begin
#include <linux/fs.h>
...
#define PSENSOR_PROC_DATA_FILE1 "Data1"
#define PSENSOR_PROC_DATA_FILE2 "Data2"
#define PSENSOR_PROC_DATA_FILE3 "Data3"
...
//liukangping  add psensor factory calibration 20161208 end
...
#define LUX_PER_COUNT	372  //ALS lux per count /*modified by ranyanhao 20161112*/
/*ALS interrupt table*/
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    epl_sensor.als.integration_time = EPL_ALS_INTT_64; /*modified by hebiao 2016413*/
    epl_sensor.als.gain = EPL_GAIN_LOW;
    epl_sensor.als.adc = EPL_PSALS_ADC_13; /*modified by hebiao 2016413*/
    epl_sensor.als.cycle = EPL_CYCLE_32; /*modified by hebiao 2016413*/
    ...
    epl_sensor.als.report_type = CMC_BIT_PRE_COUNT; //CMC_BIT_DYN_INT; /*modified by ranyanhao 20161112*/
...
//ranyanhaoranyanhao@wind-mobi.com 20160516 begin
#ifdef CONFIG_WIND_E280L_FOR_PASS_CTCC
	epl_sensor.ps.ir_drive = EPL_IR_DRIVE_10;
#else
	epl_sensor.ps.ir_drive = EPL_IR_DRIVE_100;
#endif
//ranyanhao@wind-mobi.com 20160516 end
...
//ranyanhao@wind-mobi.com 20160516 begin
#ifdef CONFIG_WIND_E280L_FOR_PASS_CTCC
	dynk_low_offset = 65;
    dynk_high_offset = 125;
#else
	//liukangping  add psensor factory calibration 20161208 begin
    // if(!(0<X1&&X1<=300 && 300<=X3&&X3<=1000 && 1000<=X2&&X2<=3000))
	 if(!(X1<X3&&X3<X2&&X1<X2))
	 {
		  dynk_low_offset = 268;
          dynk_high_offset = 770;
	 printk("wind-liukangping dynk_low_offset111= %d\n",dynk_low_offset);
		printk("wind-liukangping dynk_low_offset111=%d\n",dynk_high_offset);
	 }
	//liukangping  add psensor factory calibration 20161208 end
   // dynk_low_offset = 620; //modifid by ranyanhao 20161111
    //dynk_high_offset = 1700;
#endif
//ranyanhao@wind-mobi.com 20160516 end
#endif /*PS_DYN_K*/
...
//ranyanhao@wind-mobi.com begin
unsigned char FILTER_NUM = 3;
unsigned short als_buf[50] = {0};
bool filter_enable = 1;
//ranyanhao@wind-mobi.com end
...
//ranyanhao@wind-mobi.com begin
	...
	unsigned long filter_temp = 0;
	unsigned char i = 0;
//ranyanhao@wind-mobi.com end
//ranyanhao@wind-mobi.com begin
		if (filter_enable) {
			
			als_buf[filter_cnt] = als;  //update Slide Window data

			for (i = 0; i < FILTER_NUM; i++) {
				filter_temp += als_buf[i];
			}
			
			als = filter_temp / FILTER_NUM;
			filter_cnt++;
...
int epl_sensor_setup_eint(struct i2c_client *client)
{
	...
	if (epl_sensor_obj->irq_node)
	{
		...
		if (request_irq(epl_sensor_obj->irq, epl_eint_handler, IRQF_TRIGGER_LOW, "ALS-eint", NULL)) {
...
//ranyanhao
static ssize_t epl_sensor_show_als_lux_per_count(struct device_driver *ddri, char *buf)
{
	size_t size = 0;
...
static ssize_t epl_sensor_show_pdata1(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
    int ps_raw;
    ...
    ps_raw = epl_sensor.ps.data.data;
    APS_LOG("[%s]: ps_raw=%d \r\n", __func__, ps_raw);
    len += snprintf(buf + len, PAGE_SIZE - len, "%d", ps_raw);
...
//ranyanhao@wind-mobi.com 20161112 begin 
static ssize_t epl_sensor_store_als_fliter_enbale(struct device_driver *ddri, const char *buf, size_t count)
{
    uint16_t mode=0;
...
static ssize_t epl_sensor_show_als_fliter_enbale(struct device_driver *ddri, char *buf)
{
	size_t size = 0;
	size = sprintf(buf, "%d\n", filter_enable);
    return size;
...
static ssize_t epl_sensor_store_als_fliter_size(struct device_driver *ddri, const char *buf, size_t count)
{
    uint16_t mode=0;
...
static ssize_t epl_sensor_show_als_fliter_size(struct device_driver *ddri, char *buf)
{
	size_t size = 0;
	size = sprintf(buf, "%d\n", FILTER_NUM);
    return size;
}
//ranyanhao@wind-mobi.com 20161112 end
...
static DRIVER_ATTR(als_lux_per_count,			S_IWUSR  | S_IRUGO, epl_sensor_show_als_lux_per_count,  epl_sensor_store_als_lux_per_count			);
...
static DRIVER_ATTR(pdata1,                       S_IWUSR  | S_IRUGO, epl_sensor_show_pdata1,              NULL                                        );
...
//ranyanhao@wind-mobi.com 20161112 begin 
static DRIVER_ATTR(als_filter_enable,			S_IWUSR  | S_IRUGO, epl_sensor_show_als_fliter_enbale,   epl_sensor_store_als_fliter_enbale					);
static DRIVER_ATTR(als_fliter_size,			    S_IWUSR  | S_IRUGO, epl_sensor_show_als_fliter_size,   	epl_sensor_store_als_fliter_size					);
//ranyanhao@wind-mobi.com 20161112 end

/*----------------------------------------------------------------------------*/


/************************als_factory_calibration**************ranyanhao 20161116 begin***********************************/
...
#define LIGHTSENSOR_PROC_LUX_PRE_COUNT_FILE "lux_per_count"
...
static struct proc_dir_entry *proc_lux_per_count_file = NULL;
u16 factory_lux_per_count_800lux = 0;
#define light_bar_800lux  800
...
static ssize_t readback_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	...
	u32 lux_temp = 0;
	u16 readback_data = 0;
	static int READBACK_SEND_FLAG = 0;
	struct epl_sensor_priv *obj = epl_sensor_obj;
	epl_sensor.als.factory.lux_per_count = 1000;

	if (!READBACK_SEND_FLAG) {
		if (!obj) {
			APS_ERR("obj is null!!\n");
			return -1;
		}
		...
		readback_data = ((lux_temp / 10) * factory_lux_per_count_800lux) / 1000;
		//printk("ranyanhao_re lux_temp / 10=%d,factory_lux_per_count_800lux=%d\n",lux_temp/10,factory_lux_per_count_800lux);
		size = sprintf(buf, "%d+%d\n", readback_data, factory_lux_per_count_800lux);
		READBACK_SEND_FLAG=1;
	} else {
		READBACK_SEND_FLAG=0;
	}
...
static ssize_t calibration_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	...
	if (calibration_enable == 800) {
		...
		factory_lux_per_count_800lux = (light_bar_800lux * 1000) / report_lux;
		//printk("ranyanhao_ca report_lux=%d,factory_lux_per_count=%d\n",report_lux,factory_lux_per_count_800lux);
		return len;
	} else {
		return -1;
	}
...
static ssize_t lux_per_count_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
    int lux_per_count = 0;

    sscanf(buff, "%d",&lux_per_count);
    epl_sensor.als.factory.lux_per_count = lux_per_count;

    return len;
}
static struct file_operations proc_lux_per_count_file_ops =
{
	.owner = THIS_MODULE,
	.write = lux_per_count_write,
};

static int lightsensor_proc_init(void)
{
	...
	proc_lux_per_count_file = proc_create(LIGHTSENSOR_PROC_LUX_PRE_COUNT_FILE, (S_IWUSR|S_IRUGO|S_IWUGO), 
		lightsensor_proc_dir, &proc_lux_per_count_file_ops);
	if(proc_lux_per_count_file == NULL)
	{
		printk(" %s: proc lux_per_count file create failed!\n", __func__);
		remove_proc_entry( LIGHTSENSOR_PROC_LUX_PRE_COUNT_FILE, lightsensor_proc_dir );
		return -ENOMEM;
	}
...
static void lightsensor_proc_deinit(void)
{
	remove_proc_entry( LIGHTSENSOR_PROC_READBACK_FILE, lightsensor_proc_dir );
	remove_proc_entry( LIGHTSENSOR_PROC_CALIBRATION_FILE, lightsensor_proc_dir );
	remove_proc_entry( LIGHTSENSOR_PROC_LUX_PRE_COUNT_FILE, lightsensor_proc_dir );
...
/************************als_factory_calibration**************ranyanhao 20161116 end***********************************/
...
static ssize_t data_write3(struct file *file, const char *buf, size_t len, loff_t *pos)
{
   ...
   // if(0<=X1&&X1<=300 && 300<=X3&&X3<=1000 && 1000<=X2&&X2<=3000)
    ...
    {
       //printk("%d\n",);
	    dynk_low_offset = X3-X1; 
        dynk_high_offset = X2-X1;
...
static int psensor_proc_init(void)
{
...
/////////////////////add data 1
	proc_data_file1 = proc_create(PSENSOR_PROC_DATA_FILE1, (S_IWUSR|S_IRUGO|S_IWUGO), 
		psensor_proc_dir, &proc_data_file1_ops);
	if(proc_data_file1 == NULL)
	{
		printk(" %s: proc data file1 create failed!\n", __func__);
		remove_proc_entry( PSENSOR_PROC_DATA_FILE1, psensor_proc_dir );
		return -ENOMEM;
	}
	
	////////////////////add data 2
	proc_data_file2 = proc_create(PSENSOR_PROC_DATA_FILE2, (S_IWUSR|S_IRUGO|S_IWUGO), 
		psensor_proc_dir, &proc_data_file2_ops);
	if(proc_data_file2 == NULL)
	{
		printk(" %s: proc data file2 create failed!\n", __func__);
		remove_proc_entry( PSENSOR_PROC_DATA_FILE2, psensor_proc_dir );
		return -ENOMEM;
	}
	//////////////////add data 3
	proc_data_file3 = proc_create(PSENSOR_PROC_DATA_FILE3, (S_IWUSR|S_IRUGO|S_IWUGO), 
		psensor_proc_dir, &proc_data_file3_ops);
		
	if(proc_data_file3 == NULL)
	{
		printk(" %s: proc data file3 create failed!\n", __func__);
		remove_proc_entry( PSENSOR_PROC_DATA_FILE3, psensor_proc_dir );
		return -ENOMEM;
	}
...
static void psensor_proc_remove(void)
{
...
//ranyanhao@wind-mobi.com 20161112 begin 
    &driver_attr_als_filter_enable,
    &driver_attr_als_fliter_size,
//ranyanhao@wind-mobi.com 20161112 end
};
...
static int epl_sensor_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //liukangping  add psensor factory calibration 20161208 begin
     psensor_proc_init();
	  ...
	  //ranyanhao@wind-mobi.com 20161116 begin
	lightsensor_proc_init();
//ranyanhao@wind-mobi.com 20161116 end
