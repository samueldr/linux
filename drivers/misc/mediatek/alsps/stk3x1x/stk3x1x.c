// wangbing@wind-mobi.com 20170421 begin
/* Copyright Statement:
 *
...
// wangbing@wind-mobi.com 20170426 begin
#include "alsps_calibrator.h"
// wangbing@wind-mobi.com 20170426 end
...
#include "stk3x1x.h"
#define DRIVER_VERSION          "3.5.2 20160615v1"
...
/*oujiacheng@wind-mobi.com 20160704 begin*/ 
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	95       // wangjun@wind-mobi.com at 20170526
	#define STK_LT_N_CT	37   
	#define STK_HT_N_CT	95
#endif /* #ifdef STK_TUNE0 */
/*oujiacheng@wind-mobi.com 20160704 end*/ 
...
struct alsps_hw alsps_cust_stk;
static struct alsps_hw *hw = &alsps_cust_stk;
struct platform_device *alspsPltFmDev_stk;

/* For alsp driver get cust info */
struct alsps_hw *get_cust_alsps(void)
{
    return &alsps_cust_stk;
}
...
//	printk("stk value_init=%d\n", als_data_u32);
// wangbing@wind-mobi.com 20170428 begin
	*data = filter_als_value(als_data_u32);
// wangbing@wind-mobi.com 20170428 end
...
int stk3x1x_read_ps(struct i2c_client *client, u16 *data)
{
    ...
    printk("alsps_wind %s obj->ps = %d obj->ps_high_thd_val = %d obj->ps_low_thd_val = %d \n", __func__, (u32)(*data), atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val));
    ps_threshold_5cm = atomic_read(&obj->ps_low_thd_val);   //lihaiyan@wind-mobi.com 20170515 modified
	if(atomic_read(&obj->trace) & STK_TRC_ALS_DATA)
	{
		APS_DBG("PS: 0x%04X\n", (u32)(*data));
	}
...
//chenjiehua@wind-mobi.com change start 20160823 for plam rejection
int stk3x1x_get_ps_value_plam_rejection(void)
{
	struct stk3x1x_priv *obj = i2c_get_clientdata(stk3x1x_i2c_client);
	int err, dat, cur = 0, old = atomic_read(&obj->state_val);
	cur = old;	

	APS_LOG("%s ,obj->enable:%d\n", __FUNCTION__,(test_bit(STK_BIT_PS, &obj->enable) ? (1) : (0)));	
	
	if(0 == (test_bit(STK_BIT_PS, &obj->enable) ? (1) : (0)))
	{
		cur &= (~(0x45));
		...
		stk3x1x_write_state(stk3x1x_i2c_client, cur);
		msleep(4);
		stk3x1x_read_ps(obj->client, &obj->ps);
		stk3x1x_write_state(stk3x1x_i2c_client, old);
		enable_irq(obj->irq);
	}
	else
	{
...
//	hwm_sensor_data sensor_data;
	...
	//	APS_LOG("%s: set HT=%d, LT=%d\n", __func__, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val));		
		...
		{		
				...
				APS_LOG("%s: line=%d\n", __FUNCTION__, __LINE__);
				report_value = stk3x1x_get_ps_value(obj, obj->ps);
				if(report_value < 0)
				{
				...
				obj->ps_distance_last = report_value;
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if((obj->psi_set!=0))	
	{
		if(obj->ps_distance_last==1 && (obj->ps_thd_update==false))
		{
			...
			if(ret == 0)
			{
				...
				{
					ret = stk3x1x_master_recv(obj->client, STK_THDL1_PS_REG, buf, 0x02);
					if(ret < 0)
					{
						APS_ERR("%s fail, ret=0x%x", __func__, ret);
						return 0;
					}
					ct_value = ((buf[0] << 8) | buf[1]) - obj->stk_lt_n_ct;				

					//APS_LOG( "%s: tracking: old_ct=%d, psi=%d, HT=%d, word=%d\n", __func__, ct_value, obj->psi, obj->ps_thd_h, word_data);
					if((word_data < ct_value) && ((ct_value - word_data) > 5))  // CT tracking
					{
						obj->psi = word_data;
						...
						APS_LOG( "%s: tracking1: old_ct=%d, psi=%d, HT=%d, LT=%d\n", __func__, ct_value, obj->psi, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val));
			...
			//	APS_LOG( "%s: tracking2: stk_ps_val=%d, HT=%d, LT=%d\n", __func__, ret, obj->ps_thd_h, obj->ps_thd_l);
...
*/
		...
		//	msleep(200);
		//	stk3x1x_read_ps(obj->client, &obj->ps)
			if((obj->ps - obj->psi) > 1500)
			{
				obj->ps_thd_update = true;
								
				atomic_set(&obj->ps_high_thd_val, obj->psi + 1200); 
				atomic_set(&obj->ps_low_thd_val, obj->psi + 1100); 						
			...
			//	APS_LOG("%s: near update HT=%d, LT=%d, ps=%d, psi=%d\n", __func__, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val), obj->ps, obj->psi);
			}
		}
		else
		{
			...
			//	stk3x1x_read_ps(obj->client, &obj->ps)
				if(ret == 0)
				{
					...
					APS_LOG("%s: far update HT=%d, LT=%d, ps=%d\n", __func__, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val), obj->ps);						
    ...
    printk("alsps_wind %s obj->ps = %d obj->ps_high_thd_val = %d obj->ps_low_thd_val = %d \n",__func__, obj->ps, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val));
...
int stk3x1x_setup_eint(struct i2c_client *client)
{
	...
	alspsPltFmDev_stk = get_alsps_platformdev();
/* gpio setting */
	pinctrl = devm_pinctrl_get(&alspsPltFmDev_stk->dev);
	if (IS_ERR(pinctrl)) {
		ret = PTR_ERR(pinctrl);
...
#endif
{
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_PS_DATA:    
            ...
            printk("alsps_wind %s raw 0x%x -> value 0x%x  \n",__func__,obj->ps, dat);
            
#ifdef STK_PS_POLLING_LOG	
			APS_LOG("alsps_wind  %s:ps raw 0x%x -> value 0x%x \n",__FUNCTION__, obj->ps, dat);			
#endif			
...
//			obj->als_last = obj->als;
			*value = obj->als; 
            //printk("hb stk3x1x value=%d stk3x1x_lux_per_count=%d\n", *value, stk3x1x_lux_per_count);
//		}
//		else
//		{
...
static int ps_enable_nodata(int en)
{
	...
	//chenjiehua@wind-mobi.com change start 20160823
	if(en)
	{
		set_bit(STK_BIT_PS, &stk3x1x_obj->enable);
		APS_LOG("%s:enable=%d\n", __FUNCTION__,test_bit(STK_BIT_PS, &stk3x1x_obj->enable) ? (1) : (0));
	}
	else
	{
...
static int ps_get_data(int* value, int* status)
{
    ...
    else
    {
        *value = stk3x1x_get_ps_value(stk3x1x_obj, stk3x1x_obj->ps);
		stk3x1x_obj->ps_distance_last = *value;
        *status = SENSOR_STATUS_ACCURACY_MEDIUM;
    }
#endif //#ifdef CUSTOM_KERNEL_SENSORHUB
...
// wangbing@wind-mobi.com 20170426 begin
void stk3x1x_start_als_value(void)
{
    struct stk3x1x_priv *obj = i2c_get_clientdata(stk3x1x_i2c_client);
    stk3x1x_read_als(stk3x1x_obj->client, &obj->als);
    stk3x1x_get_als_value(obj, obj->als);
}

void stk3x1x_write_ps_value(int ps_low_threshold, int ps_high_threshold)
{
    int ret = 0;
    
    struct stk3x1x_priv *obj = i2c_get_clientdata(stk3x1x_i2c_client);
    obj->stk_max_min_diff = ps_high_threshold;      // wangjun@wind-mobi.com at 20170526
    obj->stk_ht_n_ct = ps_high_threshold;
    obj->stk_lt_n_ct = ps_low_threshold;
	...
	ps_threshold_5cm = atomic_read(&obj->ps_low_thd_val);   //lihaiyan@wind-mobi.com 20170515 modified
    printk("wangbing: %s ps_high_thd_val = %d, ps_low_thd_val = %d psi = %d \n", __FUNCTION__, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val), obj->psi);
}
// wangbing@wind-mobi.com 20170426 end
...
//	obj->hw = get_cust_alsps_hw();
	...
	atomic_set(&obj->state_val, 0);
	atomic_set(&obj->psctrl_val, 0x30);	//0x31   //hebiao modified at  20170406 
	atomic_set(&obj->alsctrl_val, 0x39); //hebiao modified at  20170406 
	obj->ledctrl_val = 0xFF;	
	obj->wait_val = 0xF; //hebiao modified at  20170406 
	obj->int_val = 0;
	obj->first_boot = true;			 
	obj->als_correct_factor = 1000;
	...
	{
		obj_ps.polling = 1;
		wake_lock_init(&ps_lock_stk,WAKE_LOCK_SUSPEND,"ps wakelock");
	}
	else
	{
...
// wangbing@wind-mobi.com 20170426 begin
    als_calibration_factor = 902;   //wangjun@wind-mobi.com 20170601
	write_ps_value_func = stk3x1x_write_ps_value;
	start_als_value_func = stk3x1x_start_als_value;
    psensor_proc_init();
...
static int __init stk3x1x_init(void)
{
    ...
    if (!hw)
        APS_ERR("stk3x1x get dts info fail\n");

    alsps_driver_add(&stk3x1x_init_info);       // hwmsen_alsps_add(&stk3x1x_init_info);

#if 0
    stk3x1x_regulator = regulator_get(&alspsPltFmDev_stk->dev, "vstk3x1x");
...
// wangbing@wind-mobi.com 20170421 end