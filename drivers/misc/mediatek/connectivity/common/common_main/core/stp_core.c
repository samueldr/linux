...
static VOID stp_sdio_trace32_dump(VOID)
{
	if (STP_IS_ENABLE_DBG(stp_core_ctx) && (stp_core_ctx.parser.type == STP_TASK_INDX) &&
		...
		if (0 == osal_strncmp("coredump end", stp_core_ctx.rx_buf + stp_core_ctx.rx_counter -
					osal_strlen("coredump end") - 2, osal_strlen("coredump end"))) {
	...
	while (i > 0) {
		...
		case MTKSTP_CRC2:
			...
			if (0x00 != stp_core_ctx.parser.crc)
				STP_ERR_FUNC
					("CRC (0x%d) is not 0 under SDIO/MAND mode, maybe something is wrong.\n",
