...
#define P2P_INDICATE_FRAME_BSS_INFO	0
...
#if P2P_INDICATE_FRAME_BSS_INFO
...
#define BEACON_FIXED_FIELD_LENGTH 12
	...
	if (ucReportDetail == 1 && ucReportIeIdsLen > 0) {
		...
		prBcnReport->aucOptElem[1] = u2BcnReportLen & 0xff; /* length of the sub-element */
		u2BcnReportLen += 2;
		...
		prBcnReport->aucOptElem[0] = 1; /* sub-element id for reported frame body */
		prBcnReport->aucOptElem[1] = u2BcnReportLen & 0xff; /* length of the sub-element */
		kalMemCopy(prBcnReport->aucOptElem, prRepParams->aucBcnFixedField, u2BcnReportLen);
		u2BcnReportLen += 2;
	}
