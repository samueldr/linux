...
//liukangping add read country code 20170904 begin

char path[] = "/dev/block/platform/mtk-msdc.0/11230000.msdc0/by-name/proinfo";
int wifi_value1,wifi_value2;

//liukangping add read country code 20170904 end
...
//liukangping add read country code 20170904 begin
int read_proinfo_countrycode_wifi(void)
{
   // part_t *part = part_get(name);
	...
	// int i;
	 ...
	 pos = 0;
	// int i=0;
	 pfile = filp_open(path, O_RDONLY, 0); //fp = open(path,O_RDONLY)
	//IS_ERR(pfile);
 /*  if(0)
	{
		printk("wind-liukangping ERROR fopen  pfile \n");
		return -EBADF;
	}
	*/
	//read(fp,buf,208);
	vfs_read(pfile, buf, 800, &pos);
	filp_close(pfile, NULL);
  //  for(i=104;i<500;i++)
    //{
    //printk("wind-liukangping get nvram value buf[%d]=%c\n",i-104,buf[i]);
    //}
	
	if(buf[203] == 'T' && buf[204] == 'W')
	{
		wifi_value1 = 0x45;
		wifi_value2 = 0x55;//EU
		printk("wind-liukangping read country code TW\n");
		printk("wind-liukangping get success nvram value buf[203]=%c\n",buf[203]);
		printk("wind-liukangping get success nvram value buf[204]=%c\n",buf[204]);
	}
	else if(buf[203] == 'W' && buf[204] == 'W')
	{
		wifi_value1 = 0x45;
		wifi_value2 = 0x55; //EU
		printk("wind-liukangping read country code WW\n"); 
		printk("wind-liukangping read country code TW\n");
		printk("wind-liukangping get success nvram value buf[203]=%c\n",buf[203]);
		printk("wind-liukangping get success nvram value buf[204]=%c\n",buf[204]);
		
	}
	else if(buf[203] == 'U' && buf[204] == 'S')
	{
		wifi_value1 = 0x55;
		wifi_value2 = 0x53;//us
		printk("wind-liukangping get success nvram value buf[203]=%c\n",buf[203]);
		printk("wind-liukangping get success nvram value buf[204]=%c\n",buf[204]);
		printk("wind-liukangping read country code US\n"); 
	}
	else if(buf[203] == 'J' && buf[204] == 'P')
	{
		wifi_value1 = 0x45;
		wifi_value2 = 0x55;//JP
		printk("wind-liukangping get success nvram value buf[203]=%c\n",buf[203]);
		printk("wind-liukangping get success nvram value buf[204]=%c\n",buf[204]);
		printk("wind-liukangping read country code JP\n"); 
	}
	else
	{
		wifi_value1 = 0x45;
		...
		printk("wind-liukangping get success nvram value buf[203]=%c\n",buf[203]);
		printk("wind-liukangping get success nvram value buf[204]=%c\n",buf[204]);
		printk("wind-liukangping read country code CN\n"); 
	}
	return 0;
}
//liukangping add read country code 20170904 end
...
static void glLoadNvram(IN P_GLUE_INFO_T prGlueInfo, OUT P_REG_INFO_T prRegInfo)
{
	...
	UINT_8 aucTmp[2];
	PUINT_8 pucDest;
  //   blkdev_t *bootdev;  //liukangping add read country code 20170904 
	ASSERT(prGlueInfo);
	ASSERT(prRegInfo);
	...
	TC1_FAC_NAME(FacReadWifiMacAddr) ((unsigned char *)prRegInfo->aucMacAddr);
        ...
        //liukangping add read country code 20170904 begin
		read_proinfo_countrycode_wifi();	
		printk("wind-liukangping entra glLoadNvram wifi");
		//prRegInfo->au2CountryCode[0] = (UINT_16) aucTmp[0];
		//prRegInfo->au2CountryCode[1] = (UINT_16) aucTmp[1];
		prRegInfo->au2CountryCode[0] = wifi_value1;
		prRegInfo->au2CountryCode[1] = wifi_value2;
        //liukangping add read country code 20170904 end
