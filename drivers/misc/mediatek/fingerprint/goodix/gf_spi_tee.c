//hebiao@wind-mobi.com 20161021 begin
...
//modify by gaohui@wind-mobi.com 20161219 begin for dark screen unlock
//#ifdef CONFIG_HAS_EARLYSUSPEND
//#include <linux/earlysuspend.h>
//#else
//#include <linux/notifier.h>
//#endif
//modify by gaohui@wind-mobi.com 20161219 end for dark screen unlock
...
//liukangping 20170112 begin	
#include "nt_smc_call.h" // add sunwave for read id
...
/* debug log setting */
u8 g_debug_level = ERR_LOG; ///LIUKANGPING
...
//#endif
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		if (GF_KEY_HOME == gf_key.key) {
			//add by lizusheng@wind-mobi.com for Navigation 20161125 begin
			break;
			//add by lizusheng@wind-mobi.com for Navigation 20161125 end
			...
			//add by lizusheng@wind-mobi.com for Navigation 20161125 begin
		} else if (GF_KEY_SINGLE == gf_key.key) {
			key_input = GF_INPUT_SINGLE_CLICK_KEY;
		} else if (GF_KEY_DOUBLE == gf_key.key) {
			key_input = GF_INPUT_DOUBLE_CLICK_KEY;
		} else if (GF_KEY_LONGPRESS == gf_key.key) {
			key_input = GF_INPUT_LONG_PRESS_KEY;
			//add by lizusheng@wind-mobi.com for Navigation 20161125 end
		} else {
			/* add special key define */
			key_input = gf_key.key;
		}
		printk("%s: received key event[%d], key=%d, value=%d\n",
				__func__, key_input, gf_key.key, gf_key.value);
			//modified by lizusheng@wind-mobi.com for Navigation 20161125 begin
		if ((GF_KEY_POWER == gf_key.key || GF_KEY_CAMERA == gf_key.key || GF_KEY_SINGLE == gf_key.key || GF_KEY_DOUBLE == gf_key.key || GF_KEY_LONGPRESS == gf_key.key)) { //zhangkaiyuan@wind-mobi.com 20170621
			input_report_key(gf_dev->input, key_input, 1);
			input_sync(gf_dev->input);
			input_report_key(gf_dev->input, key_input, 0);
			input_sync(gf_dev->input);
		}
			//modified by lizusheng@wind-mobi.com for Navigation 20161125 end
		if (GF_KEY_HOME == gf_key.key) {
		...
		break;
//zhangkaiyuan@wind-mobi.com 20170913 begin
	case GF_IOC_NAV_EVENT:
		...
		switch (nav_event) {
			...
			printk("nav long press");
			break;

		    case GF_NAV_DOUBLE_CLICK:
			nav_input = GF_NAV_INPUT_DOUBLE_CLICK;
			printk("nav double click");
			break;
//zhangkaiyuan@wind-mobi.com 20170913 end
		    default:
...
static int gf_probe(struct spi_device *spi)
{
	...
	gf_spi_clk_enable(gf_dev, 1);
/*
//hebiao@wind-mobi.com 20161020 begin for test interrupt
	printk("register inter begin\n");
	gf_irq_gpio_cfg(gf_dev);
	retval = request_threaded_irq(gf_dev->irq, NULL, gf_irq,
				IRQF_TRIGGER_RISING | IRQF_ONESHOT, "goodix_fp_irq", gf_dev);
	if (!retval)
		...
		gf_debug(ERR_LOG, "%s irq thread request failed, retval=%d\n", __func__, retval);
	printk("register inter end\n");
	gf_hw_reset(gf_dev, 0);
	printk("reset end\n");
//hebiao@wind-mobi.com 20161020 end for test interrupt		
*/
	...
	//add by lizusheng@wind-mobi.com for Navigation 20161125 begin
	__set_bit(GF_INPUT_SINGLE_CLICK_KEY, gf_dev->input->keybit);
	__set_bit(GF_INPUT_DOUBLE_CLICK_KEY, gf_dev->input->keybit);
	__set_bit(GF_INPUT_LONG_PRESS_KEY, gf_dev->input->keybit);
	//add by lizusheng@wind-mobi.com for Navigation 20161125 end
...
//hebiao@wind-mobi.com 20161105 begin
#ifdef CONFIG_WIND_DEVICE_INFO
	sprintf(wind_device_info.fp_module_info.ic_name, "%s", "gx556");
	wind_device_info.fp_module_info.vendor = 0x01; //hilitai is 0x01
	wind_device_info.fp_module_info.fwvr = 0x30020;
#endif
//hebiao@wind-mobi.com 20161105 end	
	printk("gf_out_probe\n");
	FUNC_EXIT();
...
static int __init gf_init(void)
{
	int status = 0;
	//liukangping 20170112 begin	
	uint64_t gf_vendor_id = 0x00;

	FUNC_ENTRY();
	
	get_t_device_id(&gf_vendor_id); // add sunwave for read id
	gf_debug(ERR_LOG, "%s gf_vendor_id = 0x%llx\n", __func__, gf_vendor_id);
	if(gf_vendor_id != 0x03) {
		gf_debug(ERR_LOG, "%s, Failed to read ic id, 0x%llx\n", __func__, gf_vendor_id); 
		return -EINVAL;
	}
//liukangping 20170112 end
...
//hebiao@wind-mobi.com 20161021 end