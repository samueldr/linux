...
int mas_probe(struct spi_device *spi) {
	...
	//zhangkaiyuan@wind-mobi.com 20170913 begin
    #ifdef CONFIG_WIND_DEVICE_INFO
	sprintf(wind_device_info.fp_module_info.ic_name, "%s", "microarray");
	wind_device_info.fp_module_info.vendor = 0x04;
	wind_device_info.fp_module_info.fwvr = 0x04;
    #endif
    //zhangkaiyuan@wind-mobi.com 20170913 end	
...
static int __init mas_init(void)
{
	...
	//add by microarray xuh 20170912 begin	
	uint64_t mf_vendor_id = 0x00;
	MALOGF("start");
	get_t_device_id(&mf_vendor_id); // add mircoarray for read id
	printk("%s mf_vendor_id = 0x%llx\n", __func__, mf_vendor_id);
	if(mf_vendor_id != 0x04) {
		printk("%s, Failed to read ic id, 0x%llx\n", __func__, mf_vendor_id); 
		return -EINVAL;
	}
    //add by microarray xuh 20170912 end
