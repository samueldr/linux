...
extern int flashlight_vendor_id;  // zhaozhensen@wind-mobi.com 20161219
...
//zhenglihong@wind-mobi.com 20170327 begin
...
//zhenglihong@wind-mobi.com 20170327 end	
	 else if (sensorDev == e_CAMERA_SUB_SENSOR) {
		if (strobeId == 1) {
			if (partId == 1)
				subStrobeInit(ppF);
...
//	kdStrobeDrvArg *pIdtempbuf;
	...
	// zhaozhensen@wind-mobi.com 20161219 begin
	 case FLASH_IOC_GET_STROBE_IC:
		{
			pr_err("zzs FLASH_IOC_GET_STROBE_IC\n");
			kdArg.arg = flashlight_vendor_id;
			if (copy_to_user((void __user *)arg, (void *)&kdArg, sizeof(kdStrobeDrvArg))) {
				logI("zzs [FLASH_IOC_GET_STROBE_IC] ioctl copy to user failed ~");
				return -EFAULT;
			}
			pr_err("zzs kdArg.arg = %d\n", kdArg.arg);
		}
		break;
