...
// zhaozhensen@wined-mobi.com 20161122 for hi843b_otp problem begin
extern int iReadRegI2C_OTP(u8 *a_pSendData , u16 a_sizeSendData, u8 *a_pRecvData, u16 a_sizeRecvData, u16 i2cId);
// zhaozhensen@wined-mobi.com 20161122 for hi843b_otp problem end
#endif //Hi-843B OTP
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.i2c_addr_table = {0x40, 0x40, 0x40, 0xff},
};



static imgsensor_struct imgsensor = {
	.mirror = IMAGE_H_MIRROR,				//mirrorflip information //zhaozhensen@wind-mobi.com 20161017
...
void Hi843B_Sensor_OTP_info(void)
{
    ...
    default:
        ...
        pr_debug("wangk_HI843B_Sensor: info_flag error value: %d \n ",info_flag);
        break;
    }
    ...
    if (checksum == infocheck)
    {
        ...
        pr_debug("wangk_HI843B_Sensor: Module information checksum PASS\n ");
    }
    else
    {
        LOG_INF("HI843B_Sensor: Module information checksum Fail\n ");
        pr_debug("wangk_HI843B_Sensor: Module information checksum Fail\n ");
    }
	...
	switch (wb_flag)
	{
		case 0x01:  //Group1 valid
		   ...
		   // wb_golden_g_l = HI843B_Sensor_OTP_read(0x0c77);
			wbcheck = HI843B_Sensor_OTP_read(0x0c67);
		...
		case 0x02:  //Group2 valid
			wb_unit_rg_h = HI843B_Sensor_OTP_read(0x0c68);
		    wb_unit_rg_l = HI843B_Sensor_OTP_read(0x0c69);
		    wb_unit_bg_h = HI843B_Sensor_OTP_read(0x0c6a);
		    wb_unit_bg_l = HI843B_Sensor_OTP_read(0x0c6b);
		    wb_unit_gg_h = HI843B_Sensor_OTP_read(0x0c6c);
		    wb_unit_gg_l = HI843B_Sensor_OTP_read(0x0c6d);
		   ...
		   // wb_golden_g_l = HI843B_Sensor_OTP_read(0xBD51);
			...
			{
				addr = 0x0c68 + i;
				LOG_INF("zzs HI843B_Sensor: group 2 addr = 0x%x   0x%x\n ",i+0x0c68,HI843B_Sensor_OTP_read(addr));
			}	
		    break;
		case 0x03:  //Group3 valid
		  ...
		  //  wb_golden_g_l = HI843B_Sensor_OTP_read(0xBD8A);
		    wbcheck = HI843B_Sensor_OTP_read(0x0c77);
			for(i=0;i < 6;i++)
			{
				addr = 0x0c70 + i;
				LOG_INF("zzs HI843B_Sensor: group 3 addr = 0x%x   0x%x\n ",i+0x0c70,HI843B_Sensor_OTP_read(addr));
			}	
		    break;
		default:
...
//  wb_golden_bg_l = 0x09;//
 ...
 //     + wb_golden_r_h + wb_golden_r_l + wb_golden_b_h + wb_golden_b_l + wb_golden_g_h + wb_golden_g_l ) % 0xFF + 1;
	...
	if (checksum == wbcheck)
	{
		...
		pr_debug("wangk_HI843B_Sensor: WB checksum PASS\n ");
    }
	else
    {
	    LOG_INF("HI843B_Sensor: WB checksum Fail\n ");
	    pr_debug("wangk_HI843B_Sensor: WB checksum Fail\n ");
    }
	...
	rg_golden_value = 0x0159; //((wb_golden_rg_h << 8)|wb_golden_rg_l);
	bg_golden_value = 0x012D; //((wb_golden_bg_h << 8)|wb_golden_bg_l);
  ...
  b_gain = (((OTP_MULTIPLE_FAC * bg_golden_value) / bg_value) * 0x200) / OTP_MULTIPLE_FAC;
	...
	LOG_INF("%s exit\n",__func__);
    // zhaozhensen@wined-mobi.com 20161122 for hi843b_otp problem begin
    HI843B_OTP_write_cmos_sensor(0x0a00, 0x00);
    mdelay(10);
    HI843B_OTP_write_cmos_sensor(0x003e, 0x00);
    HI843B_OTP_write_cmos_sensor(0x0a00, 0x00);
    // zhaozhensen@wined-mobi.com 20161122 for hi843b_otp problem end
...
write_cmos_sensor(0x0b22, 0xdc80);  // zhaozhensen@wind-mobi.com 20170227
...
write_cmos_sensor(0x0a04, 0x016a);  //zhaozhensen@wind-mobi.com 20161021
...
write_cmos_sensor(0x0b1e, 0x0081); 
write_cmos_sensor(0x0b20, 0x0800); 
write_cmos_sensor(0x0b22, 0xdc80);  //zhaozhensen@wind-mobi.com 20161021
write_cmos_sensor(0x0b24, 0x0000); 
write_cmos_sensor(0x0b26, 0x0001); 
write_cmos_sensor(0x0b28, 0x0807); 
...
write_cmos_sensor(0x0002, 0x0000); 
write_cmos_sensor(0x0a02, 0x0100); 
write_cmos_sensor(0x0a04, 0x014a); 	//zhaozhensen@wind-mobi.com 20161021
...
write_cmos_sensor(0x0002, 0x0000); 
write_cmos_sensor(0x0a02, 0x0100); 
write_cmos_sensor(0x0a04, 0x0168); 	//zhaozhensen@wind-mobi.com 20161021
...
static kal_uint32 open(void)
{
    ...
    //Hi843B_Sensor_OTP_update_LSC();	// zhaozhensen@wind-mobi.com 20161020
