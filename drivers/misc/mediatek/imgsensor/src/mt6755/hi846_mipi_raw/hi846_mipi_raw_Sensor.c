...
static void HI846OTPSetting(void);
static uint16_t return_moudle_id(void);
static void HI846_Sensor_update_wb_gain(kal_uint32 r_gain, kal_uint32 g_gain,kal_uint32 b_gain);
static kal_uint16 HI846_Sensor_OTP_read(kal_uint16 otp_addr);
static void HI846_Sensor_calc_wbdata(void);
...
#endif //Hi-846 OTP
// zhaozhensen@wind-mobi.com 20170317 end
...
/*zhenglihong@wind-mobi.com 20170422 begin*/
	.custom1 = {
		.pclk = 288000000,				//record different mode's pclk
		.linelength = 3800,				//record different mode's linelength
		.framelength = 2492,			//record different mode's framelength
...
/*zhenglihong@wind-mobi.com 20170422 end*/
	.margin = 6,
	.min_shutter = 6,
	...
	.custom1_delay_frame =3,//zhenglihong@wind-mobi.com 20170422 change for binning mode
...
//zhenglihong@wind-mobi.com 20170422 change for binning mode
...
// zhaozhensen@wind-mobi.com 20170329 begin
static kal_uint32 return_sensor_id(void)
{
	...
	if(sensor_id == 0x0846)
	{
		if(current_module_id == 0)
		{
			sensor_init();
			HI846OTPSetting();
			current_module_id = return_moudle_id();
			pr_err("Hi846 current_module_id = 0x%x \n", current_module_id);
		}
		if(current_module_id == 0x42)
			sensor_id ++;
	}
	pr_err("zzs cur_sensor_id = %d\n", sensor_id);
#endif
	
	return sensor_id;
}
// zhaozhensen@wind-mobi.com 20170329 end
// zhaozhensen@wind-mobi.com 20170317 begin
...
#if Hi846_OTP_FUNCTION //Hi-846 OPT fuction begin for E266L_HI846
    // zhaozhensen@wined-mobi.com 20161122 for hi846_otp problem begin
    ...
    static uint16_t return_moudle_id(void)
    {
    	...
    	{
            case 0x01:  //Group1
                moudle_id = HI846_Sensor_OTP_read(0x0202);
            break;
            
            case 0x02:
                moudle_id = HI846_Sensor_OTP_read(0x0213);
            break;
            
            case 0x03:
                moudle_id = HI846_Sensor_OTP_read(0x0224);
            break;
        ...
        pr_err("zzs return moudle id = %d\n", moudle_id);
        return moudle_id;
    }

    static void Hi846_Sensor_OTP_info(void)
    {
...
//dingyisheng@wind-mobi.com 20170808 read golden value from module begin
    static void HI846_Sensor_calc_wbdata(void)
    {
        int i = 0, addr = 0;
        uint16_t wbcheck = 0,
    	...
    	//rg_golden_value = 0x00E9;
    	//bg_golden_value = 0x0122;
    ...
    b_gain = (((OTP_MULTIPLE_FAC * bg_golden_value) / bg_value) * 0x200) / OTP_MULTIPLE_FAC;
        ...
        HI846_OTP_write_cmos_sensor(0x0a00, 0x01);
        // zhaozhensen@wined-mobi.com 20161122 for hi846_otp problem end
...
//dingyisheng@wind-mobi.com 20170808 read golden value from module begin
#else
	#if Hi846_OTP_FUNCTION //Hi-846 OPT fuction begin for E286L_HI846
	// zhaozhensen@wined-mobi.com 20161122 for hi846_otp problem begin
	static kal_uint16 read_cmos_sensor_byte(kal_uint32 addr)
	{
		kal_uint16 get_byte=0;
    ...
    static void HI846_Sensor_calc_wbdata(void)
	{
		...
		switch (wb_flag)
		{
			case 0x01:  //Group1 valid
				...
				wbcheck = HI846_Sensor_OTP_read(0x0c67);
			...
			case 0x02:  //Group2 valid
				wb_unit_rg_h = HI846_Sensor_OTP_read(0x0c68);
				wb_unit_rg_l = HI846_Sensor_OTP_read(0x0c69);
				wb_unit_bg_h = HI846_Sensor_OTP_read(0x0c6a);
				wb_unit_bg_l = HI846_Sensor_OTP_read(0x0c6b);
				wb_unit_gg_h = HI846_Sensor_OTP_read(0x0c6c);
				wb_unit_gg_l = HI846_Sensor_OTP_read(0x0c6d);

				wbcheck = HI846_Sensor_OTP_read(0x0c6f);
				for(i=0;i < 6;i++)
				{
					addr = 0x0c68 + i;
					LOG_INF("zzs HI846_Sensor: group 2 addr = 0x%x   0x%x\n ",i+0x0c68,HI846_Sensor_OTP_read(addr));
				}	
				break;
			case 0x03:  //Group3 valid
				wb_unit_rg_h = HI846_Sensor_OTP_read(0x0c70);
				wb_unit_rg_l = HI846_Sensor_OTP_read(0x0c71);
				wb_unit_bg_h = HI846_Sensor_OTP_read(0x0c72);
				wb_unit_bg_l = HI846_Sensor_OTP_read(0x0c73);
				wb_unit_gg_h = HI846_Sensor_OTP_read(0x0c74);
				wb_unit_gg_l = HI846_Sensor_OTP_read(0x0c75);

				wbcheck = HI846_Sensor_OTP_read(0x0c77);
				for(i=0;i < 6;i++)
				{
					addr = 0x0c70 + i;
					LOG_INF("zzs HI846_Sensor: group 3 addr = 0x%x   0x%x\n ",i+0x0c70,HI846_Sensor_OTP_read(addr));
				}	
				break;
			default:
				LOG_INF("HI846_Sensor: wb_flag error value: 0x%x\n ",wb_flag);
		...
		checksum = (wb_unit_rg_h + wb_unit_rg_l + wb_unit_bg_h + wb_unit_bg_l + wb_unit_gg_h + wb_unit_gg_l) % 256;
		...
		rg_golden_value = 0x0159;
		bg_golden_value = 0x012D;
...
//yeshaobo@wind-mobi.com 20170703 begin
static void sensor_init(void)
{
LOG_INF("Enter init\n");
...
write_cmos_sensor(0x0046, 0x0000); //group para off/per frame control off
 ...
 write_cmos_sensor(0x021C, 0x0003); /* zhaozhensen@wind-mobi.com 20170317 OPRN LSC*/
 write_cmos_sensor(0x021e, 0x0235); /* zhaozhensen@wind-mobi.com 20170317 OPRN LSC*/
 ...
 write_cmos_sensor(0x0A04, 0x015A); // zhaozhensen@wind-mobi.com 20170317 OPRN LSC
...
//yeshaobo@wind-mobi.com 20170703 end
static void preview_setting(void)
{
	...
	////////////////////////////////////////////////
	write_cmos_sensor(0x0A00, 0x0000); 
write_cmos_sensor(0x021C, 0x0003); //yeshaobo@wind-mobi.com 20170703 
write_cmos_sensor(0x021e, 0x0235); //yeshaobo@wind-mobi.com 20170703
	...
	write_cmos_sensor(0x0A04, 0x017A); // zhaozhensen@wind-mobi.com 20170317 OPRN LSC
	write_cmos_sensor(0x0418, 0x0000); 
	write_cmos_sensor(0x0B02, 0xE04D); 
	write_cmos_sensor(0x0B10, 0x6C21); 
	write_cmos_sensor(0x0B12, 0x0120); 
	write_cmos_sensor(0x0B14, 0x0005); 
	write_cmos_sensor(0x2008, 0x38FD); // brightness issue
	write_cmos_sensor(0x326E, 0x0000); 
	//=============================================//
	//      MIPI 4lane 360Mbps
	//=============================================//
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if( currefps == 300 )
	{
		...
		write_cmos_sensor(0x0A00, 0x0000);
		write_cmos_sensor(0x021C, 0x0003); //yeshaobo@wind-mobi.com 20170703
		write_cmos_sensor(0x021e, 0x0235); //yeshaobo@wind-mobic.om 20170703
		write_cmos_sensor(0x002E, 0x1111); 
		write_cmos_sensor(0x0032, 0x1111); 
		write_cmos_sensor(0x0026, 0x0040); 
		write_cmos_sensor(0x002C, 0x09CF); 
		...
		write_cmos_sensor(0x0B12, 0x0120);
		write_cmos_sensor(0x0B14, 0x0001); 
		write_cmos_sensor(0x2008, 0x38FD); // bright issue
		write_cmos_sensor(0x326E, 0x0000); 
		//=============================================//
		//      MIPI 4lane 720Mbps
		//=============================================//
...
//add by wanfei@wind-mobi.com 20170613 begin
...
write_cmos_sensor(0x0A12, 0x0660);
write_cmos_sensor(0x0A14, 0x04C8);
write_cmos_sensor(0x0A04, 0x016A); //disable LSC OTP
write_cmos_sensor(0x0418, 0x0000);
write_cmos_sensor(0x0B02, 0xE04D);
write_cmos_sensor(0x0B10, 0x6C21);
write_cmos_sensor(0x0B12, 0x0120);
...
//add by wanfei@wind-mobi.com 20170613 end
...
//wanfei@wind-mobi.com 20170613 begin
...
//wanfei@wind-mobi.com 20170613 end
        ...
        case SENSOR_FEATURE_GET_CROP_INFO:
            ...
            switch (*feature_data_32) {
                ...
                case MSDK_SCENARIO_ID_SLIM_VIDEO:
                    ...
                    break;
//add wanfei@wind-mobi.com 20170613 begin
                case MSDK_SCENARIO_ID_CUSTOM1:
