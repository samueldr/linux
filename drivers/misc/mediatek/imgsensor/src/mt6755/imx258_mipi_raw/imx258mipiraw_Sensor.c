...
static void load_imx258_SPC_Data(void)
{
	...
	if ( SPC_data_done == false ) {
		if (!read_imx258_eeprom_SPC(0x0D04,IMX258MIPI_SPC_Data,126)) { // zhaozhensen@wind-mobi.com 20161023
...
int read_imx258_i2c_eeprom(kal_uint16 slave_id,kal_uint32 addr, u8* data)
{
    //kal_uint16 get_byte=0;
...
u8 read_imx258_eeprom_moudle_id(void)
{
    u8 moudle_id = 0;
    pr_err("zzs OTP read moudle_id id.\n");
    
    if(read_imx258_i2c_eeprom(0xA0, 0x0005, &moudle_id) != 0)
    {
	...
	if(write_cmos_sensor(0x0A02, 0x0F)==0){
		...
		while(retry--)
		{
			 if(read_cmos_sensor(0x0A01) == 0x01)
			 {
				 ...
				 sensor_id = sensor_id;
				 LOG_INF("zzs returnsensorid = %x\n", sensor_id);
				 module_id = read_imx258_eeprom_moudle_id();
