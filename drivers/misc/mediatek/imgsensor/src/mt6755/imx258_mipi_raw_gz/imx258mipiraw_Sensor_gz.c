...
#include "imx258mipiraw_Sensor_gz.h"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX258_SENSOR_ID_GZ, //IMX258MIPI_SENSOR_ID,  /*sensor_id = 0x2680*/ //record sensor id defined in Kd_imgsensor.h
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =
	{{ 4208, 3120,	  0,	0, 4208, 3120, 2100,  1560, 0000, 0000, 2100,  1560,	  0,	0, 2100, 1560}, // Preview
	 ...
	 { 4208, 3120,	  0,	0, 4208, 3120, 2100,  1560, 0000, 0000, 2100,  1560,	  0,	0, 2100, 1560}, // video
#if 0
...
static SET_PD_BLOCK_INFO_T imgsensor_pd_info =
{
    ...
    .i4SubBlkW =16,
    .i4SubBlkH =16,
    .i4PosL = {{26,29},{42,29},{33,48},{49,48}},	// zhaozhensen@wind-mobi.com 20161023
    .i4PosR = {{25,32},{41,32},{34,45},{50,45}},	// zhaozhensen@wind-mobi.com 20161023
};
...
kal_uint16 IMX258MIPI_sensorGainMapping_gz[IMX258MIPI_MaxGainIndex][2] ={
	{64 ,0	},
	{65 ,8	},
	{66 ,16 },
	{67 ,25 },
...
extern bool read_imx258_gz_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool read_imx258_gz_eeprom_SPC( kal_uint16 addr, BYTE* data, kal_uint32 size);
...
static void load_imx258_SPC_Data(void)
{
	...
	if ( SPC_data_done == false ) {
		if (!read_imx258_gz_eeprom_SPC(0x0D04,IMX258MIPI_SPC_Data,126)) { // zhaozhensen@wind-mobi.com 20161023
...
int read_imx258_gz_i2c_eeprom(kal_uint16 slave_id,kal_uint32 addr, u8* data)
{
    //kal_uint16 get_byte=0;
...
u8 read_imx258_gz_eeprom_moudle_id(void)
{
    u8 moudle_id = 0;
    pr_err("zzs OTP read moudle_id id.\n");
    
    if(read_imx258_gz_i2c_eeprom(0xA0, 0x0005, &moudle_id) != 0)
    {
        pr_err("zzs OTP read moudle_id id failed.\n");
        return -1;
    }
    
    pr_err("zzs OTP read moudle_id id = %d.\n", moudle_id);
    return moudle_id;
}
	...
	if(write_cmos_sensor(0x0A02, 0x0F)==0){
		...
		while(retry--)
		{
			 if(read_cmos_sensor(0x0A01) == 0x01)
			 {
				 ...
				 sensor_id = (read_cmos_sensor(0x0A26) << 4) | (read_cmos_sensor(0x0A27) >> 4);
				 sensor_id = sensor_id;
				 pr_debug("zzs returnsensorid = %x\n", sensor_id);
				 module_id = read_imx258_gz_eeprom_moudle_id();
				 printk("zzs module_id = %d\n", module_id);
				 if (sensor_id  == 0x0258)
				 {
					if (module_id == 0x36)
					{
						sensor_id = sensor_id - 1;
						printk("zzs guangzhen module sensor_id = %d\n", sensor_id);
					}
				 }
				 return sensor_id;
			 }
...
static void set_shutter(unsigned long long shutter)
{
	...
	set_shutter_flag  = 1;
 	LOG_INF("ciel_test_set_shutter_Enter! shutter %llu \n", shutter);
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    ...
    for (i = 0; i < IMX258MIPI_MaxGainIndex; i++) {
        if(gain <= IMX258MIPI_sensorGainMapping_gz[i][0]){
            break;
        }
    }
    if(gain != IMX258MIPI_sensorGainMapping_gz[i][0])
            LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX258MIPI_sensorGainMapping_gz[i][0]);
    return IMX258MIPI_sensorGainMapping_gz[i][1];
}
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_imx258_gz_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
		...
		case SENSOR_FEATURE_GET_PDAF_INFO:
			...
			switch (*feature_data) {
				case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
				case MSDK_SCENARIO_ID_VIDEO_PREVIEW:	// zhaozhensen@wind-mobi.com 20161023
					if(imx258_type == IMX258_HDR_TYPE)
...
UINT32 IMX258_MIPI_RAW_SensorInit_GZ(PSENSOR_FUNCTION_STRUCT *pfFunc)
