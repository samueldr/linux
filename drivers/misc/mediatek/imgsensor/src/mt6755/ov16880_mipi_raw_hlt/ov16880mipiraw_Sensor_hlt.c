...
#include "ov16880mipiraw_Sensor_hlt.h"
...
extern bool read_eeprom_hlt( u16 addr, BYTE* data, u32 size);
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV16880_SENSOR_ID_HLT,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {	
			*sensor_id = (read_cmos_sensor_8(0x300A) << 16) | (read_cmos_sensor_8(0x300B)<<8) | read_cmos_sensor_8(0x300C);
			...
			LOGE("Read sensor id fail, write id:0x%x ,sensor Id:0x%x,imgsensor_info.sensor_id=0x%x\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) 
	{
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_eeprom_hlt((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
...
UINT32 OV16880MIPIHLTSensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
