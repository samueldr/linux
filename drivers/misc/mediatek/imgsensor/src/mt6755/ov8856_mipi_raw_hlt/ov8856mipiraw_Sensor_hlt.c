...
#include "ov8856mipiraw_Sensor_hlt.h"
...
OV8856_VERSION ov8856versionhlt = OV8856R1A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);
// zhaozhensen@wind-mobi.com 20161117 begin
#define OV8856R1A_OTP_SUPPORT
...
#endif
// zhaozhensen@wind-mobi.com 20161117 end
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	switch (image_mirror) {
		...
		case IMAGE_V_MIRROR:
			...
			write_cmos_sensor(0x5004,((read_cmos_sensor(0x5004) & 0xFB) | 0x04));				
			write_cmos_sensor(0x376b,((read_cmos_sensor(0x376b) & 0xC0) | 0x36));//shenan,ox20->0x36					
		...
		case IMAGE_HV_MIRROR:
			...
			write_cmos_sensor(0x5001,((read_cmos_sensor(0x5001) & 0xFB) | 0x04));
			write_cmos_sensor(0x5004,((read_cmos_sensor(0x5004) & 0xFB) | 0x00));	
			write_cmos_sensor(0x376b,((read_cmos_sensor(0x376b) & 0xC0) | 0x36));//shenan,0x20->0x36					
...
static void sensor_init(void)
{
    ...
    */
	...
	write_cmos_sensor(0x3706, 0x86);  //20160422 according to ov Fae from 0x87 to 0x86 for BLC performance
	write_cmos_sensor(0x370b, 0x7e); //20160422 according to ov Fae from 0x80to 0x7e for BLC performance//shenan,0x80->0x7e
	...
	write_cmos_sensor(0x59f8, 0x3d); ////20160422 according to ov Fae  for LENC h-band issue
	//;
...
int read_otp_hlt(struct otp_struct *otp_ptr)
{
	int otp_flag, addr, temp, i;
	//set 0x5001[3] to ???0??ид
	int temp1;
	int checksum2=0;
	...
	if(addr != 0) {
		...
		LOG_INF("liukun ov8855 module_integrator_id=%x\n",(*otp_ptr).module_integrator_id);
		LOG_INF("liukun ov8855 temp=%x\n",temp);
		LOG_INF("liukun ov8855 rg_ratio=%x\n",(*otp_ptr).rg_ratio);
		LOG_INF("liukun ov8855 bg_ratio=%x\n",(*otp_ptr).bg_ratio);
	}
	else {
...
int apply_otp_hlt(struct otp_struct *otp_ptr)
{
	int RG_Ratio_Typical = 0x144, BG_Ratio_Typical = 0x155;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			//yeshaobo@wind-mobi.com 20170803 begin
			if(*sensor_id==0x885A){
				*sensor_id=0x885B;
			}
			//yeshaobo@wind-mobi.com 20170803 end
			if (*sensor_id == imgsensor_info.sensor_id) {
				if((read_cmos_sensor(0x302A)) == 0XB0){
 				ov8856versionhlt = OV8856R1A;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8856versionhlt = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8856versionhlt);	
				return ERROR_NONE;
				}
				else if((read_cmos_sensor(0x302A)) == 0XB1){
 				ov8856versionhlt = OV8856R2A;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8856versionhlt = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8856versionhlt);	
				return ERROR_NONE;
				}
				//else{
			...
			LOG_INF("i2c_write_id = %d,Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
// zhaozhensen@wind-mobi.com 20161117 begin
		...
		read_otp_hlt(&otp_ptr);
		apply_otp_hlt(&otp_ptr);
	#endif
// zhaozhensen@wind-mobi.com 20161117 end
	spin_lock(&imgsensor_drv_lock);
...
UINT32 OV8856_MIPI_RAW_SensorInit_HLT(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
