/*************zhenglihong@wind-mobi.com 20170317 begin*******************/
/*****************************************************************************
 *
 * Filename:
 * ---------
...
/*===FEATURE SWITH===*/
#define FPTPDAFSUPPORT   //for pdaf switch  // zhaozhensen@wind-mobi.com 20170213
...
extern bool S5K3L8_gz_read_eeprom_pdaf( kal_uint16 addr, BYTE* data, kal_uint32 size);

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static unsigned int set_shutter_flag = 0;

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K3L8_SENSOR_ID_GZ,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =	 
{
 ...
 { 4208, 3120,	  0,  	0, 4208, 3120, 2104, 1560,   0,	0, 2104, 1560, 	 0, 0, 2104, 1560}, // custom1 //zhenglihong@wind-mobi.com 20170422 
...
*
*************************************************************************/
//zhenglihong@wind-mobi.com 20170320 begin 
static void set_shutter(unsigned long long shutter)
{
	...
	{
		int CintR = 0,Time_Farme = 0,temp = 0;
		switch(shutter)
		{
			case 97513: //1s
				CintR = 0x0c03;
				Time_Farme = 0x0cB2;
				LOG_INF("cfx:shutter= %lld,cintr = %x,Time_frame = %x\n",shutter,CintR,Time_Farme);
				break;
			case 195026: //2s
				CintR = 0x1806;
				Time_Farme = 0x1816;
			...
			default:
				LOG_INF("cfx:Error shutter\n");
				CintR = 0x0c03;
				Time_Farme = 0x0cB2;
				LOG_INF("cfx:shutter= %lld,cintr = %x,Time_frame = %x\n",shutter,CintR,Time_Farme);
				break;
		}
		LOG_INF("cfx:normal shutter to long shutter\n");
	
		
		write_cmos_sensor(0x6028,    0x4000);
                write_cmos_sensor(0x602A,    0x0100);
                write_cmos_sensor(0x6F12,    0x0000);  //Stream Off
                write_cmos_sensor(0x0306,    0x007D);  //125 main pll M
                write_cmos_sensor(0x0302,    0x0008); //vtpix 
                write_cmos_sensor(0x030E,    0x0080); //128 mipi pll M //???clock??
		...
		write_cmos_sensor(0x0200,0x00C6);  //fine
		LOG_INF("cfx: CintR&0xFFFF is : 0x%x\n",CintR&0xFFFF);
		write_cmos_sensor(0x0202,CintR&0xFFFF); //CintR
		write_cmos_sensor(0x0104,0x0000); //GP-OFF
	    temp = read_cmos_sensor(0x0202);
		LOG_INF("cfx:read regs 0x0202 : 0x%x\n",temp);
		...
		//$MIPI[Width:4208,Height:3120,Format:Raw10,Lane:4,ErrorCheck:0,PolarityData:0,PolarityClock:0,Buffer:4,DataRate:1124,useEmbData:0]
		write_cmos_sensor(0x0306,0x00B1);  //177 main pll M
		...
		LOG_INF("cfx:read regs 0x0202 : 0x%x\n",read_cmos_sensor(0x0202));
		write_cmos_sensor(0x0104,0x0100); //GP-ON
		
		/* 2016.03.01 */
		//write_cmos_sensor(0x0202,CintR&0xFFFF); //CintR   need FAE to ensure
...
//zhenglihong@wind-mobi.com 20170320 end 
...
static void hs_video_setting(void)
{
  ...
  write_cmos_sensor(0x0342, 0x16B0);
  #endif
  write_cmos_sensor(0x0340, 0x032C);
  write_cmos_sensor(0x0202, 0x0200);
  write_cmos_sensor(0x0200, 0x00C6);
  write_cmos_sensor(0x0B04, 0x0101);
  write_cmos_sensor(0x0B08, 0x0000);
...
static kal_uint32 return_sensor_id(void)
{
    ...
    kal_uint32  id = (read_cmos_sensor_byte(0x0000) << 8) | read_cmos_sensor_byte(0x0001);

     module_id =  S5K3L8_gz_read_eeprom_module_id();
    if (0x36 != module_id)
        LOG_INF("gz module error");
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			S5K3L8_gz_read_eeprom_pdaf((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;	
        /******************** PDAF END   <<< *********/
        default:
            break;
...
UINT32 S5K3L8_MIPI_RAW_SensorInit_Gz(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
...
/*************zhenglihong@wind-mobi.com 20170317 end*******************/
