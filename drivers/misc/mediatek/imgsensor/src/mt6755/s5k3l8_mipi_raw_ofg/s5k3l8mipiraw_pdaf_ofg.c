...
#ifdef S5K3L8_OFG_PDAFOTP_DEBUG
...
#define START_ADDR        0X0784
BYTE S5K3L8_ofg_eeprom_pdaf_data[DATA_SIZE]= {0};

/**************  CONFIG BY SENSOR <<< ************/

static kal_uint8 read_cmos_sensor_byte(kal_uint16 addr)
{
...
static bool _read_eeprom(kal_uint16 addr, kal_uint32 size ){
  ...
  int i=0;
  for(; i<size; i++){
    S5K3L8_ofg_eeprom_pdaf_data[i] = read_cmos_sensor_byte(addr+i);
    LOG_INF("add = 0x%x,\tvalue = 0x%x",i, S5K3L8_ofg_eeprom_pdaf_data[i]);
  }
  return true;
}

bool S5K3L8_ofg_read_eeprom_pdaf( kal_uint16 addr, BYTE* data, kal_uint32 size)
{
	addr = START_ADDR;
	size = DATA_SIZE;
	...
	memcpy(data, S5K3L8_ofg_eeprom_pdaf_data, size);
	return true;
}

kal_uint8 S5K3L8_ofg_read_eeprom_module_id(void )
{ 
          kal_uint8  module_id;

           kal_uint16 module_id_addr = 0x000F ;
           module_id =  read_cmos_sensor_byte(module_id_addr);
