...
// zhaozhensen@wind-mobi.com 20170217 begin
#define S5K3M2_EEPROM_READ_ID  0xA1
#define S5K3M2_EEPROM_WRITE_ID   0xA0
#define S5K3M2_I2C_SPEED        300  
// zhaozhensen@wind-mobi.com 20170217 end
...
static bool _read_3m2_eeprom(kal_uint16 addr, BYTE* data, kal_uint32 size ){
	...
	int offset = addr;
	for(i = 0; i < (size+5); i++) {
		if(i==0 || i==497 || i==498 || i==1305 || i==1306){
			printk("zzs escape offset = 0x%x, j = %d\n", offset, j);
			offset++;
			continue;
		}
		if(!selective_read_eeprom(offset, &data[j])){
			return false;
		}
