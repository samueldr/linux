...
static imgsensor_info_struct imgsensor_info = { 
	...
	.cap_delay_frame = 2,//modify for long explorse 2, 
	.pre_delay_frame = 2,  
	.video_delay_frame = 2,
...
//truly-j 1449
	.i4PosL = {{20,23},{72,23},{36,27},{56,27},{24,43},{68,43},{40,47},{52,47},{40,55},{52,55},{24,59},{68,59},{36,75},{56,75},{20,79},{72,79}},// zhaozhensen@wind-mobi.com 20170217
    .i4PosR = {{20,27},{72,27},{36,31},{56,31},{24,39},{68,39},{40,43},{52,43},{40,59},{52,59},{24,63},{68,63},{36,71},{56,71},{20,75},{72,75}},// zhaozhensen@wind-mobi.com 20170217
...
/*tatic kal_uint16 read_cmos_sensor_8(kal_uint16 addr)
{
    kal_uint16 get_byte=0;
    kdSetI2CSpeed(imgsensor_info.i2c_speed); // Add this func to set i2c speed by each sensor
    char pusendcmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
...
#define MAX_TIME1 9758757 // 0xffff * 0xfff0 / PCLK   us  (PCLK @ 440MHz)
#define MAX_TIME2 16105975  // 0xffff * 0xfff0 / PCLK us  (PCLK @ 266.6MHz)
...
static void write_shutter(unsigned long long shutter)
{
	...
	set_shutter_flag  = 1;
	LOG_INF("ciel_test_Enter! shutter =%lld\n",shutter);
		...
		if(shutter >= 0xFFF0 )   // need to modify line_length & PCLK
	{
		...
		unsigned long long line_length1;
		unsigned long long pclk1;

		
		exposure_time = shutter * imgsensor.line_length * 1000000/ imgsensor.pclk ;   // us
		shutter = 0xFFF0;       
	//	LOG_INF("ciel_test_long_pclk = %d, shutter = %d, imgsensor.line_length = %d, exposure_time = %d\n", imgsensor.pclk, shutter, imgsensor.line_length,exposure_time);
	
		if(exposure_time <= MAX_TIME1)  // only need to modify line_length
		{
			line_length1 = exposure_time  * imgsensor.pclk / shutter / 1000000  ;

			write_cmos_sensor_8(0x0104, 0x01);

			write_cmos_sensor(0x0342, (kal_uint16)(line_length1 & 0xFFFF));
			write_cmos_sensor(0x0340, 0x0C74);
			write_cmos_sensor(0x0202, 0x0B3A);


			write_cmos_sensor(0x303C,shutter);			
			write_cmos_sensor(0x3046,0xFFF4);
			write_cmos_sensor_8(0x3048, 0x01); 

			write_cmos_sensor_8(0x0104, 0x00);
			
		//	LOG_INF("ciel_test_shot_pclk = %d, shutter = %d, line_length = %d, exposure_time = %d\n", imgsensor.pclk, shutter, line_length1,exposure_time);
		}

		else if( (exposure_time > MAX_TIME1) && (exposure_time < MAX_TIME2))  // need to modify PCLK
		{
			pclk1 = 266600000;
			line_length1 = exposure_time * pclk1/ shutter/ 1000000;
		...
		//	LOG_INF("ciel_test_middle_pclk = %d, shutter = %d, line_length = %d, exposure_time = %d\n", pclk1, shutter, line_length1,exposure_time);			
		}

		else
		{
		//	LOG_INF("Exposure time is too long, set to the sensor max exposure time!\n exposure_time = %d\n",exposure_time);
			...
			write_cmos_sensor(0x0342, 0xFFFF);
			write_cmos_sensor(0x0340, 0x0C74);
			write_cmos_sensor(0x0202, 0x0B3A);
	...
	write_cmos_sensor_8(0x0104, 0x01);	
	write_cmos_sensor_8(0x3048, 0x00); 
	write_cmos_sensor_8(0x0104, 0x00);
