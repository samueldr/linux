...
BYTE s5k3P3_eeprom_data_sub[DATA_SIZE]= {0};
...
bool read_3P3_eeprom_sub( kal_uint16 addr, BYTE* data, kal_uint32 size){
//dingyisheng@wind-mobi.com 20170815 begin
	addr = 0x781;
//dingyisheng@wind-mobi.com 20170815 end
	size = 1404;
	//BYTE header[9]= {0};
	...
	if(!get_done || last_size != size || last_offset != addr) {
		if(!_read_3P3_eeprom(addr, s5k3P3_eeprom_data_sub, size)){
			get_done = 0;
            last_size = 0;
            last_offset = 0;
			return false;
	...
	memcpy(data, s5k3P3_eeprom_data_sub, size);
    return true;
}
