...
*
*************************************************************************/
static kal_uint16 Is_longExp = 0;
static void set_shutter(kal_uint32 shutter)
{
	unsigned long flags;
	//kal_uint16 Base_FrameLength1=0xBD2A;//0.5s
        kal_uint32 Base_FrameLength2=0x1A8EA;//1s
        kal_uint16 Multi=0;
	kal_uint16 LineShift = 0;
	kal_uint16 ShutterValue = 0;
	//unsigned long long Value = 560000000;
	spin_lock_irqsave(&imgsensor_drv_lock, flags);
	imgsensor.shutter = shutter;
	spin_unlock_irqrestore(&imgsensor_drv_lock, flags);
    printk("wjj shutter %d\n",shutter);
	...
	if(Is_longExp == 1){
 		Multi=shutter/Base_FrameLength2;
        	printk("S5K3P3.Multi=%d \n ", Multi);
		if(Multi < 4){
			if(Multi == 1){
				LineShift = 0;
				//ShutterValue = (Multi*Value - 512)/(1*5148);//1s D476
				ShutterValue=0x2170;
			}
			else{
				LineShift = 2;
				ShutterValue=0x10B8;
				//ShutterValue = (Multi*Value - 512)/(4*5148);//1s D476		
			}
		}
		else if(Multi == 4){
			LineShift = 2;
			//ShutterValue = (Multi*Value - 512)/(4*65523);//1s 5148 1 2s 5148 2
			//ShutterValue = (Multi*Value - 512)/(4*5148);
			ShutterValue=0x2170;
		}
		else if(Multi == 8){
			LineShift = 2;
		...
		printk("S5K3P3.Multi = %d,ShutterValue = 0x%x \n",Multi,ShutterValue);

				write_cmos_sensor(0x6028, 0x4000);
				write_cmos_sensor(0x602A, 0x0100);
				write_cmos_sensor(0x6F12, 0x0000);//Stream Off
				/*if(Multi == 1 || Multi == 2)
				write_cmos_sensor(0x0342, 0x141C);//line length pck//0x141c				
				else*/
				write_cmos_sensor(0x0342, 0xFFF3);//line length pck
				write_cmos_sensor(0x0216, 0x0000);//WDR off
				...
				//write_cmos_sensor(0x0216, 0x0100);//WDR LSRC separate

				write_cmos_sensor(0x305A, LineShift);//Linelength shifter
				//write_cmos_sensor(0x303E, ShutterValue);//vendor sensor coarse on 2nd frame
				write_cmos_sensor(0x0202, ShutterValue);//long coarse on 2nd frame

				//write_cmos_sensor(0x3046, 0x0020);//short analog gain on 2nd frame
				//write_cmos_sensor(0x3048, 0x0020);//long analog gain on 2nd frame

				write_cmos_sensor(0x0340, ShutterValue + 0x10);//timing frame lines 1st frame

				//write_cmos_sensor(0x304C, 0x0100);// b use 2nd frame_setting
				write_cmos_sensor(0x0100, 0x0100);//steam on
		Is_longExp = 2;
   	}else{
		if(Is_longExp == 2){
			printk("S5K3P3.Multi open 1 write_shutter\n");
			write_cmos_sensor(0x6028, 0x4000);
			write_cmos_sensor(0x0100, 0x0000);//Stream Off
			//write_cmos_sensor(0x0216, 0x0101);
			write_cmos_sensor(0x305A, 0x0000);
			write_cmos_sensor(0x0104, 0x0100); 
			write_cmos_sensor(0x0342, 0x1B94);//line length pck
			write_cmos_sensor(0x0340, 0x0E2A);//frame length lines
			write_cmos_sensor(0x0202, 0x0100);//coarse integration time
			write_cmos_sensor(0x0104, 0x0000); 
			//write_cmos_sensor(0x304C, 0x0000);//b use 2nd frame settings

			write_cmos_sensor(0x0100, 0x0100);
			Is_longExp = 0;
		}
		printk("S5K3P3.Multi open write_shutter\n");
