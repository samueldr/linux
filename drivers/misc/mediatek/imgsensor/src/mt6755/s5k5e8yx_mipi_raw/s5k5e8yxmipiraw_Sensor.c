...
//yeshaobo@wind-mobi.com 20170410 begin
...
static struct S5K5E8_otp_struct otp_date = {0};
#endif
//yeshaobo@wind-mobi.com 20170410 end
...
//zhenglihong@wind-mobi.com 20170422 change for binning mode
	...
	.i2c_addr_table = {0x20, 0x5a,0x6c,0xff},
};
...
{{ 2592, 1944,	  0,	0, 2592, 1944, 1296,  972, 0000, 0000, 1296,  972,	  0,	0, 1296,  972}, // Preview 
 ...
 { 2592, 1944,	  0,	0, 2592, 1944, 1296,  972, 0000, 0000, 1296,  972,	  0,	0, 1296,  972}, // custom1 
 }; 
//zhenglihong@wind-mobi.com 20170422 change for binning mode
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0x0B00,0x01);//shade_bypass LSC on
	//yeshaobo@wind-mobi.com 20170410 end
		// streaming ON
	write_cmos_sensor(0x3C0D,0x04);
	write_cmos_sensor(0x0100,0x01);
...
static int read_otp_s5k5e8(struct S5K5E8_otp_struct *otp_ptr)
{
	...
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x0A05; // group 1
		LOG_INF("zzs otp group 1");
	} else if((otp_flag & 0x30) == 0x10) {
		addr = 0x0A0A; // group 2
		LOG_INF("zzs otp group 2");
	} else {
		addr = 0;
		otp_ptr->flag = 0;
		LOG_INF("zzs otp invalid");
		return -1;
	}
	if(addr != 0 ) {
		otp_ptr->flag = 1;
		otp_ptr->ModuleID= read_cmos_sensor(addr);
		otp_ptr->LensID= read_cmos_sensor(addr+1);
		LOG_INF("zzs s5k5e8 read ModuleID = 0x%x, LensID = 0x%x\n", otp_ptr->ModuleID, otp_ptr->LensID);
	}
	otp_flag=read_cmos_sensor(0x0A0F);//flag of info and awb
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x0A10; // group 1
		LOG_INF("zzs otp group 1");
	} else if((otp_flag & 0x30) == 0x10) {
	...
	if(addr != 0 ) {
		...
		LOG_INF("zzs s5k5e8 read awb gain= 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",
...
//yeshaobo@wind-mobi.com 20170410 begin 
#ifdef S5K5E8_OTP_SUPPORT
	ret=read_otp_s5k5e8(&otp_date);
#endif
	sensor_init();
#ifdef S5K5E8_OTP_SUPPORT
	if(!ret){
		apply_otp_s5k5e8(&otp_date);
	}
#endif
//yeshaobo@wind-mobi.com 20170410 end
	spin_lock(&imgsensor_drv_lock);
