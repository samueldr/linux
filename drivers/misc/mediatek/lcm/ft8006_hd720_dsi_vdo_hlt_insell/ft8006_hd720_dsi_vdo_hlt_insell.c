...
extern struct platform_device * pltfm_dev ;
extern struct pinctrl *lcmbiasctrl;
extern struct pinctrl_state *lcmbias_enable;
extern struct pinctrl_state *lcmbias_disable;

static struct platform_device * pltfmtp_dev ;
static struct pinctrl *lcmtppinctrl;
static struct pinctrl_state *lcmtouchrst_2one;
static struct pinctrl_state *lcmtouchrst_2zero;
...
{
	pr_debug("[wjwind]lcmbias_probe begin!\n");
	pltfm_dev = dev;
	...
	pr_debug("[wjwind]lcmbias_probe done!\n");
	return 0;
}
...
static int lcmtppin_probe(struct platform_device *dev)
{
	pr_debug("[wjwind]lcmtppin_probe begin!\n");
	pltfmtp_dev = dev;
	
    lcmtppinctrl = devm_pinctrl_get(&pltfmtp_dev->dev);
	if (IS_ERR(lcmtppinctrl)) {
		dev_err(&pltfm_dev->dev, "Cannot find  lcmtppinctrl pinctrl!");
	}
    lcmtouchrst_2zero = pinctrl_lookup_state(lcmtppinctrl, "lcd_tp_rst_enp0_gpio");
	if (IS_ERR(lcmtouchrst_2zero)) {
		pr_debug("[wjwind]%s : pinctrl err, lcmtouchrst_2one\n", __func__);
	}

	lcmtouchrst_2one = pinctrl_lookup_state(lcmtppinctrl, "lcd_tp_rst_enp1_gpio");
	if (IS_ERR(lcmtouchrst_2one)) {
		pr_debug("[wjwind]%s : pinctrl err, lcmtouchrst_2zero\n", __func__);
	}
	pr_debug("[wjwind]lcmtppin_probe done!\n");
	return 0;
}

static int lcmtppin_remove(struct platform_device *dev)
{

	return 0;
}

static struct of_device_id lcmbias_of_match[] = {
	{ .compatible = "mediatek,lcmbias", },
	{},
};
static struct of_device_id lcmtppin_of_match[] = {
	{ .compatible = "mediatek,lcmtppin", },
	{},
};
...
static struct platform_driver lcmtppin_driver = {
	.probe = lcmtppin_probe,
	.remove = lcmtppin_remove,
	.driver = {
			.name = "lcmtppin_drv",
			.of_match_table = lcmtppin_of_match,
		   },
};
...
static int lcmbias_mod_init(void)
{
	...
	pr_debug("[wjwind]lcmbias_mod_init begin!\n");
	ret = platform_driver_register(&lcmbias_driver);
    if (ret)
		pr_debug("[wjwind]platform_driver_register(lcmbias_driver) error:(%d)\n", ret);
	else
		pr_debug("[wjwind]platform_driver_register(lcmbias_driver) done!\n");
    
    ret = platform_driver_register(&lcmtppin_driver);
	if (ret)
		pr_debug("[wjwind]platform_driver_register(lcmtppin_driver) error:(%d)\n", ret);
	else
		pr_debug("[wjwind]platform_driver_register(lcmtppin_driver) done!\n");

    pr_debug("[wjwind]lcmbias_mod_init done!\n");
...
{
	pr_debug("[wjwind]lcmbias_mod_exit\n");
	platform_driver_unregister(&lcmbias_driver);
	pr_debug("[wjwind]lcmbias_mod_exit Done!\n");
}
...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		params->dsi.vertical_sync_active				= 4;  //20;//100; 
		params->dsi.vertical_backporch				   = 12; //50;// 100; 
		params->dsi.vertical_frontporch				    = 16; //30; //100;
		params->dsi.vertical_active_line				= FRAME_HEIGHT;     
		params->dsi.horizontal_sync_active				= 16; // 10; ///4;
		params->dsi.horizontal_backporch				=48; // 60 ;//6; 
		params->dsi.horizontal_frontporch				= 16; //60;//78; 
...
//		params->dsi.CLK_TRAIL = 17;
	    ...
	    params->dsi.lcm_esd_check_table[0].para_list[0] = 0x9C;//0x80; //80
#if 0
...
{
	printk("[wjwangjun]ft8006 kernel  %s enter \n",__func__);
	...
	MDELAY(10);
    // TP_RST
    pinctrl_select_state(lcmtppinctrl, lcmtouchrst_2one);
    // LCM:RST
	SET_RESET_PIN(1);
	MDELAY(1);
	SET_RESET_PIN(0);
...
static void lcm_suspend(void) 
{
	...
	MDELAY(10);
    pinctrl_select_state(lcmtppinctrl, lcmtouchrst_2zero);
    MDELAY(10);
	printk("[wjwangjun] %s exit!\n",__func__);
...
static struct LCM_setting_table lcm_switch_chr_2[] = {
    {0x50, 1, {0x5A}},
    {REGFLAG_DELAY, 20, {}},
    {0x51, 1, {0x02}},
    {REGFLAG_DELAY, 20, {}},
...
static unsigned int lcm_compare_id(void)
{
	...
	push_table(lcm_switch_chr_2, sizeof(lcm_switch_chr_2) / sizeof(struct LCM_setting_table), 1); 

	read_reg_v2(0x80, buffer, 1);   //
	id1= buffer[0];
	...
	printf(" LK ft8006 yassy  debug: %s id=%x id1=%x id2=%x \n",__func__,id, id1, id2);
#else
	printk(" kernel ft8006 yassy horse debug: %s id=%x  \n",__func__,id);
#endif	
...
LCM_DRIVER ft8006_hd720_dsi_vdo_hlt_insell_lcm_drv =
{
	.name           	= "ft8006_hd720_dsi_vdo_hlt_insell",
