...
#define LED_PROC_FOLDER "led_orange"
#define LED_PROC_DATA_FILE1 "value"


static struct proc_dir_entry *led_proc_dir = NULL;
static struct proc_dir_entry *proc_data_value = NULL;
...
static ssize_t data_write1(struct file *file, const char *buf, size_t len, loff_t *pos)
{
	...
	x = data_read_enable;
	printk("wind-liukangpingdata_write1  x=%d\n",x);
	//size = sizeof(buf);
	return len;
...
static int led_proc_init(void)
{
	led_proc_dir = proc_mkdir(LED_PROC_FOLDER, NULL);
	if (led_proc_dir == NULL)
	{
		printk(" %s: led_proc_dir file create failed!\n", __func__);
		return -ENOMEM;
	}
/////////////////////add data 1
	proc_data_value = proc_create(LED_PROC_DATA_FILE1, (S_IWUSR|S_IRUGO|S_IWUGO), 
		led_proc_dir, &proc_data_file1_ops);
	if(proc_data_value == NULL)
	{
		printk(" %s: proc data file1 create failed!\n", __func__);
		remove_proc_entry( LED_PROC_DATA_FILE1, led_proc_dir );
		return -ENOMEM;
	}
...
static void led_proc_remove(void)
{
