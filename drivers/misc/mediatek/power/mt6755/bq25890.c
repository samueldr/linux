...
extern signed int bq25890_chr_control_interface(CHARGING_CTRL_CMD cmd, void *data);
unsigned char bq25890_dataBuffer;
int ret4 = 0;
#endif
/*liulinsheng@wind-mobi.com add for XXX 2017-09-24 end*/
static int bq25890_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
...
#ifdef CONFIG_WIND_DEF_PRO_E262L
	bq25890_dump_register();
	ret4 = bq25890_read_byte(0x14,&bq25890_dataBuffer);
	printk("[chengronghe]MTK_BQ25890_SUPPORT ret4= %d bq25890_dataBuffer = 0x%x\n",ret4,bq25890_dataBuffer);
	bq25890_dataBuffer = bq25890_dataBuffer & 0x38;
if(bq25890_dataBuffer == 0){
	battery_log(BAT_LOG_CRTI, "%s:[chengronghe]	BQ25890 exists\n", __func__);
	bq25890_dump_register();
	/* bq25890_hw_init(); //move to charging_hw_xxx.c */
	chargin_hw_init_done = true;

	/* Hook chr_control_interface with battery's interface */
	battery_charging_control = bq25890_chr_control_interface;
	printk("[chengronghe] set bq25890_chr_control_interface\n");
	battery_log(BAT_LOG_CRTI, "%s:[chengronghe]	bq25890_driver_probe suc\n", __func__);
	bq25896_exist_flag = 1;/* chengronghe@wind-mobi.com add  2017-07-07 */
}
else
	{
		battery_log(BAT_LOG_CRTI, "%s:[chengronghe] NO BQ25890 exists\n", __func__);
		return -1;
	}
#else
	/* --------------------- */
