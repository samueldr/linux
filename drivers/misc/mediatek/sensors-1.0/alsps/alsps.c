...
static int ps_enable_and_batch(void)
{
	...
	if (cxt->ps_power == 0 && cxt->ps_enable == 1) {
		printk("PS fengqian ps_power on\n");
...
printk("fengqian ps_store_active buf=%s\n", buf);
	mutex_lock(&alsps_context_obj->alsps_op_mutex);
...
err_out:
	mutex_unlock(&alsps_context_obj->alsps_op_mutex);
	printk("fengqian  ps_store_active done\n");
	return err;
}
/*----------------------------------------------------------------------------*/
				  ...
				  const char *buf, size_t count)
{
	...
	printk("fengqian ps_store_batch %s\n", buf);
	err = sscanf(buf, "%d,%d,%lld,%lld", &handle, &flag,
...
#endif
	mutex_unlock(&alsps_context_obj->alsps_op_mutex);
	printk("fengqian ps_store_batch done: %d\n", cxt->is_ps_batch_enable);
	return err;
}
	...
	for (i = 0; i < MAX_CHOOSE_ALSPS_NUM; i++) {
		...
		if (alsps_init_list[i] != 0) {
			...
			if (err == 0) {
				//ranyanhao@wind-mobi.com 20160325 begin
				#ifdef CONFIG_WIND_DEVICE_INFO         	
				wind_device_info.sensor_info_name.g_alsensor_name = alsps_init_list[i]->name;	
				#endif	
				//ranyanhao@wind-mobi.com 20160325 end
