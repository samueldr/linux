...
//yangzhigang@wind-mobi.com end	
	if(hwmsen_read_byte(client, LSM6DS3_CTRL2_G, databuf))
	{
		GYRO_PR_ERR("read LSM6DS3_CTRL2_G err!\n");
		return LSM6DS3_ERR_I2C;
	}
	else
	{
...
static int lsm6ds3_gyro_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	GYRO_LOG("mpu6515 gyro set delay = (%d) ok.\n", value);
...
/*lenovo-sw caoyi1 modify A+G sensro iic address 20150318 end*/
	...
	if(err)
	{
		GYRO_PR_ERR("lsm6ds3_gyro_device misc register failed!\n");
	...
	if(err)
	{
		GYRO_PR_ERR("lsm6ds3 create attribute err = %d\n", err);
	...
	if(err)
	{
		GYRO_PR_ERR("hwmsen_attach Gyroscope fail = %d\n", err);
		goto exit_kfree;
	}
#else
    ...
    if(err)
    {
         GYRO_PR_ERR("register acc control path err\n");
        goto exit_kfree;
    }
    ...
    if(err)
    {
        GYRO_PR_ERR("register acc data path err= %d\n", err);
        goto exit_kfree;
    }
#endif
...
static int lsm6ds3_gyro_i2c_remove(struct i2c_client *client)
{
	...
	if(err)
	{
		GYRO_PR_ERR("misc_deregister lsm6ds3_gyro_device fail: %d\n", err);
	}
	#endif
...
static int qiku_gyro_calibration(gyro_offset* qiku_gyro_offset)
{
	...
	printk("[kevin]--qiku_acc_calibration--qiku_gyro_offset.x=%d,qiku_gyro_offset.y=%d,qiku_gyro_offset.z=%d\n",qiku_gyro_offset->x,qiku_gyro_offset->y,qiku_gyro_offset->z);
		...
		while(count <20)
		{
			...
			if(hwmsen_read_block(client, LSM6DS3_OUTX_L_G, databuf, 6))
			{
				GYRO_PR_ERR("LSM6DS3 read gyroscope data  error\n");
				return -2;
			}
			else
			{
...
static int __init lsm6ds3_gyro_init(void)
{
	...
	{
		GYRO_PR_ERR("failed to register driver");
		return -ENODEV;
	}
#else
