				   ...
				   const char *buf, size_t count)
{
	...
	printk(" qiangang step_store_active handle=%d ,en=%d\n", handle, en);
	switch (handle) {
...
static int step_counter_probe(struct platform_device *pdev)
{
	...
	printk("qiangang at step_counter222\n");
	return 0;
}
...
int step_c_driver_add(struct step_c_init_info *obj)
{
	int err = 0;
	int i = 0;
    printk("qiangang at step_counter111\n");
	STEP_C_FUN();
	for (i = 0; i < MAX_CHOOSE_STEP_C_NUM; i++) {
		if (i == 0) {
...
int step_c_register_data_path(struct step_c_data_path *data)
{
	...
	if (NULL == cxt->step_c_data.get_data
		...
		printk("step_c register data path fail\n");
		return -1;
	}
	printk("step_c register data path success\n");
	return 0;
}
...
int step_c_register_control_path(struct step_c_control_path *ctl)
{
	...
	if (NULL == cxt->step_c_ctl.step_c_set_delay || NULL == cxt->step_c_ctl.open_report_data
		...
		printk("step_c register control path fail\n");
		return -1;
	}
	printk("step_c register control path ID_STEP_COUNTER%d\n",ID_STEP_COUNTER);
	printk("step_c register control path ID_STEP_DETECTOR=%d\n",ID_STEP_DETECTOR);
	printk("step_c register control path ID_SIGNIFICANT_MOTION=%d\n",ID_SIGNIFICANT_MOTION);
