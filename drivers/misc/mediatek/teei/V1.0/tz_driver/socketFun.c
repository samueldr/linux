...
int tz_socket(int family, int type, int protocol, unsigned long para_address, unsigned long buffer_addr)
{
	...
	kfree(sock_para);
	printk("socket function return value = %d\n", retval);
	...
	/* printk("---tz_socket family = %d, type = %d, protocol = %d\n", family, type, protocol); */
...
int tz_connect(int fd, struct sockaddr *address, int addrlen, unsigned long para_address, unsigned long buffer_addr)
{
	...
	printk("connect function return value = %d\n", retval);
	kfree(connect_para);
	return retval;
...
int tz_send(int fd, void *buff, size_t len, unsigned int flags, unsigned long para_address, unsigned long buffer_addr)
{
	...
	printk("send function return value = %d\n", retval);
	kfree(send_para);
			...
			unsigned flags, unsigned long para_address, unsigned long buffer_addr)
{
	...
	printk("recv function return value = %d\n", retval);
...
asmlinkage long tz_close(int fd, unsigned long para_address, unsigned long buffer_addr)
{
	...
	printk("close function return value = %d\n", retval);
	kfree(close_para);
	return retval;
...
int socket_thread_function(unsigned long virt_addr, unsigned long para_vaddr, unsigned long buff_vaddr)
{
	...
	*/
	...
	/* printk(" come into the socket_thread_function, %x in %p\n", tz_command->func, &(tz_command->func)); */
	switch (tz_command->func) {
	case TZ_SOCKET:
		/* printk(" come into the SOCKET branch\n"); */
	...
	case TZ_HTONS:
		/* printk(" come into the TZ_HTONS branch\n"); */
	...
	case TZ_INET_ADDR:
		/* printk(" come into the TZ_INET_ADDR branch\n"); */
	...
	case TZ_CONNECT:
		/* printk(" come into the TZ_CONNECT branch\n"); */
	...
	case TZ_BIND:
		/* printk(" come into the TZ_BIND branch\n"); */
	...
	case TZ_LISTEN:
		/* printk(" come into the TZ_LISTEN branch\n"); */
	...
	case TZ_ACCEPT:
		/* printk(" come into the TZ_ACCEPT branch\n"); */
	...
	case TZ_RECV:
		/* printk(" come into the TZ_RECV branch\n"); */
		sendRecv_address = 0;
	...
	return 0;
#endif
	printk("In the socket_thread_function function");
	return 0;
}
