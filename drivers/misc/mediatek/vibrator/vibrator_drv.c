...
/*zhangkaiyuan@wind-mobi.com 20170213 begin*/
#ifdef CONFIG_HAS_VIBRATION_INTENSITY
#include <mt-plat/upmu_common.h>
#endif
/*zhangkaiyuan@wind-mobi.com 20170213 end*/
...
/*zhangkaiyuan@wind-mobi.com 20170213 begin*/
#ifdef CONFIG_HAS_VIBRATION_INTENSITY
static void vib_shut_down(void)
{
	unsigned long flags;
...
static void vib_start_on(unsigned int level)
{
	unsigned long flags;
	...
	if (!vibe_state) {
		...
		pmic_set_register_value(PMIC_RG_VIBR_VOSEL, level);
    #else
	    pmic_set_register_value(MT6351_PMIC_RG_VIBR_VOSEL, level);
    #endif
		vibr_Enable();
	} else {
		spin_unlock_irqrestore(&vibe_lock, flags);
...
/*zhangkaiyuan@wind-mobi.com 20170213 begin*/
#ifdef CONFIG_HAS_VIBRATION_INTENSITY
static ssize_t store_vibr_level(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t size)
{
   // printk("wind-vibr:into store_vibr_level");
	if (buf != NULL && size != 0) {
			/* VIB_DEBUG("buf is %s and size is %d\n", buf, size); */
			if (buf[0] == '0')
                   vib_shut_down();
...
static DEVICE_ATTR(vibr_level, 0664, NULL, store_vibr_level);
#endif
/*zhangkaiyuan@wind-mobi.com 20170213 end*/
...
static int vib_mod_init(void)
{
	...
	/*zhangkaiyuan@wind-mobi.com 20170213 begin*/
	#ifdef CONFIG_HAS_VIBRATION_INTENSITY
	ret = device_create_file(mtk_vibrator.dev, &dev_attr_vibr_level);
	if (ret)
		VIB_DEBUG("device_create_file vibr_level fail!\n");
	#endif
	/*zhangkaiyuan@wind-mobi.com 20170213 end*/
