	...
	unsigned char *para_list, unsigned int count)
{
	...
	if (packet_type == 0x1A || packet_type == 0x1C) {
		...
		//modify by qiangang 20170912 begin
		if(index == 2){
 			//DISPDBG("buffer[%d]=0x%x\n", i, buffer[i]);
			//printk("sss qiangang buffer[0]=0x%x,buffer[1]=0x%x\n", buffer[0], buffer[1]);
 			if ( (buffer[0] == 0x05 && buffer[1] == 0xb9) || (buffer[0] == dsi_params->lcm_esd_check_table[index].para_list[0] && buffer[1] == dsi_params->lcm_esd_check_table[index].para_list[1])) {
 				ret = 0;
		...
		//modify by qiangang 20170912 end
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			...
			t0.Data_ID =
			    (t0.Data0 <
			     0xFF) ? DSI_DCS_READ_PACKET_ID : DSI_GERNERIC_READ_LONG_PACKET_ID;        // wangbing@wind-mobi.com 20171019
			t0.Data1 = 0;
