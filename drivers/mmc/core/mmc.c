...
//liukangping add emmc health 20170208 end

// wangjun@wind-mobi.com 20170818 begin 
#if defined(CONFIG_ASUS_EMMC_NODE)
...
// lihaiyan@wind-mobi.com begin
#if defined(CONFIG_ASUS_GET_EMMC_TOTAL_SIZE)

/*************Used for definition of Flash***********************/
#define     H9TQ26ADFTACUR_KUM                  "90014a484247346132"
#define     MT29TZZZ7D7EKKBT_107W_97V           "13014e51334a393756"
#define     H9TQ52ACLTMCUR_KUM                  "90014a484347386134"
#define     KMRC10014M_B809                     "150100524331344d42"
#define     KMRX1000BM_B614                     "150100525831424d42"
#define     KMRC1000BM_B809                     "150100524331424d42"
#define     KMQX10013M_B419                     "150100515831334d42"
#define     KMGX6001BM_B514                     "150100475836424d42"
         
#define     KMQE10013M_B318                     "150100514531334d42"
#define     H9TQ17ABJTACUR_KUM	                "90014d484147346132"
#define     H9TQ17ABJTBCUR_KUM                  "484a01906134474129"
#define     MT29TZZZ5D6EKFRL_107W_96R           "13014e51334a393652"
#define     H9TQ26ADFTBCUR_KUM		          	"90014a484247346132"  // 32 + 3
#define     H9TQ52ADFTMCUR_KUM                  "90014a484347386134"
...
eMMCinfo Cidinfo[] = {
		...
		{MT29TZZZ7D7EKKBT_107W_97V,"32","3"},
		{H9TQ52ACLTMCUR_KUM,"64","4"},
		{KMRC10014M_B809,"64","4"},
		{KMRX1000BM_B614,"32","3"},
		{KMQE10013M_B318,"16","2"},
		{KMRC1000BM_B809,"64","3"},
		{KMQX10013M_B419,"32","2"},
		{KMGX6001BM_B514,"32","3"}
	#else
		{H9TQ17ABJTBCUR_KUM,"16","2"},
		{MT29TZZZ5D6EKFRL_107W_96R,"16","3"},
		{H9TQ26ADFTBCUR_KUM,"32","3"},
		{H9TQ52ADFTMCUR_KUM,"64","3"},
	#endif
}; 
#endif

// lihaiyan@wind-mobi.com end
...
static int emmc_health_proc_init(void)
{
	...
	{
		printk(" %s: emmc_proc_dir file create failed!\n", __func__);
		return -ENOMEM;
	}
...
// wangjun@wind-mobi.com 20170818 begin 
#if defined(CONFIG_ASUS_EMMC_NODE)
static void _mmc_asusevent_log(struct  mmc_card *card)
{
	char* vendor;
	char* emmc_version;

	switch (card->cid.manfid){
		case 0x45 : vendor = "SANDISK"; break;
		case 0x11 : vendor = "TOSHIBA"; break;
		case 0x13 : vendor = "MICRON"; break;
		case 0x15 : vendor = "SAMSUNG"; break;
		case 0x90 : vendor = "HYNIX"; break;
		default: vendor = "Unknow_vendor"; break;
	}

	switch (card->ext_csd.rev){
		case 0x0 : emmc_version = "v4.0"; break;
		case 0x1 : emmc_version = "v4.1"; break;
		case 0x2 : emmc_version = "v4.2"; break;
		case 0x3 : emmc_version = "v4.3"; break;
		case 0x5 : emmc_version = "v4.41"; break;
		case 0x6 : emmc_version = "v4.5"; break;
		case 0x7 : emmc_version = "v5.0"; break;
		case 0x8 : emmc_version = "v5.1"; break;
		default: emmc_version= "Unknow_emmc_version"; break;
	}

	printk("%s:[EMMC_STATUS] vendor=%s, emmc_version=%s, emmc_size=%dG, fw_version=0x%lx, lifeA=0x%02x, lifeB=0x%02x, preEOL=0x%02x\n",
		mmc_hostname(card->host), vendor, emmc_version, card->mmc_total_size, card->ext_csd.fw_version,
		card->ext_csd.raw_device_life_time_A, card->ext_csd.raw_device_life_time_B,
		card->ext_csd.raw_device_PRE_EOL);

#if 0
	ASUSEvtlog("%s:[EMMC_STATUS] vendor=%s, emmc_version=%s, emmc_size=%dG, fw_version=0x%lx, lifeA=0x%02x, lifeB=0x%02x, preEOL=0x%02x\n",
		mmc_hostname(card->host), vendor, emmc_version, card->mmc_total_size, card->ext_csd.fw_version,
		card->ext_csd.raw_device_life_time_A, card->ext_csd.raw_device_life_time_B,
		card->ext_csd.raw_device_PRE_EOL);
#endif
...
static void mmc_asusevent_log(struct mmc_card *card, u8 *asus_ext_csd)
{

	if(asus_ext_csd == NULL) {
		pr_info("%s [EMMC_STATUS] mmc_asusevent_log NULL ext_csd\n", mmc_hostname(card->host));
	} else {
		card->ext_csd.raw_device_PRE_EOL = asus_ext_csd[267];
		card->ext_csd.raw_device_life_time_A = asus_ext_csd[268];
		card->ext_csd.raw_device_life_time_B = asus_ext_csd[269];
	}

	if(pre_life_time_A != card->ext_csd.raw_device_life_time_A ||
		pre_life_time_B != card->ext_csd.raw_device_life_time_B ||
		pre_Pre_EOL != card->ext_csd.raw_device_PRE_EOL)
		_mmc_asusevent_log(card);

	pre_Pre_EOL     = card->ext_csd.raw_device_PRE_EOL;
	pre_life_time_A = card->ext_csd.raw_device_life_time_A;
	pre_life_time_B = card->ext_csd.raw_device_life_time_B;
...
static int emmc_health_proc_read(struct seq_file *buf, void *v)
{
	...
	if (pre_life_time_B == 0x0 || pre_life_time_B > 0xB)
		return seq_printf(buf, "Not support!! 0x%02x", pre_life_time_B);

	tmp = (pre_life_time_B - 1)*10;
...
void create_emmc_health_proc_file(void)
{
	...
	struct proc_dir_entry *proc_file =
			proc_create("emmc_health", 0444, NULL, &proc_fops);

	if (!proc_file)
		pr_err("[eMMC]%s failed!\n", __func__);
...
// wangjun@wind-mobi.com 20170818 begin 
#if defined(CONFIG_ASUS_EMMC_NODE)
void Sandisk_transfer_Fw_version(struct mmc_card *card, u8 *ext_csd)
{
	int i=0;
	card->ext_csd.fw_version = 0;
	for (i=0; i<6; i++){
		card->ext_csd.fw_version = card->ext_csd.fw_version | (unsigned long)(ext_csd[EXT_CSD_FIRMWAARE_VERSION+i]-48)<< (i*4);
	}
}
#endif
// wangjun@wind-mobi.com 20170818 end
...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	printk("liukangping emmc ext_csd[EXT_CSD_STRUCTURE+73] = %d\n",ext_csd[EXT_CSD_STRUCTURE+73]); 
	//EXT_CSD_STRUCTURE+73=194+73=267 byte emmc health  liukangping
	//liukangping add emmc health 20170208 end
    ...
    // wangjun@wind-mobi.com 20170818 begin 
#if defined(CONFIG_ASUS_EMMC_NODE)
    card->ext_csd.raw_device_PRE_EOL = ext_csd[267];
    ...
    if (card->cid.manfid == 0x45)
		Sandisk_transfer_Fw_version(card,ext_csd);
	else
		card->ext_csd.fw_version =
			ext_csd[EXT_CSD_FIRMWAARE_VERSION + 0] << 0 |
			ext_csd[EXT_CSD_FIRMWAARE_VERSION + 1] << 8 |
			ext_csd[EXT_CSD_FIRMWAARE_VERSION + 2] << 16 |
			ext_csd[EXT_CSD_FIRMWAARE_VERSION + 3] << 24 |
			(unsigned long)ext_csd[EXT_CSD_FIRMWAARE_VERSION + 4] << 32 |
			(unsigned long)ext_csd[EXT_CSD_FIRMWAARE_VERSION + 5] << 40 |
			(unsigned long)ext_csd[EXT_CSD_FIRMWAARE_VERSION + 6] << 48 |
			(unsigned long)ext_csd[EXT_CSD_FIRMWAARE_VERSION + 7] << 56;
#endif
    // wangjun@wind-mobi.com 20170818 end
...
static int mmc_compare_ext_csds(struct mmc_card *card, unsigned bus_width)
{
    ...
    // wangjun@wind-mobi.com 20170818 begin 
#if defined(CONFIG_ASUS_EMMC_NODE)
    mmc_asusevent_log(card, bw_ext_csd);
#endif
    // wangjun@wind-mobi.com 20170818 end
...
/* lihaiyan@wind-mobi.com 20170531 begin */
#if defined(CONFIG_ASUS_GET_EMMC_TOTAL_SIZE)
static char* asus_get_emmc_total_size(struct mmc_card *card)
{
	char cid_tmp[40],cid_tmp_1[19]; 
	short i = 0, flash_num_sum=0;
	BUG_ON(!card);
	// 1. get emmc CID
	sprintf(cid_tmp,"%08x%08x%08x%08x\n", card->raw_cid[0], card->raw_cid[1], card->raw_cid[2], card->raw_cid[3]);
	sprintf(card->mmc_total_size,"32");   //Cidinfo[i].RamCapacity;
	
	for(i = 0;i < 18;i++){
		cid_tmp_1[i] = cid_tmp[i];
    }
	cid_tmp_1[18]='\0';
	flash_num_sum = (short)(sizeof(Cidinfo)/sizeof(Cidinfo[0]));
	printk("[wind_emmc]emmc cid_tmp= %s, cid_tmp_1=%s, flash_num_sum=%d\n", cid_tmp, cid_tmp_1, flash_num_sum); 
	
	// 2. start cmp str
	for(i = 0; i < flash_num_sum ; i++){   
		printk("[wind_emmc]emmc start cmp times = %d/%d, cid_tmp_1=%s, Cidinfo[%d].eMMCcid=%s\n", i, flash_num_sum , cid_tmp_1, i, Cidinfo[i].eMMCcid); 
		if( 0 == ( strcasecmp(cid_tmp_1,Cidinfo[i].eMMCcid) ) )   //strstr(cid_tmp, Cidinfo[i].eMMCcid) 
		{
			sprintf(card->mmc_total_size,"%s\n",Cidinfo[i].RomCapacity);   //Cidinfo[i].RamCapacity;
			printk("[wind_emmc]finded emmc cid = %08x%08x %08x%08x, mmc_total_size = %s\n", card->raw_cid[0], card->raw_cid[1],
					card->raw_cid[2], card->raw_cid[3],card->mmc_total_size);
			i = 100;
			break;
		}
	}
	if(i != 100){
		printk("[wind_emmc][ERROR]emmc cid_tmp=%s not find, set default size = 32 \n",cid_tmp_1); 
		sprintf(card->mmc_total_size,"32");   //Cidinfo[i].RamCapacity;
	}

	return card->mmc_total_size;
}
//path:  /sys/class/block/mmcblk0/device/emmc_total_size

MMC_DEV_ATTR(emmc_total_size, "%s\n", asus_get_emmc_total_size(card));

#endif
/* lihaiyan@wind-mobi.com 20170531 end */
...
MMC_DEV_ATTR(emmc_health, "%02x-%02x-%02x\n", card->ext_csd.raw_device_PRE_EOL,
		card->ext_csd.raw_device_life_time_A, card->ext_csd.raw_device_life_time_B);
