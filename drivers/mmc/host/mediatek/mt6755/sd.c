...
{
	printk("ranyanhao sdcard removed and power off VMCH first!\n");
	pmic_config_interface_nolock(0xA80, 0, 1, 1); //disable VMCH LDO
}
...
static void msdc_set_power_mode(struct msdc_host *host, u8 mode)
{
	...
	if (host->power_mode == MMC_POWER_OFF && mode != MMC_POWER_OFF) {
		...
		mdelay(300);   //lihaiyan@wind-mobi.com 20170327 
		...
		if (is_card_sdio(host) || (host->hw->flags & MSDC_SDIO_IRQ)) {
			...
			// wangbing@wind-mobi.com BEGIN 20180201
			if (host->hw->host_function == MSDC_SD) {
			...
			// wangbing@wind-mobi.com END 20180201
...
static void msdc_add_host(struct work_struct *work)
{
    ...
    // wangjun@wind-mobi.com 20170818 begin 
#if defined(CONFIG_ASUS_EMMC_NODE)
    if (!strcmp("mmc0",mmc_hostname(mmc))) {
		printk("create_emmc_health_proc_file\n");
