...
#include "tcpm.h"
#endif
//liulinsheng 20170412 begin
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
#include <linux/switch.h>
#endif
//liulinsheng 20170412 end
...
//lvwenkang@wind-mobi.com 20161121 begin
#ifdef CONFIG_WIND_DEVICE_INFO
#include <wind_device_info.h>
extern wind_device_info_t wind_device_info;
#endif
//lvwenkang@wind-mobi.com 20161121 end
//yutao@wind-mobi.com add begin
#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
static struct platform_device * platfm_dev ;
struct pinctrl *vdmctrl = NULL;
struct pinctrl_state *vdm_default= NULL;
struct pinctrl_state *vdm_high= NULL;
struct pinctrl_state *vdm_low= NULL;
extern int IMM_GetOneChannelValue_Cali(int Channel, int *voltage);
extern struct pinctrl_state *usbsw_high;
extern struct pinctrl_state *usbsw_low;
extern struct pinctrl *usbswctrl;
bool  reset_charging_current=false;
static int in_times_one=0;
static int in_times_two=0;
extern int usbsw_flag; //zhangkaiyuan@wind-mobi.com 20170609
#endif
//yutao@wind-mobi.com add end

//renyongwei@wind-mobi.com 20170923 begin
extern int force_get_tbat(kal_bool update);
extern int fgauge_get_Q_max(signed short temperature);
//renyongwei@wind-mobi.com 20170923 end
...
//liulinsheng@wind-mobi.com 20170924 begin 
...
#else
CHARGING_CONTROL battery_charging_control = chr_control_interface;  //modifiede by hebiao at 20161212
#endif
//liulinsheng@wind-mobi.com 20170924 end
...
//modified by lvwenkang 20161018 begin
...
int demoapp_flag = 0;
#endif
 unsigned int usb_temp_reach = 0;
//modified by lvwenkang 20161018 end
...
//liulinsheng@wind-mobicom 20170302
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
typedef struct {
	__s32 BTS_Temp;
	__s32 TemperatureR;
...
struct battery_data {
	...
	int BAT_batt_vol;
	int BAT_batt_temp;
	int BAT_charge_count;    //renyongwei@wind-mobi.com 20170923 add
	int BAT_bat_current;    //liulinsheng@wind-mobi.com 20171002 add
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
	int BAT_usb_ntc;     //liulinsheng@wind-mobi.com 20170309
#endif
	//liulinsheng@wind-mobi.com  for ap temp check
	#ifdef CONFIG_AUAS_AP_TEMP	
	int BAT_ap_thermal_temp;
	#endif
	//liulinsheng@wind-mobi.com  for ap temp check
	/* Add for EM */
...
static enum power_supply_property battery_props[] = {
	...
	//liulinsheng@wind-mobi.com  for ap temp check	
	#ifdef CONFIG_AUAS_AP_TEMP	
	POWER_SUPPLY_PROP_ap_thermal_temp,
	#endif
	//liulinsheng@wind-mobi.com  for ap temp check
	/* Add for EM */
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_batt_temp:
		...
		break;
	//renyongwei@wind-mobi.com 20170923 begin
	case POWER_SUPPLY_PROP_charge_count:
		val->intval = data->BAT_charge_count;
		break;
	//renyongwei@wind-mobi.com 20170923 end
	//liulinsheng@wind-mobi.com 20170923 begin
	case POWER_SUPPLY_PROP_bat_current:
		val->intval = data->BAT_bat_current;
		break;
	//liulinsheng@wind-mobi.com 20170923 end
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
	case POWER_SUPPLY_PROP_usb_ntc:
		val->intval = data->BAT_usb_ntc;       //liulinsheng@wind-mobi.com 20170309
		break;
#endif
	//liulinsheng@wind-mobi.com  for ap temp check
	#ifdef CONFIG_AUAS_AP_TEMP	
	case POWER_SUPPLY_PROP_ap_thermal_temp:
		val->intval = data->BAT_ap_thermal_temp;
		break;	
	#endif
	//liulinsheng@wind-mobi.com  for ap temp check
...
/* CC: modify to have a full power supply status */
	...
	.BAT_batt_vol = 4200,
	.BAT_batt_temp = 22,
	.BAT_charge_count = 0,         //renyongwei@wind-mobi.com 20170923 add
	.BAT_bat_current = 0,         //liulinsheng@wind-mobi.com 20171002 add
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
	.BAT_usb_ntc = 25,         //liulinsheng@wind-mobi.com 20170309
#endif
	//liulinsheng@wind-mobi.com  for ap temp check
	#ifdef CONFIG_AUAS_AP_TEMP	
	.BAT_ap_thermal_temp = 0,
	#endif
	//liulinsheng@wind-mobi.com  for ap temp check
	/* Dual battery */
...
//modified by ranyanhao 20161130 begin
#if defined(AGING_POWER_TEST)
...
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
#define AGING_POWER_TEST_PROC_CHARGING_USB_NTC "Charging_usb_ntc"   //liulinsheng@wind-mobicom 20170302
#endif
...
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
static struct proc_dir_entry *proc_Charging_usb_ntc_file = NULL;   //liulinsheng@wind-mobicom 20170302
#endif
...
static ssize_t Charging_usb_ntc_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	char *ptr = buf;
	int temp;
	unsigned int g_charger_usb_ntc = 0; 
	if (*pos) {
		return 0;
	}
	IMM_GetOneChannelValue_Cali(13, &g_charger_usb_ntc);
	battery_log(BAT_LOG_FULL, "step  : liulinsheng@wind-mobicom 20170302 Charging_usb_ntc_read=%d!!!!!!!!!\r\n",g_charger_usb_ntc);
	g_charger_usb_ntc=g_charger_usb_ntc/1000;
	temp = mtk_ts_bts_volt_to_temp(g_charger_usb_ntc);
	//battery_log(BAT_LOG_CRTI, "Charging_usb_ntc_read = %d\n", Charging_usb_ntc_read);
	ptr += sprintf(ptr,"%u\n", temp);
	*pos += ptr - buf;
	return (ptr -buf);
}
static ssize_t Charging_usb_ntc_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
   /*unsigned int temp = 0; 
   sscanf(buff, "%u", &temp);
   if(temp == 0 || temp == 1)
...
//liulinsheng@wind-mobicom 20170302
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
static struct file_operations proc_Charging_usb_ntc_file_ops =
{
	.owner = THIS_MODULE,
	.read = Charging_usb_ntc_read,
	.write = Charging_usb_ntc_write,
};
#endif
//liulinsheng@wind-mobicom 20170302
static int aging_power_test_proc_init(void)
{
	...
	//liulinsheng@wind-mobicom 20170302
	#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
	proc_Charging_usb_ntc_file = proc_create(AGING_POWER_TEST_PROC_CHARGING_USB_NTC, (S_IWUSR|S_IRUGO|S_IWUGO), 
		aging_power_test_proc_dir, &proc_Charging_usb_ntc_file_ops);
	if(proc_Charging_usb_ntc_file == NULL)
	{
		printk(" %s: proc_Charging_DemoApp_ChargeState_file create failed!\n", __func__);
		remove_proc_entry( AGING_POWER_TEST_PROC_CHARGING_USB_NTC, aging_power_test_proc_dir );
		return -ENOMEM;
	}
	#endif
	//liulinsheng@wind-mobicom 20170302
...
#endif
//modified by ranyanhao 20161130 end
...
#ifdef CONFIG_WIND_ASUS_BATTERY_LIFE_SUPPORT
	if(BATLIFE_status.Normal2Batlife == 1 && BMT_status.UI_SOC2 >= 40){
...
//liqiang@wind-mobi.com 20170321 begin 	
	...
	else if((BMT_status.UI_SOC2 == 100) && (BMT_status.charger_exist == KAL_TRUE)){
			bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_FULL;	
	}
#else			
...
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
//liulinsheng@wind-mobi.com 20170309
static ssize_t usb_ntc_charge(void)
{

	int temp;
	unsigned int usb_ntc = 0;    //error user
	IMM_GetOneChannelValue_Cali(13, &usb_ntc);   //error use
	//battery_log(BAT_LOG_FULL, "step  : liulinsheng@wind-mobicom 20170302 Charging_usb_ntc_read=%d!!!!!!!!!\r\n",g_charger_usb_ntc);
	usb_ntc=usb_ntc/1000;
	temp = mtk_ts_bts_volt_to_temp(usb_ntc);
	return temp;
}
//liulinsheng@wind-mobi.com 20170309
#endif
//renyongwei@wind-mobi.com 20170923 begin
static int mt_battery_GetBatteryCapacity(void)
{
	int temp,Qmax;
	temp = force_get_tbat(KAL_TRUE);
	Qmax = fgauge_get_Q_max(temp);
	battery_log(BAT_LOG_CRTI, "renyongwei BatteryCapacity= (%d)\n",Qmax);
	return Qmax;
}
static int mt_battery_GetBatteryChargeCounter(void)
{
	int charge_counter;
	charge_counter = BMT_status.UI_SOC2*mt_battery_GetBatteryCapacity()/100;
	battery_log(BAT_LOG_CRTI, "wind-log battery charge_counter= (%d)\n",charge_counter);
	return 4130;
}
//renyongwei@wind-mobi.com 20170923 end
//liulinsheng@wind-mobi.com 20171002 begin
static ssize_t battery_GetBatteryChargecurrent(void)
{
	int temp = 0;
    temp = BMT_status.IBattery / 10; 
	if(temp < 0){
		temp = 0;
		battery_log(BAT_LOG_CRTI, "discharge mode");
	}
	return temp;
}
//liulinsheng@wind-mobi.com 20171002 end
static void battery_update(struct battery_data *bat_data)
{
	...
	//liulinsheng@wind-mobi.com 20170331 begin
	#ifdef CONFIG_AUAS_USB_NTC_SUPPORT	
	static unsigned int pre_usb_ntc;

	#endif
	//liulinsheng@wind-mobi.com 20170331 end
	...
	bat_data->BAT_charge_count = mt_battery_GetBatteryChargeCounter();//renyongwei@wind-mobi.com 20170923 add
	bat_data->BAT_bat_current = battery_GetBatteryChargecurrent();   //liulinsheng@wind-mobi.com 20171002 add
	bat_data->BAT_PRESENT = BMT_status.bat_exist;
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
	bat_data->BAT_usb_ntc = usb_ntc_charge();        //liulinsheng@wind-mobi.com 20170309
#endif
	//liulinsheng@wind-mobi.com  for ap temp check
	#ifdef CONFIG_AUAS_AP_TEMP	
	bat_data->BAT_ap_thermal_temp = bts_cur_temp;
	#endif
	//liulinsheng@wind-mobi.com  for ap temp check
...
if((BATLIFE_status.Normal2Batlife == 1) && (BMT_status.UI_SOC2 >= 40) )
{
	BMT_status.UI_SOC_BATLIFE = 40 + (BMT_status.UI_SOC2-40)*60/40;
...
printk("wind-log:battery_update UI_SOC_BATLIFE = %d, UI_SOC2 = %d, SOC=%d\n ",BMT_status.UI_SOC_BATLIFE,
...
//liulinsheng 20170412 begin
#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
//printk("ryw [%s][%d] BAT_usb_ntc = %d usb_temp_reach = %d\n", __func__, __LINE__, bat_data->BAT_usb_ntc, usb_temp_reach);
if(bat_data->BAT_usb_ntc >= 70 ){
	usb_temp_reach = 1;
	BMT_status.bat_charging_state = CHR_ERROR;//lightning icon && Vbat
	//BMT_status.charger_type = CHARGER_UNKNOWN;//led
	//battery_charging_control(CHARGING_CMD_SET_SUSPEND_CHARGE, NULL);//Vbus
	switch_set_state(&fps_switch_data, 1);
	//printk("liulinshengusb_ntc too high =%d\n",bat_data->BAT_usb_ntc);
}
//if(usb_temp_reach == 1 && bat_data->BAT_usb_ntc >= 60 && bat_data->BAT_usb_ntc <= 70)
if(usb_temp_reach == 1 && bat_data->BAT_usb_ntc <= 60)
{
	usb_temp_reach = 0;
	...
	//battery_charging_control(CHARGING_CMD_SET_SUSPEND_CHARGE, NULL);
	//printk("liulinshengusb_ntc middle =%d\n",bat_data->BAT_usb_ntc);
}
#if 0
if(usb_temp_reach == 1 && bat_data->BAT_usb_ntc <= 60)
{
	BMT_status.bat_charging_state = CHR_ERROR;//led close && icon open 
	BMT_status.charger_type = CHARGER_UNKNOWN;
	battery_charging_control(CHARGING_CMD_SET_SUSPEND_CHARGE, NULL); 
	if(BMT_status.charger_exist == KAL_FALSE)
	{
		usb_temp_reach = 0;
		switch_set_state(&fps_switch_data, 0);
		battery_charging_control(CHARGING_CMD_SET_RECOVER_CHARGE, NULL);
	}
	//printk("liulinshengusb_ntc too low =%d\n",bat_data->BAT_usb_ntc);
}
#endif
#endif
//liulinsheng 20170412 end
	...
	//liulinsheng@wind-mobi.com 20170331 begin
	#ifdef CONFIG_AUAS_USB_NTC_SUPPORT
	else if (pre_usb_ntc != bat_data->BAT_usb_ntc) {
		/* Update when usb temp change */
		power_supply_changed(bat_psy);
		pre_usb_ntc = bat_data->BAT_usb_ntc;
		update_cnt = 0;
	}
	#endif
	//liulinsheng@wind-mobi.com 20170331 end	
...
static void ac_update(struct ac_data *ac_data)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		...
		if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||
			...
			#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
			(BMT_status.charger_type == ASUS_1_0A_5W_CHARGER)||
			(BMT_status.charger_type == ASUS_2_0A_10W_CHARGER)||
			(BMT_status.charger_type == ASUS_2_0A_18W_CHARGER)||
			(BMT_status.charger_type == ASUS_2_0A_PB_CHARGER)||
			(BMT_status.charger_type == OTHER_AC_CHARGER)|| 
			#endif
...
//lvwenkang@wind-mobi.com 20161013 begin
...
//liulinsheng@wind-mobi.com 20170821 begin
#if (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
	} else if (BMT_status.temperature >= 50) {               
	battery_log(BAT_LOG_CRTI, "[BATTERY] ASUS Battery Over 50!!\n\r");
	if (BMT_status.bat_vol >= 4080){
#else
	} else if (BMT_status.temperature >= 45) {               
	battery_log(BAT_LOG_CRTI, "[BATTERY] ASUS Battery Over 45!!\n\r");
	if (BMT_status.bat_vol >= 4100){
#endif
//liulinsheng@wind-mobi.com 20170821 end
...
#else
		if (BMT_status.temperature <= 44) {	
#endif
//liulinsheng@wind-mobi.com 20170821 end				
		battery_log(BAT_LOG_CRTI, "[BATTERY] ASUS Battery Temperature down from50 to 48, allow charging!!,(%d)\n\r", g_batt_temp_status);
		g_batt_temp_status = TEMP_POS_NORMAL;
		BMT_status.bat_charging_state = CHR_PRE;
		return PMU_STATUS_OK;
	} else {
		battery_log(BAT_LOG_CRTI, "[BATTERY] ASUS Battery Temperature down from 50 to 48, ###else###!!,(%d)\n\r", g_batt_temp_status);
		return PMU_STATUS_FAIL;
	}
} else {
...
#endif
//lvwenkang@wind-mobi.com 20161013 end
					   ...
					   unsigned int data, signed int *sum)
{
	...
	if (type == BATTERY_AVG_CURRENT) {
		if (BMT_status.charger_exist == KAL_TRUE) {
			#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
			if (previous_charger_exist == KAL_FALSE||reset_charging_current == true) {
			#else
			if (previous_charger_exist == KAL_FALSE) {
			#endif
				batteryBufferFirst = KAL_TRUE;
				previous_charger_exist = KAL_TRUE;
				//yutao@wind-mobi.com add gegin 
				#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
				reset_charging_current=false;
				
				//printk( "yutao add mtk thirdly judge the charger type is %d !!!!!!!!!\n",BMT_status.charger_type);
				if ((BMT_status.charger_type==ASUS_2_0A_10W_CHARGER)||(BMT_status.charger_type==ASUS_2_0A_18W_CHARGER)||(BMT_status.charger_type==ASUS_2_0A_PB_CHARGER))
					data = batt_cust_data.asus_2_0a_charger_current / 100;
				else if (BMT_status.charger_type == STANDARD_CHARGER||BMT_status.charger_type == ASUS_1_0A_5W_CHARGER||BMT_status.charger_type == OTHER_AC_CHARGER)
				{
					data = batt_cust_data.asus_1_0a_charger_current / 100; //if no asus high power charger, all set the current 1A.
					battery_log(BAT_LOG_FULL, "yutaowind-mobi.com debug the charging current=%d !!!!!\r\n",batt_cust_data.asus_1_0a_charger_current);
					battery_log(BAT_LOG_FULL, "yutaowind-mobi.com debug the charging current=%d !!!!!\r\n",data);
				}
				#else
...
{
	PMU_STATUS status = PMU_STATUS_OK;
	 unsigned int dpm = 4400;//lvwenkang@wind-mobi.com 20161123 
	 static int flag = 0;
#if defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		...
		if (batt_cust_data.v_charger_enable)
		{
			if (BMT_status.charger_vol <= batt_cust_data.v_charger_min)
				{
					battery_log(BAT_LOG_CRTI, "[BATTERY]Charger under voltage!!\r\n");
				//lvwenkang@wind-mobi.com 20161110 begin			
				#ifdef CONFIG_WIND_ASUS_DEMAND_SUPPORT
				BMT_status.charger_protect_status = charger_UNDER_VOL;			
				#endif			
				//lvwenkang@wind-mobi.com 20161110 end
					BMT_status.bat_charging_state = CHR_ERROR;
					status = PMU_STATUS_FAIL;
					flag = 1;
				}
		...
		//lvwenkang@wind-mobi.com 20161012 being
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		battery_log(BAT_LOG_CRTI, " wind v_charger_max from dts\r\n");
		if (BMT_status.charger_vol >= 6500) {
#else
		if (BMT_status.charger_vol >= v_charger_max) {
			...
			{
				if ((BMT_status.charger_vol < 6500)&&(BMT_status.charger_vol >= 4300)&&(flag == 1)){
								flag = 0;
								BMT_status.bat_charging_state = CHR_PRE;
							}
		      ...
		      battery_charging_control(CHARGING_CMD_SET_VINDPM, &dpm);
...
//luguosheng@wind-mobi.com  20180129 begin	
	 if(g_charger_demoapp_state == 1){ //this for demo app
	 
		if(BMT_status.UI_SOC2 > 60){
			BMT_status.bat_charging_state = CHR_ERROR;
			demoapp_flag = 1;
			battery_log(BAT_LOG_CRTI,"BMT_status.bat_charging_state BMT_status.UI_SOC2 = %d Stop\n", BMT_status.UI_SOC2);	
			battery_charging_control(CHARGING_CMD_SET_SUSPEND_CHARGE, NULL);               //add by liulinsheng  20171002		
		}else if((BMT_status.UI_SOC2 <= 55)  || ((BMT_status.UI_SOC2 > 55) && (demoapp_flag == 0) && (BMT_status.UI_SOC2 < 60))){
			BMT_status.bat_charging_state = CHR_PRE;			
			demoapp_flag = 0;
			battery_log(BAT_LOG_CRTI,"(BMT_status.UI_SOC2 <= 55)  || ((BMT_status.UI_SOC2 > 55) && (demoapp_flag == 0) && (BMT_status.UI_SOC2 < 60) BMT_status.UI_SOC2 = %d start\n", BMT_status.UI_SOC2);
			battery_charging_control(CHARGING_CMD_SET_RECOVER_CHARGE, NULL);
		}else if((BMT_status.UI_SOC2 > 55) && (demoapp_flag == 1) && (BMT_status.UI_SOC2 <= 60)){			
			BMT_status.bat_charging_state = CHR_ERROR;
			battery_log(BAT_LOG_CRTI,"(BMT_status.UI_SOC2 > 55) && (demoapp_flag == 1) && (BMT_status.UI_SOC2 <= 60) BMT_status.UI_SOC2 = %d", BMT_status.UI_SOC2);
			battery_charging_control(CHARGING_CMD_SET_RECOVER_CHARGE, NULL); 
		}else if((BMT_status.UI_SOC2 == 60) && (demoapp_flag == 0)){
			battery_charging_control(CHARGING_CMD_SET_RECOVER_CHARGE, NULL); 
			BMT_status.bat_charging_state = CHR_ERROR;
			demoapp_flag = 1;
			battery_log(BAT_LOG_CRTI,"(BMT_status.UI_SOC2 == 60) && (demoapp_flag == 0) BMT_status.UI_SOC2 = %d", BMT_status.UI_SOC2);
		}
...
//luguosheng@wind-mobi.com  20180129 end	
#endif
	//modified by lvwenkang 20161018 end
...
//added by lvwenkang@wind-mobi.com 20161012 b--
static int temperature_num_1 = 0;
static int temp_over_first_time_1 = 0;
//added by lvwenkang@wind-mobi.com 20161012 e--
...
static void mt_battery_notify_VBatTemp_check(void)
{
...
#endif
	//lvwenkang@wind-mobi.com 20161012	 begin
#if defined(BATTERY_DTS_SUPPORT) 
	...
	//lvwenkang@wind-mobi.com 20161012       end
#endif
...
static void mt_battery_notify_VCharger_check(void)
{
	...
	if (BMT_status.charger_vol > v_charger_max) {
		...
		//battery_log(BAT_LOG_CRTI, "[BATTERY] BMT_status.charger_vol(%d) > %d mV\n",BMT_status.charger_vol, batt_cust_data.v_charger_max);
			battery_log(BAT_LOG_CRTI, "[BATTERY] BMT_status.charger_vol(%d) > 6500 mV\n",
			    BMT_status.charger_vol);//modified by lvwenkang@wind-mobi.com 20161028
	} else {
		g_BatteryNotifyCode &= ~(0x0001);
	}
...
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
    #ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
	if (BMT_status.charger_type == CHARGER_UNKNOWN||BMT_status.charger_type==STANDARD_CHARGER) {
	#else
...
*/
	...
	//yutao@wind-mobi.com add begin
#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT	
   static int chg_id_volt_1=0;
   static int chg_id_volt_2=0;
   ...
   //yutao@wind-mobi.com add end
	...
	battery_log(BAT_LOG_FULL, "step  : yutao ,now in upmu is chr det !!!!!!!!!\r\n");
	BMT_status.charger_vol = battery_meter_get_charger_voltage();
		...
		if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
			...
			//lihaiyan@wind-mobi.com 20170106 begin
		if(BMT_status.charger_type == NONSTANDARD_CHARGER){	
  //      	    printk("<0> [USB] STEP 3....%s...%d... \n",__func__,__LINE__);
            	    mt_charger_type_detection();
        	}else{
  //        	  printk("<0> [USB] STEP 4....%s...%d... \n",__func__,__LINE__);
       		}
        	//lihaiyan@wind-mobi.com 20170106 end
...
//yutao@wind-mobi.com add below
	#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
	//printk( "yutao add mtk firstly judge the charger type is %d !!!!!!!!!\n",BMT_status.charger_type);	
		...
		if(BMT_status.charger_type==STANDARD_CHARGER||BMT_status.charger_type==NONSTANDARD_CHARGER)
		{
			in_times_one++;
			in_times_two++;
			battery_log(BAT_LOG_FULL, "yutao add now goes into ac charging state !!!!!!!!!\r\n");
			battery_log(BAT_LOG_FULL, "yutao add now goes into ac charger detect times_1=%d !!!!!!!!!\r\n",in_times_one);
			battery_log(BAT_LOG_FULL, "yutao add now goes into ac charger detect times_2=%d !!!!!!!!!\r\n",in_times_two);
			if(in_times_one==1&&in_times_two==1){
				pinctrl_select_state(usbswctrl,usbsw_high);
				pinctrl_select_state(vdmctrl,vdm_low);
			}
			if(in_times_one==4){ //modified by renyongwei@wind-mobi.com 20171129 begin
				in_times_one=0;	
				for(i=0;i<5;i++)
				{
					ret = IMM_GetOneChannelValue_Cali(14, &chg_id_volt_1);
					if(ret!=0)
						battery_log(BAT_LOG_FULL, "step  : yutao return adc value fail !!!!!!!!!\r\n");
					else
						battery_log(BAT_LOG_FULL, "step  : yutao return adc value chg_id_volt_1=%d!!!!!!!!!\r\n",chg_id_volt_1);
					
					AdcVal[i]=chg_id_volt_1;
					if(i>=1){
					#ifdef    CONFIG_WIND_DEF_PRO_E266L
						if((AdcVal[i]>=AdcVal[i-1])&&((AdcVal[i]-AdcVal[i-1])>2000))  //20mv
					#elif defined CONFIG_WIND_DEF_PRO_E267L
						if((AdcVal[i]>=AdcVal[i-1])&&((AdcVal[i]-AdcVal[i-1])>2000)) 
					/*liulinsheng@wind-mobi.com 20170803 begin*/		
					#elif (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
						if((AdcVal[i]>=AdcVal[i-1])&&((AdcVal[i]-AdcVal[i-1])>2000)) 
					/*liulinsheng@wind-mobi.com 20170803 end*/		
					#else
						if((AdcVal[i]>=AdcVal[i-1])&&((AdcVal[i]-AdcVal[i-1])>5000))
					#endif
						{
							AdcVal[0]=AdcVal[i];
							i=0;
					...
					#ifdef CONFIG_WIND_DEF_PRO_E266L
						else if((AdcVal[i]<=AdcVal[i-1])&&((AdcVal[i-1]-AdcVal[i])>2000))
					#elif defined CONFIG_WIND_DEF_PRO_E267L
						else if((AdcVal[i]<=AdcVal[i-1])&&((AdcVal[i-1]-AdcVal[i])>2000)) 
                    /*liulinsheng@wind-mobi.com 20170803 begin*/
					#elif (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
                    ...
                    /*liulinsheng@wind-mobi.com 20170803 end*/							
					#else
						else if((AdcVal[i]<=AdcVal[i-1])&&((AdcVal[i-1]-AdcVal[i])>5000))
					#endif
						{
							AdcVal[0] = AdcVal[i];
							i=0;
				...
				//modified by renyongwei@wind-mobi.com 20171129 begin
				if(chg_id_volt_1>=900000&&chg_id_volt_1<=1500000)	
				{
					BMT_status.charger_type=ASUS_2_0A_PB_CHARGER;  
				}
				//modified by renyongwei@wind-mobi.com 20171129 end
				pinctrl_select_state(vdmctrl,vdm_high);
			}
				
			if(in_times_two==6){ //modified by renyongwei@wind-mobi.com 20171129 begin
				in_times_one=0;
				in_times_two=0;
				for(j=0;j<5;j++)
				{
					ret = IMM_GetOneChannelValue_Cali(14, &chg_id_volt_2);
					if(ret!=0)
						battery_log(BAT_LOG_FULL, "step  : yutao return adc value !!!!!!!!!\r\n");
					else
						battery_log(BAT_LOG_FULL, "step  : yutao return adc value chg_id_volt_2= %d!!!!!!!!!\r\n",chg_id_volt_2);
					
					AdcVal[j]=chg_id_volt_2;
					if(i>=1){
					#ifdef  CONFIG_WIND_DEF_PRO_E266L
						if((AdcVal[j]>=AdcVal[j-1])&&((AdcVal[j]-AdcVal[j-1])>2000))
					#elif defined CONFIG_WIND_DEF_PRO_E267L
						if((AdcVal[j]>=AdcVal[j-1])&&((AdcVal[j]-AdcVal[j-1])>2000))  
					/*liulinsheng@wind-mobi.com 20170803 begin*/		
					#elif (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
				    ...
				    /*liulinsheng@wind-mobi.com 20170803 end*/
					#else
						if((AdcVal[j]>=AdcVal[j-1])&&((AdcVal[j]-AdcVal[j-1])>5000))
					#endif			
						{
							AdcVal[0]=AdcVal[j];
							j=0;
					...
					#ifdef    CONFIG_WIND_DEF_PRO_E266L
						else if((AdcVal[j]<=AdcVal[j-1])&&((AdcVal[j-1]-AdcVal[j])>2000))
					#elif defined CONFIG_WIND_DEF_PRO_E267L
						else if((AdcVal[j]<=AdcVal[j-1])&&((AdcVal[j-1]-AdcVal[j])>2000))
					/*liulinsheng@wind-mobi.com 20170803 begin*/
					#elif (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
					...
					/*liulinsheng@wind-mobi.com 20170803 end*/
					#else
						else if((AdcVal[j]<=AdcVal[j-1])&&((AdcVal[j-1]-AdcVal[j])>5000))
					#endif
						{
							AdcVal[0]=AdcVal[j];
							j=0;
				...
				battery_log(BAT_LOG_FULL, "step  : yutao return adc value chg_id_volt_2= %d,chg_id_volt_1= %d!!!!!!!!!\r\n",chg_id_volt_2,chg_id_volt_1);
				

					
			
				#ifdef    CONFIG_WIND_DEF_PRO_E266L   // 5w => 0.8v - 1.3v
				if(chg_id_volt_1<=200000&&chg_id_volt_2>=850000&&chg_id_volt_2<=1200000)
				#elif defined CONFIG_WIND_DEF_PRO_E267L
				if(chg_id_volt_1<=200000&&chg_id_volt_2>=850000&&chg_id_volt_2<=1200000)
				/*liulinsheng@wind-mobi.com 20170803 begin*/
				#elif (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
				if(chg_id_volt_1<=80000&&chg_id_volt_2>=1000000&&chg_id_volt_2<=1300000)
				/*liulinsheng@wind-mobi.com 20170803 end*/
				#else
				if(chg_id_volt_1<=200000&&chg_id_volt_2>=850000&&chg_id_volt_2<=1200000)
				#endif
				...
				#ifdef    CONFIG_WIND_DEF_PRO_E266L   // 10w => 0.5v - 0.8v
				else if(chg_id_volt_1<=200000&&chg_id_volt_2>=550000&&chg_id_volt_2<=800000)
				#elif defined CONFIG_WIND_DEF_PRO_E267L
				else if(chg_id_volt_1<=200000&&chg_id_volt_2>=550000&&chg_id_volt_2<=800000)
				/*liulinsheng@wind-mobi.com 20170803 begin*/
				#elif (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
				else if((chg_id_volt_1<=80000&&chg_id_volt_2>=550000&&chg_id_volt_2<=700000) || (chg_id_volt_1<=180000&&chg_id_volt_2>=590000&&chg_id_volt_2<=750000) ||  (chg_id_volt_1<=180000&&chg_id_volt_2>=1000000&&chg_id_volt_2<=1150000))//lugusoheng@wind-mobi.com 20180123 add
				/*liulinsheng@wind-mobi.com 20170803 end*/
				#else
				else if(chg_id_volt_1<=200000&&chg_id_volt_2>=400000&&chg_id_volt_2<=800000)
				#endif
				{
					BMT_status.charger_type=ASUS_2_0A_10W_CHARGER;
				    
				}
				#ifdef    CONFIG_WIND_DEF_PRO_E266L   // 18w => 0.1v - 0.5v
				else if(chg_id_volt_1<=200000&&chg_id_volt_2>=100000&&chg_id_volt_2<=400000)
				#elif defined CONFIG_WIND_DEF_PRO_E267L
				else if(chg_id_volt_1<=200000&&chg_id_volt_2>=100000&&chg_id_volt_2<=400000)
				/*liulinsheng@wind-mobi.com 20170803 begin*/
				#elif (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
				else if((chg_id_volt_1<=80000&&chg_id_volt_2>=200000&&chg_id_volt_2<=450000) || (chg_id_volt_1<=180000&&chg_id_volt_2>=200000&&chg_id_volt_2<=450000))//renyongwei@wind-mobi.com 20170926 add
				/*liulinsheng@wind-mobi.com 20170803 end*/
				#else
				else if(chg_id_volt_1<=200000&&chg_id_volt_2>=100000&&chg_id_volt_2<=400000)
				#endif
				...
				//modified by renyongwei@wind-mobi.com 20171129 begin
				/*else if((chg_id_volt_1>=300000&&chg_id_volt_1<=500000&&chg_id_volt_2>=500000&&chg_id_volt_2<=700000))	
				{
					BMT_status.charger_type=ASUS_2_0A_PB_CHARGER;
				...
				battery_charging_control(CHARGING_CMD_SW_INIT, NULL);     //add by liulinsheng  20170113
				battery_log(BAT_LOG_FULL, "[BAT_thread]Cable in, CHR_Type_num=%d\r\n",
			    BMT_status.charger_type);
				#if 0 //def CONFIG_WIND_DEVICE_INFO
				#include <wind_device_info.h>
				//extern wind_device_info_t wind_device_info;
				wind_device_info.chg_module_info.charger_name = chg_name;
				
				#endif
					reset_charging_current=true;
				   pinctrl_select_state(vdmctrl,vdm_low);
...
*/
		...
		in_times_two=0;
        pinctrl_select_state(vdmctrl,vdm_low);
		//zhangkaiyuan@wind-mobi.com 20170609 begin
		if(usbsw_flag == 0)
		pinctrl_select_state(usbswctrl,usbsw_low);
		//printk("yutao-plugout:usbsw_low!!!!\n");
		//zhangkaiyuan@wind-mobi.com 20170609 end
		battery_log(BAT_LOG_FULL, "step  : yutao when plug out ,all return default value !!!!!!!!!\r\n");
#endif
	}
}
...
{
	/*liulinsheng@wind-mobi.com@wind-mobi.com change interface XXX 2017-9-24  */
	#ifdef CONFIG_WIND_DEF_PRO_E262L
...
//liulinsheng@wind-mobi.com 20170821 begin
#if (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
	__batt_parse_node(np, "max_charge_temperature_2",
		&batt_cust_data.max_charge_temperature);

	__batt_parse_node(np, "max_charge_temperature_minus_x_degree_2",
		&batt_cust_data.max_charge_temperature_minus_x_degree);
#endif
//liulinsheng@wind-mobi.com 20170821 end
...
//yutao@wind-mobi.com add begin
#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT  
		__batt_parse_node(np, "asus_1_0a_charger_current",
		&batt_cust_data.asus_1_0a_charger_current);
		__batt_parse_node(np, "asus_2_0a_charger_current",
		&batt_cust_data.asus_2_0a_charger_current);
#endif
//yutao@wind-mobi.com add end
//liulinsheng@wind-mobi.com 20170821 begin
#if (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
		__batt_parse_node(np, "asus_2_0a_charger_current_2",
		&batt_cust_data.asus_2_0a_charger_current);
#endif
//liulinsheng@wind-mobi.com 20170821 end
...
//lvwenkang@wind-mobi.com 20161121 begin
#ifdef CONFIG_WIND_DEVICE_INFO
	sprintf(wind_device_info.battery_data.BAT_Model_Name, "%s", "C11P1611");
...
#ifdef CONFIG_WIND_DEF_PRO_E266L
	sprintf(wind_device_info.battery_data.Image_Version, "%s", "14.00.1703.1");   //add by liulinsheng 20170323
#elif defined CONFIG_WIND_DEF_PRO_E267L
	sprintf(wind_device_info.battery_data.Image_Version, "%s", "14.00.1703.1");  
/*renyongwei@wind-mobi.com 20170925 begin*/
#elif defined CONFIG_WIND_DEF_PRO_E262L
	sprintf(wind_device_info.battery_data.Image_Version, "%s", "14.02.1708.1"); 
/*renyongwei@wind-mobi.com 20170925 end*/
/*liulinsheng@wind-mobi.com 20170803 begin*/	
#elif defined CONFIG_WIND_DEF_PRO_E260L
	sprintf(wind_device_info.battery_data.Image_Version, "%s", "14.00.1706.1"); 
/*liulinsheng@wind-mobi.com 20170803 end*/
#else
	sprintf(wind_device_info.battery_data.Image_Version, "%s", "14.0.0.1");   //add by liulinsheng 20170323
#endif
...
//lihaiyan@wind-mobi.com 20170110 begin
//#ifdef BATTERY_MODULE_INIT
#if 0
//lihaiyan@wind-mobi.com 20170110 end
...
//yutao add below 
#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
static int vdm_probe(struct platform_device *dev)
{
	//printk("[usbsw]vdm_probe begin!\n");
	platfm_dev = dev;
	
	vdmctrl = devm_pinctrl_get(&platfm_dev->dev);
	if (IS_ERR(vdmctrl)) {
		//printk( "Cannot find  vdmctrl pinctrl!\n");
	}
	vdm_low = pinctrl_lookup_state(vdmctrl, "vdm_enp0_gpio");
	if (IS_ERR(vdm_low)) {
		//printk("%s : pinctrl err, vdm_low_enable\n", __func__);
	}

	vdm_high = pinctrl_lookup_state(vdmctrl, "vdm_enp1_gpio");
	if (IS_ERR(vdm_high)) {
		//printk("%s : pinctrl err, vdm_high_disable\n", __func__);
	}
	return 0;
}

static int vdm_remove(struct platform_device *dev)
{

	return 0;
}

struct of_device_id vdm_of_match[]={
{.compatible = "mediatek,vdmgpio",},
{},
};
...
.name="vdmgpio_drv",
.owner=THIS_MODULE,
.of_match_table=vdm_of_match,
},
};
static int vdmgpio_pin_init(void)
{
	int ret = 0;

	//printk("[usbsw]usbsw_pin_init begin!!!!!!!!!!!!\n");
	ret = platform_driver_register(&vdm_driver);
	if (ret!=0)
		printk("[vdm]platform_driver_register error:%d\n", ret);
	else
		printk("[vdm]platform_driver_register done!\n");
	if(!ret)
	printk("vdm init work fail\n");
	return ret;

}
static void vdmgpio_pin_uninit(void)
{
	
	 platform_driver_unregister(&vdm_driver);
}

module_init(vdmgpio_pin_init);
module_exit(vdmgpio_pin_uninit);
#endif
//yutao add end
