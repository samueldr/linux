...
unsigned int set_bat_charging_current_limit(int current_limit)
{
	...
	//lvwenkang@wind-mobi.com 20161012 being
		battery_log(BAT_LOG_CRTI, "[BATTERY] set_bat_charging_current_limit (%d)\r\n",
				current_limit);
			//return 0;                 //modified by liulinsheng 20170323
	...
	if (current_limit != -1) {
		...
		case ASUS_2_0A_10W_CHARGER:
		case ASUS_2_0A_18W_CHARGER:
		case ASUS_2_0A_PB_CHARGER:
			chr_type_ichg = batt_cust_data.asus_2_0a_charger_current;
			mtk_pep20_set_charging_current(&chr_type_ichg, &chr_type_aicr);
			mtk_pep_set_charging_current(&chr_type_ichg, &chr_type_aicr);
			break;
		case OTHER_AC_CHARGER:
		case ASUS_1_0A_5W_CHARGER:
			chr_type_ichg = batt_cust_data.asus_1_0a_charger_current;
			break;
		#endif
...
void select_charging_current(void)
{
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			//yutao@wind-mobi.com add begin
			#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
			g_temp_input_CC_value = batt_cust_data.ac_charger_current;      //modified by liulinsheng  20170118
			g_temp_CC_value = batt_cust_data.ac_charger_current;            //modified by liulinsheng  20170118
			#else
			g_temp_CC_value = batt_cust_data.ac_charger_current;
			#endif //liqiang@wind-mobi.com 20161230 add 
		...
		else if (BMT_status.charger_type == ASUS_1_0A_5W_CHARGER||BMT_status.charger_type ==OTHER_AC_CHARGER) {
			g_temp_input_CC_value = batt_cust_data.asus_1_0a_charger_current;
			g_temp_CC_value = batt_cust_data.asus_1_0a_charger_current;
			//mtk_pep_set_charging_current(&g_temp_CC_value, &g_temp_input_CC_value);
			//mtk_pep20_set_charging_current(&g_temp_CC_value, &g_temp_input_CC_value);
		}
		#endif
...
#if defined(CONFIG_WIND_ASUS_DEMAND_SUPPORT) && defined(CONFIG_MTK_ASUS_CHARGER_SUPPORT)
static int asus_vbat_ov_flag = 0; //add by lvwenkang@wind-mobile.com 20161107
#endif
...
static void mtk_select_ichg_aicr(void)
{
	...
	battery_log(BAT_LOG_CRTI,
		...
		//lvwenkang@wind-mobi.com 20161013 begin
          ...
          #ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
			if((BMT_status.temperature > 0) && (BMT_status.temperature < 10)&&(BMT_status.charger_type==STANDARD_CHARGER||BMT_status.charger_type==ASUS_2_0A_10W_CHARGER||BMT_status.charger_type==ASUS_2_0A_18W_CHARGER||BMT_status.charger_type==ASUS_2_0A_PB_CHARGER))
		  #else
			if((BMT_status.temperature > 0) && (BMT_status.temperature < 10)&&(BMT_status.charger_type==STANDARD_CHARGER))  
		  #endif
	  //liulinsheng@wind-mobi.com 20170821 begin
	#if (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
				{						 
							battery_log(BAT_LOG_CRTI,"wind temp at 0--10 set ac_charger_current = %d\n",g_temp_CC_value);
				}		  
		  #else  
	//liulinsheng@wind-mobi.com 20170821 end
          ...
          #ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
			if((BMT_status.temperature > 20) && (BMT_status.temperature < 50)&& (BMT_status.bat_vol  >  4200)&&(BMT_status.charger_type==STANDARD_CHARGER||BMT_status.charger_type==ASUS_2_0A_10W_CHARGER||BMT_status.charger_type==ASUS_2_0A_18W_CHARGER||BMT_status.charger_type==ASUS_2_0A_PB_CHARGER))
		  #else
				if((BMT_status.temperature > 20) && (BMT_status.temperature < 50)&& (BMT_status.bat_vol  >  4200)&&(BMT_status.charger_type==STANDARD_CHARGER))  
          #endif	
  //liulinsheng@wind-mobi.com 20170821 begin
  #if (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
   				{	 
							battery_log(BAT_LOG_CRTI,"wind temp at 20--50,  bat_vol at 4.2    set ac_charger_current = %d\n",g_temp_CC_value);
				}
   #else
  //liulinsheng@wind-mobi.com 20170821 end
				{	 
			...
			#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
			 if(asus_vbat_ov_flag == 1 && BMT_status.bat_vol  >  4000&&(BMT_status.charger_type==STANDARD_CHARGER||BMT_status.charger_type==ASUS_2_0A_10W_CHARGER||BMT_status.charger_type==ASUS_2_0A_18W_CHARGER||BMT_status.charger_type==ASUS_2_0A_PB_CHARGER))
			#else
			  if(asus_vbat_ov_flag == 1 && BMT_status.bat_vol  >  4000&&(BMT_status.charger_type==STANDARD_CHARGER))	
			#endif
  //liulinsheng@wind-mobi.com 20170821 begin
 #if (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
   			{
			 				battery_log(BAT_LOG_CRTI,"wind temp at 20--50,  bat_vol at 4.0  set ac_charger_current = %d\n",g_temp_CC_value);
			 	}
   #else
  //liulinsheng@wind-mobi.com 20170821 end
				{
   ...
   #endif
			//add by liulinsheng@wind-mobi.com  20170116
			#ifdef CONFIG_MTK_ASUS_CHARGER_SUPPORT
			 if( BMT_status.bat_vol  <=  4000&&(BMT_status.charger_type==STANDARD_CHARGER||BMT_status.charger_type==ASUS_2_0A_10W_CHARGER||BMT_status.charger_type==ASUS_2_0A_18W_CHARGER||BMT_status.charger_type==ASUS_2_0A_PB_CHARGER))
			#else
			  if( BMT_status.bat_vol  <=  4000&&(BMT_status.charger_type==STANDARD_CHARGER))	
			#endif
  //liulinsheng@wind-mobi.com 20170821 begin
		#if (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
	   
	   			{
			 			battery_log(BAT_LOG_CRTI,"wind temp at 20--50,  bat_vol below 4.0 ac_charger_current = %d\n",g_temp_CC_value);
			 	}
	   #else
  //liulinsheng@wind-mobi.com 20170821 end
			{
		...
		/*=========================*/
    battery_log(BAT_LOG_CRTI,"yutao add    set ac_charger_current g_temp_CC_value= %d,g_temp_input_CC_value=%d,\n",g_temp_CC_value,g_temp_input_CC_value);
...
static void mtk_select_cv(void)
{
...
#if (defined CONFIG_WIND_DEF_PRO_E260L)||(defined CONFIG_WIND_DEF_PRO_E262L)
	if (batt_cust_data.high_battery_voltage_support)
		cv_voltage = BATTERY_VOLT_04_360000_V;//liulinsheng@wind-mobi.com 20170822
#else
	if (batt_cust_data.high_battery_voltage_support)
		cv_voltage = BATTERY_VOLT_04_387500_V;//lvwenkang@wind-mobi.com 20161012  
#endif
	...
	if (ret == 0) {
		...
		battery_log(BAT_LOG_FULL, "%s: set dynamic cv = %dmV\n",
			__func__, dynamic_cv);
 		battery_log(BAT_LOG_FULL, "wind mtk_get_dynamic_cv  not open!!\n ");//lvwenkang@wind-mobi.com 20161012  
	}
...
//liqiang@wind-mobi.com 20170311 begin 
#ifdef CONFIG_WIND_ASUS_BATTERY_LIFE_SUPPORT
	if(BATLIFE_status.Normal2Batlife == 1 && BMT_status.UI_SOC2 >= 81)
	{
		charging_enable = KAL_FALSE;
...
PMU_STATUS BAT_BatteryHoldAction(void)
{
	...
	//added by lvwenkang@wind-mobi.com  20161116  
extern unsigned int g_charger_demoapp_state; 
extern int demoapp_flag;
extern int usb_temp_reach ;
	//added by lvwenkang@wind-mobi.com  20161116 
PMU_STATUS BAT_BatteryStatusFailAction(void)
{
	...
	charging_enable = KAL_FALSE;	
	//added by lvwenkang@wind-mobi.com  20161116 for AGING_POWER_TEST begin
if((g_charger_state == 0) || (demoapp_flag ==1 && g_charger_demoapp_state == 1))
	...
	//added by lvwenkang@wind-mobi.com  20161116 for AGING_POWER_TEST end
	//added by liulinsheng@wind-mobi.com  20170920 for AGING_POWER_TEST end
    if(usb_temp_reach == 1)
		{
			charging_enable = KAL_FALSE;
			battery_log(BAT_LOG_CRTI,
				    "usb_temp_reach stop charging");
				g_temp_input_CC_value = CHARGE_CURRENT_0_00_MA;
				g_temp_CC_value = CHARGE_CURRENT_0_00_MA;
				battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,
		&g_temp_input_CC_value);
