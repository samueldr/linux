...
static void qg_notify_charger(struct qpnp_qg *chip)
{
	...
	pr_info("Notified charger on float voltage and FCC chg_iterm_ma = %d\n", chip->chg_iterm_ma);
}
...
#define EMPTY_SOC		0
#define FULL_SOC		100
#define FG_DIFF_CHG_TOPOFF_CUR_MA		30
			       ...
			       const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		if (pval->intval)
			chip->dt.iterm_ma = pval->intval + FG_DIFF_CHG_TOPOFF_CUR_MA;
		else
...
static int qg_parse_dt(struct qpnp_qg *chip)
{
	...
	qg_dbg(chip, QG_DEBUG_PON, "tcss_enable=%d, tcss_entry_soc=%d\n",
				chip->dt.tcss_enable, chip->dt.tcss_entry_soc);
