...
struct batt_cap_learn_info {
	...
	struct delayed_work	bcl_integral_work;
...
#if defined(CONFIG_SOC_QCOM)
#define CURRENT_COUNTER_ZTE_OFFSET POWER_SUPPLY_PROP_CHARGE_COUNTER
#elif defined(CONFIG_SOC_SPRD)
#define CURRENT_COUNTER_ZTE_OFFSET POWER_SUPPLY_PROP_CURRENT_COUNTER_ZTE
#elif defined(CONFIG_SOC_MTK)
#define CURRENT_COUNTER_ZTE_OFFSET POWER_SUPPLY_PROP_CURRENT_COUNTER_ZTE
#else
#define CURRENT_COUNTER_ZTE_OFFSET POWER_SUPPLY_PROP_CHARGE_COUNTER
#endif
...
static int batt_cap_learn_linearization_reduce_voltage(struct batt_cap_learn_info *chip)
{
	...
	char *debug_info = chip->show_capacity.debug_buffer;
	...
	snprintf(debug_info + strlen(debug_info), DEBUG_BUFFER_LEN - strlen(debug_info),
				"DV/CAP:%d/%d, ", delta_voltage, linear_capacity);
...
static int batt_cap_learn_linearization_age_lost(struct batt_cap_learn_info *chip)
{
	...
	snprintf(debug_info + strlen(debug_info), DEBUG_BUFFER_LEN - strlen(debug_info),
				"AGE/CAP:%d/%d, ", age, linear_capacity);
...
/********************************************************
******************bcl zte coulomb counting algorithm***********
*********************************************************/
...
/********************************************************
******************bcl platform coulomb delta algorithm***********
*********************************************************/
...
static int batt_cap_learn_platform_delta_begin(struct batt_cap_learn_info *chip)
{
	...
	rc = power_supply_get_property(chip->battery_psy,
				CURRENT_COUNTER_ZTE_OFFSET, &capacity_counter_prop);
	if (rc < 0) {
		pr_alarm("[delta] Failed to get CHARGE_FULL property rc=%d\n", rc);
		goto failed_loop;
	}

	pr_alarm("[delta] capacity_counter_prop %d\n",
									capacity_counter_prop.intval);
...
static int batt_cap_learn_platform_delta_stop(struct batt_cap_learn_info *chip)
{
	...
	if (chip->platform_bcl_delta.begin_flag == true) {
		...
		chip->platform_bcl_delta.uah_total = 0;
	}
...
static int batt_cap_learn_platform_delta_save(struct batt_cap_learn_info *chip)
{
	union power_supply_propval capacity_prop = {0, };
	int rc = 0;
	...
	rc = power_supply_get_property(chip->battery_psy,
		CURRENT_COUNTER_ZTE_OFFSET, &capacity_prop);
	if (rc < 0) {
		pr_alarm("Failed to get present property rc=%d\n", rc);
		goto failed_loop;
	}

	chip->platform_bcl_delta.uah_total = capacity_prop.intval - chip->platform_bcl_delta.delta_init;
	chip->platform_bcl_delta.uah_total += chip->platform_bcl_delta.uah_init;

	pr_alarm("[0] platform delta mode, init: %d, end: %d, total: %d\n",
				chip->platform_bcl_delta.delta_init, capacity_prop.intval,
				chip->platform_bcl_delta.uah_total);
...
static int batt_cap_learn_platform_delta_algorithm(struct batt_cap_learn_info *chip)
{
	union power_supply_propval batt_prop = {0, };
	int rc = 0;
	...
	case POWER_SUPPLY_STATUS_FULL:
		if (chip->platform_bcl_delta.begin_flag) {
			batt_cap_learn_platform_delta_save(chip);
			batt_cap_learn_capacity_compensate(chip);
			batt_cap_learn_platform_delta_stop(chip);
		}
		break;
...
/*************************************************************
******************bcl platform coulomb counting algorithm***********
**************************************************************/

static int batt_cap_learn_platform_blackbox_check(struct batt_cap_learn_info *chip)
{
	union power_supply_propval capacity_prop = {0, };
...
static int batt_cap_learn_capacity_compensate(struct batt_cap_learn_info *chip)
{
	int capacity_learn = 0, limit_capacity = 0;
	char *debug_info = chip->show_capacity.debug_buffer;
	...
	if (chip->config_par.algorithm_select & USE_PLATFORM_BCL_ALGO) {
		if (chip->config_par.algorithm_select & PLATFORM_BCL_ALGO_DELTA_MODE) {
			capacity_learn = chip->platform_bcl_delta.uah_total;
		} else {
			if (batt_cap_learn_platform_blackbox_check(chip) != 0) {
				goto failed_loop;
			}
			capacity_learn = chip->integral_unshake;
		}
	} else { /*USE_ZTE_BCL_ALGO*/
		capacity_learn = chip->bcl_data.learned_uah;
	}

	/*check capacity < 1000mAH*/
	if (capacity_learn < 1000000) {
		pr_alarm("capacity_learn < 1000mAH\n");
		goto failed_loop;
	}

	memset(debug_info, 0, DEBUG_BUFFER_LEN);

	/*calc and save raw capacity to show in "charge_now"*/
	chip->show_capacity.show_capacity_raw = capacity_learn;

	snprintf(debug_info + strlen(debug_info), DEBUG_BUFFER_LEN - strlen(debug_info),
				"RC:%d, ", chip->show_capacity.show_capacity_raw);
	...
	snprintf(debug_info + strlen(debug_info), DEBUG_BUFFER_LEN - strlen(debug_info),
				"SHOW/MAX+/NOW:%d/%d/%d, ", chip->show_capacity.show_capacity,
										limit_capacity, capacity_learn);
	...
	snprintf(debug_info + strlen(debug_info), DEBUG_BUFFER_LEN - strlen(debug_info),
				"SHOW/MIN-/NOW:%d/%d/%d, ", chip->show_capacity.show_capacity,
										limit_capacity, capacity_learn);
	...
	snprintf(debug_info + strlen(debug_info), DEBUG_BUFFER_LEN - strlen(debug_info),
				"FULL/MAX/NOW:%d/%d/%d, ", chip->show_capacity.show_capacity_full,
										limit_capacity, capacity_learn);
	...
	snprintf(debug_info + strlen(debug_info), DEBUG_BUFFER_LEN - strlen(debug_info),
				"FULL/MIN/NOW:%d/%d/%d;", chip->show_capacity.show_capacity_full,
										limit_capacity, capacity_learn);
...
static void batt_cap_learn_notify_handler_work(struct work_struct *work)
{
	...
	if (chip->config_par.algorithm_select & USE_PLATFORM_BCL_ALGO) {
		if (chip->config_par.algorithm_select & PLATFORM_BCL_ALGO_DELTA_MODE) {
			batt_cap_learn_platform_delta_algorithm(chip);
		} else {
			batt_cap_learn_capacity_compensate(chip);
		}
	}
...
static void batt_cap_learn_integral_work(struct work_struct *work)
{
	struct batt_cap_learn_info *chip =
			container_of(work, struct batt_cap_learn_info, bcl_integral_work.work);
	...
	if (!chip->config_par.battery_current_polarity)
	...
	pr_debug("uas now: %lld, uas delta: %d, delta count: %lld, delta now: %d\n",
				chip->bcl_data.uas_count, current_now.intval,
				chip->bcl_data.delta_count, (current_now.intval - chip->bcl_data.per_current) / 2);
...
static enum hrtimer_restart batt_cap_learn_timer(struct hrtimer *timer)
{
	...
	queue_delayed_work(chip->bcl_probe_wq, &chip->bcl_integral_work, msecs_to_jiffies(0));
...
static int batt_cap_learn_init_calibration_data(struct batt_cap_learn_info *chip)
{
	...
	for (i = 0; i < chip->config_par.battery_num; i++) {
		if (batt_prop.intval == chip->config_par.battery_info[i].id) {
			...
			break;
		} else if (batt_prop.intval > 1000) {
			/* 1000 is 1K, id value is in the middle of config , diff is + - 5% */
			bat_id_min = (95 * chip->config_par.battery_info[i].id) / 100;
			bat_id_max = (105 * chip->config_par.battery_info[i].id) / 100;
			if ((batt_prop.intval > bat_id_min) && (batt_prop.intval < bat_id_max)) {
				pr_alarm("match RESISTANCE_ID: %d\n", batt_prop.intval);
				chip->config_par.battery_now = &(chip->config_par.battery_info[i]);
				break;
			}
...
static int batt_cap_learn_parse_dt(struct batt_cap_learn_info *chip)
{
	...
	OF_READ_PROPERTY(chip->config_par.battery_current_polarity,
					"battery-current-polarity", retval, 1);
				...
				union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_SERIAL_NUMBER:
		pval->strval = chip->show_capacity.debug_buffer;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN:
		pval->intval = chip->config_par.battery_now->battery_voltage;
		break;
	default:
...
int bcl_demon_switch_set(const char *val, const void *arg)
{
	struct batt_cap_learn_info *chip = (struct batt_cap_learn_info *) arg;
	int demon_switch_enable = 0;
	...
	sscanf(val, "%d", &demon_switch_enable);

	chip->bcl_data.demon_switch = demon_switch_enable;

	pr_alarm("demon_switch = %d\n",chip->bcl_data.demon_switch);

	return 0;
}

int bcl_demon_switch_get(char *val, const void *arg)
{
	struct batt_cap_learn_info *chip = (struct batt_cap_learn_info *) arg;
	...
	return snprintf(val, PAGE_SIZE, "%u", chip->bcl_data.demon_switch);
}

static struct zte_misc_ops bcl_demon_switch_node = {
	.node_name = "bcl_demon_switch",
	.set = bcl_demon_switch_set,
	.get = bcl_demon_switch_get,
	.free = NULL,
	.arg = NULL,
};
...
static void batt_cap_learn_probe_work(struct work_struct *work)
{
	...
	INIT_DELAYED_WORK(&chip->bcl_integral_work, batt_cap_learn_integral_work);

	batt_cap_learn_init_calibration_data(chip);

	zte_misc_register_callback(&bcl_demon_switch_node, chip);
