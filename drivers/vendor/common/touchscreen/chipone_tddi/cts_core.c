...
static int cts_init_device_hwdata(struct cts_device *cts_dev, u32 hwid, u16 fwid)
{
	int i;
...
int cts_irq_handler(struct cts_device *cts_dev)
{
	...
	if (tpd_fw_cdev.lcd_reset_processing) {
		return -EINVAL;
	}
...
#endif
#ifdef CONFIG_CTS_POINT_REPORT_CHECK
		cancel_delayed_work(&cts_data->point_report_check_work);
		queue_delayed_work(cts_data->workqueue, &cts_data->point_report_check_work, msecs_to_jiffies(100));
#endif
...
int cts_resume_device(struct cts_device *cts_dev)
{
	...
	if (retries < 0) {
		...
		if (firmware) {
			...
			cts_err("Request default firmware failed, please update manually!!");
	...
	ret = cts_fw_reg_writeb(cts_dev, CTS_DEVICE_FW_REG_EDGE_PARAM, cts_data->edge_limit_level);
	if (ret < 0) {
...
int cts_enter_normal_mode(struct cts_device *cts_dev)
{
	...
	if(ret){
		...
		if(fwid==CTS_FWID_ICNL9911 || fwid==CTS_FWID_ICNL9911S)
		{
...
#endif /* CONFIG_CTS_ESD_PROTECTION */
	cancel_work_sync(&cts_data->upgrade_work);
	cancel_work_sync(&cts_data->pdata->ts_irq_work);
	cancel_work_sync(&cts_data->ts_resume_work);
...
int cts_get_hwid(struct cts_device *cts_dev, u32 *hwid)
{
	...
	ret = cts_hw_reg_readl_retry(cts_dev, 0x30000, hwid, 5, 0);
	if (ret) {
		goto err_out;
	}
	...
	*hwid &=0xfffffff0;
	...
	cts_info("Device hardware id: %06x", *hwid);

	if (!cts_is_hwid_valid(*hwid)) {
		cts_warn("Device hardware id %06x invalid", *hwid);
		ret = -EINVAL;
		goto err_out;
	}
