...
#define MATCH_FWID(firmware, fwid) \
	...
	cts_info("Request driver builtin if match hwid: %06x fwid: %04x && ver > %04x", hwid, fwid, device_fw_ver);
...
const struct cts_firmware *cts_request_newer_firmware_from_fs(struct cts_device *cts_dev,
			const char *fwname, u16 curr_version)
{
	struct cts_firmware *firmware;
	struct file *file;
	int ret, read_size;
	...
	snprintf(filepath, sizeof(filepath), "%s%s", "/sdcard/", fwname);
	file = filp_open(filepath, O_RDONLY, 0);
	if (IS_ERR(file)) {
		cts_err("open %s file fail, try open /vendor/firmware/", filepath);
		snprintf(filepath, sizeof(filepath), "%s%s", "/vendor/firmware/", fwname);
		file = filp_open(filepath, O_RDONLY, 0);
		if (IS_ERR(file)) {
			cts_err("open %s file fail", filepath);
			goto err_free_firmware;
		}
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	offset = FIRMWARE_VERSION_OFFSET;
	if (kernel_read(file, buff, 2, &offset) < 0) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	offset = 0;
	read_size = kernel_read(file, firmware->data, firmware->size, &offset);
#else
...
const struct cts_firmware *cts_request_firmware_from_fs(struct cts_device *cts_dev, const char *fwname)
{
	cts_info("Request from file '%s'", fwname);

	return cts_request_newer_firmware_from_fs(cts_dev, fwname, 0);
}
...
const struct cts_firmware *cts_request_firmware(struct cts_device *cts_dev, u32 hwid, u16 fwid, u16 curr_firmware_ver)
{
	...
	cts_info("Request newer if match hwid: %06x fwid: %04x && ver > %04x", hwid, fwid, curr_firmware_ver);
...
void cts_release_firmware(const struct cts_firmware *firmware)
{
	...
	if (firmware && firmware->name == NULL) {
		cts_info("Release firmware->data");
		vfree(firmware->data);
		kfree(firmware);
	}
}
...
*/
		ret = cts_erase_flash(cts_dev, 0, cts_dev->flash->total_size);
		if (ret) {
