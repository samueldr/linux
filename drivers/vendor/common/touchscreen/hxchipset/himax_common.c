...
#endif
#if defined(CONFIG_DRM_MSM) || defined(CONFIG_DRM_PANEL_NOTIFIER)
...
#ifdef CONFIG_DRM_PANEL_NOTIFIER
static int himax_drm_register(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (PTR_ERR(panel) == -ENODEV) {
			...
			E("%s: device has not been probed yet\n", __func__);
			retval = -EPROBE_DEFER;
		}
...
#ifndef CONFIG_DRM_PANEL_NOTIFIER
#ifdef CONFIG_FB
...
#endif	/* CONFIG_FB */
#endif /* CONFIG_DRM_PANEL_NOTIFIER */
