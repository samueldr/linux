...
#include "himax_common_interface.h"
...
uint16_t gest_key_def[GEST_SUP_NUM] = {
	KEY_GESTURE_DOUBLEC, HX_KEY_UP, HX_KEY_DOWN, HX_KEY_LEFT,
...
static int i_get_FW(void)
{
	...
	ret = request_firmware(&hxfw, i_CTPM_firmware_name, private_ts->dev);
	if (ret < 0) {
#if defined(__EMBEDDED_FW__)
		hxfw = &g_embedded_fw;
...
static void himax_wake_event_report(void)
{
	...
	if (KEY_EVENT) {
		...
		if (KEY_EVENT == KEY_GESTURE_DOUBLEC) {
			if (tpd_fw_cdev.tpd_report_uevent != NULL) {
				I("Double Click Uevent\n");
				tpd_fw_cdev.tpd_report_uevent(double_tap);
			}
		} else {
			I(" %s no support WAKEUP KEY event.\n", __func__);
		}
#if defined(HX_GESTURE_TRACK)
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		if (valid) {
			...
			if (!finger_down[i] && (ts->nFinger_support <= HIMAX_MAX_TOUCH_NUM)) {
				finger_down[i] = 1;
				I("%s:touch down id: %d, coord [%d:%d]\n", __func__,
					i, g_target_report_data->x[i], g_target_report_data->y[i]);
			}
#ifdef HX_REPORT_BY_ZTE_ALGO
			tpd_touch_press(ts->input_dev, g_target_report_data->x[i], g_target_report_data->y[i],
							i, g_target_report_data->w[i], g_target_report_data->w[i]);
#else
#if !defined(HX_PROTOCOL_A)
...
#ifdef HX_REPORT_BY_ZTE_ALGO
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		tpd_touch_release(ts->input_dev, loop_i);
	}
#endif
#endif
