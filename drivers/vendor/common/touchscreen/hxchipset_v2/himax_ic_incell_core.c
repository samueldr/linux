		...
		int len, uint8_t *data, uint8_t bank)
{
	...
	if (w_data == NULL) {
		E("%s Allocate w_data buf failed\n", __func__);
		ret = false;
		goto exit;
	}
	...
	int ret = -1;
		...
		if (suspended == 0) {
			if (state == 0x01) {
				...
				ret = g_core_fp.fp_register_write(pfw_op->addr_headline, DATA_LEN_4, tmp_data, 0);
				if (ret < 0) {
...
static bool himax_mrotation_set(uint8_t mrotation)
{
	...
	case mRotatin_0:
		himax_parse_assign_cmd(fw_data_safe_mode_release_pw_reset, tmp_data, 4);
		ret = g_core_fp.fp_register_write(pfw_op->addr_mrotation, DATA_LEN_4, tmp_data, 0);
		if (ret < 0) {
		...
		break;
	case mRotatin_90:
		himax_parse_assign_cmd(fw_func_mrotation90_pwd, tmp_data, 4);
		ret = g_core_fp.fp_register_write(pfw_op->addr_mrotation, DATA_LEN_4, tmp_data, 0);
		if (ret < 0) {
			I("%s: mrotation writer fail!\n", __func__);
			return false;
	...
	case mRotatin_180:
		...
		break;
	case mRotatin_270:
		himax_parse_assign_cmd(fw_func_mrotation270_pwd, tmp_data, 4);
		ret = g_core_fp.fp_register_write(pfw_op->addr_mrotation, DATA_LEN_4, tmp_data, 0);
		if (ret < 0) {
			I("%s: mrotation writer fail!\n", __func__);
			return false;
...
static bool himax_edge_limit_level_set(uint8_t level)
{
	...
	himax_parse_assign_cmd(edge_limit_level_pwd, tmp_data, 4);
	ret = g_core_fp.fp_register_write(pfw_op->addr_edge_limit_level, DATA_LEN_4, tmp_data, 0);
	if (ret < 0) {
...
#endif
#ifdef HX_DISPLAY_ROTATION
	himax_parse_assign_cmd(fw_mrotation_addr,
		pfw_op->addr_mrotation, sizeof(pfw_op->addr_mrotation));
#endif
#ifdef HX_EDGE_LIMIT
	himax_parse_assign_cmd(fw_edge_limit_level_addr,
