...
static uint32_t himax_wait_sorting_mode(uint8_t checktype)
{
	...
	I("%s:NowType=%d, Except=0x%02X%02X\n", __func__, checktype, wait_pwd[1], wait_pwd[0]);
	do {
		if (gHX_INSPT_DBG)
...
static int himax_self_test_data_init(void)
{
	...
	char *file_name_2 = hx_criteria_csv_name;
	int ret = HX_INSP_OK;
	int err = 0;
	int i = 0;
	...
	snprintf(g_file_path, sizeof(g_file_path), "%s%s", g_hx_save_file_path, g_hx_save_file_name);
	file_w_flag = true;
	return ret;
