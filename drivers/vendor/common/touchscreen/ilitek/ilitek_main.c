...
extern int ilitek_register_fw_class(void);
...
static void ilitek_tddi_wq_bat_check(struct work_struct *work)
{
	...
	if (strnstr(str, "Charging", sizeof(str)) != NULL || strnstr(str, "Full", sizeof(str)) != NULL
		|| strnstr(str, "Fully charged", sizeof(str)) != NULL) {
		if (charge_mode != 1) {
...
int ilitek_tddi_sleep_handler(int mode)
{
	...
	case TP_DEEP_SLEEP:
		...
		idev->skip_wake = false;
#ifdef RESET_LOW_SUSPEND
		gpio_direction_output(idev->tp_rst, 0);
#endif
		break;
	case TP_RESUME:
...
void ilitek_tddi_report_handler(void)
{
	...
	if (!idev->report || atomic_read(&idev->tp_reset) || atomic_read(&idev->fw_stat) ||
		atomic_read(&idev->tp_sw_mode) || atomic_read(&idev->mp_stat) ||
		atomic_read(&idev->tp_sleep) || tpd_fw_cdev.lcd_reset_processing) {
...
int ilitek_tddi_init(void)
{
	...
	atomic_set(&idev->irq_wake_stat, DISABLE);
	atomic_set(&idev->ice_stat, DISABLE);
	atomic_set(&idev->tp_reset, END);
	atomic_set(&idev->fw_stat, END);
	atomic_set(&idev->mp_stat, DISABLE);
...
void ilitek_tddi_release_resource(void)
{
	ipio_info("release_resource\n");
