...
extern int ili9881x_register_fw_class(void);
extern void ili9881x_ex_mode_recovery(void);
...
int ili_report_handler(void)
{
	...
	if (!ilits->report || atomic_read(&ilits->tp_reset) ||
		...
		tpd_fw_cdev.lcd_reset_processing) {
...
void ili_tddi_release_resource(void)
{
	ILI_INFO("release_resource\n");
