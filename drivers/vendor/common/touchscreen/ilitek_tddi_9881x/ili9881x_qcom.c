...
static int ilitek_plat_probe(void)
{
	...
	if (get_tp_chip_id() == 0) {
		if ((tpd_fw_cdev.tp_chip_id != TS_CHIP_MAX) && (tpd_fw_cdev.tp_chip_id != TS_CHIP_ILITEK	)) {
			ILI_INFO("this tp is not used,return.\n");
			return -EPERM;
		}
	}
#endif
	...
	return 0;
err_tddi_init_failed:
	ili_tddi_release_resource();
	return -ENODEV;
}
