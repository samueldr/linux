...
#define RESET_ON_RESUME_DELAY_MS 0
...
extern void synaptics_tpd_register_fw_class(struct syna_tcm_hcd *tcm_hcd);
...
DECLARE_COMPLETION(baseline_report_complete);
...
static struct work_struct delay_probe_work;
static struct workqueue_struct *delay_probe_workqueue;
static struct platform_device *tmp_pdev;
static lcdstate current_lcd_state = SCREEN_ON;

/*for lcd low power mode*/
static struct syna_tcm_hcd *lp_tcm_hcd;
...
SHOW_STORE_PROTOTYPE(syna_tcm, enable_closed_cover)
SHOW_STORE_PROTOTYPE(syna_tcm, enable_fp_gesture)
SHOW_PROTOTYPE(syna_tcm, baseline_report)
...
static struct device_attribute *dynamic_config_attrs[] = {
	...
	ATTRIFY(enable_closed_cover),
	ATTRIFY(enable_fp_gesture),
};

int syna_tcm_get_app_info(struct syna_tcm_hcd *tcm_hcd);
static int syna_tcm_suspend_all(struct device *dev);
static int syna_tcm_enable_report(struct syna_tcm_hcd *tcm_hcd, unsigned char cmd, bool en);
static void change_tp_state(lcdchange lcd_change);
...
static ssize_t syna_tcm_sysfs_baseline_report_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	unsigned int count;
	...
	if (tcm_hcd->app_status != APP_STATUS_OK) {
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	size = cols * rows + hybrid * (cols + rows) + buttons;

	if (tcm_hcd->baseline_data_words == NULL) {
		tcm_hcd->baseline_data_words = kcalloc(size, sizeof(unsigned int), GFP_KERNEL);
		if (!tcm_hcd->baseline_data_words) {
			LOGE(&tcm_hcd->pdev->dev,
				"Failed to allocate memory for baseline_data_words\n");
			goto exit;
		}
	}

#if (KERNEL_VERSION(3, 13, 0) <= LINUX_VERSION_CODE)
	reinit_completion(&baseline_report_complete);
#else
	INIT_COMPLETION(baseline_report_complete);
#endif

	retval = syna_tcm_enable_report(tcm_hcd, REPORT_BASELINE, true);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to enable report baseline\n");
		goto exit;
	}
	...
	retval = wait_for_completion_timeout(&baseline_report_complete,
			msecs_to_jiffies(timeout));
	if (retval == 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Timed out waiting for report baseline\n");
	} else {
		completed = true;
	}

	retval = syna_tcm_enable_report(tcm_hcd, REPORT_BASELINE, false);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to disable report baseline\n");
		goto exit;
	}
	...
	for (y = 0; y < rows; y++) {
		for (x = 0; x < cols; x++) {
			retval = snprintf(buf, PAGE_SIZE - count,
				"%d, ", tcm_hcd->baseline_data_words[y * cols + x]);
			if (retval < 0)
				goto exit;
...
exit:
	...
	kfree(tcm_hcd->baseline_data_words);
	tcm_hcd->baseline_data_words = NULL;
...
dynamic_config_sysfs(enable_closed_cover, DC_ENABLE_CLOSED_COVER)

dynamic_config_sysfs(enable_fp_gesture, DC_ENABLE_FINGERPRINT_GESTURE)
...
static void syna_tcm_dispatch_baseline_report(struct syna_tcm_hcd *tcm_hcd)
{
	...
	size = (cols * rows + (hybrid * (cols + rows)) + buttons);
	data_length = tcm_hcd->payload_length;
	if (data_length != size * 2) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Baseline data size is mismatch, data_length = %u: baseline_data_length = %u\n",
				data_length, size);
		goto exit;
	}

	for (idx = 0; idx < size; idx++) {
		tcm_hcd->baseline_data_words[idx] = le2_to_uint(&tcm_hcd->in.buf[MESSAGE_HEADER_SIZE + idx * 2]);
	}

exit:
	complete(&baseline_report_complete);
}
...
static void syna_tcm_dispatch_message(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->status_report_code >= REPORT_IDENTIFY) {
		...
		if (tcm_hcd->status_report_code == REPORT_BASELINE) {
			syna_tcm_dispatch_baseline_report(tcm_hcd);
			return;
		}
...
static void syna_tcm_single_tap_work(struct work_struct *work)
{
	struct delayed_work *delay_work =
			container_of(work, struct delayed_work, work);
	struct syna_tcm_hcd *tcm_hcd =
			container_of(delay_work, struct syna_tcm_hcd,
			single_tap_work);

	spin_lock(&tcm_hcd->fp_sintap_lock);
	tcm_hcd->is_off_single_tap = false;
	spin_unlock(&tcm_hcd->fp_sintap_lock);
}
...
static int syna_tcm_get_regulator(struct syna_tcm_hcd *tcm_hcd, bool get)
{
	...
	if (bdata->pwr_reg_name != NULL && *bdata->pwr_reg_name != 0) {
		...
		LOGN(tcm_hcd->pdev->dev.parent,
					" set power regulator 2.9-3.0V\n");
		regulator_set_voltage(tcm_hcd->pwr_reg , 2900000, 3000000);
	}
...
static int syna_tcm_enable_report(struct syna_tcm_hcd *tcm_hcd, unsigned char cmd, bool en)
{
	...
	unsigned char command = (en == true ? CMD_ENABLE_REPORT : CMD_DISABLE_REPORT);
...
static int syna_tcm_resume(struct device *dev)
{
	...
	queue_work(tcm_hcd->resume_suspend_workqueue, &tcm_hcd->resume_work);
...
#ifdef WAKEUP_GESTURE
	status = tcm_hcd->is_single_tap | tcm_hcd->in_wakeup_gesture << 1;
#endif

	if (!tcm_hcd->in_suspend) {
		pr_notice("%s: not_in_suspend!, exit\n", __func__);
...
smart_covert:
	...
	retval = tcm_hcd->set_dynamic_config(tcm_hcd,
			DC_ENABLE_CLOSED_COVER, tcm_hcd->is_smart_cover);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to set dynamic config\n");
	}
	...
	if (tcm_hcd->gesture_set_change_in_sus != tcm_hcd->in_wakeup_gesture)
		tcm_hcd->in_wakeup_gesture = tcm_hcd->gesture_set_change_in_sus;

	if (tcm_hcd->is_single_tap != tcm_hcd->single_set_change_in_sus)
		tcm_hcd->is_single_tap = tcm_hcd->single_set_change_in_sus;

exit:
	tcm_hcd->in_suspend = false;
	pr_notice("%s: resume end! retval = %d\n", __func__, retval);
}

static void syna_tcm_suspend(struct device *dev)
{
	int retval = 0;
	int num_reset = 0;
#ifdef WAKEUP_GESTURE
	...
	if (!list_empty(&mod_pool.list)) {
		list_for_each_entry(mod_handler, &mod_pool.list, link) {
			if (!mod_handler->insert &&
				...
				retval = mod_handler->mod_cb->suspend(tcm_hcd);
		}
	}
	mutex_unlock(&mod_pool.mutex);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to do suspend\n");
		num_reset++;
		if (num_reset < 3)
			goto do_suspend;
		else
...
{
	struct syna_tcm_hcd *tcm_hcd = lp_tcm_hcd;

	spin_lock(&tcm_hcd->fp_logo_lock);
	tcm_hcd->in_low_power = false;
...
static inline void lcd_off_thing(void)
{
	...
	spin_lock(&tcm_hcd->fp_logo_lock);
	tcm_hcd->in_low_power = false;
	tcm_hcd->in_fp_logo = 0;
	spin_unlock(&tcm_hcd->fp_logo_lock);
	spin_lock(&tcm_hcd->fp_sintap_lock);
	tcm_hcd->is_off_single_tap = false;
	spin_unlock(&tcm_hcd->fp_sintap_lock);
	syna_tcm_suspend_all(&tcm_hcd->pdev->dev);
}
...
static inline void lcd_doze_thing(void)
{
	...
	spin_lock(&tcm_hcd->fp_logo_lock);
	tcm_hcd->in_low_power = true;
	if (tcm_hcd->in_fp_logo) {
		spin_unlock(&tcm_hcd->fp_logo_lock);
		syna_tcm_resume(&tcm_hcd->pdev->dev);
	} else {
		spin_unlock(&tcm_hcd->fp_logo_lock);
...
#ifdef CONFIG_SCREEN_ON_TEMP_SUPPORT
static inline void lcd_on_temp_thing(struct syna_tcm_hcd *tcm_hcd)
{
	spin_lock(&tcm_hcd->fp_sintap_lock);
	if (tcm_hcd->frame_is_light) {
		spin_unlock(&tcm_hcd->fp_sintap_lock);
		change_tp_state(ON);
		return;
	}
...
static inline void screen_on(lcdchange lcd_change)
{
	switch (lcd_change) {
	case ENTER_LP:
		current_lcd_state = DOZE;
		lcd_doze_thing();
		break;
	case OFF:
		current_lcd_state = SCREEN_OFF;
		lcd_off_thing();
		break;
	default:
		pr_notice("synapti:ignore err lcd change");
	}
}

static inline void screen_off(lcdchange lcd_change)
{
	struct syna_tcm_hcd *tcm_hcd = lp_tcm_hcd;
...
#ifdef CONFIG_SCREEN_ON_TEMP_SUPPORT
		spin_lock(&tcm_hcd->fp_sintap_lock);
		if (tcm_hcd->is_off_single_tap) {
			current_lcd_state = SCREEN_ON_TEMP;
			spin_unlock(&tcm_hcd->fp_sintap_lock);
			lcd_on_temp_thing(tcm_hcd);
		} else {
			spin_unlock(&tcm_hcd->fp_sintap_lock);
	...
	case OFF:
		current_lcd_state = SCREEN_OFF;
		lcd_off_thing();
		pr_err("synapti: err lcd off change");
		break;
	default:
		pr_notice("synapti: ignore err lcd change");
	}
...
static inline void doze(lcdchange lcd_change)
{
	switch (lcd_change) {
	case EXIT_LP:
		current_lcd_state = SCREEN_ON;
...
#ifdef CONFIG_SCREEN_ON_TEMP_SUPPORT
static inline void screen_on_temp(lcdchange lcd_change)
{
	switch (lcd_change) {
	case ON:
		current_lcd_state = SCREEN_ON;
...
static void change_tp_state(lcdchange lcd_change)
{
	pr_notice("synapti: current_lcd_state %d", current_lcd_state);
	switch (current_lcd_state) {
	case SCREEN_ON:
	...
	default:
		current_lcd_state = SCREEN_ON;
		lcd_on_thing();
		pr_err("synapti: err lcd light change");
	}
}
...
#ifdef CONFIG_FB
static void syna_tcm_early_suspend(struct device *dev)
{
	struct syna_tcm_module_handler *mod_handler;
	struct syna_tcm_hcd *tcm_hcd = dev_get_drvdata(dev);
...
static int syna_tcm_suspend_all(struct device *dev)
{
	...
	queue_work(tcm_hcd->resume_suspend_workqueue, &tcm_hcd->suspend_work);

	pr_notice("%s: end!\n", __func__);
	return 0;
}

int syna_tcm_node_notifier_cb(bool enable)
{
	int retval = 0;
	struct syna_tcm_hcd *tcm_hcd = lp_tcm_hcd;

	if (!tcm_hcd) {
		pr_notice("%s: tp driver is failed, exit!\n");
		return 0;
	}

	if (atomic_read(&tcm_hcd->firmware_flashing) && !enable) {
...
/*for lcd low power mode*/
int syna_tcm_notifier_cb(bool in_lp)
{
	int retval = 0;
	struct syna_tcm_hcd *tcm_hcd = lp_tcm_hcd;
...
EXPORT_SYMBOL(syna_tcm_notifier_cb);

static inline void report_uevent(struct kobject *kobj, char **envp)
{
	int retval;
	...
	if (retval != 0)
		pr_err("syna: lcd state uevent send failed!\n");
}

void syna_tcm_report_lcd_state(void)
{
	struct syna_tcm_hcd *tcm_hcd = lp_tcm_hcd;
	char *envp[2];

	report_uevent(&(tcm_hcd->pdev->dev.kobj), envp);
}
EXPORT_SYMBOL(syna_tcm_report_lcd_state);

#ifdef CONFIG_SCREEN_ON_TEMP_SUPPORT
int syna_tcm_frame_notifier_cb(bool is_light)
{
	int retval = 0;
	struct syna_tcm_hcd *tcm_hcd = lp_tcm_hcd;
	...
	spin_lock(&tcm_hcd->fp_sintap_lock);
	if (tcm_hcd->is_off_single_tap) {
		if (current_lcd_state == SCREEN_ON_TEMP && is_light) {
			spin_unlock(&tcm_hcd->fp_sintap_lock);
			change_tp_state(ON);
		} else {
			spin_unlock(&tcm_hcd->fp_sintap_lock);
	...
	spin_lock(&tcm_hcd->fp_sintap_lock);
	tcm_hcd->frame_is_light = is_light;
	spin_unlock(&tcm_hcd->fp_sintap_lock);

	return retval;
}
EXPORT_SYMBOL(syna_tcm_frame_notifier_cb);
#endif

#ifdef CONFIG_DRM_PANEL
static int syna_tcm_fb_notifier_drm(struct notifier_block *nb,
		unsigned long action, void *data)
{
	...
	if (atomic_read(&tcm_hcd->firmware_flashing) && *transition == DRM_PANEL_BLANK_POWERDOWN) {
		retval = wait_event_interruptible_timeout(tcm_hcd->reflash_wq,
				!atomic_read(&tcm_hcd->firmware_flashing),
				msecs_to_jiffies(RESPONSE_TIMEOUT_MS));
		if (retval == 0) {
		...
		unsigned long action, void *data)
{
	...
	if (action == FB_EARLY_EVENT_BLANK) {
		...
		if (*transition == FB_BLANK_POWERDOWN) {
			if (current_lcd_state != SCREEN_OFF) {
				change_tp_state(OFF);
				tcm_hcd->fb_ready = 0;
				pr_notice("synapti: off!\n");
			}
		} else if (*transition == FB_BLANK_UNBLANK) {
			if (current_lcd_state != SCREEN_ON) {
				change_tp_state(ON);
				tcm_hcd->fb_ready++;
				pr_notice("synapti: on!\n");
			}
...
#ifdef CONFIG_DRM_PANEL
static int syna_drm_register(void)
{
	struct device *dev = tmp_pdev->dev.parent;
	int retval = -1;
	int i;
	int count;
	...
	for (i = 0; i < count; i++) {
		...
		if (PTR_ERR(panel) == -ENODEV) {
			pr_err("syna: no device!\n");
			retval = -ENODEV;
		} else if (PTR_ERR(panel) == -EPROBE_DEFER) {
			pr_err("syna: device has not been probed yet\n");
			retval = -EPROBE_DEFER;
		}
...
static void syna_tcm_delay_probe(struct work_struct *work)
{
	int retval;
	int idx;
	struct syna_tcm_hcd *tcm_hcd;
	...
	pr_notice("%s: probe start!\n", __func__);
#ifdef CONFIG_TP_DETECT_BY_LCDINFO
	retval =  get_tp_chip_id();
	if (retval == 0) {
		if ((tpd_fw_cdev.tp_chip_id != TS_CHIP_MAX) && (tpd_fw_cdev.tp_chip_id != TS_CHIP_SYNAPTICS)) {
			pr_notice("this tp is not used,return.\n");
			return;
		}
	}
#endif
	...
	if (tpd_fw_cdev.TP_have_registered) {
		pr_err("TP have registered by other TP.\n");
		return;
	}
	...
	tcm_hcd->in_low_power = false;
	tcm_hcd->in_fp_logo = false;
	tcm_hcd->is_smart_cover = false;
	tcm_hcd->is_irq_wake = false;
#ifdef WAKEUP_GESTURE
	tcm_hcd->in_wakeup_gesture = false;
	tcm_hcd->gesture_set_change_in_sus = false;
	tcm_hcd->is_single_tap = 0;
	tcm_hcd->single_set_change_in_sus = 0;
#endif
	tcm_hcd->baseline_data_words = NULL;
	tcm_hcd->is_off_single_tap = false;
#ifdef CONFIG_SCREEN_ON_TEMP_SUPPORT
	tcm_hcd->frame_is_light = false;
#endif
	tcm_hcd->tp_suspending = false;
	if (bdata->irq_gpio >= 0)
...
#ifdef CONFIG_SCREEN_ON_TEMP_SUPPORT
	spin_lock_init(&tcm_hcd->fp_logo_lock);
#endif
	spin_lock_init(&tcm_hcd->fp_sintap_lock);
...
*/
#ifdef CONFIG_FB
#ifdef CONFIG_DRM_PANEL
		tcm_hcd->fb_notifier.notifier_call = syna_tcm_fb_notifier_drm;
		if (syna_drm_register() == 0) {
			retval = drm_panel_notifier_register(active_panel, &tcm_hcd->fb_notifier);
			if (retval < 0)
				pr_err("syna: Failed to register FB notifier client\n");
		}
#else
	tcm_hcd->fb_notifier.notifier_call = syna_tcm_fb_notifier_cb;
	retval = fb_register_client(&tcm_hcd->fb_notifier);
	...
	tcm_hcd->resume_suspend_workqueue =
			alloc_ordered_workqueue("syna_resume_suspend", 0);
	...
	tcm_hcd->single_tap_workqueue =
			create_singlethread_workqueue("single_tap_cancel");
	INIT_DELAYED_WORK(&tcm_hcd->single_tap_work, syna_tcm_single_tap_work);
	...
	synaptics_tpd_register_fw_class(tcm_hcd);
	tpd_fw_cdev.TP_have_registered = true;

	pr_notice("%s: probe success end!\n", __func__);
	return;
...
err_enable_irq:
	...
	flush_workqueue(tcm_hcd->resume_suspend_workqueue);
	destroy_workqueue(tcm_hcd->resume_suspend_workqueue);

	cancel_delayed_work_sync(&tcm_hcd->single_tap_work);
	flush_workqueue(tcm_hcd->single_tap_workqueue);
	destroy_workqueue(tcm_hcd->single_tap_workqueue);
...
#ifdef CONFIG_DRM_PANEL
	if (active_panel)
		drm_panel_notifier_unregister(active_panel, &tcm_hcd->fb_notifier);
#else
...
static int syna_tcm_probe(struct platform_device *pdev)
{
	...
	delay_probe_workqueue = create_singlethread_workqueue("synatcm_delay_probe");
	if (!delay_probe_workqueue) {
		pr_err("%s: create delay_probe_workqueue faile!\n", __func__);
		retval = -ENOMEM;
		goto exit;
	}
	INIT_WORK(&delay_probe_work, syna_tcm_delay_probe);

	tmp_pdev = pdev;

	queue_work(delay_probe_workqueue, &delay_probe_work);
	retval = 0;
...
static int syna_tcm_remove(struct platform_device *pdev)
{
	...
	cancel_work_sync(&delay_probe_work);
	flush_workqueue(delay_probe_workqueue);
	destroy_workqueue(delay_probe_workqueue);
