...
static int get_profile_from_list(char *buf, size_t buf_len, int id);
					...
					int next_profile, int *vstep)
{
	...
	/* tfa98xx_tapdet_check_update(tfa98xx);  //dll for record mic */
					      ...
					      bool unregister)
{
	...
	for (i = 0; i < tfaContMaxProfile(tfa98xx->handle); i++) {
		profile_name = tfaContProfileName(tfa98xx->handle, i);
		if (strnstr(profile_name, ".tap", strlen(profile_name))) {
			tap_profile = true;
...
static int tfa98xx_dbgfs_otc_get(void *data, u64 *val)
{
	...
	struct tfa98xx_control *otc =
	    &(handles_local[tfa98xx->handle].dev_ops.controls.otc);
...
static int tfa98xx_dbgfs_mtpex_set(void *data, u64 val)
{
	...
	struct tfa98xx_control *mtpex =
	    &(handles_local[tfa98xx->handle].dev_ops.controls.mtpex);
	enum Tfa98xx_Error err, status;
					 ...
					 int calibrateDone)
{
	...
	struct tfa98xx_control *calib =
	    &(handles_local[handle].dev_ops.controls.calib);
				      ...
				      size_t count, loff_t *ppos)
{
	...
	if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
		...
		error = tfaContWriteFile(tfa98xx->handle, msg_file, 0, 0);
		if (error)
			pr_err("tfaContWriteFile error: %d\n", error);
	} else {
		error =
		    dsp_msg(tfa98xx->handle, msg_file->size, msg_file->data);
		if (error)
...
#define TFA98XX_DEBUGFS_REG_CREATE_FILE(__reg, __name)				\
	debugfs_create_file(TOSTRING(__reg) "-" TOSTRING(__name), S_IRUGO|S_IWUSR, dbg_reg_dir,\
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("OTC", S_IRUGO | S_IWUSR, tfa98xx->dbg_dir,
			    i2c, &tfa98xx_dbgfs_calib_otc_fops);
	debugfs_create_file("MTPEX", S_IRUGO | S_IWUSR, tfa98xx->dbg_dir,
			    i2c, &tfa98xx_dbgfs_calib_mtpex_fops);
	debugfs_create_file("TEMP", S_IRUGO | S_IWUSR, tfa98xx->dbg_dir,
			    i2c, &tfa98xx_dbgfs_calib_temp_fops);
	debugfs_create_file("calibrate", S_IRUGO | S_IWUSR, tfa98xx->dbg_dir,
	...
	debugfs_create_file("dsp-state", S_IRUGO | S_IWUSR, tfa98xx->dbg_dir,
			    i2c, &tfa98xx_dbgfs_dsp_state_fops);
	debugfs_create_file("fw-state", S_IRUGO | S_IWUSR, tfa98xx->dbg_dir,
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (profile < 0) {
		pr_err
		    ("tfa98xx: %s: invalid profile %d (mixer_profile=%d, rate=%d)\n",
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	vstep = tfa98xx_prof_vsteps[profile];
	vsteps = tfacont_get_max_vstep(tfa98xx->handle, profile);
	...
	if (profile == tfa98xx_profile) {
		...
		if (ready) {
			...
			if (err) {
				...
				pr_err("Successfully changed vstep index!\n");
			}
		}
			      ...
			      struct snd_ctl_elem_info *uinfo)
{
	...
	uinfo->value.integer.max =
	    max(0, tfacont_get_max_vstep(tfa98xx->handle, profile) - 1);
	pr_err("vsteps count: %d [prof=%d]\n",
	       tfacont_get_max_vstep(tfa98xx->handle, profile), profile);
	return 0;
}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (prof_idx < 0) {
		pr_err
		    ("tfa98xx: sample rate [%d] not supported for this mixer profile [%d].\n",
...
static int get_profile_from_list(char *buf, size_t buf_len, int id)
{
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if (bprof->item_id == id) {
			strlcpy(buf, bprof->basename, buf_len);
			return 0;
		}
	}
 ...
 * samplerate, on error or if not found return RET_ERR
 */
static int get_profile_id_for_sr(int id, unsigned int rate)
{
	int idx = 0;
			     ...
			     int profile)
{
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if (strncmp(bprof->basename, basename, len) == 0) {
			...
			if (!sr) {
				pr_err
				    ("unable to identify supported sample rate for %s\n",
				     bprof->basename);
				return RET_ERR;
			}
				...
				struct snd_ctl_elem_info *uinfo)
{
	...
	err = get_profile_from_list(profile_name, sizeof(profile_name), uinfo->value.enumerated.item);
	if (err != 0)
		return -EINVAL;
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		...
		bprofile =
		    devm_kzalloc(tfa98xx->codec->dev, sizeof(*bprofile),
				 GFP_KERNEL);
		if (!bprofile)
			return -ENOMEM;
		...
		get_profile_basename(bprofile->basename,
				     tfaContProfileName(tfa98xx->handle, prof));
		...
		if (is_profile_in_list(bprofile->basename, bprofile->len) == 0
		    &&
		    is_calibration_profile(tfaContProfileName
					   (tfa98xx->handle, prof)) == 0) {
	...
	if (tfa98xx_dev_family(tfa98xx->handle) == 1) {
		...
		name =
		    devm_kzalloc(tfa98xx->codec->dev, MAX_CONTROL_NAME,
				 GFP_KERNEL);
		if (!name)
			return -ENOMEM;
...
static void tfa98xx_add_widgets(struct tfa98xx *tfa98xx)
{
	...
	tfa98xx_append_i2c_address(&tfa98xx->i2c->dev,
				   tfa98xx->i2c,
				   widgets, num_dapm_widgets, NULL, 0);
					   ...
					   unsigned short *val)
{
 ...
 retry:
		...
		if (ret < 0) {
			pr_warn
			    ("i2c error at subaddress 0x%x, retries left: %d\n",
			     subaddress, retries);
			if (retries) {
				retries--;
				msleep(I2C_RETRY_DELAY);
...
static void tfa98xx_interrupt_setup_tfa1(struct tfa98xx *tfa98xx)
{
	...
	ie_reg = TFA98XX_INTERRUPT_REG_VDDD |
	    ...
	    TFA98XX_INTERRUPT_REG_SPKD | TFA98XX_INTERRUPT_REG_WDD;
...
static void tfa98xx_interrupt_restore_tfa1(struct tfa98xx *tfa98xx)
{
	...
	if (ie_reg != handles_local[tfa98xx->handle].interrupt_enable[0]) {
		...
		dev_dbg(&tfa98xx->i2c->dev,
			"Restored interrupts: ICR = 0x%04x\n", ie_reg);
	} else {
...
{
	handles_local[tfa98xx->handle].interrupt_enable[0] &=
	    ~TFA98XX_INTERRUPT_REG_INT;
	handles_local[tfa98xx->handle].interrupt_enable[0] |=
	    enable << TFA98XX_INTERRUPT_REG_INT_POS;
...
static void tfa98xx_interrupt_setup_tfa2(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->flags & TFA98XX_FLAG_REMOVE_PLOP_NOISE)
		TFA_SET_BF_VALUE(tfa98xx->handle, IENOCLK, 1, &ie_reg);
	...
	if (tfa98xx->flags & TFA98XX_FLAG_LP_MODES) {
		TFA_SET_BF_VALUE(tfa98xx->handle, IELP0, 1, &ie_reg);
		TFA_SET_BF_VALUE(tfa98xx->handle, IELP1, 1, &ie_reg);
	}
	handles_local[tfa98xx->handle].interrupt_enable[2] = ie_reg;
}
...
/* static char *fw7_name = "tfa9897.cnt"; */
/* static char *fw6_name = "tfa9896.cnt"; */
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (tfa98xx->flags & TFA98XX_FLAG_DSP_START_ON_MUTE) {
		...
		pr_err("dll+++++in tfa98xx_interrupt_enable\n");
		return;
	}

	if (tfa98xx->handle == 0) {
		pr_err("dll+++++tfa98xx->handle == 0\n");
		mutex_lock(&tfa98xx->dsp_lock);
		ret =
		    tfa98xx_tfa_start(tfa98xx, tfa98xx_profile, tfa98xx_vsteps);
		if (ret == Tfa98xx_Error_Ok) {
			pr_err("dll+++++ret == Tfa98xx_Error_Ok\n");
			tfa98xx->dsp_init = TFA98XX_DSP_INIT_DONE;
		} else if (ret == Tfa98xx_Error_Not_Supported) {
			pr_err
			    ("dll+++++ret == ret == Tfa98xx_Error_Not_Supported\n");
			tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_FAIL;
		}
		mutex_unlock(&tfa98xx->dsp_lock);
	}
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	if (!tfa98xx->input) {
		...
		if (tfa98xx->flags & TFA98XX_FLAG_TDM_DEVICE) {
			val =
			    snd_soc_read(tfa98xx->codec, TFA98XX_STATUS_FLAGS1);
...
static void tfa98xx_interrupt(struct work_struct *work)
{
	...
	/* check for tap interrupt (using new tfa_irq functions) */
	if (tfa_irq_get(tfa98xx->handle, tfa9912_irq_sttapdet))
		tfa98xx_tapdet(tfa98xx);

	if (out1) {
		/* No clock detected */
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTNOCLK, out1) > 0) {
			...
			if ((tfa98xx->handle == 0) && (no_clk == 1)) {
				...
				int sw_profil = TFA_GET_BF(tfa98xx->handle, SWPROFIL);

				tfa98xx_vsteps[0] = TFA_GET_BF(tfa98xx->handle, SWVSTEP);
				pr_info("No clock detected. Resetting the I2CR to avoid pop on 72!\n");
				err = (enum Tfa98xx_Error)tfa98xx_tfa_start(tfa98xx, sw_profil, tfa98xx_vsteps);
				if (err != Tfa98xx_Error_Ok) {
				 ...
				 */
				profile_name = tfaContProfileName(tfa98xx->handle, sw_profil);
				if (strnstr(profile_name, ".saam", strlen(profile_name))) {
					pr_info("Powering down from a SAAM profile, workaround PLMA4766 used!\n");
					TFA_SET_BF(tfa98xx->handle, PWDN, 1);
					TFA_SET_BF(tfa98xx->handle, AMPE, 0);
					TFA_SET_BF(tfa98xx->handle, SAMMODE, 0);
				}
			}

			/* If clk is stable set polarity to check for LOW (no clock) */
			if (no_clk == 0)
				TFA_SET_BF(tfa98xx->handle, IPONOCLK, 1);
			else
				TFA_SET_BF(tfa98xx->handle, IPONOCLK, 0);
		}
	...
	if (out2) {
		...
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTMWSRC, out2) > 0) {
			...
			if (manwait1 > 0) {
				...
				pr_info
				    ("setting AMPC and AMPE to 1 (default)\n");
				...
				if (tfa98xx->set_mtp_cal) {
					...
					tfa98xx_open(tfa98xx->handle);
					err =
					    tfa_mtp_set_calibration(tfa98xx->handle,
								    tfa98xx->cal_data);
					tfa98xx_close(tfa98xx->handle);
					if (err != Tfa98xx_Error_Ok) {
						pr_err
						    ("Error, setting calibration value in mtp, err=%d\n",
						     err);
					} else {
						tfa98xx->set_mtp_cal = false;
						pr_info
						    ("Calibration value (%d) set in mtp\n",
						     tfa98xx->cal_data);
					}
				}
			}
	...
	if (out3) {
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTLP0, out3) > 0) {
			...
			TFA_SET_BF(tfa98xx->handle, IPOLP0, (lp0 == 0));
		}

		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTLP1, out3) > 0) {
			...
			TFA_SET_BF(tfa98xx->handle, IPOLP1, (lp1 == 0));
		}
			   ...
			   struct snd_soc_dai *dai)
{
	...
	get_profile_basename(basename,
			     tfaContProfileName(tfa98xx->handle,
						tfa98xx_profile));
	len = strlen(basename);
	pr_err("dll+++basename is %s\n", basename);
	...
	for (prof = 0; prof < nprof; prof++) {
		...
		if (0 ==
		    strncmp(basename, tfaContProfileName(tfa98xx->handle, prof),
			    len)) {
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (no_start) {
		pr_err
		    ("no_start parameter set no tfa_start or tfa_stop, returning\n");
		return 0;
	}
...
static struct snd_soc_dai_driver tfa98xx_dai[] = {
	{
	 ...
	 .ops = &tfa98xx_dai_ops,
	 .symmetric_rates = 1,
#if 0				/* LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0)    dll */
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	INIT_DELAYED_WORK(&tfa98xx->monitor_work, tfa98xx_monitor);	/* dll */
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (no_start == 0) {
		...
		case 0x92:	/* tfa9891 */
			...
			tfa98xx->flags |= TFA98XX_FLAG_TFA9890_FAM_DEV;
			break;
		case 0x12:	/* tfa9895|tfa9887 */
			pr_info("TFA9895 or TFA9887 detected\n");
			tfa98xx->flags |= TFA98XX_FLAG_DSP_START_ON_MUTE;
			tfa98xx->flags |= TFA98XX_FLAG_SKIP_INTERRUPTS;
			tfa98xx->flags |= TFA98XX_FLAG_TFA9890_FAM_DEV;
			break;
		case 0x97:
			...
			tfa98xx->flags |= TFA98XX_FLAG_TFA9890_FAM_DEV;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
			pr_err("The chip is 9897,fw_name is %s\n", fw_name);
			break;
		case 0x96:
			...
			tfa98xx->flags |= TFA98XX_FLAG_TFA9890_FAM_DEV;
			tfa98xx->flags |= TFA98XX_FLAG_TDM_DEVICE;
			pr_err("The chip is 9896,fw_name is %s\n", fw_name);
			break;
		default:
...
MODULE_PARM_DESC(trace_level,
		 "TFA98xx debug trace level (0=off, bits:1=verbose,2=regdmesg,3=regftrace).");
