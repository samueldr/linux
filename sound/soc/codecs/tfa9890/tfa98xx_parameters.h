...
typedef struct nxpTfaLsCompensationFilter {
	...
	/* Loudspeaker compensation on/off;
	 * When 'off', the DSP code doesn't apply,
	 * the bwExt => bwExtOn GUI flag should be gray to avoid confusion */
	uint8_t lsCompOn;
...
typedef struct nxpTfaProfileList {
	...
	nxpTfaDescPtr_t list[];	/* items list (length-1 items) */
} nxpTfaProfileList_t;
