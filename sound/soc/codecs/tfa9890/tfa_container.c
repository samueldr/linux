...
static const char errorname[] = "!ERROR!";
static const char nonename[] = "NONE";
...
int tfacont_get_max_vstep(int dev_idx, int prof_idx)
{
	...
	vp = (nxpTfaVolumeStep2File_t *) tfacont_getfiledata(dev_idx, prof_idx,
							     volstepHdr);
	if (vp == NULL)
		return 0;
 ...
 * tree. There can only be one type of file within profile or device.
  */
nxpTfaFileDsc_t *tfacont_getfiledata(int dev_idx, int prof_idx,
				     enum nxpTfaHeaderType type)
{
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscFile) {
			file =
			    (nxpTfaFileDsc_t *) (dev->list[i].offset +
						 (uint8_t *) g_cont);
			if (file != NULL) {
	...
	for (i = 0; i < prof->length; i++) {
		if (prof->list[i].type == dscFile) {
			file =
			    (nxpTfaFileDsc_t *) (prof->list[i].offset +
						 (uint8_t *) g_cont);
			if (file != NULL) {
				hdr = (nxpTfaHeader_t *) file->data;
				if (hdr != NULL) {
...
static const char nostring[] = "Undefined string";
 ...
 */
static enum Tfa98xx_Error tfaContWriteVstep(int dev_idx,
					    nxpTfaVolumeStep2File_t *vp,
					    int vstep)
{
	...
	if (vstep < vp->vsteps) {
		...
		err =
		    tfa98xx_dsp_write_preset(dev_idx,
					     sizeof(vp->vstep[0].preset),
					     vp->vstep[vstep].preset);
		if (err != Tfa98xx_Error_Ok)
			return err;
		err =
		    tfa_cont_write_filterbank(dev_idx, vp->vstep[vstep].filter);
...
static struct nxpTfaVolumeStepMessageInfo *tfaContGetmsgInfoFromReg(struct
								    nxpTfaVolumeStepRegisterInfo
								    *regInfo)
{
	char *p = (char *)regInfo;
...
static struct nxpTfaVolumeStepMessageInfo *tfaContGetNextmsgInfo(struct
								 nxpTfaVolumeStepMessageInfo
								 *msgInfo)
{
	char *p = (char *)msgInfo;
...
static struct nxpTfaVolumeStepRegisterInfo *tfaContGetNextRegFromEndInfo(struct
									 nxpTfaVolumeStepMessageInfo
									 *msgInfo)
{
	char *p = (char *)msgInfo;
...
static enum Tfa98xx_Error tfaContWriteVstepMax2_One(int dev_idx,
						    struct
						    nxpTfaVolumeStepMessageInfo
						    *new_msg,
						    struct
						    ...
						    int enable_partial_update)
{
	...
	if ((enable_partial_update) && (new_msg->MessageType == 1)) {
		 ...
		 */
		enable_partial_update = 0;
		if (memcmp(old_msg->ParameterData, new_msg->ParameterData, len)
		    == 0) {
			pr_debug
			    ("No change in message type 1 - discarding %d bytes\n",
			     len);
			len = 0;
		} else {
	...
	if (enable_partial_update) {
		partial =
		    kmalloc((sizeof(uint8_t) * len) +
			    sizeof(struct tfa_partial_msg_block), GFP_KERNEL);
		if (!partial)
...
static enum Tfa98xx_Error tfaContWriteVstepMax2(int dev_idx,
						...
						int vstep_msg_idx)
{
	...
	for (i = 0; i < nrMessages; i++) {
		...
		if ((vstep_msg_idx >= TFA_MAX_VSTEP_MSG_MARKER)
		    || (vstep_msg_idx == i)) {
			err =
			    tfaContWriteVstepMax2_One(dev_idx, msgInfo,
						      p_msgInfo, enp);
			if (err != Tfa98xx_Error_Ok) {
				/*
	...
	for (i = 0; i < regInfo->NrOfRegisters * 2; i++) {
		...
		bitF.field =
		    (uint16_t) (regInfo->registerInfo[i] >> 8) |
...
enum Tfa98xx_Error tfaContWriteDrcFile(int dev_idx, int size, uint8_t data[])
{
	...
	if (modified == 1) {
		...
		err =
		    tfa_dsp_msg_id(dev_idx, size - 3, (const char *)data,
				   cmdid_changed);
	} else {
 ...
 */
enum Tfa98xx_Error tfaContWriteFile(int dev_idx, nxpTfaFileDsc_t *file,
				    int vstep_idx, int vstep_msg_idx)
{
	...
	case msgHdr:		/* generic DSP message */
		...
		err =
		    dsp_msg(dev_idx, size,
			    (const char *)((nxpTfaMsgFile_t *) hdr)->data);
		break;
	case volstepHdr:
		if (tfa98xx_dev_family(dev_idx) == 2) {
			err =
			    tfaContWriteVstepMax2(dev_idx,
						  (nxpTfaVolumeStepMax2File_t *)
						  hdr, vstep_idx,
						  vstep_msg_idx);
		} else {
			err =
			    tfaContWriteVstep(dev_idx,
					      (nxpTfaVolumeStep2File_t *) hdr,
					      vstep_idx);
		}
	...
	case speakerHdr:
		if (tfa98xx_dev_family(dev_idx) == 2) {
			...
			size =
			    hdr->size - sizeof(struct nxpTfaSpkHeader) -
			    sizeof(struct nxpTfaFWVer);

			err = dsp_msg(dev_idx, size,
				      (const char
				       *)(((nxpTfaSpeakerFile_t *) hdr)->data +
					  (sizeof(struct nxpTfaFWVer))));
		} else {
			size = hdr->size - sizeof(nxpTfaSpeakerFile_t);
			err =
			    tfa98xx_dsp_write_speaker_parameters(dev_idx, size,
								 (const unsigned
								  char
								  *)((nxpTfaSpeakerFile_t *) hdr)->data);
		}
	...
	case presetHdr:
		...
		err =
		    tfa98xx_dsp_write_preset(dev_idx, size,
					     (const unsigned char
					      *)((nxpTfaPreset_t *) hdr)->data);
		break;
	case equalizerHdr:
		err =
		    tfa_cont_write_filterbank(dev_idx,
					      ((nxpTfaEqualizerFile_t *) hdr)->
					      filter);
		break;
	case patchHdr:
		size = hdr->size - sizeof(nxpTfaPatch_t);	/* size is total length */
		err =
		    tfa_dsp_patch(dev_idx, size,
				  (const unsigned char *)((nxpTfaPatch_t *)
							  hdr)->data);
		break;
	case configHdr:
		size = hdr->size - sizeof(nxpTfaConfig_t);
		err =
		    tfa98xx_dsp_write_config(dev_idx, size,
					     (const unsigned char
					      *)((nxpTfaConfig_t *) hdr)->data);
		break;
	case drcHdr:
		if (hdr->version[0] == NXPTFA_DR3_VERSION) {
			...
			err =
			    tfaContWriteDrcFile(dev_idx, size,
						((nxpTfaDrc2_t *) hdr)->data);
		} else {
			/*
			...
			err =
			    tfa98xx_dsp_write_drc(dev_idx, size,
						  ((const unsigned char
						    *)((nxpTfaDrc_t *) hdr)->
						   data + 381));
		}
 ...
 */
nxpTfaDescPtr_t *tfa_cnt_get_dsc(nxpTfaContainer_t *cnt,
				 nxpTfaDescriptorType_t type, int dev_idx)
{
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == (uint32_t) type) {
			this =
			    (nxpTfaDescPtr_t *) (dev->list[i].offset +
						 (uint8_t *) cnt);
			return this;
		}
 ...
 * write a bit field
 */
enum Tfa98xx_Error tfaRunWriteBitfield(Tfa98xx_handle_t dev_idx,
				       nxpTfaBitfield_t bf)
{
	...
	if (tfa98xx_cnt_verbose)
		pr_debug("bitfield: %s=0x%x (0x%x[%d..%d]=0x%x)\n",
			 tfaContBfName(bfUni.field,
				       tfa98xx_dev_revision(dev_idx)), value,
 ...
 * read a bit field
 */
enum Tfa98xx_Error tfaRunReadBitfield(Tfa98xx_handle_t dev_idx,
				      nxpTfaBitfield_t *bf)
{
	enum Tfa98xx_Error error;
	union {
	...
	error =
	    tfa_reg_read(dev_idx, (unsigned char)(bfUni.Enum.address), &regvalue);
	if (error)
		return error;
 ...
 */
enum Tfa98xx_Error tfaRunWriteDspMem(Tfa98xx_handle_t dev,
				     nxpTfaDspMem_t *cfmem)
{
	...
	for (i = 0; i < cfmem->size; i++) {
		...
		error =
		    mem_write(dev, cfmem->address++, cfmem->words[i],
			      cfmem->type);
		if (error)
			return error;
	}
 ...
 */
enum Tfa98xx_Error tfaRunWriteFilter(Tfa98xx_handle_t dev,
				     nxpTfaContBiquad_t *bq)
{
	...
	for (i = 0; i < runs; i++) {
		...
		if (dmem == Tfa98xx_DMEM_ERR) {
			pr_debug
			    (" Warning: No memory location found to write filter settings! Skipped!\n");
		...
		if (tfa98xx_dev_family(dev) == 2) {
			error =
			    tfa_dsp_cmd_id_write(dev, MODULE_FRAMEWORK,
						 FW_PAR_ID_SET_MEMORY,
						 sizeof(data), data);
		} else {
			error =
			    tfa_dsp_cmd_id_write(dev, MODULE_FRAMEWORK,
						 4/* param */, sizeof(data), data);
		}
	}
 ...
 */
enum Tfa98xx_Error tfaRunWriteRegister(Tfa98xx_handle_t handle,
				       nxpTfaRegpatch_t *reg)
{
	...
	error = tfa_reg_read(handle, reg->address, &value);
	if (error)
		return error;
...
enum Tfa98xx_Error tfaContWriteRegsDev(int dev_idx)
{
	...
	for (i = 0; i < dev->length; i++) {
		...
		if (dev->list[i].type == dscBitfield) {
			bitF =
			    (nxpTfaBitfield_t *) (dev->list[i].offset +
						  (uint8_t *) g_cont);
			err = tfaRunWriteBitfield(dev_idx, *bitF);
		}
...
enum Tfa98xx_Error tfaContWriteRegsProf(int dev_idx, int prof_idx)
{
	...
	for (i = 0; i < prof->length; i++) {
		...
		if (prof->list[i].type == dscBitfield) {
			bitf =
			    (nxpTfaBitfield_t *) (prof->list[i].offset +
						  (uint8_t *) g_cont);
			err = tfaRunWriteBitfield(dev_idx, *bitf);
		}
		if (prof->list[i].type == dscRegister) {
			err =
			    tfaRunWriteRegister(dev_idx,
						(nxpTfaRegpatch_t *) (prof->list[i].offset + (char *)g_cont));
		}
		if (err)
			break;
	}
...
enum Tfa98xx_Error tfaContWriteFiles(int dev_idx)
{
	...
	for (i = 0; i < dev->length; i++) {
		...
		if (dev->list[i].type == dscCfMem) {
			err =
			    tfaRunWriteDspMem(dev_idx,
					      (nxpTfaDspMem_t *) (dev->list[i].offset + (uint8_t *)g_cont));
		}
 ...
 */
enum Tfa98xx_Error tfaContWriteFilesProf(int dev_idx, int prof_idx,
					 int vstep_idx)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	nxpTfaProfileList_t *prof = tfaContProfile(dev_idx, prof_idx);
	...
	for (i = 0; i < prof->length; i++) {
		...
		case dscFile:
			file =
			    (nxpTfaFileDsc_t *) (prof->list[i].offset +
						 (uint8_t *) g_cont);
			err =
			    tfaContWriteFile(dev_idx, file, vstep_idx,
		...
		case dscCfMem:
			err =
			    tfaRunWriteDspMem(dev_idx,
					      (nxpTfaDspMem_t *) (prof->list[i].offset + (uint8_t *)g_cont));
			break;
...
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	enum Tfa98xx_Error tmp_err = Tfa98xx_Error_Ok;
	/* nxpTfaFileDsc_t *file; */
	nxpTfaRegpatch_t *reg;
	nxpTfaMode_t *cas;
	nxpTfaBitfield_t *bitf;
	...
	case dscMode:
		...
		if (cas->value == Tfa98xx_Mode_RCV)
			tmp_err = tfa98xx_select_mode(dev_idx, Tfa98xx_Mode_RCV);
		else
			tmp_err = tfa98xx_select_mode(dev_idx, Tfa98xx_Mode_Normal);
		if (tmp_err != Tfa98xx_Error_Ok) {
			pr_debug("tfa98xx_select_mode return %d\n", tmp_err);
		}
		break;
	case dscCfMem:
		err =
		    tfaRunWriteDspMem(dev_idx,
				      (nxpTfaDspMem_t *) (dsc->offset +
							  (uint8_t *) g_cont));
		break;
	case dscBitfield:
	...
	case dscFilter:
		return tfaRunWriteFilter(dev_idx,
					 (nxpTfaContBiquad_t *) (dsc->offset +
								 (uint8_t *)
								 g_cont));
		break;
	}
...
enum Tfa98xx_Error tfa_write_filters(int dev_idx, int prof_idx)
{
	...
	for (i = 0; i < prof->length; i++) {
		if (prof->list[i].type == dscFilter) {
			if (tfaContWriteItem(dev_idx, &prof->list[i]) !=
			    Tfa98xx_Error_Ok)
				return Tfa98xx_Error_Bad_Parameter;
		}
	}
...
unsigned int tfa98xx_get_profile_sr(int dev_idx, unsigned int prof_idx)
{
	...
	pr_debug("%s - profile fs: 0x%x = %dHz (%d - %d)\n", __func__,
		 fs_profile, tfa98xx_sr_from_field(fs_profile), dev_idx,
		 prof_idx);
	if (fs_profile != -1)
...
enum Tfa98xx_Error get_sample_rate_info(int dev_idx, nxpTfaProfileList_t *prof,
					...
					int fs_previous_profile)
{
	...
	for (i = 0; i < previous_prof->length; i++) {
		...
		if (i < previous_prof->length) {
			if (previous_prof->list[i].type == dscBitfield) {
				bitf =
				    (nxpTfaBitfield_t *) (previous_prof->list[i].offset + (uint8_t *)g_cont);
				if (bitf->field == TFA_FAM(dev_idx, AUDFS)) {
	 ...
	 */
	pr_err("dll++++test fs_next_profile:%d, fs_default_profile:%d\n ",
	       fs_next_profile, fs_default_profile);
	if (fs_next_profile != fs_default_profile) {
		 ...
		 */
		pr_err("dll++test1 tfa98xx_cnt_verbose+++ ");
		 ...
		 */
		pr_err("dll++test1 fs_default_profile != fs_previous_profile ");
...
enum Tfa98xx_Error tfaContWriteProfile(int dev_idx, int prof_idx, int vstep_idx)
{
	...
	nxpTfaProfileList_t *prof = tfaContProfile(dev_idx, prof_idx);
	nxpTfaProfileList_t *previous_prof =
	    tfaContProfile(dev_idx, tfa_get_swprof(dev_idx));
	char buffer[(MEMTRACK_MAX_WORDS * 3) + 3] = { 0 };	/* every word requires 3 bytes, and 3 is the msg */
	...
	/* Get the slave */
	err = tfaContGetSlave(dev_idx, &slave_address);
	...
	if (prof->group == previous_prof->group && prof->group != 0) {
		if (tfa98xx_cnt_verbose) {
			pr_debug
			    ("The new profile (%s) is in the same group as the current profile (%s)\n",
			     ...
			     tfaContGetString(&previous_prof->name));
		}
	} else {
		/* mute */
		tfaRunMute(dev_idx);
		 ...
		 * */
		if (((dev_family == 2) && (TFA_GET_BF(dev_idx, MANSTATE) == 9))
		    || (dev_family != 2)) {
			...
			do {
				err =
				    tfa98xx_dsp_system_stable(dev_idx, &ready);
				if (!ready) {
					break;
				}
	...
	for (i = 0; i < previous_prof->length; i++) {
		...
		if (i < previous_prof->length) {
			if (tfaContWriteItem(dev_idx, &previous_prof->list[i])
			    != Tfa98xx_Error_Ok)
				return Tfa98xx_Error_Bad_Parameter;
		}
	}
	...
	err =
	    get_sample_rate_info(dev_idx, prof, previous_prof,
				 fs_previous_profile);
	if (err)
		return err;
		...
		for (i = 0; i < previous_prof->length; i++) {
			...
			if (i < previous_prof->length) {
				if (previous_prof->list[i].type == dscFile
					...
					file =
					    (nxpTfaFileDsc_t *) (previous_prof->list[i].offset + (uint8_t *)g_cont);
					err =
					    tfaContWriteFile(dev_idx, file,
							     vstep_idx,
							     TFA_MAX_VSTEP_MSG_MARKER);
				}
			}
	...
	for (i = j; i < prof->length; i++) {
		...
		case dscSetSensesDelay:
		case dscSetMBDrc:
			create_dsp_buffer_msg((nxpTfaMsg_t *)(prof->list[i].offset + (char *)g_cont), buffer, &size);
			err = dsp_msg(dev_idx, size, buffer);
 ...
 *
 */
enum Tfa98xx_Error tfaContWriteFilesVstep(int dev_idx, int prof_idx,
					  int vstep_idx)
{
	nxpTfaProfileList_t *prof = tfaContProfile(dev_idx, prof_idx);
	unsigned int i;
...
int tfaContGetCalProfile(int dev_idx)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		profile_name = tfaContProfileName(dev_idx, prof);
...
int tfaContIsTapProfile(int dev_idx, int prof_idx)
{
	...
	profile_name = tfaContProfileName(dev_idx, prof_idx);
	if (strnstr(profile_name, ".tap", strlen(profile_name)) != NULL) {
	...
	*hw_feature_register = -1;
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscFeatures) {
			features =
			    (nxpTfaFeatures_t *) (dev->list[i].offset +
						  (uint8_t *) g_cont);
	...
	if (result) {
		...
		if (result == 0) {
			...
			if (currentValue + delta < 15) {
				TFA_SET_BF_VOLATILE(dev_idx, DCMCC,
						    currentValue + delta);
				if (tfa98xx_cnt_verbose)
					pr_debug
