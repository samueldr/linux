...
int tfa9887_is87(Tfa98xx_handle_t handle);
...
int tfa_irq_get(Tfa98xx_handle_t handle, enum tfa9912_irq bit)
{
	...
	if (bit < tfa9912_irq_max) {
		...
		mask = 1 << (bit & 0x0f);
		tfa_reg_read(handle, (unsigned char)reg, &value);
	} else
		return RET_ERR;
...
int tfa_irq_ena(Tfa98xx_handle_t handle, enum tfa9912_irq bit, int state)
{
	...
	if (bit == tfa9912_irq_all) {
		...
		for (reg = TFA98XX_INTERRUPT_ENABLE_REG1;
			...
			/* all bits */
			handles_local[handle].interrupt_enable[reg - TFA98XX_INTERRUPT_ENABLE_REG1]
				= state ? 0xffff : 0;
		}
	} else if (bit < tfa9912_irq_max) {
		/* only this bit */
		...
		if (new_value != value) {
			...
			handles_local[handle].interrupt_enable[reg -
...
void tfa_irq_update(Tfa98xx_handle_t handle)
{
	...
	if (!maxregs) {
		...
		for (i = 0; i < maxregs; i++) {
			...
			tfa_reg_read(handle, (unsigned char)reg,
...
static void tfa_set_query_info(int dev_idx)
{
	...
	case 0x72:
		/* tfa9872 */
		handles_local[dev_idx].supportDrc = supportYes;
	...
	case 0x12:
		...
		handles_local[dev_idx].spkr_count = 1;
		handles_local[dev_idx].daimap = Tfa98xx_DAI_I2S;
		if (tfa9887_is87(dev_idx) == 1) {
/***MCH_TO_TEST***/
			handles_local[dev_idx].Is87Device = true;
			handles_local[dev_idx].supportDrc = supportNo;
			handles_local[dev_idx].supportFramework = supportNo;
			tfa9887_ops(&handles_local[dev_idx].dev_ops);	/* register device operations */
			pr_info("87 devices\n");
		} else if (tfa9887_is87(dev_idx) == 0) {
			handles_local[dev_idx].Is87Device = false;
	...
	case (0x1a << 1):	/* TODO properly implement foreign i2c addressing like uda1355 */
		...
		*pHandle = idx;
		...
		tfa98xx_trace_printk("slave:0x%02x revid:0x%04x\n",
				     slave_address, rev);
		pr_err("slave:0x%02x revid:0x%04x\n", slave_address, rev);
#endif
		break;
	default:
 ...
 */
enum Tfa98xx_DMEM tfa98xx_filter_mem(Tfa98xx_handle_t dev, int filter_index,
 ...
 */
enum Tfa98xx_Error tfa98xx_supported_speakers(Tfa98xx_handle_t handle,
					      int *spkr_count)
{
	if (tfa98xx_handle_is_open(handle)) {
 ...
 */
enum Tfa98xx_Error tfa98xx_supported_dai(Tfa98xx_handle_t handle,
 ...
 */
enum Tfa98xx_Error tfa98xx_supported_saam(Tfa98xx_handle_t handle,
					  enum Tfa98xx_saam *saam)
{
	...
	*saam =
	    handles_local[handle].support_saam ==
			supportYes ? Tfa98xx_saam : Tfa98xx_saam_none;
 ...
 */
enum Tfa98xx_Error tfa98xx_compare_features(Tfa98xx_handle_t handle,
...
enum Tfa98xx_Error tfa98xx_dsp_reset(Tfa98xx_handle_t dev_idx, int state)
{
	...
	if (tfa98xx_handle_is_open(dev_idx)) {
		if (handles_local[dev_idx].dev_ops.tfa_dsp_reset)
			error =
			    (*handles_local[dev_idx].dev_ops.tfa_dsp_reset)(dev_idx, state);
		else
			/* generic function */
...
void tfa98xx_apply_deferred_calibration(Tfa98xx_handle_t handle)
{
	...
	if (controls->otc.deferrable && controls->otc.triggered) {
		...
		if (err != Tfa98xx_Error_Ok) {
			pr_err
			    ("Unable to apply deferred MTP OTC write. Error=%d\n",
			     err);
		} else {
	...
	if (controls->calib.triggered) {
		...
		if (err) {
			...
			pr_err("Deferred calibration ok\n");
...
enum Tfa98xx_Error tfa98xx_dsp_system_stable(Tfa98xx_handle_t dev_idx,
					     int *ready)
{
	enum Tfa98xx_Error error;
	...
	if (handles_local[dev_idx].dev_ops.tfa_dsp_system_stable) {
		pr_err("dll+++++test!!!\n");
		error =
		    (*handles_local[dev_idx].dev_ops.tfa_dsp_system_stable)(dev_idx, ready);
	} else {
		/* generic function */
		pr_err
		    ("dll+++++test+_tfa98xx_dsp_system_stable, dev_idx : %d!!!\n",
		     dev_idx);
...
enum Tfa98xx_Error tfa98xx_dsp_write_tables(Tfa98xx_handle_t handle,
					    int sample_rate)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	...
	if (!tfa98xx_handle_is_open(handle))
		return Tfa98xx_Error_NotOpen;
	pr_err("dll++test  samplerate is :%d\n", sample_rate);
	if (handles_local[handle].dev_ops.tfa_dsp_write_tables)
		error =
		    (*handles_local[handle].dev_ops.tfa_dsp_write_tables)(handle, sample_rate);
...
enum Tfa98xx_Error tfa98xx_dsp_get_memory(Tfa98xx_handle_t handle,
...
enum Tfa98xx_Error tfa98xx_dsp_set_memory(Tfa98xx_handle_t handle,
					  int memoryType, int offset,
...
void tfa98xx_key2(Tfa98xx_handle_t handle, int lock)
{
	...
	reg_write(handle, (tfa98xx_dev_family(handle) == 1) ? 0x40 : 0x0F,
		  0x5A6B);
...
int tfa_calibrate(Tfa98xx_handle_t handle)
{
	...
	/* clear mtpex */
	error =
	    tfa98xx_set_mtp(handle, 0, TFA98XX_KEY2_PROTECTED_MTP0_MTPEX_MSK);
	if (error)
		return error;
			     ...
			     const unsigned char patchheader[])
{
	...
	if (checkaddress != 0xFFFF) {
		...
		if (error == Tfa98xx_Error_Ok) {
			error = mem_read(handle, checkaddress, 1, &value);
		}
		if (error == Tfa98xx_Error_Ok) {
			if (value != checkvalue) {
				pr_err
				    ("patch file romid type check failed [0x%04x]: expected 0x%02x, actual 0x%02x\n",
		...
		if (checkvalue != 0xFFFFFF && checkvalue != 0) {
			...
			if (revid != handles_local[handle].rev) {
				pr_err
				    ("patch file device type check failed: expected 0x%02x, actual 0x%02x\n",
	      ...
	      const unsigned char *patchBytes)
{
	...
	error =
	    tfa98xx_process_patch_file(handle,
...
enum Tfa98xx_Error tfa_dsp_msg_write(Tfa98xx_handle_t handle, int length,
				     const char *buffer)
{
	int offset = 0;
...
enum Tfa98xx_Error tfa_dsp_msg_write_id(Tfa98xx_handle_t handle, int length,
					const char *buffer, uint8_t cmdid[3])
{
	...
	/* write cmd-id */
	error =
	    tfa98xx_write_data(handle, FAM_TFA98XX_CF_MEM, 3,
...
enum Tfa98xx_Error tfa_dsp_msg_read(Tfa98xx_handle_t handle, int length,
				    unsigned char *bytes)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	...
	while (num_bytes > 0) {
		burst_size =
		    ROUND_DOWN(handles_local[handle].buffer_size,
			       bytes_per_word);
		if (num_bytes < burst_size)
			burst_size = num_bytes;
		error =
		    tfa98xx_read_data(handle, FAM_TFA98XX_CF_MEM, burst_size,
				      bytes + offset);
		if (error != Tfa98xx_Error_Ok)
			return error;
...
void tfa_dsp_reg_register(int dev_idx, reg_read_t read_func,
			  reg_write_t write_func)
{
	handles_local[dev_idx].dev_ops.tfa_reg_read  = read_func;
	handles_local[dev_idx].dev_ops.reg_write = write_func;
}

void tfa_dsp_mem_register(int dev_idx, mem_read_t read_func,
			  mem_write_t write_func)
{
...
enum Tfa98xx_Error dsp_msg(Tfa98xx_handle_t handle, int length, const char *buf)
{
	...
	if (handles_local[handle].dev_ops.dsp_msg) {
		...
		if (handles_local[handle].ext_dsp == 1) {
			...
			lastmessage = tfa_tib_dsp_msgblob(handle, length, buf);
			if (lastmessage == -1)
			...
			if (lastmessage == 1) {
				...
				error =
				    (*handles_local[handle].dev_ops.dsp_msg)(handle, length, (const char *)blob);
				if (error != Tfa98xx_Error_Ok)
			...
			error =
			    (*handles_local[handle].dev_ops.dsp_msg) (handle,
								      length,
								      buf);
			if (error != Tfa98xx_Error_Ok)
				/* Get actual error code from softDSP */
		...
		error = tfa_dsp_msg(handle, length, buf);	/* Use default */
	}

	return error;
}

enum Tfa98xx_Error dsp_msg_read(Tfa98xx_handle_t handle, int length,
				unsigned char *bytes)
{
	enum Tfa98xx_Error error;
	...
	if (handles_local[handle].dev_ops.dsp_msg_read) {
		error =
		    (*handles_local[handle].dev_ops.dsp_msg_read)(handle, length, bytes);
		if (error != Tfa98xx_Error_Ok) {
			/* Get actual error code from softDSP */
			error = (enum Tfa98xx_Error)(error + Tfa98xx_Error_RpcBase);
		}
	} else {
		error = tfa_dsp_msg_read(handle, length, bytes);	/* Use default */
	}

	return error;
}

enum Tfa98xx_Error tfa_reg_read(Tfa98xx_handle_t handle, unsigned char subaddress,
			    unsigned short *value)
{
	enum Tfa98xx_Error error;

	if (handles_local[handle].dev_ops.tfa_reg_read) {
		error =
		    (*handles_local[handle].dev_ops.tfa_reg_read)(handle, subaddress, value);
		if (error != Tfa98xx_Error_Ok) {
			/* Get actual error code from softDSP */
			error = (enum Tfa98xx_Error)(error + Tfa98xx_Error_RpcBase);
		}
...
enum Tfa98xx_Error reg_write(Tfa98xx_handle_t handle, unsigned char subaddress,
			     unsigned short value)
{
	enum Tfa98xx_Error error;
	...
	if (handles_local[handle].dev_ops.reg_write) {
		error =
		    (*handles_local[handle].dev_ops.reg_write)(handle, subaddress, value);
		if (error != Tfa98xx_Error_Ok) {
			/* Get actual error code from softDSP */
			error = (enum Tfa98xx_Error)(error + Tfa98xx_Error_RpcBase);
		}
...
enum Tfa98xx_Error mem_read(Tfa98xx_handle_t handle, unsigned int start_offset,
			    int num_words, int *pValues)
{
	...
	if (handles_local[handle].dev_ops.mem_read) {
		error =
		    (*handles_local[handle].dev_ops.mem_read)(handle, start_offset, num_words, pValues);
		if (error != Tfa98xx_Error_Ok) {
			/* Get actual error code from softDSP */
			error = (enum Tfa98xx_Error)(error + Tfa98xx_Error_RpcBase);
		}
	} else {
		error = tfa98xx_dsp_read_mem(handle, start_offset, num_words, pValues);	/* Use default */
	}

	return error;
}

enum Tfa98xx_Error mem_write(Tfa98xx_handle_t handle, unsigned short address,
			     int value, int memtype)
{
	...
	if (handles_local[handle].dev_ops.mem_write) {
		error =
		    (*handles_local[handle].dev_ops.mem_write)(handle, address, value, memtype);
		if (error != Tfa98xx_Error_Ok) {
			/* Get actual error code from softDSP */
			error = (enum Tfa98xx_Error)(error + Tfa98xx_Error_RpcBase);
		}
...
enum Tfa98xx_Error tfa_dsp_msg(Tfa98xx_handle_t handle, int length,
			       const char *buf)
{
	enum Tfa98xx_Error error;
	...
	if (rpc_status != Tfa98xx_I2C_Req_Done) {
		...
		error =
		    (enum Tfa98xx_Error)(rpc_status + Tfa98xx_Error_RpcBase);
 ...
 */
enum Tfa98xx_Error tfa_dsp_msg_id(Tfa98xx_handle_t handle, int length,
...
enum Tfa98xx_Error
tfa98xx_dsp_write_mem_word(Tfa98xx_handle_t handle, unsigned short address,
			   int value, int memtype)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
enum Tfa98xx_Error tfa_cont_write_filterbank(int device,
					     nxpTfaFilter_t *filter)
{
	...
	for (biquad_index = 0; biquad_index < 10; biquad_index++) {
		if (filter[biquad_index].enabled) {
			error = tfa_dsp_cmd_id_write(device, MODULE_BIQUADFILTERBANK, biquad_index + 1,	/* start @1 */
						     sizeof(filter[biquad_index].biquad.bytes),
						     filter[biquad_index].biquad.bytes);
		} else {
			error =
			    Tfa98xx_DspBiquad_Disable(device, biquad_index + 1);
		}
		if (error)
			return error;
...
enum Tfa98xx_Error tfa98xx_dsp_support_tcoefA(Tfa98xx_handle_t handle,
					      int *pbSupporttCoefA)
{
	*pbSupporttCoefA = handles_local[handle].Is87Device;
...
enum Tfa98xx_Error tfa98xx_dsp_get_state_info(Tfa98xx_handle_t handle,
					      unsigned char bytes[],
					      unsigned int *statesize)
{
	...
	if (err == Tfa98xx_Error_Ok) {
		if (bSupportFramework) {
			err =
			    tfa_dsp_cmd_id_write_read(handle, MODULE_FRAMEWORK,
						      FW_PARAM_GET_STATE,
						      3 * stateSize, bytes);
		} else {
			...
			stateSize = 8;
			err =
			    tfa_dsp_cmd_id_write_read(handle,
						      MODULE_SPEAKERBOOST,
						      SB_PARAM_GET_STATE,
						      3 * stateSize, bytes);
		}
					   ...
					   int *pbSupportDrc)
{
	...
	*pbSupportDrc = 0;
	...
	if (handles_local[handle].supportDrc != supportNotSet) {
		*pbSupportDrc =
		    (handles_local[handle].supportDrc == supportYes);
	} else {
		int featureBits[2];
		...
		if (error == Tfa98xx_Error_Ok) {
			handles_local[handle].supportDrc =
...
int tfa_set_bf(Tfa98xx_handle_t dev_idx, const uint16_t bf,
	       const uint16_t value)
{
	...
	err = tfa_reg_read(dev_idx, address, &regvalue);
	if (err) {
	...
	regvalue &= ~msk;
	regvalue |= value << pos;
	/* pr_err("dl+++1+ The address is:%#x, the value is :%#x", address, regvalue); */
	...
	if (oldvalue != regvalue) {
		...
		pr_err("dll++2++ The address is:%#x, the value is :%#x\n",
		       address, regvalue);
		if (err) {
...
int tfa_set_bf_volatile(Tfa98xx_handle_t dev_idx, const uint16_t bf,
			const uint16_t value)
{
	enum Tfa98xx_Error err;
...
int tfa_write_reg(Tfa98xx_handle_t dev_idx, const uint16_t bf,
...
/*
 * warm powerup the tfa and coolflux subsystem and wait for it
 */
 ...
 */
static enum Tfa98xx_Error tfa98xx_aec_output(Tfa98xx_handle_t handle,
					     int enable)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
...
enum Tfa98xx_Error show_current_state(Tfa98xx_handle_t handle)
{
	...
	switch (manstate) {
	case 0:
		pr_err("power_down_state\n");
		break;
	case 1:
		pr_err("wait_for_source_settings_state\n");
		break;
	case 2:
		pr_err("connnect_pll_input_state\n");
		break;
	case 3:
		pr_err("disconnect_pll_input_state\n");
		break;
	case 4:
		pr_err("enable_pll_state\n");
		break;
	case 5:
		pr_err("enable_cgu_state\n");
		break;
	case 6:
		pr_err("init_cf_state\n");
		break;
	case 7:
		pr_err("enable_amplifier_state\n");
		break;
	case 8:
		pr_err("alarm_state\n");
		break;
	case 9:
		pr_err("operating_state\n");
		break;
	case 10:
		pr_err("mute_audio_state\n");
		break;
	case 11:
		pr_err("disable_cgu_pll_state\n");
		break;
	default:
 ...
 *
 */
enum Tfa98xx_Error tfaRunSpeakerBoost(Tfa98xx_handle_t handle, int force,
				      int profile)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	int value;
	...
	pr_err("dll++%s,+++clod value:%d\n", __func__, value);

#ifdef __KERNEL__		/* TODO try to combine this with the pr_err below */
	...
	if ((value == 1) && (!istap_prof)) {
		pr_err("dll++%s,+++test ++istap_prof is:%d\n", __func__,
		       istap_prof);
...
/*Nxp patch for no sound when boost begin*/
/*
	switch(handles_local[handle].rev & 0xff){
		case 0x96:
	...
	*/
/*Nxp patch for no sound when boost end*/
		...
		if (!handles_local[handle].ext_dsp) {
			...
			/* SL: Added dsp reset workaround for 96 and 97 */
			switch (TFA_GET_BF(handle, REV) & 0xff) {
			case 0x96:
			case 0x97:
			...
			err = tfaSetCalibrationValues(handle);
		}
...
enum Tfa98xx_Error tfaRunSpeakerStartup(Tfa98xx_handle_t handle, int force,
					int profile)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;

	pr_err("coldstart%s :", force ? " (forced)" : "");
	...
	if (!force) {		/* in case of force CF already running */
		...
		if (tfa_cf_enabled(handle) == 0)
			return err;
		pr_err("dll++++tfa+11111++RunSpeakerStartup\n");
		if (!handles_local[handle].ext_dsp) {
			pr_err("dll++++tfa+222222++RunSpeakerStartup\n");
...
/* Necessary otherwise we are thrown out of operating mode in kernel (because of internal clock) */
#ifdef __KERNEL__
	profile_name = tfaContProfileName(handle, profile);
	if ((strnstr(profile_name, ".cal", strlen(profile_name)) == NULL)
	...
	err = tfa98xx_read_register16(handle, 0x00, &value);
	pr_err("dll+++SYS_STATUS0: 0x%04x\n", value);
	...
	if (err != Tfa98xx_Error_Ok) {
		if ((tfa98xx_dev_family(handle) == 2)
			...
			pr_err
			    ("Unable to calibrate the device with the internal clock!\n");
		}
	}
 ...
 */
enum Tfa98xx_Error tfaSetCalibrationValues(Tfa98xx_handle_t handle)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	...
	if (TFA_GET_BF(handle, MTPEX) == 1) {
		/* mono */
		if (tfa98xx_cnt_max_device() == 1) {
			err = tfa_reg_read(handle, 0xF5, &value);
			...
			err = tfa_reg_read(0, 0xF5, &value);
			dsp_cal_value_left = (value * 65536) / 1000;

			err = tfa_reg_read(1, 0xF5, &value);
	...
	err =
	    tfa_dsp_cmd_id_write(handle, MODULE_SPEAKERBOOST,
...
enum Tfa98xx_Error tfaRunStartDSP(Tfa98xx_handle_t handle)
{
	...
	if (err) {		/* patch load is fatal so return immediately */
		pr_err("dll++tfa_run_load_patch error  %s\n", __func__);
		return err;
	}
	...
	pr_err("dll++tfa98xx_dsp_write_tables  %s\n", __func__);
	PRINT_ASSERT(err);
...
enum Tfa98xx_Error tfaRunStartup(Tfa98xx_handle_t handle, int profile)
{
	...
	if (handles_local[handle].Is87Device) {	/* MCH_TO_TEST */
		pr_err("device here here 9887\n");
		msleep_interruptible(10);
		err = tfa98xx_write_register16(handle, 0x70, 0x01);
		err = tfa98xx_read_register16(handle, 0x9, &value);
		PRINT_ASSERT(err);
		value |= TFA98XX_SYS_CTRL_CFE_MSK;	/*(1<<2) */
		err = tfa98xx_write_register16(handle, 0x09, value);

	} else
		pr_info("device looks not 9887\n");
	...
	case 0x80:
		err = tfa98xx_dsp_reset(handle, 1);
		pr_err("RunStartup Reset DSP after power on\n");
		break;
	default:
		break;
	}
	...
	if (!handles_local[handle].ext_dsp) {
		/* We dont need to verify for a clock if CFE is disabled */
		if (tfa_cf_enabled(handle)) {
			/*  wait until the PLL is ready
			 *    note that the DSP CPU is not running (RST=1) */
			if (tfa98xx_runtime_verbose) {
				if ((tfa98xx_dev_family(handle) == 2)
				    && TFA_GET_BF(handle, NOCLK))
					pr_err("Using internal clock\n");
				pr_err("Waiting for DSP system stable...\n");
			}
...
enum Tfa98xx_Error tfaRunColdStartup(Tfa98xx_handle_t handle, int profile)
{
	...
	pr_err("dll++++tfa+++RunColdStartup\n");
	err = tfaRunStartup(handle, profile);
	PRINT_ASSERT(err);
	if (err)
		return err;
	pr_err("dll++++tfa++2+RunColdStartup\n");
	if ((handles_local[handle].rev & 0xFF) != 0x72) {
		pr_err
		    ("dll++tfa  RunColdStartup++33+handles_local[handle].rev & 0xFF) != 0x72");
		/* force cold boot */
	...
	pr_err("dll++++tfa+444++RunColdStartup\n");
	/* start */
	err = tfaRunStartDSP(handle);
	PRINT_ASSERT(err);
 ...
 */
enum Tfa98xx_Error tfaRunWaitCalibration(Tfa98xx_handle_t handle,
					 int *calibrateDone)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	...
	*calibrateDone = 0;
	...
	while ((*calibrateDone != 1) && (tries < TFA98XX_API_WAITRESULT_NTRIES)) {
		err =
		    mem_read(handle, TFA_FW_XMEM_CALIBRATION_DONE, 1,
			     calibrateDone);
		tries++;
	}
...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
	...
	for (dev = 0; dev < devcount; dev++) {
		 ...
		 * For softDSP or 72 we can't switch so we only run on the profile given
		 */
		...
		if (tfa98xx_runtime_verbose) {
			pr_err("active_profile:%s, next_profile:%s\n",
			       ...
			       tfaContProfileName(dev, profile));
			pr_err("Starting device [%s]\n",
		...
		err = tfaRunSpeakerBoost(dev, 0, profile);
		pr_err("dll+++%s++test++++err is: %d\n", __func__, err);
		if (err != Tfa98xx_Error_Ok)
			goto error_exit;
	}
	...
	for (dev = 0; dev < devcount; dev++) {
		...
		if ((next_profile != active_profile && active_profile >= 0)
		    || (istap_prof == 1)) {
			err =
			    tfaContWriteProfile(dev, next_profile, vstep[dev]);
			if (err != Tfa98xx_Error_Ok)
				goto error_exit;
		}
		 ...
		 */
		profile_name = tfaContProfileName(dev, next_profile);
		if (strnstr(profile_name, ".standby", strlen(profile_name)) != NULL) {
		 ...
		 */
		if (tfa98xx_dev_family(dev) == 2) {
			/* If we are in state 0 or 1 we can skip setting SBSL and RST, but also filters and
			 * vstep (since there is no clk).
			 * We can be in state 0 or 1 because of standby profile or switching profile with
			 * internal clock.
		...
		if (((handles_local[dev].rev & 0xff) == 0x72)
		    && (!handles_local[dev].ext_dsp)) {
			/* Only run this when there is an actual DSP! */
		} else if (vstep[dev] != tfaContGetCurrentVstep(dev)) {
			err =
			    tfaContWriteFilesVstep(dev, next_profile,
						   vstep[dev]);
			if (err != Tfa98xx_Error_Ok)
				goto error_exit;
		}
 ...
 error_exit:
	...
	return (enum tfa_error)err; /* xxx_zte fix compile err. */
}
	...
	if (error == Tfa98xx_Error_Ok) {
		if ((handles_local[handle].rev & 0xff) == 0x72) {
			if (TFA_GET_BF(handle, MTPOTC)
				...
				pr_err
				    ("Getting calibration values from Speakerboost\n");
			...
			error =
			    tfa_dsp_cmd_id_write_read(handle,
						      ...
						      nr_bytes, bytes);
			if (error == Tfa98xx_Error_Ok) {
				tfa98xx_convert_bytes2data(nr_bytes, bytes,
							   data);
				for (i = 0; i < spkr_count; i++) {
					handles_local[handle].mohm[i] =
...
enum Tfa98xx_Error tfa_mtp_get_calibration(Tfa98xx_handle_t handle,
					   uint16_t cal_data[1])
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;

	cal_data[0] =
	    (uint16_t) tfa_read_reg(handle, (uint16_t) TFA_MK_BF(0xF5, 0, 16));

	return err;
}

enum Tfa98xx_Error tfa_mtp_set_calibration(Tfa98xx_handle_t handle,
					   uint16_t cal_data)
{
	int loop;
	...
	tfa_write_reg(handle, (uint16_t) TFA_MK_BF(0xF5, 0, 16), cal_data);
	/* CIMTP=1 start copying all the data from i2c regs_mtp to mtp */
	TFA_SET_BF(handle, CIMTP, 1);
	/* no check for MTPBUSY here, i2c delay assumed to be enough */
	tfa98xx_key2(handle, 1);	/* lock */
...
enum Tfa98xx_Error tfa98xx_set_boost_trip_level(void)
{
	...
	for (dev = 0; dev < devcount; dev++) {
		...
		if (Re25C == 0) {
			...
			Re25C =
			    (unsigned short)tfa_read_reg(dev,
							 (uint16_t)
							 TFA_MK_BF((mtp_reg_addr), 0, 16));
		}

		if (handles_local[dev].dev_ops.tfa_set_boost_trip_level)
			error =
			    (*handles_local[dev].dev_ops.tfa_set_boost_trip_level)(dev, Re25C);
 ...
 */
static int tfa_dsp_handle_event(Tfa98xx_handle_t handle,
				enum tfadsp_event_en tfadsp_event)
{
	...
	case TFADSP_CMD_READY:	/*Ready to receive commands */
		...
		handles_local[handle].ext_dsp = 1;
		err =
		    tfa_start(handles_local[handle].profile,
