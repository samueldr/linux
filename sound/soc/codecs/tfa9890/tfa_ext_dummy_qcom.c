...
/*     Create directory `/sys/kernel/debug/tfa_ext_dummy`.*/
	...
	if (!dir) {
		/* Abort module load. */
		pr_alert(
		       "debugfs_tfa_ext_dummy: failed to create /sys/kernel/debug/tfa_ext_dummy\n");
		return RET_ERR;
	}

/* Create file `/sys/kernel/debug/tfa_ext_dummy/event`.*/

	junk = debugfs_create_file("event", 0220, dir, NULL, &add_fops);
	if (!junk) {
		/* Abort module load. */
		pr_alert("debugfs_tfa_ext_dummy: failed to create /sys/kernel/debug/tfa_ext_dummy/event\n");
		return RET_ERR;
	}
...
*/
	...
	if (!junk) {
		pr_alert("debugfs_tfa_ext_dummy: failed to create /sys/kernel/debug/tfa_ext_dummy/status\n");
		return RET_ERR;
	}
...
static int tfa_ext_registry(void)
{
	...
	if (tfa_ext_register
	    (NULL, tfa_ext_dsp_msg, tfa_ext_dsp_msg_read,
	     &tfa_ext_handle_event)) {
