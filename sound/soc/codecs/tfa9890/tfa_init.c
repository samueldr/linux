...
int tfa9887_is87(Tfa98xx_handle_t handle)
{
	...
	if ((save_value & 0x0400) == 0) {	/* if clear it's 87 */
		pr_info("Value if clear it's 87\n");
		return 1;
	}
	...
	pr_info("Value to be resturned is %d\n", check_value != save_value);
...
static enum Tfa98xx_Error tfa9887_specific(Tfa98xx_handle_t handle)
{
	...
	if (Tfa98xx_Error_Ok == error) {
		error = tfa98xx_read_register16(handle, 0x09, &value);
	}
	if (Tfa98xx_Error_Ok == error) {
		/* DSP must be in control of the amplifier to avoid plops */
		value &= ~(TFA98XX_SYS_CTRL_AMPE_MSK |	/* clear AMPE to prevent starting the amp too early */
			   TFA98XX_SYS_CTRL_CFE_MSK);
		error = tfa98xx_write_register16(handle, 0x09, value);
	}

	/* some other registers must be set for optimal amplifier behaviour */
	if (Tfa98xx_Error_Ok == error) {
		error = tfa98xx_write_register16(handle, 0x05, 0x13AB);
	}
	if (Tfa98xx_Error_Ok == error) {
		error = tfa98xx_write_register16(handle, 0x06, 0x001F);
	}
	if (Tfa98xx_Error_Ok == error) {
		error = tfa98xx_write_register16(handle, 0x08, 0x3C4E);
	}
	if (Tfa98xx_Error_Ok == error) {	/*TFA98XX_SYSCTRL_DCA=0 */
		error = tfa98xx_write_register16(handle, 0x09, 0x024D);
	}
	if (Tfa98xx_Error_Ok == error) {
		error = tfa98xx_write_register16(handle, 0x0A, 0x3EC3);
	}
	if (Tfa98xx_Error_Ok == error) {
		error = tfa98xx_write_register16(handle, 0x41, 0x0308);
	}
	if (Tfa98xx_Error_Ok == error) {
...
static enum Tfa98xx_Error tfa9890_specific(Tfa98xx_handle_t handle)
{
	...
	if (error)
		return error;
	tfa_reg_read(handle, 0x59, &regRead);
	regRead |= 0x3;
...
static enum Tfa98xx_Error tfa9896_specific(Tfa98xx_handle_t handle)
{
	 ...
	 */
	error = tfa_reg_read(handle, 0x49, &check_value);
	check_value &= ~0x1;
...
static enum Tfa98xx_Error tfa9888_specific(Tfa98xx_handle_t handle)
{
	...
	error = tfa_reg_read(handle, 0xFB, &value);
	xor = value ^ 0x005A;
...
static enum Tfa98xx_Error tfa9872_specific(Tfa98xx_handle_t handle)
{
	...
	error = tfa_reg_read(handle, 0xB0, &value);
	value |= 1 << 3;
...
static enum Tfa98xx_Error tfa9912_specific(Tfa98xx_handle_t handle)
{
	...
	if (handles_local[handle].rev == 0x1a13) {
		/* ----- generated code start ----- */
		/* -----  version 1.32 ----- */
		reg_write(handle, 0x00, 0x0245);	/* POR=0x0045 */
 ...
 *
 */
static enum Tfa98xx_Error tfa9890_dsp_system_stable(Tfa98xx_handle_t handle,
						    int *ready)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	...
	*ready = !((TFA_GET_BF_VALUE(handle, AREFS, status) == 0)
	...
	error = tfa_reg_read(handle, 0x84, &mtp0);
	if (error)
		goto errorExit;
...
static enum Tfa98xx_Error tfa9890_clockgating(Tfa98xx_handle_t handle, int on)
{
	...
	error = tfa_reg_read(handle, TFA98XX_CURRENTSENSE4, &value);
	if (error)
		return error;
 ...
 */
static enum Tfa98xx_Error tfa9896_dsp_write_vsfwdelay_table(Tfa98xx_handle_t
							    handle)
{
	enum Tfa98xx_Error error;
	...
	error = tfa_dsp_cmd_id_write(handle, MODULE_FRAMEWORK,
				     ...
				     sizeof(vsfwdelay_table), vsfwdelay_table);
	return error;
}
...
static enum Tfa98xx_Error tfa9896_tfa_dsp_write_tables(Tfa98xx_handle_t dev_idx,
						       int sample_rate)
{
	enum Tfa98xx_Error error;
...
static enum Tfa98xx_Error tfa9896_tfa_set_boost_trip_level(Tfa98xx_handle_t
							   handle, int Re25C)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	int trip_value;
	...
	pr_err("New boost_trip_lvl is set to %d\n", trip_value);
 ...
 *  so moving this to the tfa98xx API requires also updating all patches
 */
static enum Tfa98xx_Error tfa9897_dsp_write_vsfwdelay_table(Tfa98xx_handle_t
							    handle)
{
	enum Tfa98xx_Error error;
...
static enum Tfa98xx_Error tfa9897_tfa_dsp_write_tables(Tfa98xx_handle_t dev_idx,
						       int sample_rate)
{
	enum Tfa98xx_Error error;
...
static enum Tfa98xx_Error tfa9888_tfa_dsp_write_tables(Tfa98xx_handle_t handle,
						       int sample_rate)
{
...
static enum Tfa98xx_Error tfa9912_tfa_dsp_write_tables(Tfa98xx_handle_t handle,
						       int sample_rate)
{
	unsigned char buffer[15] = { 0 };
	int size = 15 * sizeof(char);
