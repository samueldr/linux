...
typedef enum Tfa98xx_Error (*dsp_msg_t)(Tfa98xx_handle_t handle, int length,
					 const char *buf);
...
struct tfa_device_ops {
	...
	enum Tfa98xx_Error (*tfa_dsp_reset)(Tfa98xx_handle_t dev_idx,
					     int state);
	enum Tfa98xx_Error (*tfa_dsp_system_stable)(Tfa98xx_handle_t handle,
						     int *ready);
	enum Tfa98xx_Error (*tfa_dsp_write_tables)(Tfa98xx_handle_t dev_idx,
						    int sample_rate);
	enum Tfa98xx_Error (*tfa_set_boost_trip_level)(Tfa98xx_handle_t handle,
							int Re25C);
	dsp_msg_t dsp_msg;
	dsp_msg_read_t dsp_msg_read;
	reg_read_t tfa_reg_read;
...
TFA_INTERNAL enum Tfa98xx_Error tfa98xx_check_rpc_status(Tfa98xx_handle_t
							 handle,
							 int *pRpcStatus);
TFA_INTERNAL enum Tfa98xx_Error tfa98xx_wait_result(Tfa98xx_handle_t handle,
						    int waitRetryCount);
TFA_INTERNAL void tfa98xx_apply_deferred_calibration(Tfa98xx_handle_t handle);
TFA_INTERNAL void tfa98xx_deferred_calibration_status(Tfa98xx_handle_t handle,
						      int calibrateDone);
TFA_INTERNAL int print_calibration(Tfa98xx_handle_t handle, char *str,
				   size_t size);
