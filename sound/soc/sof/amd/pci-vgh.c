...
static const struct resource vangogh_res[] = {
	{
		.start = 0,
		.end = ACP5x_REG_END - ACP5x_REG_START,
		.name = "acp_mem",
...
static const struct sof_amd_acp_desc vangogh_chip_info = {
	...
	.i2s_pin_config_offset = ACP5X_I2S_PIN_CONFIG,
...
static int acp_pci_vgh_probe(struct pci_dev *pci, const struct pci_device_id *pci_id)
{
	...
	res = devm_kzalloc(&pci->dev, sizeof(struct resource) * ARRAY_SIZE(vangogh_res),
			   GFP_KERNEL);
	if (!res) {
	...
	res_i2s = vangogh_res;
	for (i = 0; i < ARRAY_SIZE(vangogh_res); i++, res_i2s++) {
	...
	pdevinfo.name = "acp_asoc_vangogh";
	pdevinfo.id = 0;
	pdevinfo.parent = &pci->dev;
	pdevinfo.num_res = ARRAY_SIZE(vangogh_res);
	pdevinfo.res = &res[0];
